...
int main(int argc, char *argv[])
{
    ...
    wordList boundaryPatchTypes(boundaryFaces.size(), directPolyPatch::typeName);
    word defaultFacesName = "defaultFaces";
    word defaultFacesType = directPolyPatch::typeName;
    ...
    if (isA<directPolyPatch>(pp))
    {
        ...
        // Go through all the patchFaces and find corresponding face in dpp.
        forAll(patchFaces, patchi)
        {
            ...
            forAll(pFaces, i)
            {
                ...
                // Find face in dpp using all vertices of f.
                label patchFacei = findFace(dpp, f);

                if (patchFacei != -1)
                {
                    label meshFacei = dpp.start() + patchFacei;
