...
int main(int argc, char *argv[])
{
    ...
    forAll(regionNames, regionI)
    {
        ...
        if (readDict)
        {
            ...
            writeMaps = renumberDict.lookup<bool>("writeMaps");
            if (writeMaps)
            {
        ...
        // Update sets
        topoSet::topoChange(mesh.facesInstance(), map(), cellSets);
        topoSet::topoChange(mesh.facesInstance(), map(), faceSets);
        topoSet::topoChange(mesh.facesInstance(), map(), pointSets);
