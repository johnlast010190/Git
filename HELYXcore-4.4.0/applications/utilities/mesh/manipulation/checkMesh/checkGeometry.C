...
)
{
            ...
            if (cpp.owner())
            {
                ...
                (
                    ...
                    (
                        ...
                        cpp,
                        cpp.points(),
                        cpp.primitivePatch::faceCentres()
                    )
                );
    ...
    reduce(
        std::tie(nErrorFaces, avgMismatch, nCoupledPoints),
        ParallelOp<sumOp<label>, maxOp<scalar>, sumOp<label>>{}
    );
...
)
{
    ...
    Info<< "    Overall domain volume " << gSum(mesh.cellVolumes()) << endl;
    ...
    {
        faceSet faces(mesh, "minAreaFaces", mesh.nFaces()/100+1);
        if (mesh.checkFaceAreas(true, minFaceArea, &faces))
        {
            noFailedChecks++;
            ...
            {
                Info<< "  <<Writing " << nFaces
                    << " min area faces to set " << faces.name() << endl;
...

    {
        ...
        if (mesh.checkFacePyramids(true, -doubleScalarSMALL, &faces))
        {
            noFailedChecks++;
            ...
            if (isA<cyclicAMIPolyPatch>(pbm[patchi]))
            {
                ...
                if (cpp.owner())
                {
                    ...
                    }
                    {
                        ...
                        (
                            mesh,
                            cpp.nbrPatch().localFaces(),
                            cpp.nbrPatch().meshPoints(),
                            cpp.nbrPatch().meshPointMap(),
    ...
    if (allGeometry)
    {
        ...
        // Print message if there are non-conformal cyclics
        forAll(patches, patchi)
        {
            if (isA<nonConformalCyclicPolyPatch>(patches[patchi]))
            {
                Info<< "\nWriting non-conformal coupling coverage" << endl;
                break;
            }
        }

        // Compute coverage for all NCC original patches
        ...
        forAll(patches, nccPatchi)
        {
            if (isA<nonConformalCyclicPolyPatch>(patches[nccPatchi]))
            {
                ...
                if (nccPp.owner())
                {
                    ...
                    patchCoverage[origPp.index()] += intersection.srcCoverage();
