...
)
{
...

    {
        ...
        else
        {
            ...
            if (allTopology)
            {
                ...
                Pstream::listCombineReduce(regionDisconnected, andOp<bool>());
            }
    ...
    // Check how well parallel mesh balanced
    if (Pstream::parRun())
    {
        label minNumberCells = mesh.nCells();
        label maxNumberCells = mesh.nCells();
        reduce(
            std::tie(minNumberCells, maxNumberCells),
            ParallelOp<minOp<label>, maxOp<label>>{}
        );
        scalar inbalance(scalar(minNumberCells)/scalar(maxNumberCells));
        Info<< "    Processor balance " << inbalance << endl;
    }
...

    {
        ...
        forAll(patches, patchi)
        {
            ...
            nameWidth = max(nameWidth, label(pp.name().length()));
        }

        Info<< " "
            << setw(nameWidth) << "Patch"
            << " "
            << setw(9) << "Faces"
            << " "
            << setw(9) << "Points"
        ...
        forAll(patches, patchi)
        {
            ...
            if (isA<directPolyPatch>(pp))
            {
                ...
                {
                    scalar pArea = gSum(pp.magFaceAreas());
                    vector pNorm = gSum(pp.faceAreas());
                    ...
                    if (!Pstream::parRun())
                    {
                        primitivePatch::surfaceTopo pTyp = dpp.surfaceType();
                        ...
                        {
                            if (dpp.checkPointManifold(true, &points))
                            {
                                Info<< setw(34)
                            ...
                            // Add points on non-manifold edges to make
                            // set complete
                            dpp.checkTopology(false, &points);
                        }
                        else
                        {
                            dpp.checkTopology(false, &points);
