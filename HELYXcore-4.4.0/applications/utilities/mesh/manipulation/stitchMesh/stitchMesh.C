...
void removeFacesFromExistingZone
(
    polyMesh& mesh,
...
{
    const polyPatch& masterPatch = mesh.boundaryMesh()[masterName];
    const polyPatch& slavePatch = mesh.boundaryMesh()[slaveName];

    boolList masterSlaveFaces(mesh.nFaces(), false);
    forAll(masterPatch, i)
    {
        masterSlaveFaces[masterPatch.start() + i] = true;
    }
    forAll(slavePatch, i)
    {
        masterSlaveFaces[slavePatch.start() + i] = true;
    }

    forAll(mesh.faceZones(), zoneI)
    {
        const faceZone& fz = mesh.faceZones()[zoneI];
        DynamicList<label> ufz(fz.size());
        DynamicList<bool> flipMap(fz.size());
        boolList fMap = fz.flipMap();

        forAll(fz, i)
        {
            if (!masterSlaveFaces[fz[i]])
            {
                ufz.append(fz[i]);
                flipMap.append(fMap[i]);
            }
        }
        ufz.shrink();
        flipMap.shrink();
...
int main(int argc, char *argv[])
{
    ...
    removeFacesFromExistingZone(mesh, masterPatchName, slavePatchName);
    ...
    //Clear to force meshPhi to not be written
    mesh.clearOut();
