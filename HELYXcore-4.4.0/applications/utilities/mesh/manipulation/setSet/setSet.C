...
// #include "output/foamVtkWriteCellSetFaces.H"
// #include "output/foamVtkWriteFaceSet.H"
// #include "output/foamVtkWritePointSet.H"
    ...
    try
    {
        ...
        if (currentSetPtr.valid())
        {
            ...
            Info<< "    Set:" << currentSet.name()
                << "  Size:" << returnReduceToMaster(currentSet.size(), sumOp<label>())
                << "  Action:" << actionName
                << endl;
            ...
            if (action != topoSetSource::LIST)
            {
                ...
                if (writeVTKFile)
                {
                    ...
                    Info<< "    Writing " << currentSet.name()
                        << " (size "
                        << returnReduceToMaster(currentSet.size(), sumOp<label>())
                        << ") to "
                        << currentSet.instance()/currentSet.local()
                           /currentSet.name()
                        << " and to vtk file " << vtkName << endl << endl;

                    if (currentSet.readOpt() == IOobject::NO_READ)
                    {
                        //force read for construction
                        currentSet.readOpt() = IOobject::MUST_READ;
                    }
