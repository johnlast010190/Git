...
#include "include/createMesh.H"
    ...
    {
        meshQuality = removalDict.lookup<bool>("meshQualityRemoval");
    }

    Switch multiRegion(true);
    if (removalDict.found("multiRegionRemoval"))
    {
        multiRegion = removalDict.lookup<bool>("multiRegionRemoval");
    }

    Switch holeDetection(true);
    if (removalDict.found("holeDetection"))
    {
        holeDetection = removalDict.lookup<bool>("holeDetection");
    }

    Switch splitMesh(false);
    if (removalDict.found("splitMesh"))
    {
        splitMesh = removalDict.lookup<bool>("splitMesh");
    }

    Switch extrudeRepair(false);
    if (removalDict.found("extrudeRepair"))
    {
        extrudeRepair = removalDict.lookup<bool>("extrudeRepair");
    }
    ...
    if (multiRegion)
    {
        ...
        if (extrudeRepair)
        {
            ...
            const scalar thickness(extrudeModelDict.lookup<scalar>("thickness"));
        ...
        if (holeDetection)
        {
            ...
            {
                nFaces = removalDict.lookup<label>("nFaces");
            }
    ...
    if (returnReduce(nRemoved, sumOp<label>()) > 0)
    {
        ...
        if (Pstream::parRun())
        {
            ...
            const scalar mergeTol =
                removalDict.lookup<scalar>("mergeTolerance");
