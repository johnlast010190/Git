...
)
{
    ...
    forAllConstIter(typename HashTable<const GeoField*>, fields, iter)
    {
        ...
        forAll(tSubFld().boundaryField(), patchi)
        {
            ...
            {
                tSubFld.ref().boundaryFieldRef()[patchi].forceAssign(
...
)
{
    // Create dummy system/fv*
    {
        ...
        Info<< "Testing: " << io.relativeObjectPath() <<endl;
...
)
{
    ...
    Pstream::listCombineReduce(cellsInZone, plusOp<label>());
...
(
    ...
    const List<wordRe>& excludeZones,
    labelList& zoneID,
    labelList& neiZoneID
)
{
    ...
    forAll(cellZones, zoneI)
    {
        ...
        if (!findStrings(excludeZones, cz.name()))
        {
            forAll(cz, i)
            {
                label celli = cz[i];
        ...
        {
            Info<<"Excluding cellZone: "<<cz.name()<<endl;
        }
    }
...
)
{
    ...
    getZoneID(mesh, cellZones, excludeZones, zoneID, neiZoneID);
    ...
    forAll(regionToZone, regionI)
    {
        ...
        {
            regionNames[regionI] = "domain" + Foam::name(nUnmatched);
            nUnmatched++;
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    (
        "excludeZones",
        "wordReList",
        "a list of zones to exclude - eg '( inlet \".*Wall\" )' "
    );
    argList::addOption
    (
        "zoneToRegionMap",
        "List<Tuple2<word, wordReList>>",
        "map from zones to regions - eg '( (domain1 (z1 z2)) (domain2 (z3 z4)))' "
    );
    ...
    List<Tuple2<word, List<wordRe>>> zoneToRegionMap;
    args.optionReadIfPresent("zoneToRegionMap", zoneToRegionMap);
    ...
    if
    (
        zoneToRegionMap.size()
     && (useCellZones || blockedFacesName.size() || useCellZonesOnly
        || useCellZonesFile || insidePoint || largestOnly || makeCellZones
        || sloppyCellZones || useFaceZones)
    )
    {
        FatalErrorIn(args.executable())
            << "Option -zoneToRegionMap is not compatible with any other"
            << " splitMeshRegion specific options."
            << exit(FatalError);
    }
    ...
    if (args.optionFound("excludeZones"))
    {
        args.optionLookup("excludeZones")() >> excludeZones;

        Info<< "Not including zones " << excludeZones << nl << endl;
    }
    ...
    if (zoneToRegionMap.size())
    {
        Info<< nl << "Using zoneToRegionMap to split mesh into regions:"
            << nl << "Region" << tab << "Zones" << nl
            <<       "------" << tab << "-----" << nl;

        word defaultRegion("default");

        label nValidRegions = 0;
        forAll(zoneToRegionMap, ri)
        {
            word regionName = zoneToRegionMap[ri].first();
            if (regionName == defaultRegion)
            {
                continue;
            }

            Info<< zoneToRegionMap[ri].first()
                 << tab;

            forAll(zoneToRegionMap[ri].second(), zi)
            {
                Info<< zoneToRegionMap[ri].second()[zi] << " ";
            }
            Info<< endl;
            nValidRegions++;
        }

        nCellRegions = nValidRegions;
        regionNames.setSize(nCellRegions);
        regionToZone.setSize(nCellRegions);
        zoneToRegion.setSize(cellZones.size());
        zoneToRegion = -1;
        ...
        forAll(zoneToRegionMap, ri)
        {
            ...
            regionNames[nValidRegions] = regionName;

            regionToZone[nValidRegions] = nValidRegions;
            const List<wordRe>& rZonesRe(zoneToRegionMap[ri].second());

            forAll(cellZones, zoneI)
            {
                const cellZone& cz = cellZones[zoneI];

                if (findStrings(rZonesRe, cz.name()))
                {
                    zoneToRegion[zoneI] = nValidRegions;
                    ...
                    forAll(cz, i)
                    {
                        ...
                        {
                            cellRegion[cellI] = nValidRegions;
                        }
                        else
                        {
                            FatalErrorIn("getZoneID(..)")
                                << "Cell " << cellI << " with cell centre "
                                << mesh.cellCentres()[cellI]
                                << " has been assigned to multiple regions."
                                << " This is not allowed." << endl
                                << "It is in region "
                                << regionNames[cellRegion[cellI]]
                                << " and has now been added to region "
                                << regionNames[nValidRegions]
                                << exit(FatalError);
                        }
                    }
                }
        ...
        //check if any cells have not been assigned to a region, if so add
        //another region called "default" and add all the leftovers to it
        bool addDefaultRegion = false;
        forAll(cellRegion, rI)
        {
            if (cellRegion[rI] == -1)
            {
                cellRegion[rI] = nCellRegions;
                addDefaultRegion = true;
            }
        }
        reduce(addDefaultRegion, orOp<bool>());
        ...
        if (addDefaultRegion)
        {
            ...
            nCellRegions++;
            regionNames.setSize(nCellRegions);
            regionNames[nCellRegions -1] = defaultRegion;
            regionToZone.setSize(nCellRegions);
            regionToZone[nCellRegions -1] = nCellRegions -1;
        }
    }
    else if (useCellZonesOnly)
    {
    ...
    else if (useCellZonesFile)
    {
        ...
        getZoneID(mesh, newCellZones, excludeZones, newZoneID, newNeiZoneID);
    ...
    else
    {
        ...
        if (blockedFacesName.size())
        {
            ...
            Info<< "Read "
                << returnReduceToMaster(blockedFaceSet.size(), sumOp<label>())
    ...
    Pstream::listCombineReduce(regionSizes, sumOp<label>{});

    Info<< "Region\tCells\tName" << nl
        << "------\t-----\t----" << endl;

    forAll(regionSizes, regionI)
    {
        Info<< regionI << '\t' << regionSizes[regionI]
    ...
    else if (makeCellZones)
    {
        ...
        if (insidePoint)
        {
            ...
            Info<< nl << "Found point " << insidePoint << " in cell "
                << cellI << endl;

            if (cellI != -1)
            {
                insideRegionI = cellRegion[cellI];
            }

            reduce(insideRegionI, maxOp<label>());

            Info<< nl
                << "Subsetting region " << insideRegionI
        ...
        for (label regionI = 0; regionI < nCellRegions; regionI++)
        {
            if (insidePoint && (regionI != insideRegionI))
            {
                continue;
            }
