...
)
{
    ...
    Pstream::listCombineReduce(zoneSidePatch, plusOp<label>());
    Pstream::listCombineReduce(zoneZonePatch, plusOp<label>());
}
...
)
{
    ...
    forAll(zoneNames, zoneI)
    {
        ...
        {
            interRegionTopPatch[zoneI] = addPatch<directPolyPatch>
            (
                mesh.boundaryMesh(),
...
)
{
    ...
    forAll(interMeshBottomPatch, zoneI)
    {
        interMeshBottomPatch[zoneI] = oldToNew[interMeshBottomPatch[zoneI]];
    }

    forAll(interMeshTopPatch, zoneI)
    {
        interMeshTopPatch[zoneI] = oldToNew[interMeshTopPatch[zoneI]];
    }
}
...
)
{
    ...
    forAll(zoneSidePatch, zoneI)
    {
        ...
        else if (zoneSidePatch[zoneI] > 0)
        {
            ...
            zoneSidePatch[zoneI] = addPatch<directPolyPatch>
            (
                mesh.boundaryMesh(),
                patchName,
                newPatches
...
{
    argList::addNote
        ("Create region mesh by extruding a faceZone, faceSet or patches");
    ...
    //face source selection is based on presidence faceZone>faceSet>patch
    word faceSource("");
    if (dict.found("faceZones")) faceSource = "zone";
    else if (dict.found("faceSets")) faceSource = "set";
    else if (dict.found("patches")) faceSource = "patch";
    else
    {
        FatalIOErrorInFunction(dict)
            << "No valid face source specified for extrusion. Add 'faceZones'"
            << " 'faceSets' or 'patches' entry."
            << exit(FatalIOError);
    }

    if (faceSource == "zone")
    {
        zoneNames = dict.lookup<wordList>("faceZones");
        dict.readIfPresent("faceZonesShadow", zoneShadowNames);
    }
    else if (faceSource == "set")
    {
        zoneNames = dict.lookup<wordList>("faceSets");
        dict.readIfPresent("faceSetsShadow", zoneShadowNames);
    }
    else if (faceSource == "patch")
    {
        zoneNames = dict.lookup<wordList>("patches");
        dict.readIfPresent("patchesShadow", zoneShadowNames);
    }
    ...
    if (oneD)
    {
        ...
        oneDPatchType = dict.lookup<word>("oneDPolyPatchType");
    }
    ...
    {
        Info<< "Extruding patches " << zoneNames
            << " on mesh " << regionName
            << " into shell mesh " << shellRegionName
            << endl;
    }
    ...
    else if (faceSource == "patch")
    {
        ...
        forAll(zoneNames, i)
        {
            Info<< "Loading patch " << zoneNames[i] << endl;
            patchToFace ptf(mesh, zoneNames[i]);
            zones.set(i, new faceSet(mesh, zoneNames[i], 0));
            ptf.applyToSet
            (
                topoSetSource::ADD,
                zones[i]
        ...
        if (zoneShadowNames.size())
        {
            ...
            forAll(zoneShadowNames, i)
            {
                patchToFace ptf(mesh, zoneShadowNames[i]);
                shadowZones.set(i, new faceSet(mesh, zoneShadowNames[i], 0));
                ptf.applyToSet
                (
                    topoSetSource::ADD,
                    shadowZones[i]
    ...
    Pstream::listCombineReduce(isInternal, orOp<bool>());
    ...
    if (Pstream::parRun())
    {
        //ensure processor patches at end of list
        reorderProcessorPatches(mesh, interMeshTopPatch, interMeshBottomPatch);
    }
