...
#include "polyTopoChange/edgeCollapser/edgeCollapser.H"
...
void createPatch(fvMesh& mesh, const word& name)
{
...
void updateCellSet(const polyTopoChangeMap& map, labelHashSet& cellLabels)
{
    ...
    cellLabels = std::move(newCellLabels);
}
...
int main(int argc, char *argv[])
{
    ...
    word oldInstance = runTimeExtruded.timeName();
    ...
    fileName dictName(runTimeExtruded.system()/"extrudeMeshDict");
    ...
    if (mode == PATCH || mode == MESH)
    {
        ...
        {
            sourceCasePath = args.rootPath()/args.globalCaseName()
                /sourceCasePath;
            sourceCasePath.expand();
        }
        ...
        wordList sourcePatches = dict.lookup<wordList>("sourcePatches");
        ...
        if (mode == PATCH)
        {
            backPatchName = dict.lookup<word>("exposedPatchName");

            createPatch(mesh, backPatchName);
        }
    ...
    if (mergeDim > 0)
    {
        ...
        if (returnReduce(anyChange, orOp<bool>()))
        {
    ...
    {
        runTimeExtruded++;
        Info<< "Writing mesh to " << runTimeExtruded.timeName() << nl << endl;
    }
    else
    {
        mesh.setInstance(oldInstance);
        Info<< "Writing mesh to " << oldInstance << nl << endl;
    }
