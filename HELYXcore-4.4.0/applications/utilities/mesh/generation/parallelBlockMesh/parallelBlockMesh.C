...
Copyright
    ...
    Based on code published on github at
    https://github.com/venugopalansgr/OpenFOAM/tree/master/parallelBlockMesh
    created by
...
Description
    ...
    /HELYXcore/HELYXcore-version/etc/exampleDictionaries/blockMeshDict.parallelBlockMesh

Usage

    - helyxRun -np <nprocs> -parallel parallelBlockMesh
...
#include "mpi.h"
#include "../../manipulation/mergeMeshes/mergePolyMesh.H"

using namespace Foam;

const List<label> sortedPatchVertices(const ITstream&, const label);

class parallelBlockMesh
{
    public:
...
parallelBlockMesh::parallelBlockMesh
(
    word & regionName_,
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
void parallelBlockMesh::createMesh()
{
    if
    (
           meshDict.lookup("blocks")[13] != label(0)
        && meshDict.lookup("blocks")[14] != label(0)
        && meshDict.lookup("blocks")[15] != label(0)
    )
    {
        ...
        if (meshDict.found("mergePatchPairs"))
        {
            ...
            {
                polyMesh& mesh = *this->mesh_;
                #include "../blockMesh/mergePatchPairs.H"
            }
        }
        else
        {
        ...
        if (nZones > 0)
        {
            ...
            forAllConstIter(HashTable<label>, zoneMap, iter)
            {
                ...
                cellSet cset(*mesh_, iter.key(), zoneCells[zoneI].shrink());
                cset.write();
            }

            mesh_->pointZones().setSize(0);
            mesh_->faceZones().setSize(0);
            mesh_->cellZones().setSize(0);
            mesh_->addZones(List<pointZone*>(0), List<faceZone*>(0), cz);
        }
    }
    else
    {
        FatalErrorInFunction
            << "The number of processors exceeds the number of cells "
            << "in the direction in which the mesh is parallelised. "
            << exit(FatalError);
    }
}


void parallelBlockMesh::writeMesh()
{
...
const List<label> sortedPatchVertices
(
    const ITstream& blockBoundaryStream,
    const label patchVerticesStartDistanceFromStreamEnd
)
{
    label blockBoundaryStartIndex =
        blockBoundaryStream.size()-patchVerticesStartDistanceFromStreamEnd;

    List<label> faceVertices
        (
            {blockBoundaryStream[blockBoundaryStartIndex].labelToken(),
             blockBoundaryStream[blockBoundaryStartIndex+1].labelToken(),
             blockBoundaryStream[blockBoundaryStartIndex+2].labelToken(),
             blockBoundaryStream[blockBoundaryStartIndex+3].labelToken()}
        );
...
//    blockMesh::verbose(true);
    ...
    parallelBlockMesh** meshObject = new parallelBlockMesh* [nThreads];
    ...
    const ITstream boundaryStream = (meshDict.lookup("boundary"));

    // Check that the last two patches listed in the blockMeshDict is indeed the
    // two patches normal to the parallelisisation direction

    // When parallelising in the x-direction:
    if
    (
        dotx
     && !(sortedPatchVertices(boundaryStream, 9) == List<label> ({1, 2, 5, 6}))
    )
    {
        if (Pstream::master())
        {
            FatalErrorInFunction
                << "Patches in the wrong order for use with "
                << "parallelBlockMesh. When parallelising in the "
                << "x-direction, the patch normal to the x-axis with "
                << "larger x-coordinate values (vertices numbers "
                << "(2 6 5 1)) should be listed last in the 'boundary' "
                << "section of blockMeshDict."
                << exit(FatalError);
        }
        else
        {
    ...
    (
        dotx
     && !(sortedPatchVertices(boundaryStream, 25) == List<label> ({0, 3, 4, 7}))
    )
    {
        if (Pstream::master())
        {
            FatalErrorInFunction
                ...
                << "parallelBlockMesh. When parallelising in the "
                << "x-direction, the patch normal to the x-axis with "
                << "smaller x-coordinate values (vertices numbers "
                << "(0 4 7 3)) should be listed second to last in the "
                << "'boundary' section of blockMeshDict."
                << exit(FatalError);
        }
        else
        {
    ...
    // When parallelising in the y-direction:
    else if
    (
        doty
     && !(sortedPatchVertices(boundaryStream, 9) == List<label> ({2, 3, 6, 7}))
    )
    {
        if (Pstream::master())
        {
            FatalErrorInFunction
                << "Patches in the wrong order for use with "
                << "parallelBlockMesh. When parallelising in the "
                << "y-direction, the patch normal to the y-axis with "
                << "larger y-coordinate values (vertices numbers "
                << "(3 7 6 2)) should be listed last in the 'boundary' "
                << "section of blockMeshDict."
                << exit(FatalError);
        }
        else
    ...
    (
        doty
     && !(sortedPatchVertices(boundaryStream, 25) == List<label> ({0, 1, 4, 5}))
    )
    {
        if (Pstream::master())
        {
            FatalErrorInFunction
                ...
                << "parallelBlockMesh. When parallelising in the "
                << "y-direction, the patch normal to the y-axis with "
                << "smaller y-coordinate values (vertices numbers "
                << "(1 5 4 0)) should be listed second to last in the "
                << "'boundary' section of blockMeshDict."
                << exit(FatalError);
        }
        else
    ...
    // When parallelising in the z-direction:
    else if
    (
        dotz
     && !(sortedPatchVertices(boundaryStream, 9) == List<label> ({4, 5, 6, 7}))
    )
    {
        if (Pstream::master())
        {
            FatalErrorInFunction
                << "Patches in the wrong order for use with "
                << "parallelBlockMesh. When parallelising in the "
                << "z-direction, the patch normal to the z-axis with "
                << "larger z-coordinate values (vertices numbers "
                << "(4 5 6 7)) should be listed last in the 'boundary' "
                << "section of blockMeshDict."
                << exit(FatalError);
        }
        else
    ...
    (
        dotz
     && !(sortedPatchVertices(boundaryStream, 25) == List<label> ({0, 1, 2, 3}))
    )
    {
        if (Pstream::master())
        {
            FatalErrorInFunction
                ...
                << "parallelBlockMesh. When parallelising in the "
                << "z-direction, the patch normal to the z-axis with "
                << "smaller z-coordinate values (vertices numbers "
                << "(0 3 2 1)) should be listed second to last in the "
                << "'boundary' section of blockMeshDict."
                << exit(FatalError);
        }
        else
...
//    for (int i=iStart+1;i<L1.size();i++)
    for (int i=iStart+1;i<iEnd;i++)
    {
        ...
        scalar locr =
        (::fabs(locRatio - 1.0) < 1e-4)
      ? 1.0 : Foam::pow(locRatio,(1.0/(locNumCells - 1)));
        label oldSize = zvalues.size();
        scalar locA =
        (::fabs(locRatio - 1.0) < 1e-4)
      ? locDt/locNumCells : (locDt*(1 - locr))/(1 - pow(locr,locNumCells));
    ...
    scalar localStartX, localStartY, localStartZ;
    scalar localEndX, localEndY, localEndZ;
    ...
    {
        scalar evalr =
            (zLocal[indCnt] - zLocal[indCnt-1])
          / (zLocal[indCnt-1] - zLocal[indCnt-2]);
...
//    Info<< "Amended stream now is = " << is << endl;
    ...
    meshObject[myRank] = new parallelBlockMesh(regionNameLocal, meshDict,runTime, myRank);
    ...
    label perCellCount = meshObject[myRank]->mesh_->nCells();
    label perFaceCount = meshObject[myRank]->mesh_->nFaces();
    label perPointCount = meshObject[myRank]->mesh_->nPoints();
