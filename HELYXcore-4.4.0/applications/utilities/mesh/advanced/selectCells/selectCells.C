...
#ifdef HELYX_USE_OPENVDB
        const VDBTree& tree = querySurf.tree();
        Info<<"   VDBtree search" << endl;
#else
        const indexedOctree<treeDataTriSurface>& tree = querySurf.tree();
        Info<<"   Octree search" << endl;
#endif
...
int main(int argc, char *argv[])
{
    ...
    bool useSurface(refineDict.lookup<bool>("useSurface"));
    bool selectCut(refineDict.lookup<bool>("selectCut"));
    bool selectInside(refineDict.lookup<bool>("selectInside"));
    bool selectOutside(refineDict.lookup<bool>("selectOutside"));
    ...
    forAll(outsidePts, outsideI)
    {
        ...
        if (returnReduceToMaster(celli, maxOp<label>()) == -1)
        {
            FatalErrorInFunction
    ...
    if (useSurface)
    {
        ...
        // Search engine on surface.
        querySurf.reset(new triSurfaceSearch(surf, refineDict));
