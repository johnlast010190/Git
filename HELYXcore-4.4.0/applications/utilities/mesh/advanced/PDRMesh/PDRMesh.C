    ...
    )
    {
        ...
        forAll(fieldBf, patchi)
        {
            if (map.oldPatchSizes()[patchi] == 0)
            {
                ...
                {
                    fieldBf[patchi].forceAssign(initValue);
                }
            }
        }
    }
...
label findPatch(const polyBoundaryMesh& patches, const word& patchName)
{
    ...
    // Check same patch for all procs
    {
        ...
        reduceToMaster(newPatch, minOp<label>());

        HELYX_ASSERT(newPatch == patchi) {
            FatalErrorInFunction
                << "Patch " << patchName
...
int main(int argc, char *argv[])
{
    ...
    autoPtr<polyTopoChangeMap> map = meshMod.changeMesh(subsetter.subMesh(), false);
    ...
    topoSet::topoChange(subsetter.subMesh().facesInstance(), map, cellSets);
    topoSet::topoChange(subsetter.subMesh().facesInstance(), map, faceSets);
    topoSet::topoChange(subsetter.subMesh().facesInstance(), map, pointSets);
