...
Description
    Generate Abaqus pressure loads mapping Helyx results (based on AMI).
    ...
    -flipNorm Abaqus and Helyx normals need to point in opposite directions
...
(
    ...
    autoPtr<AMIInterpolation> amiInterpolatePtr,
    const labelList& eid,
    PtrList<GeoField>& fields,
...
(
    ...
    const word& sourcePatchName,
    const scalar& loadDir,
    const AMIInterpolation& amiInterpolate,
    const labelList& eid,
    GeoField& field,
    const fileName& abaqusDir,
    simpleVTKWriter& sourceVTK,
...
int main(int argc, char *argv[])
{
    ...
    (
        "sourceSurface",
        "source patch from Helyx or Elements case"
    );
    argList::validOptions.insert
    (
        "fields",
...
#include "include/createMesh.H"
    ...
    autoPtr<AMIInterpolation> amiInterpolatePtr(nullptr);
    std::map<word,KNN> bndMaps;
    std::map<word,std::vector<label>> faceIdMaps;
