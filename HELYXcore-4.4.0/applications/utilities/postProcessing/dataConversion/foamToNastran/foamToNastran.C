...
)
{
    ...
    IFstream OBJfile(typeFilePath<searchableSurface>(surfaceIO()));
    ...
    // 0. opening data output file in ascii format
      ...
      if (!ofs)
      {
       ...
       if (cellScalars.size())
       {
            ...
            {
                std::cerr
                    << " ***ERROR:foamToNastran::writeVTKModified()(2): "
                    << "wrong dimension of provided scalar field ="
                    << cellScalars.size() << " vs ele=" << nelements
                    << std::endl;
               return;
           }
...
)
{
    ...
    if (isWritePoints)
    {
        if (someScalars.size())
        {
            ...
            {
                std::cerr
                    << " ***ERROR:foamToNastran::writeVTKModified()(3): "
                    << "wrong dimension of provided scalar field ="
...
)
{
    ...
    forAll(faces, i)
    {
        ...
        {
            std::cerr
                << " ***ERROR:foamToNastran::writeSTLModified(): "
...
)
{
    ...
    {
        std::cerr
            << " foamToNastran::readPrevPressureComputeError(): Input "
...
)
{
    ...
    {
        std::cerr
            << " foamToNastran::readPrevPointsComputeDist(): "
            << "Input file " << infile << " could not be opened."
            << std::endl;
        return;
    }
    ...
    {
        std::cerr
            << " foamToNastran::readPrevPointsComputeDist(): point "
    ...
    for (unsigned int i = 0; i < nnodes; i++)
    {
        ...
        dist =
            sqrt
            (
                (x - allPoints[i][0])*(x - allPoints[i][0])
...
)
{
    ...
    const scalar dotProduct = triNormal & vect;
    ...
    {
        const scalar time =
            -(triNormal & (linept - inputTri.a()))/dotProduct;

        // Here ther is a lot of problems, related what is the orientation,
        // what starts where, etc.
        ipoint = linept + vect*time;

        Foam::triangle<point, const point&> prom0
        (
            inputTri.a(),
            inputTri.b(),
            ipoint
        );
        Foam::triangle<point, const point&> prom1
        (
            inputTri.b(),
            inputTri.c(),
            ipoint
        );
        Foam::triangle<point, const point&> prom2
        (
            inputTri.c(),
            inputTri.a(),
            ipoint
        ...
        if
        (
            (prom0.areaNormal() & triNormal) < 0.0
         && (prom1.areaNormal() & triNormal) < 0.0
         && (prom2.areaNormal() & triNormal) < 0.0
        )
        {
            return true;
        }
...
int main(int argc, char *argv[])
{
    ...
    // Exclude 0 by default in time options unless -withZero specified
    ...
    {
        unsteady = nastranDict.lookup<bool>("unsteady");
    }
    else
    {
    ...
    forAll(targetDicts, i)
    {
        ...
        {
            targetBackPressure[i] = dict.lookup<scalar>("backPressure");
        }
        else
        {
    ...
    forAll(sourceFileNames, i)
    {
        ...
        {
            FatalError << "Source surface file "
                       << sourceFileNames[i] << " is not a "
                       << "nastran format file."
                       << exit(FatalError);
        }
    }
   ...
   // Setup indirectPrimitivePatch interpolation
    ...
    // Transport properties header
    IOobject transportPropertiesHeader
    (
        "transportProperties",
        runTime.constant(),
        mesh,
    ...
    autoPtr<IOdictionary> transportProperties
    (
        transportPropertiesHeader.headerOk()
      ? new IOdictionary(transportPropertiesHeader)
      : nullptr
    );

    dimensionedScalar rho
    (
        transportProperties.valid()
      ? dimensionedScalar
        (
            "rho",
            dimDensity,
    ...
    if (isSourceSurfacesPresent)
    {
        Info<< nl << " WARNING: foamToNastran::main(): perfoming SURFACE "
        ...
        forAll(surfaceFileNames, surfI)
        {
            ...
            if (Pstream::master() || !Pstream::parRun())
            {
                ...
                forAll(targetOutputPIDs[surfI], i)
                {
                    ...
                    fileName ploadFileName =
                        targetFileNames[surfI].lessExt()
                      + "_pid" + name(pid)
                      + "_time" + Times[timeI].name()
                      + "." + "pload";
                    if (ldebug)
                    {
                        Info<< "Writing PLOAD file: " << ploadFileName << endl;
                    }
                    if (Pstream::parRun())
                    {
                        ploadFileName =
