...
#include "include/createTime.H"
    ...
    (
        "fuel",
        thermo(mesh, thermoData.subDict(fuelName))
    );
    Info<< "fuel " << FUEL << ';' << endl;
    FUEL *= FUEL.W();
    ...
    (
        "O2",
        thermo(mesh, thermoData.subDict("O2"))
    );
    O2 *= O2.W();
    ...
    (
        "N2",
        thermo(mesh, thermoData.subDict("N2"))
    );
    N2 *= N2.W();
    ...
    (
        "CO2",
        thermo(mesh, thermoData.subDict("CO2"))
    );
    CO2 *= CO2.W();
    ...
    (
        "H2O",
        thermo(mesh, thermoData.subDict("H2O"))
    );
    H2O *= H2O.W();
