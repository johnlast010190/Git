...
)
{
    switch (rType)
    {
        case irreversible:
        {
            reactions_.append
            (
                new IrreversibleReaction<dummyThermo, ReactionRateType>
                (
                    ReactionProxy<dummyThermo>
                    (
                        speciesTable_,
                        speciesThermoList(),
                        lhs.shrink(),
        ...
        case reversible:
        {
            reactions_.append
            (
                new ReversibleReaction<dummyThermo, ReactionRateType>
                (
                    ReactionProxy<dummyThermo>
                    (
                        speciesTable_,
...
{
    Reaction<dummyThermo>::TlowDefault = 0;
    Reaction<dummyThermo>::ThighDefault = GREAT;
    ...
    // Use binary mode in case we read binary.
    // Causes windows reading to fail if we don't.
    std::ifstream CHEMKINStream
    (
        CHEMKINFileName.c_str(),
        ios_base::in|ios_base::binary
    );
#else
