...
)
{
    ...
    (
        ...
        (
            ...
            clone(vdbGrids.points()),
            std::move(vdbGrids.faces()),
            std::move(vdbGrids.owner()),
            std::move(vdbGrids.neighbour())
        )
    );
...
int main(int argc, char *argv[])
{
    ...
    const word dictName("helyxVDBMapDict");
    ...
    if (samplingBox)
    {
        ...
        if (boxDict.found("sampleGeometry") && Pstream::master())
        {
            ...
            helyxVDB ovdb(minEdge, maxLevel);
    ...
    if (mapDict.found("sourceTime"))
    {
        ...
        {
            scalar sourceTimeScalar = mapDict.lookup<scalar>("sourceTime");
    ...
    if (samplingBox && visualization && Pstream::master())
    {
        ...
        helyxVDB ovdb(outputVoxelSize, /*maxLevel*/0);
