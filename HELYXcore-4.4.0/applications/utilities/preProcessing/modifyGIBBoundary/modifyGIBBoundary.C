...
int main(int argc, char *argv[])
{
    ...
    if (args.optionFound("addGIB")||args.optionFound("removeGIB"))
    {
        ...
        {
            mesh.setInstance(runTime.time().constant());
        }
        ...
        if (args.optionFound("addGIB"))
        {
            ...
            // Check if the multi-motion dynamic library is active
            ...
            IOdictionary cGIBDict =
                readOrCreateDictionary(runTime, args, meshWithChangers);
            ...
            //- add boundary
            {
                ...
                if (initType == "dynamicGIBFvMesh")
                {
                    // Legacy dynamic library
                    if (meshWithChangers)
                    {
                        FatalErrorInFunction
                            << "This GIB initialization is not compatible with "
                            << "the new dynamic mesh structure using changers."
                            << nl << "Please set the 'meshChangers' flag to "
                            << "'false' in the controlDict dictionary or use "
                            << "the 'GIBChanger' initialization type instead."
                            << exit(FatalError);
                    }
                    ...
                    const auto ctor =
                        ctorTableLookup
                        (
                            "dynamicGIBFvMesh type",
                            dynamicGIBFvMesh::IOobjectConstructorTable_(),
                    ...
                    autoPtr<dynamicGIBFvMesh> dGIBMesht(ctor(io, dInit));
                    ...
                    if (args.optionFound("constant"))
                    {
                        dGIBMesh.setInstance(runTime.time().constant());
                    }
                ...
                else if (initType == "GIBChanger")
                {
                    // Multi-motion dynamic library
                    if (!meshWithChangers)
                    {
                        FatalErrorInFunction
                            << "This GIB initialization is not compatible with "
                            << "the legacy dynamic mesh structure."
                            << nl << "Please set the 'meshChangers' flag to "
                            << "'true' in the controlDict dictionary and use "
                            << "the new multi-motion dynamic library instead."
                            << exit(FatalError);
                    }

                    const word changerType(dInit.lookup("type"));

                    Info<< "\nCreating fvMeshGIBChanger " << changerType
                        << " for the GIB initialization" << endl;

                    libs.open(dInit, "libs");

                    const auto ctor =
                        ctorTableLookup
                        (
                            "fvMeshGIBChanger",
                            fvMeshGIBChanger::fvMeshConstructorTable_(),
                            changerType
                        );

                    if (args.optionFound("constant"))
                    {
                        mesh.polyMesh::instance() = runTime.time().constant();
                    }

                    autoPtr<fvMeshGIBChanger> GIBChangerPtr(ctor(mesh, dInit));
                    fvMeshGIBChanger& GIBChanger = GIBChangerPtr();

                    fvMesh& thisMesh = GIBChanger.mesh();

                    GIBChanger.updateInit(faceZoneName);

                    if (args.optionFound("constant"))
                    {
                        thisMesh.setInstance(runTime.time().constant());
                    }

                    thisMesh.write();
                    thisMesh.boundaryMesh().write();

                    if (dInit.found("exportGeometry"))
                    {
                        const word surfaceName = dInit.lookup("exportGeometry");
                        GIBChanger.writeGeometry(surfaceName);
                    }

                    rm(thisMesh.phi().filePath());

                    if (args.optionFound("changeFields"))
                    {
                        Info<< "Changing boundaryFields:" << endl;
                        const dictionary bfs =
                            cGIBDict.subDict("boundaryFields");

                        forAllConstIter(IDLList<entry>, bfs, iter)
                        {
                            word name(iter().keyword());
                            dictionary bf = iter().dict();
                            addGIBBC<scalar>(bf, name, mesh, thisMesh);
                            addGIBBC<vector>(bf, name, mesh, thisMesh);
                            addGIBBC<sphericalTensor>(bf, name, mesh, thisMesh);
                            addGIBBC<symmTensor>(bf, name, mesh, thisMesh);
                            addGIBBC<tensor>(bf, name, mesh, thisMesh);
                        }
                    }
                }
                else //- no initialization
                {
                    ...
                    if (dInit.found("exportGeometry"))
                    {
                        ...
                        MeshedSurface<face> surf(mpp, true);
                        ...
                        if (Pstream::master())
                        {
                            ...
                            {
                                Info<< "Path to surface does not exist." << nl
                                    << "Creating path: "
                                    << globalCasePath.path() << endl;
