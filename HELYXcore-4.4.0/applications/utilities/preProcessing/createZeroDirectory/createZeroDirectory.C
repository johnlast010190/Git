...
const word getClassType(const word& pType)
{
    ...
    {
        return VolField<scalar>::typeName;
    }
    else if (pType == pTraits<vector>::typeName)
    {
        return VolField<vector>::typeName;
    }
    else if (pType == pTraits<sphericalTensor>::typeName)
    {
        return VolField<sphericalTensor>::typeName;
    }
    else if (pType == pTraits<symmTensor>::typeName)
    {
        return VolField<symmTensor>::typeName;
    }
    else if (pType == pTraits<tensor>::typeName)
    {
        return VolField<tensor>::typeName;
    }
    else
    {
        // Error
...
int main(int argc, char *argv[])
{
    ...
    fileName baseDir
    (
        "${HELYX_PROJECT_DIR}/etc/caseDicts/preProcessing/createZeroDirectoryTemplates"
    );
