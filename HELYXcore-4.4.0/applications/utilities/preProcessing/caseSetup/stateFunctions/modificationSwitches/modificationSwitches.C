...
:
    ...
    sequentialBoundaryMeshWrite_(false)
{}
...
:
    ...
    sequentialBoundaryMeshWrite_(ms.sequentialBoundaryMeshWrite_)
{
    merge(dict);
}
...
void Foam::modificationSwitches::merge(const dictionary& dict)
{
    if (dict.found("modificationSwitches"))
    {
        ...
        sequentialBoundaryMeshWrite_
            = msd.lookupOrDefault<Switch>
            ("sequentialBoundaryMeshWrite", sequentialBoundaryMeshWrite_);
    }
}
