...
dictionary stateFunction::etcDictionary
(
    const fileName& fn,
    bool mustfind
)
...
{
    // Insert buoyant settings if they are available
    ...
    // Do a preliminary merge of materialProperties defaults to get defaults
    // from the state that may affect buoyancy
    dictionary matInputDict = defaults.subOrEmptyDict("materialProperties");
    matInputDict.merge
    (
        materials.size() == 1
      ? input.subOrEmptyDict("materialProperties").subOrEmptyDict
        (
            materials.first()
        )
      : input.subOrEmptyDict("materialProperties")
    );

    if (materials.size())
    {
        isMultiphase = (phaseNames().size() > 1);
        ...
        (
            matInputDict.isDict(materials.first())
         && matInputDict.subDict(materials.first()).found("buoyancyModel")
        )
        {
...
dictionary stateFunction::readDictionaries
(
...
)
{
    ...
    autoPtr<stateFunction> stateFn
    (
        ctor(name, input, defaults, master, index, meshName)
    );
    stateFn->initialMerge(input, defaults, master);
    return stateFn;
}
...
void stateFunction::initialMerge
(
    const dictionary& input,
    const dictionary& defaults,
    const stateFunction& master
...
{
    //assemble input components (not for global state)
    if (input.found("state"))
    {
...
void stateFunction::finalise()
{
    ...
    {
        const dictionary& inputSys = input_.subDict("system");

        // Merge together the 'libs' entries for input and state, instead of
        // overwriting with input
        fileNameList allLibs;
        if
        (
            inputSys.found("controlDict")
         && inputSys.subDict("controlDict").found("libs")
        )
        {
            allLibs =
                fileNameList(inputSys.subDict("controlDict").lookup("libs"));
        }
        if
        (
            system().found("controlDict")
         && system().subDict("controlDict").found("libs")
        )
        {
            allLibs.append
            (
                fileNameList(system().subDict("controlDict").lookup("libs"))
            );
        }
        ...
        if (allLibs.size())
        {
            system().subDict("controlDict").add("libs", allLibs, true);
        }
    }
