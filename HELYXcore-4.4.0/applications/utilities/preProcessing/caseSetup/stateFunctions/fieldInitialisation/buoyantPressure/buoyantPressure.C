...
void Foam::fieldInitializations::buoyantPressure::correct()
{
    ...
    if (basicThermo::dictName == basicThermo::matDictName)
    {
        ...
        if (!thermoPtr->buoyant())
        {
            FatalErrorInFunction
                << "Buoyancy model is not activated in materialProperties"
                << nl << exit(FatalError);
        }
        distinctBuoyancy = thermoPtr->distinctBuoyancy();
    }

    if (initDict().found("value"))
    {
        // Pre-initialise with supplied value in order to bootstrap the density
        // calculation
        p_rgh.primitiveFieldRef() =
            scalarField(word("value"), initDict(), p_rgh.size());
        initCoupledBoundaries();
    }
    else if (!thermoPtr->isochoric())
    {
        IOWarningInFunction(initDict())
            << "No 'value' entry specified for starting iteration of "
    ...
    word UName = initDict().lookupOrDefault<word>("UName", "U");
    ...
    {
        // Try phasic versions of each phase
        if (isA<multiphaseThermo>(*thermoPtr))
        {
            const wordList& phaseNames =
                refCast<multiphaseThermo>(*thermoPtr).phases();
            for (const word& phaseName : phaseNames)
            {
                word phasicUName = IOobject::groupName(UName, phaseName);
                const volVectorField* UPtr =
                    localDb().lookupObjectPtr<volVectorField>(phasicUName);
                if (!UPtr)
                {
                    FatalErrorInFunction
                        << "No U field named " << UName << " or " << phasicUName
                        << " was found." << exit(FatalError);
                }

                surfaceScalarField* phivPtr =
                    new surfaceScalarField
                    (
                        IOobject
                        (
                            IOobject::groupName("phiv", phaseName),
                            runTime.timeName(),
                            localDb(),
                            IOobject::READ_IF_PRESENT,
                            IOobject::AUTO_WRITE
        ...
        {
            // Error out with a logical message
            localDb().lookupObjectPtr<volVectorField>(UName);
        }
    }
    ...
    for (label i=0; i<nNonOrthCorr; i++)
    {
        ...
        if (isA<multiphaseThermo>(*thermoPtr))
        {
            multiphaseThermo& mpThermo = refCast<multiphaseThermo>(*thermoPtr);
            for (basicThermo& phaseThermo : mpThermo.thermos())
            {
                phaseThermo.he().forceAssign
                (
                    phaseThermo.he(p, phaseThermo.T())
                );
            }
        }
        thermoPtr->correct();
