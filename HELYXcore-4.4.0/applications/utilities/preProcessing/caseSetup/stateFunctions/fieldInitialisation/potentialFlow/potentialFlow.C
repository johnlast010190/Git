...
) const
{
    ...
    if (pPtr->dimensions() == dimensionSet(1, -1, -2, 0, 0, 0, 0))
    {
        ...
        {
            rhoRef.value() = potDict.lookup<scalar>("rhoRef");
    ...
    // Create a dummy 'psi' variable for total pressure boundaries.
    // Value = 0 since incompressibility is inherent in the potential flow
    // assumption
    volScalarField psi
    (
        "psi",
    ...
    if (pTotPSystem)
    {
        ...
        wordList pcorrTypes(pPtr().boundaryField().types());
    ...
    dictionary fvSchemes = refMesh.schemes().localSchemeDict();
    {
        ...
        char pPotLapl[] = "Gauss linear limited 0.333";

        fvSchemes.subDict("laplacianSchemes").add
        (
            word("laplacian(1,pPot)"),
            pPotLapl,
            false
        );
        fvSchemes.subDict("laplacianSchemes").add
        (
            word("laplacian((1|A(U)),pPot)"),
            pPotLapl,
            false
        );
    ...
    for (label corr = 0; corr < nCorrectors; corr++)
    {
        ...
        for (int nonOrth=0; nonOrth<=nNonOrthCorr; nonOrth++)
        {
            fvScalarMatrix pEqn
            (
                fvm::laplacian(rUA, pPtr()) == fvc::div(phi)
            );
        ...
        const scalar phiInterpolError =
            (
                sqrt(sum(sqr((fvc::interpolate(UPtr()) & mesh().Sf()) - phi)))
                /sum(mesh().magSf())
            ).value();

        Info<< "   Interpolated Velocity error = " << phiInterpolError << endl;
    }
...
void Foam::fieldInitializations::potentialFlow::correct()
{
    ...
    if (potDict.lookupOrAddDefault<Switch>("initMaterials", false))
    {
        // Construct thermo in case some thermo variables are needed.
        // Not a generic solution requires mass and volume fractions
        // to be already written down, otherwise thermo will not calculate
        // the properties correctly
        const word phaseName =
            initDict().lookupOrDefault<word>("phaseName", "");
    ...
    {
        volVectorField& f =
            localDb().lookupObjectRef<volVectorField>(name());
        ...
        {
            blThickness.value() =
                initDict().lookup<scalar>("boundaryLayerThickness");
        }
        ...
        forAll(f, cellI)
        {
            ...
            {
                f[cellI] =
                    f[cellI]*Foam::pow(d[cellI]/blThickness.value(), (1.0/7.0));
            }
        }
    ...
    {
        volScalarField& f =
            localDb().lookupObjectRef<volScalarField>(name());
        const volScalarField& pPot =
            localDb().lookupObject<volScalarField>("pPot");

        f.primitiveFieldRef() = pPot.primitiveField();

        forAll(f.boundaryField(), pI)
        {
            f.boundaryFieldRef()[pI].fvPatchScalarField::operator=
            (
                fvPatchScalarField(pPot.boundaryField()[pI])
            );
        }
    }
