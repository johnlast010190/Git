...
void regionState::clearNonConformalSetup(const bool initFields)
{
    ...
    {
        // Return early for conformal meshes. Needed by the HELYX GUI.
    ...
    forAllConstIter(dictionary, fieldDef, iter)
    {
        ...
        forAllIter(dictionary, cbcDictRef, iter)
        {
            ...
            (
             ...
             || pfType == nonConformalDiscreteMixingFvPatch::typeName
            )
            {
    ...
    if (initFields)
    {
        // Write down the mesh after cleaning non-conformal patches.
        // Needed by the HELYX GUI.
        ...
        // Exclude CBC entries for removed non-conformal patches.
        // Needed by the HELYX GUI.
        forAllConstIter(dictionary, fieldDef, iter)
        {
            const word fieldName(iter().keyword());
            ...
            forAllIter(dictionary, cbcDict, iter)
            {
                const entry& e = iter();
                const word pfType(e.dict().lookup("type"));
                ...
                {
                    cbcDict.remove(e.keyword());
                }
            }

            cbcDict.regIOobject::write();
        }
    }
}


void regionState::writeAMITransforms()
{
    const polyBoundaryMesh& patches = mesh().boundaryMesh();
    forAll(patches, patchi)
    {
        ...
        if (!isA<cyclicAMIPolyPatch>(pp)) continue;

        // Write down complete AMI transforms
        {
            if (distributed_ || collated_)
            {
                mesh().boundaryMesh().write();
...
dictionary regionState::initialPatchDict(const label& patchI) const
{
    ...
    // Type entry
    tpd.add("type", polyPatches[patchI].type(), true);

    // Size entry
    tpd.add("nFaces", polyPatches[patchI].size(), true);

    // Start entry
    tpd.add("startFace", polyPatches[patchI].start(), true);
...
{
    word bType = patchDict.lookup<word>("type");
    ...
    if (pType != "" && bType == "patch")
    {
        ...
        {
            patchDict.add<word>("physicalType", pType, true);
            patchDict.add<word>("type", bType, true);
        }
        else
        {
            FatalError << pType
               << " is a currently unsupported physicalType for patch "
               << "boundaries. Currently supported physicalTypes are: "
...
)
{
    ...
    Info<< "Modifying boundary patch: " << polyPatches[patchI].name() << endl;
    ...
    const word patchName =
        patchDict.lookupOrDefault<word>("name", polyPatches[patchI].name());
    patchDict.remove("name");

    polyPatches.set
    (
        patchI,
        polyPatch::New(patchName, patchDict, patchI, polyPatches)
    );

    fvPatches.set
    (
        patchI,
        fvPatch::New(polyPatches[patchI], mesh.boundary())
    );
}
...
)
{
    ...
    forAllConstIter(dictionary, bmDict, iter)
    {
        if (iter().isDict() && !iter().keyword().isPattern())
        {
            ...
            {
                newNamePatchi = bmesh.findPatchID(indict.lookup<word>("name"));
            ...
            if (patchi != -1)
            {
                ...
                {
                    dictionary mergedDict(patchDefaults.subDict(boundaryType));
                    mergedDict.merge(patchDict); // overwrites existing
                    patchDict.transfer(mergedDict);
                }

                modifyBoundaryPatch(mesh(), patchDict, patchi);
    ...
    if (bmDict.size())
    {
        ...
        )
        {
            ...
            if (e.isDict() && !e.keyword().isPattern())
            {
                ...
                (
                    bmesh.findIndices(e.keyword(), true)
                );
...
template<>
void regionState::storeInternalField(const word& name, const Field<scalar>& v)
{
    scalarFields_.append(new Tuple2<word, Field<scalar>>(name, v));
}
...
{
    vectorFields_.append(new Tuple2<word, Field<vector>>(name, v));
}
...
{
    tensorFields_.append(new Tuple2<word, Field<tensor>>(name, v));
}
...
{
    symmTensorFields_.append(new Tuple2<word, Field<symmTensor>>(name, v));
}
...
)
{
    ...
    (
        new Tuple2<word, Field<sphericalTensor>>(name, v)
    );
}
...
{
    typedef VolField<scalar> GeoField;

    if (localDb().registry().foundObject<GeoField>(name))
    {
        GeoField& f = localDb().registry().lookupObjectRef<GeoField>(name);
...
{
    typedef VolField<vector> GeoField;
...
{
    typedef VolField<tensor> GeoField;
...
{
    typedef VolField<symmTensor> GeoField;
...
{
    typedef VolField<sphericalTensor> GeoField;
...
void regionState::readInitialFields()
{
    ...
    forAll(fnList, i)
    {
        ...
        fieldType ft(stateFunction::fieldTypeNames_[word(fd.lookup("type"))]);
        ...
        switch (ft)
        {
            ...
            {
                FatalErrorInFunction
                    << "Invalid field type: '" << word(fd.lookup("type"))
                    << "'" << exit(FatalError);
            }
        }
    }
    ...
    forAllConstIter(dictionary, fieldDef, iter)
    {
        ...
        else if (!switches().resetInternalFields())
        {
            ...
            if (initialFieldBoundaryDictsPtr_->found(fieldName))
            {
                ...
                dictionary& newFieldBCs = fd.subDict("boundaryConditions");
        ...
        fieldType ft(stateFunction::fieldTypeNames_[fd.lookup("type")]);
...
void regionState::initialiseFields()
{
    ...
    for (label pr = 1; pr <= 100; pr++)
    {
        forAll(fIS, i)
        {
            if
            (
                fIS[i].initDict().lookupOrDefault<scalar>("priority", 10) == pr
            )
            {
...
{
    const word timeInstance = mesh().time().timeName();
...
void regionState::clearInstanceFields()
{
    ...
    forAll(localDb().registry().names(), nameI)
    {
        if
        (
            localDb().registry().foundObject<regIOobject>
            (
                localDb().registry().names()[nameI]
            )
        )
        {
            const word name = localDb().registry().names()[nameI];
    ...
    for (auto const& i : namesToCheckout)
    {
        ...
        localDb().registry().checkOut
        (
            localDb().registry().lookupObjectRef<regIOobject>(i)
        );
    }
    scalarFields_.clear();
    vectorFields_.clear();
...
void regionState::createObjects(const Time& runTime)
{
    ...
    solutionRegPtr_.set
    (
        new fvSolutionRegistry(runTime, *meshPtr_, regionName())
    );
    ...
    // If AMI patches are present, make sure that all transforms are fully
    // defined and written to disk. It is a helper workaround function for the
    // HELYX GUI, but this workflow should be refactored in the future.
    writeAMITransforms();
}
...
void regionState::modifyMesh(scalar writePause)
{
    ...
    {
        boolList modifiedPatch(mesh().boundaryMesh().size(), false);
        modifyBoundaryMesh(defaults(), modifiedPatch);

        if (!modificationSwitches().sequentialBoundaryMeshWrite() || distributed_ || collated_)
        {
...
void regionState::createFields()
{
    ...
    // HELYX GUI to be able to clean previous NCC setups, but this workflow
    ...
    // Create fields
    {
        ...
        {
            Info<< nl << nl << "Boundary conditions" << nl
                <<             "-------------------" << endl;
        }
