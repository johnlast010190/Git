...
Description
    Geometric population of a helyxHexMeshDict.
...
#include "include/createTime.H"
    ...
    fileName baseDir
    (
        "${HELYX_PROJECT_DIR}/etc/caseDicts/preProcessing/createCase"
    );

    if (isFile(runTime.time().system()/"helyxHexMeshDict") && overwrite)
    {
        //Make a backup of helyxHexMeshDict
        cp
        (
            runTime.time().system()/"helyxHexMeshDict",
            runTime.time().system()/"helyxHexMeshDict.backup"
        );
        // copy a templated helyxHexMeshDict

        cp
        (
            baseDir + "/createCase.helyxHexMeshDict",
            runTime.time().system()/"helyxHexMeshDict"
        );
    }
    else if (!isFile(runTime.time().system()/"helyxHexMeshDict") || overwrite)
    {
        //helyxHexMeshDict
        cp
        (
