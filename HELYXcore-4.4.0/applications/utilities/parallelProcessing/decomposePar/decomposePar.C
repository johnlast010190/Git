...
Usage
    ...
    Options:
      ...
      - \par -linkUniform
        Link any \a uniform directories instead of copying them.
...
)
{
    ...
    if (fileHandler().isDir(runTime.timePath()/uniformDir))
    {
        ...
        if (linkUniform && !distributeUniform)
        {
...
int main(int argc, char *argv[])
{
    ...
    (
        "linkUniform",
        "link any uniform/ directories instead of copying them"
    );
    argList::addBoolOption
    (
        "fields",
    ...
    bool linkUniform             = args.optionFound("linkUniform");
    ...
    fileName decomposeDictFile;
    args.optionReadIfPresent("decomposeParDict", decomposeDictFile);

    // Set time from database
    Info<< "Create time\n" << endl;
    processorRunTimes runTimes(Time::controlDictName, args, decomposeDictFile);
    ...
    forAll(regionNames, regioni)
    {
        ...
        (
            ...
            (
                ...
                (
                    ...
                    (
                        "decomposeParDict",
                        runTimes.completeTime().time().system(),
        ...
        (
            decomposeParDict.lookup("numberOfSubdomains")
        );
        ...
        forAll(times, timeI)
        {
            ...
            Info<< "Time = " << runTimes.completeTime().timeName() << endl;
            ...
            else if (copyZero)
            {
                ...
                for (label proci = 0; proci < meshes.nProcs(); proci++)
                {
                    ...
                    processorDb.setTime(runTimes.completeTime().time());
            ...
            else
            {
                ...
                //- delete allocated memory (if any) in particle cloud
                {
                    forAll(cellParticles, cPI)
                    {
                        List<SLList<indexedParticle*>*>& cellParticlesI =
                            cellParticles[cPI];

                        forAll(cellParticlesI, cPII)
                        {
                            SLList<indexedParticle*>* cellParticlesII =
                                cellParticlesI[cPII];

                            if (cellParticlesII)
                            {
                                delete cellParticlesII;
                            }
                        }
                    }
                }
