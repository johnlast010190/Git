...
)
{
    ...
    if (!haveMesh)
    {
        ...
        // Is this mesh conformal?
        bool conformalMesh = true;
        ...
        forAll(patchEntries, patchi)
        {
            ...
            (
             ...
             && type != nonConformalProcessorCyclicPolyPatch::typeName
            )
            {
            ...
            if (type == nonConformalCyclicPolyPatch::typeName)
            {
                conformalMesh = false;
            }
        }
        ...
        // Generate a null finite-volume addressing for the dummy meshes, if
        // the original meshes are non-conformal. Set dummy data also for the
        // face geometry, since it should not be used.
        if (!conformalMesh)
        {
            surfaceLabelField::Boundary polyFacesBf
            (
                surfaceLabelField::null(),
                dummyMesh.polyFacesBf()
            );
            surfaceVectorField Sf(dummyMesh.Sf().cloneUnSliced());
            surfaceVectorField Cf(dummyMesh.Cf().cloneUnSliced());
