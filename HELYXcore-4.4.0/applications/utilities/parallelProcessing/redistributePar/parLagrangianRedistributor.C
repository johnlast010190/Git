...
:
    ...
    distMap_(distMap)
{
    const distributionMap& cellMap = distMap_.cellMap();
...
Foam::autoPtr<Foam::distributionMapBase>
...
) const
{
    ...
    // Work the send indices (subMap) into a distributionMapBase
    ...
    // Construct map
    return autoPtr<distributionMapBase>
    (
        new distributionMapBase
        (
            constructSize,
            std::move(subMap),
