...
void checkWriteGIBMeshData
(
    Time& runTime,
    const fvMesh& mesh,
    const labelListList& pointProcAddressing,
    const word& regionDir,
    const PtrList<Time>& dTimes

)
{
    IOobject basePoints0
    (
        "basePoints",
        dTimes[0].timeName(),
        regionDir/polyMesh::meshSubDir,
    ...
    //- Check if base point exist in processor zero.
    if (!basePoints0.typeHeaderOk<pointIOField>(false))
    {
        return;
    }

    // Read the field for all the processors
    PtrList<pointIOField> procsPoints(dTimes.size());

    forAll(procsPoints, proci)
    {
        procsPoints.set
        (
            proci,
    ...
    (
        ...
        (
            "basePoints",
            runTime.time().findInstance(mesh.meshDir(), "points"),
            mesh.meshDir(),
            mesh,
            IOobject::NO_READ,
            IOobject::NO_WRITE
        ),
        pointField( mesh.nPoints(), vector::zero)
    );
    ...
    forAll(pointProcAddressing, proci)
    {
        ...
        {
            basePoints[pointProcAddressingI[pointi]] = procPoints[pointi];
        }
    }
    ...
    forAll(timeDirs, timeI)
    {
        ...
        forAll(regionNames, regionI)
        {
            ...
            if (!facesIO.typeHeaderOk<faceCompactIOList>(false))
            {
                ...
                {
                    Info<< "faceZones found. Reconstructing" << nl << endl;
                }
            }

            Info<< "\n\nReconstructing for mesh " << regionName << nl
                << endl;
