...
)
{
    ...
    const smoluchowskiJumpTFvPatchScalarField& thftptf
        = refCast<const smoluchowskiJumpTFvPatchScalarField>(ptf);

    Twall_.rmap(thftptf.Twall_, addr);
}


void Foam::smoluchowskiJumpTFvPatchScalarField::autoMapGIB
(
    const gibFvPatchFieldMapper& mapper
)
{
    mixedFvPatchField<scalar>::autoMapGIB(mapper);
    mapper.map(Twall_, gAverage(Twall_));
}
...
void Foam::smoluchowskiJumpTFvPatchScalarField::updateCoeffs()
{
    ...
    (
        "Pr",
        dimless,
        basicThermo::matDictName == basicThermo::dictName
      ? thermophysicalProperties.subDict("kappaModelCoeffs").lookup("Pr")
