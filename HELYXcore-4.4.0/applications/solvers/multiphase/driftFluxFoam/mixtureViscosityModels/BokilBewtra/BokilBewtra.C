...
:
    ...
    alphac_("alphac", dimless, BokilBewtraCoeffs_.lookup("alphac")),
    alpha_
    (
...
Foam::mixtureViscosityModels::BokilBewtra::mu(const volScalarField& muc) const
{
    ...
    (
        min(max((alphac_ - alpha_)/alphac_, scalar(0.0)), scalar(1.0))
    );
    ...
    return
        ...
        (
           ...
           *pow(scalar(10), BokilBewtraViscosityExponent_*alpha_),
            muMax_
        );
}
