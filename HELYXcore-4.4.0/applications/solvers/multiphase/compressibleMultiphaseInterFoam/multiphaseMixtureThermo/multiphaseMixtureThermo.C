...
void Foam::multiphaseMixtureThermo::correct()
{
    ...
    kappa_ = phasei()*phasei().thermo().kappa();
    Cp_ = phasei()*phasei().thermo().Cp();
    Cv_ = phasei()*phasei().thermo().Cv();
...
) const
{
    ...
    tmp<scalarField> ths
    (
        scalarField(phasei(), cells)*phasei().thermo().hs(p, T, cells)
    );
...
Foam::tmp<Foam::volScalarField> Foam::multiphaseMixtureThermo::hf() const
{
    PtrDictionary<phaseModel>::const_iterator phasei = phases_.begin();

    tmp<volScalarField> thf(phasei()*phasei().thermo().hf());

    for (++phasei; phasei != phases_.end(); ++phasei)
    {
        thf.ref() += phasei()*phasei().thermo().hf();
    }
...
const Foam::volScalarField& Foam::multiphaseMixtureThermo::Cp() const
{
    return Cp_;
}
...
const Foam::volScalarField& Foam::multiphaseMixtureThermo::Cv() const
{
    return Cv_;
}
...
const Foam::volScalarField& Foam::multiphaseMixtureThermo::Cpv() const
{
    PtrDictionary<phaseModel>::const_iterator phasei = phases_.begin();
    if
    (
        phasei().thermo().he().member() == "h"
     || phasei().thermo().he().member() == "ha"
    )
    {
        return Cp_;
    }
...
const Foam::volScalarField& Foam::multiphaseMixtureThermo::kappa() const
{
    return kappa_;
}
