...
void Foam::incompressibleThreePhaseDriftMixture::calcNu()
{
    ...
    const volScalarField limitedAlpha1(max(alpha1_, scalar(0)));
    const volScalarField limitedAlpha2(max(alpha2_, scalar(0)));
    const volScalarField limitedAlpha3(max(alpha3_, scalar(0)));
...
:
    ...
    nu_
    (
        IOobject("nu", U.time().timeName(), U.db()),
        U.mesh(),
    ...
    nuModel3_(viscosityModel::New("nu3", subDict(phase3Name_), U, phi)),
    ...
    UdmModelPtr_
    (
        relativeVelocityModel::New("driftVel2", subDict(phase2Name_), alpha2_)
    )
{
...
Foam::incompressibleThreePhaseDriftMixture::muf() const
{
    ...
    const surfaceScalarField alpha2f(fvc::interpolate(limitedAlpha2));
    const surfaceScalarField alpha3f(fvc::interpolate(limitedAlpha3));
