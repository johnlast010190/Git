...
:
    ...
    (
        lookup("phases"),
        phase::iNew(U, phi, this->lookupOrDefault("alphaMax", 1.0))
    ),
    ...
    )
{
    ...
    incompressibleMultiphaseInteractingMixture::correct();
    incompressibleMultiphaseInteractingMixture::correctUdm();
}
...
Foam::incompressibleMultiphaseInteractingMixture::tauDrift() const
{
    ...
    tauDrift +=
       ...
       *(
            sqr(H())
          + (sqr(H() + U()) - sqr(U()) - sqr(H()))
          - (sqr(U()+(Hm()/rho())) - sqr(Hm()/rho()) - sqr(U()))
          - (sqr((Hm()/rho())+H()) - sqr(Hm()/rho()) - sqr(H()))
       );

    return volSymmTensorField::New("tauDm", ttauDrift);
}
...
bool Foam::incompressibleMultiphaseInteractingMixture::read()
{
    if (regIOobject::read())
    {
        alphaMax_ = this->lookupOrDefault<scalar>("alphaMax", 1);
        return true;
    }
    else
    {
