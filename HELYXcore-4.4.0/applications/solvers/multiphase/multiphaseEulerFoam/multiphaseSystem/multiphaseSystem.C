...
#include "alphaContactAngleMPFvPatchScalarField.H"
    ...
    forAllIter(PtrDictionary<phaseModel>, phases_, iter)
    {
        ...
        forAllIter(PtrDictionary<phaseModel>, phases_, iter2)
        {
            ...
            if (cAlpha != cAlphas_.end())
            {
                ...
                {
                    fvsPatchScalarField& phicp = phic.boundaryFieldRef()[patchi];
...
) const
{
    ...
    {
        if (isA<alphaContactAngleMPFvPatchScalarField>(gbf[patchi]))
        {
            const alphaContactAngleMPFvPatchScalarField& acap =
                refCast<const alphaContactAngleMPFvPatchScalarField>(gbf[patchi]);
            ...
            alphaContactAngleMPFvPatchScalarField::thetaPropsTable::
                const_iterator tp =
    ...
    )
{
    ...
    // update alpha boundary conditions:
    PtrDictionary<phaseModel>::iterator iter = phases_.begin();
...
Foam::tmp<Foam::volScalarField> Foam::multiphaseSystem::rho() const
{
    ...
    tmp<volScalarField> trho = boundedAlpha(iter())*iter().rho();
    volScalarField& rho = trho.ref();

    for (++iter; iter != phases_.end(); ++iter)
    {
        rho += boundedAlpha(iter())*iter().rho();
    }
...
Foam::multiphaseSystem::rho(const label patchi) const
{
    ...
    tmp<scalarField> trho =
        boundedAlpha(iter())().boundaryField()[patchi]*iter().rho().value();
    scalarField& rho = trho.ref();

    for (++iter; iter != phases_.end(); ++iter)
    {
        rho += boundedAlpha(iter())().boundaryField()[patchi]*iter().rho().value();
    }
...
const Foam::volScalarField& Foam::multiphaseSystem::Cp() const
{
    NotImplemented;
}


Foam::tmp<Foam::volScalarField> Foam::multiphaseSystem::Prt() const
{
    NotImplemented;
}


Foam::tmp<Foam::volScalarField> Foam::multiphaseSystem::Pr() const
{
    NotImplemented;
}


Foam::tmp<Foam::volScalarField> Foam::multiphaseSystem::lambda() const
{
    NotImplemented;
}


Foam::tmp<Foam::volScalarField> Foam::multiphaseSystem::alphaLam() const
{
    NotImplemented;
}
...
Foam::tmp<Foam::volScalarField> Foam::multiphaseSystem::nu() const
{
    ...
    tmp<volScalarField> tmu = boundedAlpha(iter())*(iter().rho()*iter().nu());
    volScalarField& mu = tmu.ref();

    for (++iter; iter != phases_.end(); ++iter)
    {
        mu += boundedAlpha(iter())*(iter().rho()*iter().nu());
    }
...
Foam::multiphaseSystem::nu(const label patchi) const
{
    ...
    tmp<scalarField> tmu =
        boundedAlpha(iter())().boundaryField()[patchi]
...
Foam::tmp<Foam::volScalarField>
Foam::multiphaseSystem::boundedAlpha
(
    const phaseModel& phase
) const
{
    return min(max(phase, scalar(0)), scalar(1));
}
...
bool Foam::multiphaseSystem::read()
{
    if (regIOobject::read())
    {
        ...
        sigmas_ = lookup<scalarCoeffSymmTable>("sigmas");
        cAlphas_ = lookup<scalarCoeffSymmTable>("interfaceCompression");
        Cvms_ = lookup<scalarCoeffTable>("virtualMass");
