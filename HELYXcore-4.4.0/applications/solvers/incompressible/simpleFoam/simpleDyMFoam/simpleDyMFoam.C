...
int main(int argc, char *argv[])
{
    ...
    bool correctPhi
    (
        simple.dict().lookupOrDefault<Switch>("correctPhi", false)
    );

    // ensure divergence-free flux at simulation start
    {
        IOobjectList objects(mesh, runTime.timeName());
        if (!objects.lookup("phi"))
        {
            ...
            forAll(U.boundaryField(), patchI)
            {
                if (!U.boundaryField()[patchI].coupled())
                {
                    phi.boundaryFieldRef()[patchI]
                        ...
                        & fvc::interpolate(U)().boundaryField()[patchI];
                }
            }
