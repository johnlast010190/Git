...
{
    scalar dt( timeDict.lookup<scalar>("deltaT") );
...
)
{
    const auto ctor =
        ...
        (
            ...
            postProcessingWavesConstructorTable_(),
            action
        );
    return autoPtr<postProcessingWaves>(ctor( rT, actionProp, action));
}
