...
void Foam::jjc2014Zone::checkNegativeResistance(dimensionedVector& resist)
{
    ...
    if (minCmpt < 0)
    {
        FatalErrorIn
        (
            "Foam::jjc2014Zone::jjc2014Zone::adjustNegativeResistance"
            "(dimensionedVector&)"
...
:
    ...
    coordSys_(dict, mesh),
#elif OFPLUSBRANCH==1
    coordSys_(dict, coordinateSystem::typeName_()),
#else
    #if OFVERSION<230
        coordSys_(dict, mesh),
    #else
    ...
    porosity_(dict_.lookup<scalar>("porosity")),
    addedMassCoeff_(dict_.lookup<scalar>("gammaAddedMass")),
...
void Foam::jjc2014Zone::porosity
(
    volScalarField & poro
) const
{
...
void Foam::jjc2014Zone::addResistance(fvVectorMatrix& UEqn) const
{
    if (cellZoneID_ == -1)
    {
        return;
...
void Foam::jjc2014Zone::addResistance
(
...
void Foam::jjc2014Zone::writeDict(Ostream& os, bool subDict) const
{
    if (subDict)
    {
        os  << indent << token::BEGIN_BLOCK << incrIndent << nl;
        os.writeEntry("name", zoneName());
    }
    else
    {
        os.beginBlock(zoneName());
    }

    if (dict_.found("note"))
    {
        os.writeEntry("note", string(dict_.lookup("note")));
    }

    coordSys_.writeEntry(coordSys_.name(), os);

    if (dict_.found("porosity"))
    {
        os.writeEntry("porosity", porosity());
    }
...
Foam::Ostream& Foam::operator<<(Ostream& os, const jjc2014Zone& pZone)
{
    pZone.writeDict(os);
    return os;
}
