...
:
    ...
    eliminatePre_(dict.lookup<bool>("eliminatePre")),
    eliminatePost_(dict.lookup<bool>("eliminatePost")),
    out_(this->outputDir(), this->owner().time()),
    search_(this->owner().mesh()),
    nrPre_(0)
{
    out_.addSpec("eliminated.*", "pre post");
}
...
{
    if (this->owner().mesh().changing())
    {
        Info<< this->modelName() << ":" << this->owner().name()
            << ":" << this->modelType() << ": Mesh moving" << endl;

        search_.correct();
    }

    Info<< this->modelName() << ":" << this->owner().name()
        << ":" << this->modelType() << ": Checking pre" << endl;
    ...
    if (eliminatePre_)
    {
        if (!eliminatedPrePtr_.valid())
        {
            ...
            (
                ...
                (
                    this->owner().name() + this->modelName() + "EliminatedPre"
                ).ptr()
            );
        }
...
void Foam::EliminateOutsideParticles<CloudType>::postEvolve()
{
    ...
    reduce
    (
        std::tie(nrEliminatedPre, nrEliminatedPost),
    ...
    if (totalEliminatedPre > 0 || totalEliminatedPost > 0)
    {
        Info<< this->modelName() << ":" << this->owner().name()
            << ":" << this->modelType() << " : " << nrEliminatedPre
            << " parcels eliminated before timestep. "
...
template<class CloudType>
Foam::label Foam::EliminateOutsideParticles<CloudType>::checkInside
(
    Cloud<parcelType>& eliminate
)
{
    label cnt = 0;
    label outCnt = 0;
    ...
    forAllIter(typename CloudType, this->owner(), iter)
    {
        ...
        label cellI = search_.findCell(p.position(), oldCellI);
        if (cellI < 0)
        {
            cnt++;
