...
:
    ...
    writeTimeline_(dict.lookup<bool>("writeTimeline")),
    writeDistribution_(dict.lookup<bool>("writeDistribution")),
    startup_(false),
    startTime_("none")
{
    if (writeTimeline_ && writeDistribution_)
    {
        WarningIn("distributionFunctionObject::distributionFunctionObject")
            << "Both 'writeTimeline' and 'writeDistribution' set in "
            << name << ". " << "This is usually not a good idea"
            << " because of the amount of data written" << endl;
    }
    if (!writeTimeline_ && writeDistribution_)
    {
        WarningIn("distributionFunctionObject::distributionFunctionObject")
            << "Both 'writeTimeline' and 'writeDistribution' not set. "
            << "This makes calculating the distribution futile" << endl;
    }
}
...
void distributionFunctionObject::writeSimple()
{
    ...
    if (distScalar_.valid() && distScalar_->maxNrBins() < 0)
    {
        zeroDistribution = true;
    }
    if (distVector_.valid() && distVector_->maxNrBins() < 0)
    {
        zeroDistribution = true;
    }
    if (distTensor_.valid() && distTensor_->maxNrBins() < 0)
    {
        zeroDistribution = true;
    }
    if (distSymmTensor_.valid() && distSymmTensor_->maxNrBins() < 0)
    {
        zeroDistribution = true;
    }
    if (distSphericalTensor_.valid() && distSphericalTensor_->maxNrBins() < 0)
    {
        zeroDistribution = true;
    }
