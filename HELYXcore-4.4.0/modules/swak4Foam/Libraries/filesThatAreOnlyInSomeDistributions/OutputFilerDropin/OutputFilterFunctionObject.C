...
template<class OutputFilter>
void Foam::OutputFilterFunctionObject<OutputFilter>::checkPostProcess() {
    Dbug<< this->name() << " OutputFilterFunctionObject::checkPostProcess()" << endl;
    if (functionObject::postProcess)
    {
        ptr_->setPostProcess(true);
    }
}
...
template<class OutputFilter>
void Foam::OutputFilterFunctionObject<OutputFilter>::topoChange
(
    const polyTopoChangeMap& map
)
{
    if (active() && map.mesh().name() == regionName_)
    {
        ptr_->topoChange(map);
    }
}
