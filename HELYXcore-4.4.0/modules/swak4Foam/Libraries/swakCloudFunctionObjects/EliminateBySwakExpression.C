...
:
    ...
    expression_(dict.lookup("eliminationExpression"), dict),
    nrPre_(0)
{
...
{
    Info<< this->modelName() << ":" << this->owner().name()
        << ":" << this->modelType() << ": Checking post" << endl;
...
template<class CloudType>
Foam::label Foam::EliminateBySwakExpression<CloudType>::checkExpression
(
...
)
{
    ...
    if (driver_.getResultType()!=pTraits<bool>::typeName)
    {
        FatalErrorIn
        (
            "Foam::EliminateBySwakExpression<CloudType>::checkExpression"
        )
            << "Expected 'bool' as result of " << expression_ << nl
            << "Got " << driver_.getResultType() << nl
            << exit(FatalError);
    }
