...
Foam::binaryOperationSearchableSurface::coordinates() const
{
    ...
    label aCoordsSize(aCoords.size());
    label bCoordsSize(bCoords.size());

    tmp<pointField> tpoints(new pointField(aCoordsSize + bCoordsSize, Zero));
    ...
    forAll(bCoords,i) {
        points[i+aCoordsSize]=bCoords[i];
    }
    if (debug) {
        Info<< "A: " << aCoordsSize << " B: " << bCoordsSize
            << " -> " << points.size() << "(" << this->size() << ")" << endl;
    }
