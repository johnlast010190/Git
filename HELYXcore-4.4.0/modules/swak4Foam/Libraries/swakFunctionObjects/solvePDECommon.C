...
void Foam::solvePDECommon::read(const dictionary& dict)
{
    ...
    if (active_) {
        ...
        steady_=dict.lookup<bool>("steady");
        if (steady_) {
            if (dict.found("restoreNonConvergedSteady")) {
                restoreNonConvergedSteady_=dict.lookup<bool>("restoreNonConvergedSteady");
            } else {
            ...
            if (dict.found("relaxUnsteady")) {
                relaxUnsteady_=dict.lookup<bool>("relaxUnsteady");
            } else {
                WarningIn("solvePDECommon::read(const dictionary& dict)")
        ...
        if (steady_ || relaxUnsteady_) {
            if (dict.found("relaxLastIteration")) {
                relaxLastIteration_=dict.lookup<bool>("relaxLastIteration");
            } else {
                WarningIn("solvePDECommon::read(const dictionary& dict)")
