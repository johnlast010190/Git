    ...
    velocityName_("none")
{
    ...
    if (isA<dynamicFvMesh>(obr)) {
        ...
        makePhiRelative_=dict.lookup<bool>("makePhiRelative");
    }
    read(dict);
    ...
    if (active_) {
        ...
        if (dict.found("sourceImplicit")) {
            ...
            if (dict.found("sourceImplicitUseSuSp")) {
                sourceImplicitUseSuSp_=dict.lookup<bool>("sourceImplicitUseSuSp");
            } else {
...
void Foam::solveTransportPDE::solve()
{
    if (active_) {
        ...
        dictionary sol=mesh.solution().dict().subDict(fieldName_+"TransportPDE");
