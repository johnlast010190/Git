...
// #include "include/swakPTraitsSpecialization.H"
    ...
    needsReset_(false)
{
    ...
    if (
        ...
        if (isSingleValue_) {
            ...
            label fs=dict.lookup<label>("fieldSize");
...
)
{
    ...
    if (unset) {
        ...
        return autoPtr<ExpressionResult>(ctor());
    } else {
        const auto ctor =
            ctorTableLookup
            (
        ...
        return autoPtr<ExpressionResult>(ctor(dict));
    }
}
        ...
        if (valType_==pTraits<scalar>::typeName) {
            ...
            WarningIn("ExpressionResult::uglyDelete()")
                << "Unknown type " << valType_
                    << " probable memory loss" << endl;
            delete static_cast<char *>(valPtr_);  // Avoid compiler warning
        }
    }
    valType_="Void";
    valPtr_=NULL;
        ...
        if (data.size() <= 0) {
            ...
            if (data.isSingleValue_) {
                ...
                if (data.valType_==pTraits<scalar>::typeName) {
                    ...
                    out << "nonuniform " << Field<vector>(0, Zero);
                } else if (data.valType_==tensor::typeName) {
                    out << "nonuniform " << Field<tensor>(0, Zero);
                } else if (data.valType_==symmTensor::typeName) {
                    out << "nonuniform " << Field<symmTensor>(0, Zero);
                } else if (data.valType_==sphericalTensor::typeName) {
                    out << "nonuniform " << Field<sphericalTensor>(0, Zero);
                } else if (data.valType_==pTraits<bool>::typeName) {
