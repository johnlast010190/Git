...
{
    if (debug) {
        Info<< "PatchValueExpressionDriver - patch+dict+obr constructor" << endl;
    }
...
{
    if (debug) {
        Info<< "PatchValueExpressionDriver - id+mesh+obr constructor" << endl;
    }
}
    ...
    mappingInterpolationSchemes_(old.mappingInterpolationSchemes_)
{
    if (debug) {
        ...
        Info<< "Patch + Driver names "<<obr_.names()<<endl;
    }
}
...
const objectRegistry& PatchValueExpressionDriver::db() const
{
    return obr_;
}
...
tmp<scalarField> PatchValueExpressionDriver::makeNearDistField()
{
    ...
    const volScalarField::Boundary& dist = nearWallDist::New(this->mesh()).y();
