...
#include "helpers/MeshDistFromPatch.H"
...
tmp<surfaceVectorField> FieldValueExpressionDriver::makeFaceProjectionField()
{
    ...
    {
        const labelUList& cNumbers = mesh_.boundaryMesh()[patchI].faceCells();
...
tmp<volScalarField> FieldValueExpressionDriver::makeDistanceField()
{
    ...
    const_cast<volScalarField&>(f()).forceAssign(dist.y());
#else
    const_cast<volScalarField&>(f()).forceAssign(dist);
#endif
...
tmp<volScalarField> FieldValueExpressionDriver::makeNearDistanceField()
{
    ...
    f->dimensions().reset(mesh_.C().dimensions());
    const volScalarField::Boundary& dist = nearWallDist::New(mesh_).y();
    const_cast<volScalarField::GeometricBoundaryField&>(f->boundaryField()).forceAssign(dist);
    f->dimensions().reset(dimless);
...
) const
{
    ...
    reduce(
        std::tie(isCell, isFace),
        UniformParallelOp<andOp<bool>, 2>{}
    );
