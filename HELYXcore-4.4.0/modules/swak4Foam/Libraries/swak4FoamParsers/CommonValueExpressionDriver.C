...
#include "entryToExpression/entryToExpression.H"
...
label CommonValueExpressionDriver::readForeignMeshInfo(
    ...
    forAll(names,i) {
        ...
        const scalar time(info.lookup<scalar>("time"));
...
)
{
    ...
    const auto ctor =
        ctorTableLookup
        (
            "CommonValueExpressionDriver type",
...
{
    if (
        db().foundObject<regIOobject>(name)
        &&
        !variableNameIdenticalToField_
    ) {
        const regIOobject &ob=db().lookupObject<regIOobject>(name);
