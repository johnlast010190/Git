...
struct CDelaBella0 : IDelaBella0
{
	struct Face;

	struct Vert : DelaBella0_Vertex
	{
		Unsigned28 z;
		Face* sew;
	...
	struct Face : DelaBella0_Triangle
	{
		Norm n;
	...
	int Triangulate()
	{
		...
		if (points < 3)
		{
			if (points == 2)
			{
				...
				vert_alloc[0].next = (DelaBella0_Vertex*)vert_alloc + 1;
				vert_alloc[1].next = 0;
			}
			else
			{
		...
		*/
		...
		for (; i < points; i++)
		{
			...
			[[maybe_unused]] int del = 0;
			[[maybe_unused]] int add = 0;
	...
	virtual int Triangulate(int points, const float* x, const float* y = 0, int advance_bytes = 0)
	{
		...
		if ((size_t) advance_bytes < sizeof(float) * 2)
	...
	virtual int Triangulate(int points, const double* x, const double* y, int advance_bytes)
	{
		...
		if ((size_t) advance_bytes < sizeof(double) * 2)
	...
	virtual const DelaBella0_Triangle* GetFirstDelaunayTriangle() const
	{
		return first_dela_face;
	}

	virtual const DelaBella0_Triangle* GetFirstHullTriangle() const
	{
		return first_hull_face;
	}

	virtual const DelaBella0_Vertex* GetFirstHullVertex() const
	{
...
IDelaBella0* IDelaBella0::Create()
{
	CDelaBella0* db = new CDelaBella0;
	if (!db)
		return 0;
...
void* DelaBella0_Create()
{
	return IDelaBella0::Create();
}

void  DelaBella0_Destroy(void* db)
{
	((IDelaBella0*)db)->Destroy();
}

void  DelaBella0_SetErrLog(void* db, int(*proc)(void* stream, const char* fmt, ...), void* stream)
{
	((IDelaBella0*)db)->SetErrLog(proc, stream);
}

int   DelaBella0_TriangulateFloat(void* db, int points, float* x, float* y, int advance_bytes)
{
	return ((IDelaBella0*)db)->Triangulate(points, x, y, advance_bytes);
}

int   DelaBella0_TriangulateDouble(void* db, int points, double* x, double* y, int advance_bytes)
{
	return ((IDelaBella0*)db)->Triangulate(points, x, y, advance_bytes);
}

int   DelaBella0_GetNumInputPoints(void* db)
{
	return ((IDelaBella0*)db)->GetNumInputPoints();
}

int   DelaBella0_GetNumOutputVerts(void* db)
{
	return ((IDelaBella0*)db)->GetNumOutputVerts();
}

const DelaBella0_Triangle* GetFirstDelaunayTriangle(void* db)
{
	return ((IDelaBella0*)db)->GetFirstDelaunayTriangle();
}

const DelaBella0_Triangle* GetFirstHullTriangle(void* db)
{
	return ((IDelaBella0*)db)->GetFirstHullTriangle();
}

const DelaBella0_Vertex*   GetFirstHullVertex(void* db)
{
	return ((IDelaBella0*)db)->GetFirstHullVertex();
}

// depreciated!
int DelaBella0(int points, const double* xy, int* abc, int(*errlog)(const char* fmt, ...))
{
	if (errlog)
	...
	IDelaBella0* db = IDelaBella0::Create();
	...
	{
		int tris = verts / 3;
		const DelaBella0_Triangle* dela = db->GetFirstDelaunayTriangle();
	...
	{
		int pnts = -verts;
		const DelaBella0_Vertex* line = db->GetFirstHullVertex();
