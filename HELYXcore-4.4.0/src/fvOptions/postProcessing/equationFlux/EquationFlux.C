...
template<class Type>
void Foam::fv::EquationFlux<Type>::sourceFields(wordList& fieldNames)
{
    fieldNames.setSize(1, coeffs_.lookup<word>("equation"));
}
...
)
{
    ...
    tmp<VolField<Type>> tflux;
    if
    (
        !storeField_
     || !obr_.foundObject<VolField<Type>>
        (
            outputFieldName_
        )
    )
    ...
    VolField<Type>& flux =
        (
            tflux.valid()
          ? tflux.ref()
          : obr_.lookupObjectRef<VolField<Type>>(outputFieldName_)
        );
    ...
    tmp<SurfaceField<Type>> teqnFlux = eqn.flux();
