...
#include "cellSetOption/sweptCellMRF/sweptCellMRF.H"
...
namespace Foam
{
    ...
    const NamedEnum<fv::cellSetOption::selectionModeType, 6>
...
void Foam::fv::cellSetOption::setSelection(const dictionary& dict)
{
    switch (selectionMode_)
    {
        ...
        case smCellSet:
        {
            cellSetName_ = dict.lookup<word>("cellSet");
            break;
        }
        case smCellZone:
        {
            cellSetName_ = dict.lookup<word>("cellZone");
            break;
        }
        case smSweptCell:
        {
...
void Foam::fv::cellSetOption::setCellSet()
{
    switch (selectionMode_)
    {
        ...
        {
            Info<< indent
                << "- selection cells using automatic cellZone detection " << endl;

            sweptCellMRF sweptCells(dict_, mesh_);
            cells_ = sweptCells.getMRFCells();
            break;
        }
        case smAll:
        {
...
:
    ...
    selectionMode_
    (
        selectionModeTypeNames_.read
        (
            IStringStream
            (coeffs_.lookupOrDefault<word>("selectionMode", "none"))()
        )
    ),
