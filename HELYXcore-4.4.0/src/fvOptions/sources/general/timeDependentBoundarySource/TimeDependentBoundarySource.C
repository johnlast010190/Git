...
template<class Type>
void Foam::fv::TimeDependentBoundarySource<Type>::setTimeSeries
(
...
)
{
    ...
    forAll(fieldNames, i)
    {
        boundarySourcePatchIDs.set(fieldNames[i], patchIDs);
    }
...
template<class Type>
void Foam::fv::TimeDependentBoundarySource<Type>::boundarySourceFieldsAndPatches
(
    HashTable<labelList>& boundarySourcePatchIDs
)
{
    setTimeSeries(coeffs_, boundarySourcePatchIDs);
}
