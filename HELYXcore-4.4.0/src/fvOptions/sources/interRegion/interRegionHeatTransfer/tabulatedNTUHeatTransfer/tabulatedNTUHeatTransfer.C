    ...
    if (Ain_ < 0 && !userGeomIsInit_)
    {
        ...
        switch (geometryMode_)
        {
            case gmCalculated:
            {
                ...
                scalar alpha(coeffs_.lookup<scalar>("inletBlockageRatio"));
                ...
                scalar alphaNbr
                (
                    coeffs_.lookup<scalar>("inletBlockageRatioNbr")
                );
                ...
                scalar beta(coeffs_.lookup<scalar>("coreBlockageRatio"));
            ...
            case gmUser:
            {
                ...
                {
                    mDot_ = coeffs_.lookup<scalar>("mDot");
                    mDotNbr_ = coeffs_.lookup<scalar>("mDotNbr");
                }
                else if (
                    coeffs_.found("Ain")
                ...
                {
                    Ain_ = coeffs_.lookup<scalar>("Ain");
                    AinNbr_ = coeffs_.lookup<scalar>("AinNbr");
                }
                else if (
                ...
                )
                {
                    ...
                    CpNbr_ = coeffs_.lookup<scalar>("CpNbr");
                }
...
void Foam::fv::tabulatedNTUHeatTransfer::calculateHtc(scalar deltaT)
{
    ...
    scalarField Cp(thermo.Cp().primitiveField());
    scalarField CpNbr(interpolate(thermoNbr.Cp().primitiveField()));
