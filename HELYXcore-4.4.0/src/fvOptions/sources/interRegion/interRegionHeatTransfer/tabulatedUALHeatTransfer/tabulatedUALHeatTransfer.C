    ...
    if (Vcore_ < 0)
    {
        ...
        if (!coeffs_.readIfPresent("Vcore", Vcore_))
        {
            ...
            (
                ...
                mesh_,
                dimensionedScalar(dimTemperature, 2)
            );
...
Foam::fv::tabulatedUALHeatTransfer::massFlowRate() const
{
    ...
    if (mDot_ < 0)
    {
        if (Ain_ < 0)
        {
            ...
            const fvPatchScalarField& rhof =
                obr_.lookupObject<volScalarField>(rhoName_).boundaryField()
                [
                    patchI
                ];
...
Foam::tmp<Foam::scalarField>
Foam::fv::tabulatedUALHeatTransfer::massFlowRateNbr() const
{
    const fvMesh& nbrMesh =
        mesh_.time().lookupObject<fvMesh>(nbrRegionName());
