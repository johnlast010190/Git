...
void Foam::fv::interRegionExplicitPorositySource::init()
{
    if (!firstIter_)
    {
        return;
...
:
    interRegionOption(name, modelType, dict, obr),
...
void Foam::fv::interRegionExplicitPorositySource::sourceFields
(
    wordList& fieldNames
)
{
...
)
{
    ...
    const volScalarField& mu =
        obr_.lookupObject<volScalarField>(muName_);
