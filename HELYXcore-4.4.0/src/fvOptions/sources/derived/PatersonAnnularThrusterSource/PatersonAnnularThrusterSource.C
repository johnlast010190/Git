...
:
    ...
    rhoRef_(coeffs_.subDict("shipForce").lookup<scalar>("rhoInf")),
    R_(coeffs_.lookup<scalar>("propTipRadius")),
    rh_(coeffs_.lookup<scalar>("propHubRadius")),
    propPosition_(coeffs_.lookup("propPosition")),
    diskDir_(coeffs_.lookup("propDirection")),
    tProp_(coeffs_.lookup<scalar>("propThickness")),
    qPropDir_(coeffs_.lookupOrDefault<scalar>("torqueDirection", 1.0)),
    diskFraction_(coeffs_.lookup<scalar>("diskFraction")),
    J_(coeffs_.lookup<scalar>("J")),
    a_(coeffs_.lookup("a")),
    b_(coeffs_.lookup("b")),
...
void Foam::fv::PatersonAnnularThrusterSource::sourceFields
(
    wordList& fieldNames
)
{
    fieldNames = List<word>(1, "U");
}
...
bool Foam::fv::PatersonAnnularThrusterSource::read(const dictionary& dict)
{
    if (cellSetOption::read(dict))
    {
        ...
        Uship_ = coeffs_.lookup<vector>("Uship");
