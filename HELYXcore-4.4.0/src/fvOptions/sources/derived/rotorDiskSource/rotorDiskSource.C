    ...
    switch (selectionMode())
    {
        ...
        case smAll:
        {
            ...
            switch (inletFlow_)
            {
                case ifFixed:
                {
                    inletVelocity_ = coeffs_.lookup<vector>("inletVelocity");
                    break;
                }
...
void Foam::fv::rotorDiskSource::createCoordinateSystem()
{
    ...
    switch (gm)
    {
        case gmAuto:
        {
            ...
            refDir = coeffs_.lookup<vector>("refDirection");
        ...
        case gmSpecified:
        {
            origin = coeffs_.lookup<vector>("origin");
            axis = coeffs_.lookup<vector>("axis");
            refDir = coeffs_.lookup<vector>("refDirection");
...
void Foam::fv::rotorDiskSource::sourceFields
(
    wordList& fieldNames
)
{
...
bool Foam::fv::rotorDiskSource::read(const dictionary& dict)
{
    if (cellSetOption::read(dict))
    {
        ...
        nBlades_ = coeffs_.lookup<label>("nBlades");

        inletFlow_ = inletFlowTypeNames_.read(coeffs_.lookup("inletFlowType"));

        tipEffect_ = coeffs_.lookup<scalar>("tipEffect");

        prandtlDrella_ =
            coeffs_.lookupOrDefault<Switch>("prandtlDrellaMethod", "false");

        if (prandtlDrella_)
        {
            // Reference diameter for calculating prandtl tip loss factor
            coeffs_.lookup("diameterRef") >> diameterRef_;

            // Reference velocity for calculating propeller efficiency
            coeffs_.lookup("refVelEta") >> refVelEta_;
        }

        const dictionary& flapCoeffs(coeffs_.subDict("flapCoeffs"));
        flap_.beta0 = flapCoeffs.lookup<scalar>("beta0");
        flap_.beta1c = flapCoeffs.lookup<scalar>("beta1c");
        flap_.beta2s = flapCoeffs.lookup<scalar>("beta2s");
