...
Foam::labelList
Foam::fv::effectivenessHeatExchangerSource::zoneBoundaryFaces() const
{
...
void Foam::fv::effectivenessHeatExchangerSource::determineUpwindCells()
{
    ...
    {
        upwindCells_[i] =
            (zoneCells[own[faceId_[i]]]) ? nei[faceId_[i]] : own[faceId_[i]];
    }
...
Foam::fv::effectivenessHeatExchangerSource::getRho() const
{
    ...
    else
    {
        ...
        (
            "rho",
            obr_.lookupObject<dictionary>("transportProperties").lookup("rho")
        );

        return volScalarField::New("rho", obr_, mesh_, rho);
    }
}
...
:
    ...
    secondaryCp_(coeffs_.lookup<scalar>("secondaryCp")),
    ...
    zoneBoundaryFaces_()
{
    if (zoneID_ < 0)
    {
        FatalErrorInFunction
            << effectivenessHeatExchangerSource::type()
            << " " << this->name() << ": "
...
void Foam::fv::effectivenessHeatExchangerSource::sourceFields
(
    wordList& fieldNames
)
{
...
)
{
    ...
    *(CpfMean/faceZoneArea_)*mag(totalPhi);
    ...
    {
        secondaryOutletT =
            secondaryInletT_ - Qt/(secondaryCp_*secondaryMassFlowRate_);
    }

    if (Pstream::master())
    {
        Info<< indent << "\nfvOption:effectivenessHeatExchangerSource:"
            << this->name() << nl
            << indent << "  Net mass flux [Kg/s] = " << totalPhi << nl
            << indent << "  Total energy exchange [W] = " << Qt << nl
            << indent << "  TPrimaryInlet [K] = " << primaryInletT_ << nl
            << indent << "  TSecondaryOutlet [K] = " << secondaryOutletT << nl
...
bool Foam::fv::effectivenessHeatExchangerSource::read(const dictionary& dict)
{
    if (cellSetOption::read(dict))
    {
        secondaryMassFlowRate_ =
            coeffs_.lookup<scalar>("secondaryMassFlowRate");
        secondaryInletT_ = coeffs_.lookup<scalar>("secondaryInletT");
        secondaryCp_ = coeffs_.lookup<scalar>("secondaryCp");
