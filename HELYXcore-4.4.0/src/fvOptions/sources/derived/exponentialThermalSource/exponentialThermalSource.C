...
:
    ...
    eName_(coeffs_.lookupOrDefault<word>("eName", word::null)),
...
void Foam::fv::exponentialThermalSource::sourceFields(wordList& fieldNames)
{
    if (eName_ == word::null)
    {
...
)
{
    ...
    if (obr_.foundObject<fluidThermo>(basicThermo::dictName))
    {
        ...
        forAll(cells_, i)
        {
            ...
            const scalar h = V[celli] * Cm_*(1.0-exp(-mag(U[celli])/Ce_));
