...
:
    ...
    acceleration_(dict.lookupOrDefault("acceleration", true)),
    conservative_(dict.lookupOrDefault("conservative", false)),
    ddtPhiCorr_(dict.lookupOrDefault("ddtPhiCorr", false))
{}
...
) const
{
    ...
    if (coorFrame().validParentFrame())
    {
        ...
        const vectorField cellCentres(UIndirectList<vector>(mesh_.C(), cells_));
        tmp<vectorField> tframev = parentFrame.frameVelocity(cellCentres);
