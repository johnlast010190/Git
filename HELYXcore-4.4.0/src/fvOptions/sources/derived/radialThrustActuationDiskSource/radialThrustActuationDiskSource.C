...
:
    ...
    PoverD_(coeffs_.lookup<scalar>("PoverD")),
    R_(coeffs_.lookup<scalar>("R")),
    rh_(coeffs_.lookup<scalar>("rh")),
    ...
    shipResistanceThrust_
    (
        coeffs_.lookupOrDefault("shipResistanceThrust", false)
    ),
    tr_(0.0),
    Kts_(0.0),
    n_(0.0),
    ...
    uDisk_(Zero)
{
    ...
    if (propertiesFile.good())
    {
        ...
        propPosition_ = propertiesDict.lookup<vector>("propPosition");
    }
...
bool Foam::fv::radialThrustActuationDiskSource::updatePropellerPosition
(
    const vector& propPosition
)
{
...
bool Foam::fv::radialThrustActuationDiskSource::updatePropellerOrientation
(
    const quaternion& propOrientation
)
{
    ...
    while (idx < thrustCoeffs_.size())
    {
        if (neg(diffCurves[idx]))
        {
            ...
            else
            {
                ...
                Info<< "KT from OW curves " << Kts_ << nl
                    << "Thrust from OW curves "
                    << (Kts_*rho_*pow(n_,2)*pow(2*R_,4)) << nl
                    << "J " << Jts << nl
                    << "Rotational speed " << n_ << endl;
    ...
    switch (diskType_)
    {
        ...
        case mdOWCurves:
        {
            if (!coeffs_.found("wakeFraction") || coeffs_.found("sampleDisk"))
            {
                ...
                uDisk_ =
                    sampledSurfaces_.returnAverage<volVectorField>(sampleU)[0];
            }
            break;
        }
    }
...
bool Foam::fv::radialThrustActuationDiskSource::read(const dictionary& dict)
{
    if (option::read(dict))
    {
        ...
        propPosition_ = coeffs_.lookup<vector>("propPosition");
        propOrientation_ = coeffs_.lookup<quaternion>("propOrientation");
        shipResistanceThrust_ =
            coeffs_.lookupOrDefault("shipResistanceThrust", false);
        checkData();

        uShip_ = coeffs_.lookup("vesselSpeed");
        rho_ = coeffs_.lookup<scalar>("rhoInf");
        ...
        switch (diskType_)
        {
            ...
            case mdOWCurves:
            {
                thrustCoeffs_ = coeffs_.lookup<List<scalar>>("KT");
                torqueCoeffs_ = coeffs_.lookup<List<scalar>>("KQ");
                advanceCoeffs_ = coeffs_.lookup<List<scalar>>("J");
        ...
        {
            tr_ = coeffs_.subDict("controller").lookup<scalar>("startTime");
            propulsionType_ =
                propulsionModeTypeNames_.read
                (
                    coeffs_.lookup("propulsionTarget")
                );
            ...
            switch (diskType_)
            {
                ...
                {
                    n_ =
                        coeffs_.subDict("controller").lookup<scalar>
                        (
                            "estimatedRPS"
                        );
