...
)
{
    ...
    // mass flow rate
    monitoredQuantities[0] = fsum;
    // flux-ave temperature in deg C
    monitoredQuantities[1] =
        Tsum/fsum+basicThermo::TRefIfFound(mesh).value()-273.15;
...
bool Foam::fv::externalHeatExchangerSource::initialise()
{
    ...
    (
        ...
        (
            ...
            mesh_,
            dimensionedScalar(dimensionSet(1, -1, -3, 0, 0, 0, 0), 0) // [W/m3]
        )
    );
...
void Foam::fv::externalHeatExchangerSource::sourceFields
(
    wordList& fieldNames
)
{
