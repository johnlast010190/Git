...
void Foam::fv::thermalSource::updateRate(const fvMatrix<scalar>& eqn)
{
    ...
    if (heatTransferMode_ == hmScaledTinf)
    {
        ...
        if (Tinf_ < -basicThermo::TRefIfFound(obr_).value())
        {
            Tinf_ = -basicThermo::TRefIfFound(obr_).value();
            WarningInFunction << "To produce the specified heat transfer "
                << "in zone " << cellSetName()
                << " requires negative absolute Tinf." << nl
...
)
{
    ...
    (
        ...
        mesh_,
        dimensionedScalar(Su.dimensions()/he.dimensions(), 0)
    );
...
void Foam::fv::thermalSource::sourceFields
(
    wordList& fieldNames
)
{
