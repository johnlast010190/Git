...
) const
{
    ...
    forAll(pbm, patchi)
    {
        if (isA<directPolyPatch>(pbm[patchi]))
        {
            const directPolyPatch& dpp =
                refCast<const directPolyPatch>(pbm[patchi]);
            if (dpp.coupled() && !isA<processorPolyPatch>(dpp))
            {
                SubList<label>
                (
                    nbr,
                    dpp.size(),
                    dpp.start()-mesh.nInternalFaces()
                ) = dpp.faceCells();
            }
        }
    }
    ...
    forAll(pbm, patchi)
    {
        if (isA<directPolyPatch>(pbm[patchi]))
        {
            ...
            (
                dpp.coupled()
            && !isA<processorPolyPatch>(dpp)
            &&  refCast<const coupledPolyPatch>(dpp).owner()
            )
            {
                const labelUList& faceCells = dpp.faceCells();
                forAll(faceCells, i)
                {
                    label bFacei = dpp.start()+i-mesh.nInternalFaces();
