...
void Foam::npGrid::readTextFile(word& input, const word& fname)
{
...
Foam::npGrid::npGrid(const word& blkdef, const word& npfiles)
:
    gridField()
{
    wordList vtmp;
    ...
    if (!is)
    {
        FatalErrorInFunction
            << "Error reading np field, block definition file "
            << blkdef << " not found.\n"
            << exit(FatalError);
    }
    ...
    helyxMap::stringTokenize(vtmp, line, " ");

    if (vtmp.size() != 9)
    {
        FatalErrorInFunction
            << "Error reading np field, incorrect block definition: "
            << line << "\n"
            << exit(FatalError);
    }
    ...
    wordList vfiles;
    helyxMap::stringTokenize(vfiles, npfiles, ",");

    for (label i = 0; i < vfiles.size(); i++)
    {
        word fname = vfiles[i];
        helyxMap::stringTokenize(vtmp, fname, ":");

        if (vtmp.size() == 1 && fname != blkdef)
        {
            scalarSamples_.append(fname);
        }
        else if (vtmp.size() == 2)
        {
            if (vectorSamples_.empty())
            {
                vectorSamples_.append(vtmp[0]);
            }
    ...
    for (label f = 0; f < scalarSamples_.size(); f++)
    {
...
void Foam::npGrid::combineVectorField()
{
    DynamicList<point> vecField;
    vecField.setSize(nNodes());

    for (label ic = 0; ic < vectorSamples_.size(); ic++)
    {
        ...
        {
            FatalErrorInFunction
                << "Error constructing vector field, "
                << "x-component field not found.\n"
                << exit(FatalError);
        }
    ...
    Info<< "Number of vector fields constructed: "
        << vectorFields_.size() << endl;
}


void Foam::npGrid::writeMeshDict()
{
    ...
    helyxSample::writeBanner(os, "dictionary", "blockMeshDict");
    ...
    os  << ");\n\n";
    os  << "blocks\n\(\n";
    os  << "  hex (0 1 2 3 4 5 6 7) (" << nx_ + 1 << " "
        << ny_ + 1 << " " << nzpt() << " )simpleGrading (1 1 1)\n";
...
void Foam::npGrid::writeVisualization()
{
    word casedir = cwd();

    const fileName sysdir(casedir + "/system");
    mkDir(sysdir);

    const fileName constdir(casedir + "/constant");
    mkDir(constdir);

    const fileName meshdir(constdir + "/polyMesh");
    mkDir(meshdir);

    const fileName runtimedir(casedir + "/" + timeName());
    mkDir(runtimedir);
...
void Foam::npGrid::writeVectorField(label k)
{
    word subdir = cwd();

    const fileName runtimedir(subdir + "/1");
    mkDir(runtimedir);

    for (label iv = 0; iv < vectorSamples_.size(); iv++)
    {
        word name = vectorSamples_[iv];
        word fname = runtimedir + "/" + name;

        OFstream os(fname.c_str());
        if (!os)
        {
            FatalErrorInFunction
                << "Error writing vector sample field, file: \n"
                << fname <<" cannot be opened."
                << exit(FatalError);
        }

        helyxSample::writeBanner(os, "volVectorField", name);

        os  << "dimensions      [0 1 -1 0 0 0 0];\n\n\n";
        os  << "internalField   nonuniform List<vector>\n";

        os  << (nx_ + 1)*(ny_ + 1)*nzpt() << endl;
        os  << "(\n";
...
void Foam::npGrid::writeScalarField(label k)
{
    word subdir = cwd();
    ...
    for (label isc = 0; isc < scalarSamples_.size(); isc++)
    {
        ...
        {
            FatalErrorInFunction
                << "Error writing scalar sample field, file: \n"
                << fname << " cannot be opened."
                << exit(FatalError);
        }

        helyxSample::writeBanner(os, "volScalarField", name);
...
void Foam::npGrid::getScalarField
(
    const word& fname,
    DynamicList<scalar>& scfield
)
{
    word input;
    readTextFile(input, fname);

    wordList vtmp;
    helyxMap::stringTokenize(vtmp, input, "\n");
    ...
    for (label i = 0; i < vtmp.size(); i++)
    {
        ...
        helyxMap::trimLeft(str, "[");
        helyxMap::trimRight(str, "]");

        wordList vtmp1;
        helyxMap::stringTokenize(vtmp1, str, ",");

        DynamicList<scalar> ddat;
        ddat.setSize(vtmp1.size());

        for (label k = 0; k < vtmp1.size(); k++)
        {
    ...
    Info<< "Number of data items for scalar " << fname
        << ": " << vdata.size() << endl;
}
