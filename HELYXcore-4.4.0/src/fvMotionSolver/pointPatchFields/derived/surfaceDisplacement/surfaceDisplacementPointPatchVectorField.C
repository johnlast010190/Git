...
#include "pointPatchFields/derived/surfaceDisplacement/surfaceDisplacementPointPatchVectorField.H"
...
) const
{
    ...
    // mesh mover (if valid)
    const fvMesh& thisMesh = refCast<const fvMesh>(mesh);
    const motionSolver& motion =
        thisMesh.hasChangers()
      ? refCast<const fvMeshMovers::motionSolver>(thisMesh.mover()).motion()
...
void surfaceDisplacementPointPatchVectorField::updateCoeffs()
{
    ...
    this->forceAssign(currentDisplacement+offset);
