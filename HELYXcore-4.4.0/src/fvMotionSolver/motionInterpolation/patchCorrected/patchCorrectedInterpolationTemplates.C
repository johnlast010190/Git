...
(
    const VolField<Type>& cellDisplacement,
    PointField<Type>& pointDisplacement
) const
{
    ...
    // boundary conditions
    pointUncorrectedDisplacement.forceAssign
    (
...
) const
{
    ...
    forAll(patchGroups_, patchGroupI)
    {
        ...
        (
            ...
            data.mesh(),
            dimensionedScalar(data.dimensions(), 0),
            zeroGradientPointPatchField<scalar>::typeName
        );
        PointField<Type> patchData(data);
