/*---------------------------------------------------------------------------*\
|       o        |
|    o     o     |  HELYX (R) : Open-source CFD for Enterprise
|   o   O   o    |  Version : 4.4.0
|    o     o     |  ENGYS Ltd. <http://engys.com/>
|       o        |
\*---------------------------------------------------------------------------
License
    This file is part of HELYXcore.
    HELYXcore is based on OpenFOAM (R) <http://www.openfoam.org/>.

    HELYXcore is free software: you can redistribute it and/or modify it
    under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    HELYXcore is distributed in the hope that it will be useful, but WITHOUT
    ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or
    FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License
    for more details.

    You should have received a copy of the GNU General Public License
    along with HELYXcore.  If not, see <http://www.gnu.org/licenses/>.

Copyright
    (c) 2013 Engys Ltd.
    (c) 2011-2016 OpenFOAM Foundation
    (c) 2015-2016 OpenCFD Ltd.

\*---------------------------------------------------------------------------*/

#include "fvMotionSolvers/displacement/laplacian/displacementLaplacianFvMotionSolver.H"
#include "motionInterpolation/motionInterpolation/motionInterpolation.H"
#include "motionDiffusivity/motionDiffusivity/motionDiffusivity.H"
#include "finiteVolume/fvm/fvmLaplacian.H"
#include "db/runTimeSelection/construction/addToRunTimeSelectionTable.H"
#include "db/IOstreams/Fstreams/OFstream.H"
#include "meshTools/meshTools.H"
#include "meshes/polyMesh/polyTopoChangeMap/polyTopoChangeMap.H"
#include "fvMatrices/fvMatrices.H"
#include "meshQualityMetrics/meshQualityMetrics.H"

// * * * * * * * * * * * * * * Static Data Members * * * * * * * * * * * * * //

namespace Foam
{
    defineTypeNameAndDebug(displacementLaplacianFvMotionSolver, 0);

    addToRunTimeSelectionTable
    (
        motionSolver,
        displacementLaplacianFvMotionSolver,
        dictionary
    );

    addToRunTimeSelectionTable
    (
        displacementMotionSolver,
        displacementLaplacianFvMotionSolver,
        displacement
    );
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::displacementLaplacianFvMotionSolver::displacementLaplacianFvMotionSolver
(
    const polyMesh& mesh,
    const dictionary& dict
)
:
    displacementMotionSolver(mesh, dict, typeName),
    fvMotionSolver(mesh),
    cellDisplacement_
    (
        IOobject
        (
            "cellDisplacement",
            mesh.time().timeName(),
            mesh,
            IOobject::READ_IF_PRESENT,
            IOobject::AUTO_WRITE
        ),
        fvMesh_,
        dimensionedVector
        (
            "cellDisplacement",
            pointDisplacement_.dimensions(),
            Zero
        ),
        cellMotionBoundaryTypes<vector>(pointDisplacement_.boundaryField())
    ),
    pointLocation_(nullptr),
    interpolationPtr_
    (
        coeffDict().found("interpolation")
      ? motionInterpolation::New(fvMesh_, coeffDict().lookup("interpolation"))
      : motionInterpolation::New(fvMesh_)
    ),
    diffusivityPtr_
    (
        motionDiffusivity::New(fvMesh_, coeffDict().lookup("diffusivity"))
    ),
    frozenPointsZone_
    (
        coeffDict().found("frozenPointsZone")
      ? fvMesh_.pointZones().findZoneID(coeffDict().lookup("frozenPointsZone"))
      : -1
    ),
    writeBuffer_(coeffDict().lookupOrDefault<Switch>("writeBuffer", false)),
    meshObjectName_
    (
        coeffDict().lookupOrDefault<word>
        (
            "meshObjectName", "none"
        )
    )
{
    IOobject io
    (
        "pointLocation",
        fvMesh_.time().timeName(),
        fvMesh_,
        IOobject::MUST_READ,
        IOobject::AUTO_WRITE
    );

    if (debug)
    {
        Info<< "displacementLaplacianFvMotionSolver:" << nl
            << "    diffusivity       : " << diffusivityPtr_().type() << nl
            << "    frozenPoints zone : " << frozenPointsZone_ << endl;
    }


    if (io.typeHeaderOk<pointVectorField>(true))
    {
        pointLocation_.reset
        (
            new pointVectorField
            (
                io,
                pointMesh::New(fvMesh_)
            )
        );

        if (debug)
        {
            Info<< "displacementLaplacianFvMotionSolver :"
                << " Read pointVectorField "
                << io.name()
                << " to be used for boundary conditions on points."
                << nl
                << "Boundary conditions:"
                << pointLocation_().boundaryField().types() << endl;
        }
    }
}


Foam::displacementLaplacianFvMotionSolver::
displacementLaplacianFvMotionSolver
(
    const polyMesh& mesh,
    const dictionary& dict,
    const pointVectorField& pointDisplacement,
    const pointVectorField& points0
)
:
    displacementMotionSolver(mesh, dict, pointDisplacement, points0, typeName),
    fvMotionSolver(mesh),
    cellDisplacement_
    (
        IOobject
        (
            "cellDisplacement",
            mesh.time().timeName(),
            mesh,
            IOobject::READ_IF_PRESENT,
            IOobject::AUTO_WRITE
        ),
        fvMesh_,
        dimensionedVector
        (
            "cellDisplacement",
            pointDisplacement_.dimensions(),
            Zero
        ),
        cellMotionBoundaryTypes<vector>(pointDisplacement_.boundaryField())
    ),
    pointLocation_(nullptr),
    interpolationPtr_
    (
        coeffDict().found("interpolation")
      ? motionInterpolation::New(fvMesh_, coeffDict().lookup("interpolation"))
      : motionInterpolation::New(fvMesh_)
    ),
    diffusivityPtr_
    (
        motionDiffusivity::New(fvMesh_, coeffDict().lookup("diffusivity"))
    ),
    frozenPointsZone_
    (
        coeffDict().found("frozenPointsZone")
        ? fvMesh_.pointZones().findZoneID(coeffDict().lookup("frozenPointsZone"))
        : -1
    ),
    writeBuffer_(coeffDict().lookupOrDefault<Switch>("writeBuffer", false)),
    meshObjectName_
    (
        coeffDict().lookupOrDefault<word>
        (
            "meshObjectName", "none"
        )
    )
{
    IOobject io
    (
        "pointLocation",
        fvMesh_.time().timeName(),
        fvMesh_,
        IOobject::MUST_READ,
        IOobject::AUTO_WRITE
    );

    if (debug)
    {
        Info<< "displacementLaplacianFvMotionSolver:" << nl
            << "    diffusivity       : " << diffusivityPtr_().type() << nl
            << "    frozenPoints zone : " << frozenPointsZone_ << endl;
    }


    if (io.typeHeaderOk<pointVectorField>(true))
    {
        pointLocation_.reset
        (
            new pointVectorField
            (
                io,
                pointMesh::New(fvMesh_)
            )
        );

        if (debug)
        {
            Info<< "displacementLaplacianFvMotionSolver :"
                << " Read pointVectorField "
                << io.name()
                << " to be used for boundary conditions on points."
                << nl
                << "Boundary conditions:"
                << pointLocation_().boundaryField().types() << endl;
        }
    }
}


// * * * * * * * * * * * * * * * * Destructor  * * * * * * * * * * * * * * * //

Foam::displacementLaplacianFvMotionSolver::
~displacementLaplacianFvMotionSolver()
{}


// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

Foam::motionDiffusivity&
Foam::displacementLaplacianFvMotionSolver::diffusivity()
{
    if (!diffusivityPtr_.valid())
    {
        diffusivityPtr_ = motionDiffusivity::New
        (
            fvMesh_,
            coeffDict().lookup("diffusivity")
        );
    }
    return diffusivityPtr_();
}


void Foam::displacementLaplacianFvMotionSolver::constrainInternalPoints
(
    pointField& curPoints
) const
{
    vectorField fixedDisplacement
    (
        fvMesh_.nPoints(),
        vector(GREAT, GREAT, GREAT)
    );

    // Compute point displacement in buffer zone(s)
    forAll(pointDisplacement_.boundaryField(), patchI)
    {
        pointDisplacement_.boundaryFieldRef()[patchI].setField
        (
            points0(),
            fixedDisplacement
        );
    }

    // Fix location in marked points
    forAll(fixedDisplacement, pointI)
    {
        if (fixedDisplacement[pointI] != vector(GREAT, GREAT, GREAT))
        {
            curPoints[pointI] = points0()[pointI] +
                fixedDisplacement[pointI];
        }
    }

    // Implement frozen points
    // Highly suspicious as it does not constrain cellDisplacement_
    // and likely to crash if motionDiffusivity in the zone is non-zero
    if (frozenPointsZone_ != -1)
    {
        const pointZone& pz = fvMesh_.pointZones()[frozenPointsZone_];

        forAll(pz, i)
        {
            curPoints[pz[i]] = points0()[pz[i]];
        }
    }

    twoDCorrectPoints(curPoints);
}


void Foam::displacementLaplacianFvMotionSolver::setBufferZone
(
    fvVectorMatrix& TEqn
)
{
    // Sanity check - has cell already been set y/n
    // Only needed when multiple patches are constrained
    boolList marked(fvMesh_.nCells(), false);

    // List of cellID's to fix
    DynamicList<label> cells(fvMesh_.nCells());
    // List of corresponding values to set
    DynamicList<vector> fixedValues(fvMesh_.nCells());

    autoPtr<volScalarField> pointDisplacementBufferZone;
    if (writeBuffer_ && fvMesh_.time().outputTime())
    {
        pointDisplacementBufferZone.reset
        (
            new volScalarField
            (
                IOobject
                (
                    "pointDisplacementBufferZone",
                    fvMesh_.time().timeName(),
                    fvMesh_,
                    IOobject::READ_IF_PRESENT,
                    IOobject::NO_WRITE
                ),
                fvMesh_,
                dimensionedScalar(dimless, 0)
            )
        );
    }

    forAll(pointDisplacement_.boundaryField(), patchI)
    {
        // List of cell labels in buffer zone
        // Gets filled by pointDisplacement_ bc
        labelList clabels(0);
        // Cell centre displacement field in buffer zone
        // Gets filled by pointDisplacement_ bc
        Field<vector> fv(0);

        // Compute cell centre displacement in buffer zone
        pointDisplacement_.boundaryFieldRef()[patchI].manipulateMatrix
        (
            points0(),
            clabels,
            fv
        );

        forAll(clabels, i)
        {
            label cellI = clabels[i];

            if (!marked[cellI])
            {
                marked[cellI] = true;
                cells.append(cellI);
                fixedValues.append(fv[i]);
            }
            else
            {
                WarningInFunction << "Cell " << cellI << " already fixed by "
                    << "another bc. Please check your setup." << endl;
            }
        }

        if (writeBuffer_ && fvMesh_.time().outputTime())
        {
            forAll(clabels, i)
            {
                // assign cellID
                pointDisplacementBufferZone()[clabels[i]] = clabels[i];
            }
        }
    }

    if (writeBuffer_ && fvMesh_.time().outputTime())
    {
        Info<< "writing pointDisplacementBufferZone" << endl;
        pointDisplacementBufferZone().write();
    }

    // Resize dynamic lists
    cells.shrink();
    fixedValues.shrink();

    if (cells.size())
    {
        // Fix matrix solution in buffer zone
        TEqn.setValues(cells, vectorField(fixedValues));
    }
}


Foam::tmp<Foam::pointField>
Foam::displacementLaplacianFvMotionSolver::curPoints() const
{
    interpolationPtr_->interpolate
    (
        cellDisplacement_,
        pointDisplacement_
    );

    if (pointLocation_.valid())
    {
        if (debug)
        {
            Info<< "displacementLaplacianFvMotionSolver : applying "
                << " boundary conditions on " << pointLocation_().name()
                << " to new point location."
                << endl;
        }

        pointLocation_().primitiveFieldRef() =
            points0()
          + pointDisplacement_.primitiveField();

        pointLocation_().correctBoundaryConditions();

        constrainInternalPoints(pointLocation_().primitiveFieldRef());

        return tmp<pointField>(pointLocation_().primitiveField());
    }
    else
    {
        tmp<pointField> tcurPoints
        (
            points0() + pointDisplacement_.primitiveField()
        );
        pointField& curPoints = tcurPoints.ref();

        constrainInternalPoints(curPoints);

        // Update pointDisplacement after correction
        pointDisplacement_.primitiveFieldRef() = curPoints-points0();

        return tcurPoints;
    }
}


void Foam::displacementLaplacianFvMotionSolver::solve()
{
    // The points have moved so before interpolation update
    // the motionSolver accordingly
    movePoints(fvMesh_.points());

    diffusivity().correct();
    pointDisplacement_.boundaryFieldRef().updateCoeffs();

    // We explicitly do NOT want to interpolate the motion inbetween
    // different regions so bypass all the matrix manipulation.
    fvVectorMatrix TEqn
    (
        fvm::laplacian
        (
            ((meshObjectName_ == "none") ?
            diffusivity().operator()()
          : constrainDiffusivity()),
            cellDisplacement_,
            "laplacian(diffusivity,cellDisplacement)"
        )
    );

    setBufferZone(TEqn);

    TEqn.solveSegregatedOrCoupled(TEqn.solverDict());
}


Foam::tmp<Foam::surfaceScalarField>
Foam::displacementLaplacianFvMotionSolver::constrainDiffusivity()
{
    surfaceScalarField diffCoeff = diffusivity().operator()()();

    const meshQualityMetrics& mqMetrics =
        meshQualityMetrics::New(fvMesh_, meshObjectName_);

    const boolList& markedCells = mqMetrics.badQualityCells();

    forAll(diffCoeff, fI)
    {
        const label own = fvMesh_.owner()[fI];
        const label nei = fvMesh_.neighbour()[fI];

        if (markedCells[own] || markedCells[nei])
        {
            diffCoeff[fI] = 100;
        }
    }

    if (fvMesh_.time().outputTime() && mqMetrics.writeField())
    {
        volScalarField outField
        (
            IOobject
            (
                meshObjectName_ + "badQualityCells",
                fvMesh_.time().timeName(),
                fvMesh_,
                IOobject::NO_READ,
                IOobject::AUTO_WRITE,
                false
            ),
            fvMesh_,
            dimensionedScalar(dimless, 0),
            "zeroGradient"
        );
        forAll(markedCells, cI)
        {
            if (markedCells[cI]) outField[cI] = 1;
        }
        outField.write();
    }

    return tmp<surfaceScalarField>(new surfaceScalarField(diffCoeff));
}


void Foam::displacementLaplacianFvMotionSolver::topoChange
(
    const polyTopoChangeMap& map
)
{
    displacementMotionSolver::topoChange(map);
    diffusivityPtr_.clear();
}


void Foam::displacementLaplacianFvMotionSolver::mapMesh
(
    const polyMeshMap& map
)
{
    displacementMotionSolver::mapMesh(map);
    diffusivityPtr_.clear();
}


// ************************************************************************* //
