    ...
    if (io.typeHeaderOk<pointVectorField>(true))
    {
        ...
        if (debug)
        {
            Info<< "solidBodyDisplacementLaplacianFvMotionSolver :"
                << " Read pointVectorField " << io.name()
                << " to be used for boundary conditions on points." << nl
                << "Boundary conditions:"
...
Foam::solidBodyDisplacementLaplacianFvMotionSolver::curPoints() const
{
    ...
    else
    {
        ...
        forAll(setPointIndices(), i)
        {
            const label pointi = setPointIndices()[i];
            curPoints[pointi] =
                newPoints[pointi] + pointDisplacement_.primitiveField()[pointi];
        }
...
void Foam::solidBodyDisplacementLaplacianFvMotionSolver::topoChange
(
