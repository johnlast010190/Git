...
:
    ...
    nNonOrthogonalCorr_(coeffDict().lookup<label>("nNonOrthogonalCorr")),
        ...
        forAll(hit, facei)
        {
            if (hit[facei].hit())
            {
                ...
                if (rotCellId != -1)
                {
                    ...
                    &&
                        (
                            Foam::cos(degToRad(minAng_)) > cosMax
                    ...
                    )
                    {
                        ...
                        if (magRot > 0)
                        {
                            const scalar theta = Foam::asin(magRot);
...
void Foam::surfaceAlignedSBRStressFvMotionSolver::solve()
{
    ...
    (
        volScalarField::New("mu", fvMesh_, dimensionedScalar(dimless, 0))
    );
