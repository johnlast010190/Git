...
#include "motionSolvers/sinus/sinusMotionSolver.H"
...
namespace Foam
{
    defineTypeNameAndDebug(sinusMotionSolver, 0);
...
Foam::sinusMotionSolver::sinusMotionSolver
(
    const polyMesh& mesh,
...
Foam::sinusMotionSolver::curPoints() const
{
    ...
    (
       ...
       *(amplitude_*Foam::sin(radFrequency_*mesh().time().value() + phase_))
    );
...
void Foam::sinusMotionSolver::movePoints(const pointField& p)
{
    // No local data that needs adapting.
}


void Foam::sinusMotionSolver::topoChange(const polyTopoChangeMap& map)
{
