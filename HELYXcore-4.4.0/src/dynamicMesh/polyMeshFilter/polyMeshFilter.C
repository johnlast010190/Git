    ...
    )
    {
        ...
        if (controlMeshQuality())
        {
            ...
            Info<< nl << "    Number of bad faces     : " << nBadFaces << nl
                ...
                << returnReduceToMaster(isErrorPoint.count(), sumOp<unsigned int>())
                << endl;
...
) const
{
    ...
    reduce(
        std::tie(sum, min, max, validElements),
        ParallelOp<sumOp<scalar>, minOp<scalar>, maxOp<scalar>, sumOp<label>>{}
    );
    const label totFieldSize = returnReduceToMaster(fld.size(), sumOp<label>());
