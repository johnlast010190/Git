...
)
{
    ...
    forAll(patches0, patchi)
    {
        ...
        else
        {
            ...
            const directPolyPatch& dpp =
                refCast<const directPolyPatch>(patches0[patchi]);
    ...
    forAll(from1ToAllPatches, patchi)
    {
        ...
        if (allPatchi >= patches0.size())
        {
            ...
            else
            {
                filteredPatchi = allPatches.size();
                const directPolyPatch& dpp =
                    refCast<const directPolyPatch>(patches1[patchi]);
    ...
    return List<polyPatch*>(allPatches, true);
}
...
)
{
    ...
    DynamicList<polyPatch*> indPatches(allPatchNames.size());
    label nIndPatches = 0;
    label nIndPatches1 = 0;
    ...
    labelList indMap0(from0ToAllPatches.size(), -1);
    labelList indMap1(from1ToAllPatches.size(), -1);

    forAll(from0ToAllPatches, patch0)
    {
        if (!isA<directPolyPatch>(patches0[patch0]))
        {
            indMap0[patch0] = patch0;
        }
    }
    forAll(from1ToAllPatches, patch1)
    {
        if (!isA<directPolyPatch>(patches1[patch1]))
        {
            indMap1[patch1] = patch1;
        }
    }
        ...
        else
        {
            ...
            else
            {
                const indirectPolyPatch& ipp =
                    refCast<const indirectPolyPatch>(allPatches[patch0]);
                indPatches.append
                (
                    ipp.clone
                    (
                ...
                from0ToAllPatches[patch0] = allPatchi+nIndPatches;
                nIndPatches++;
            }
        }
    }
    ...
    forAll(from1ToAllPatches, patch1)
    {
        ...
        if (uncompactAllPatchi >= from0ToAllPatches.size()-nIndPatches)
        {
            // Patch has not been merged with any mesh0 patch.
            ...
            else
            {
                ...
                {
                    if (from0ToAllPatches.size()==0)
                    {
                    const indirectPolyPatch& ipp =
                        refCast<const indirectPolyPatch>(patches1[patch1]);
                        indPatches.append
                        (
                            ipp.clone
                            (
                        ...
                        from1ToAllPatches[patch1] = allPatchi+nIndPatches;

                        nIndPatches++;
                    }
                    else
                    {
                        from1ToAllPatches[patch1] = allPatchi+nIndPatches1;
                        nIndPatches1++;
                    }
                }
            }
    ...
    forAll(indMap0, patch0)
    {
        if (indMap0[patch0]>0)
        {
            from0ToAllPatches[patch0] += nUnique;
        }
    }
    ...
    label oldSize = allPatches.size();
    label newSize = oldSize+nIndPatches;
    allPatches.setSize(newSize);
