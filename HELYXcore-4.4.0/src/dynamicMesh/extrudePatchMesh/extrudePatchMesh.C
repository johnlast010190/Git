...
#include "extrudePatchMesh/extrudePatchMesh.H"
    ...
    dict_(dict)
{
    ...
    forAll(dicts, patchi)
    {
        patchNames[patchi] = dicts[patchi].lookup<word>("name");
        patchTypes[patchi] = dicts[patchi].lookup<word>("type");
    }
...
void extrudePatchMesh::extrudeMesh(const List<polyPatch*>& regionPatches)
{
    ...
    {
        bool columnCells = dict_.lookup<bool>("columnCells");
        ...
        const directPolyPatch& dpp =
            refCast<const directPolyPatch>(extrudedPatch_);
