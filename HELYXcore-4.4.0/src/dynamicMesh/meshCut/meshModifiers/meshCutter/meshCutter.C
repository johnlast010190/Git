...
)
{
    ...
    else
    {
        ...
        //if face reversed then also flip zone
        if (zoneID >= 0)
        {
            zoneFlip = (zoneFlip ? false : true);
        }
...
void Foam::meshCutter::topoChange(const polyTopoChangeMap& map)
{
...

    {
        ...
        addedCells_ = std::move(newAddedCells);
    }

    {
        ...
        addedFaces_ = std::move(newAddedFaces);
    }

    {
        ...
        // Copy
        addedPoints_ = std::move(newAddedPoints);
    }
}
