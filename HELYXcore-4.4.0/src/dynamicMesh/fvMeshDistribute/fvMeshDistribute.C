...
)
{
    ...
    (
        nbrProc,
        indices,
        fvLessProcPatches(nbrProc, referPatchID, referNbrPatchID)
    );
...
(
    const labelList& distribution,
    PtrList<fvMesh>& myProcMeshes, /*= fvDummyMeshList*/
    PtrList<polyDistributionMap>& mapsDist /*= fvDummyMapList*/
)
{
    ...
    (
        fieldNames(volVector1Field::typeName, nFields)
    );
    const wordList volVectors4
    (
        fieldNames(volVector4Field::typeName, nFields)
    );
    const wordList volTensors4
    (
        fieldNames(volTensor4Field::typeName, nFields)
    );
    ...
    (
        fieldNames(volVector1Field::Internal::typeName, nFields)
    );
    const wordList dimVectors4
    (
        fieldNames(volVector4Field::Internal::typeName, nFields)
    );
    const wordList dimTensors4
    (
        fieldNames(volTensor4Field::Internal::typeName, nFields)
    );
    ...
    if (nFinalProcs_ > Pstream::nProcs())
    {
        ...
        forAll(myProcMeshes, i)
        {
            ...
            correctCoupledPatchFields<volScalarField>(mesh);
            correctCoupledPatchFields<volVectorField>(mesh);
            correctCoupledPatchFields<volSphericalTensorField>(mesh);
            correctCoupledPatchFields<volSymmTensorField>(mesh);
            correctCoupledPatchFields<volTensorField>(mesh);
            correctCoupledPatchFields<volVector1Field>(mesh);
            correctCoupledPatchFields<volVector4Field>(mesh);
            correctCoupledPatchFields<volTensor4Field>(mesh);
    ...
    else // standard behaviour where every proc has one mesh
    {
        ...
        // Correct processor patch fields
        correctCoupledPatchFields<volScalarField>(mesh_);
        correctCoupledPatchFields<volVectorField>(mesh_);
        correctCoupledPatchFields<volSphericalTensorField>(mesh_);
        correctCoupledPatchFields<volSymmTensorField>(mesh_);
        correctCoupledPatchFields<volTensorField>(mesh_);
        correctCoupledPatchFields<volVector1Field>(mesh_);
        correctCoupledPatchFields<volVector4Field>(mesh_);
        correctCoupledPatchFields<volTensor4Field>(mesh_);
