...
) const
{
    ...
    forAll(edges, edgeI)
    {
        wght[edgeI] = 1.0/(edges[edgeI].mag(points)+VSMALL);
    }
    return twght;
}
...
)
{
    ...
    if (debug)
    {
        ...
        forAll(ppMeshPoints, patchPointi)
        {
            ...
            if (mag(newDisp-patchDisp[patchPointi]) > VSMALL)
            {
...
Foam::pointField Foam::motionSmootherAlgo::curPoints() const
{
    ...
    pointField tnewPoints(oldPoints_ + totalDisplacement.primitiveField());
    ...
    forAll(resetDisp, pointI)
    {
        if (resetDisp[pointI])
        {
            tnewPoints[pointI] = oldPoints_[pointI];
            nReset++;
        }
    }
...
)
{
    ...
    else
    {
        ...
        if (mag(errorReduction) < SMALL)
        {
            ...
            wrongFaces = std::move(newWrongFaces);
            wrongFaces.sync(mesh_);
        }
...
void Foam::motionSmootherAlgo::topoChange(bool patchesRenumbered)
{
