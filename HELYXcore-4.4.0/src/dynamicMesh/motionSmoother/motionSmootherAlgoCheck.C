...
)
{
    ...
    if (report) {
        // If the detailed report is needed, then reduce all the counters
        // seperately so we can make a nice breakdown of what's wrong:
        reduce(
            std::tie(
                nBadDotProdFaces,
        ...
        if (doSnapVolumeCheck) {
            Info<< "    faces on cells with snap volume ratio < "
                 << setw(3)
                 << max(minSnapRelativeVolume,minSnapRelativeTetVolume)
                 << "            : "
    ...
    // If not reporting, this is going to just be a count of errors found by
    // this node. If we did reporting above, this will be all errors everywhere.
    nWrongFaces =
        nBadDotProdFaces +
        nBadFacePyramidFaces +
        nBadFaceTetFaces +
    ...
    if (meshCutterPtr)
    {
        const scalar relEdgeLength
        (
            dict.lookupOrDefault<scalar>("relativeEdgeLength", scalar(-1), true)
        );
        const scalar relVol
        (
            dict.lookupOrDefault<scalar>("relativeVol", scalar(-1), true)
        );
        const scalar relFaceArea
        (
            dict.lookupOrDefault<scalar>("relativeFaceArea", scalar(-1), true)
        );

        if (relVol > 0 || relFaceArea > 0 || relEdgeLength > 0)
        {
            label nRelWrongFace = wrongFaces.size();
            bool doRelativeEdgeLength = relEdgeLength > 0;
            label nBadRelativeEdgeLength = 0;
            if (doRelativeEdgeLength)
            {
                polyMeshGeometry::checkRelativeEdgeLength
                (
                    verbose,
                    relEdgeLength,
                    *meshCutterPtr,
                ...
                nBadRelativeEdgeLength = wrongFaces.size() - nRelWrongFace;
                nRelWrongFace += nBadRelativeEdgeLength;
            }

            bool doRelativeVolume = relVol > 0;
            label nBadRelativeVolFaces = 0;
            if (doRelativeVolume)
            {
                polyMeshGeometry::checkRelativeCellVol
                (
                    verbose,
                    relVol,
                    *meshCutterPtr,
                ...
                nBadRelativeVolFaces = wrongFaces.size() - nRelWrongFace;
                nRelWrongFace += nBadRelativeVolFaces;
            }

            bool doRelativeFaceArea = relFaceArea > 0;
            label nBadRelativeAreaFaces = 0;
            if (doRelativeFaceArea)
            {
                polyMeshGeometry::checkRelativeFaceArea
                (
                    verbose,
                    relFaceArea,
                    *meshCutterPtr,
                ...
                nBadRelativeAreaFaces = wrongFaces.size() - nRelWrongFace;
                nRelWrongFace += nBadRelativeAreaFaces;
            }
            ...
            if (report)
            {
                ...
                (
                    std::tie
                    (
                        nBadRelativeEdgeLength,
                        nBadRelativeVolFaces,
                        nBadRelativeAreaFaces
                    ),
                ...
                if (doRelativeEdgeLength)
                {
                    Info<< "    faces with relative edge length < "
                        << setw(5) << relEdgeLength
                        << "                : "
                        << nBadRelativeEdgeLength << endl;
                }
                if (doRelativeVolume)
                {
                    Info<< "    faces on cells with relative volume < "
                        << setw(5) << relVol
                        << "            : "
                        << nBadRelativeVolFaces << endl;
                }
                if (doRelativeFaceArea)
                {
                    Info<< "    faces with relative area < "
                        << setw(5) << relFaceArea
                        << "                       : "
                        << nBadRelativeAreaFaces << endl;
                }
            }
            nWrongFaces += nBadRelativeEdgeLength + nBadRelativeVolFaces
               + nBadRelativeAreaFaces;
        }
    }

    if (!report) {
        // If we didn't do the reporting, then we didn't distribute all the
        // various `n<SpecificProblem>Faces`, so the above calculation of
        // `nWrongFaces` will count only errors found on this node, so we
        // must do a reduce of just that one value to get the overall sum.
        // This way, we only do the amount of reducing we actually need: just
        // this one integer if not reporting, but all of the little counters
        // if the breakdown is needed.
...
)
{
    ...
    // TODO: Uh, this function seems to be the same as the other checkMesh
    // function above, except this one skips snapVolumeFaces and
    // GaussGreenCentroidFaces. ... Consider using a constexpr bool
    // non-type-template-parameter to redefine as one function with zero
    // runtime overhead. A kitten dies evey time you copy-paste a 500 line
    // function.

    if (report) {
        // If the detailed report is needed, then reduce all the counters
        // separately so we can make a nice breakdown of what's wrong:
        reduce(
            std::tie(
                nBadDotProdFaces,
                nBadFacePyramidFaces,
    ...
    if (!report) {
        ...
        // `nWrongFaces` will count only errors found on this node, so we
        // must do a reduce of just that one value to get the overall sum.
        // This way, we only do the amount of reducing we actually need:
        // just this one integer if not reporting, but all of the little
        // counters if the breakdown is needed.
        reduce(nWrongFaces, sumOp<label>{});
    }
