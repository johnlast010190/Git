...
)
{
    ...
    const scalar severeNonorthogonalityThreshold = Foam::cos(degToRad(orthWarn));
    const scalar severeFaceCentreOrthogonalityThreshold =
        Foam::cos(degToRad(maxFaceCentreNonOrtho));
    ...
    forAll(checkFaces, i)
    {
        ...
        if (mesh.isInternalFace(facei))
        {
            ...
            scalar orthoAngle =
                radToDeg(Foam::acos(min(scalar(1.0), max(scalar(-1.0), dDotS))));
            sumDDotS += orthoAngle;
            nDDotS++;
            if (fieldIndex != -1)
            {
...
bool Foam::polyMeshGeometry::checkRelativeCellVol
(
    const bool verbose,
    const scalar warnRelVol,
...
)
{
    ...
    const scalar edge0Length = meshCutter.level0EdgeLength();

    const scalarField& cellVolumes = mesh.cellVolumes();

    scalar minRelVol = GREAT;
    scalar sumRelVol = 0.0;
    label nSumRelVol = 0;
    label nWarnRelVol = 0;
    ...
    forAll(affectedCells, i)
    {
        ...
        scalar vol = cellVolumes[celli];
        label clevel = cellLevel[celli];
        if (clevel > -1)
        {
            scalar len = edge0Length / (1<<clevel);
            scalar vol0 = pow(len,3);
            if (vol0 > SMALL)
            {
               scalar vratio = min(vol,vol0)/max(vol,vol0);
               minRelVol = min(minRelVol, vratio);
               sumRelVol += vratio;
               nSumRelVol++;
               if (vratio < warnRelVol)
               {
                   if (setPtr)
                   {
                       // Insert all faces of the cell.
    ...
    reduce
    (
        std::tie(minRelVol, sumRelVol, nSumRelVol, nWarnRelVol),
        ParallelOp<minOp<scalar>, sumOp<scalar>, sumOp<label>, sumOp<label>>{}
    );
    ...
    if (verbose)
    {
        if (nSumRelVol > 0)
        {
            Info<< "Cell relative volume (1 = unchanged) : average = "
                << sumRelVol / nSumRelVol << "  min = " << minRelVol << endl;
        }

        if (nWarnRelVol > 0)
        {
            Info<< "There are " << nWarnRelVol
                << " cells with relative volumes < " << warnRelVol << '.'
                << nl << endl;
        }
        else
        {
            Info<< "All faces have relative volumes > " << warnRelVol
                << '.' << nl << endl;
        }
    }
    ...
    if (nWarnRelVol > 0)
    {
        ...
        {
            WarningInFunction
                << nWarnRelVol << " cells with relative volume < " << warnRelVol
                << " found.\n"
                << endl;
        }
...
bool Foam::polyMeshGeometry::checkRelativeFaceArea
(
    const bool verbose,
    const scalar warnRelFaceArea,
    const hexRef8& meshCutter,
    const labelList& checkFaces,
    labelHashSet* setPtr
)
{
    ...
    scalar minRelFaceArea = GREAT;
    scalar sumRelFaceArea = 0.0;
    label nSumRelFaceArea = 0;
    label nWarnRelFaceArea = 0;

    forAll(checkFaces, i)
    {
        label facei = checkFaces[i];
        scalar faceArea = mag(Sf[facei]);
        label flevel = meshCutter.faceLevel(facei);
        if (flevel > -1)
        {
            scalar len = edge0Length / (1<<flevel);
            scalar faceArea0 = len*len;
            if (faceArea0 > SMALL)
            {
               scalar aratio = min(faceArea,faceArea0)/max(faceArea,faceArea0);
               minRelFaceArea = min(minRelFaceArea, aratio);
               sumRelFaceArea += aratio;
               nSumRelFaceArea++;
               if (aratio < warnRelFaceArea)
               {
                   if (setPtr)
                   {
                       setPtr->insert(facei);
    ...
    if (verbose)
    {
        if (nSumRelFaceArea > 0)
        {
            Info<< "Face relative face area (1 = unchanged) : average = "
                << sumRelFaceArea / nSumRelFaceArea
                << "  min = " << minRelFaceArea << endl;
        }

        if (nWarnRelFaceArea > 0)
        {
            Info<< "There are " << nWarnRelFaceArea
                << " faces with relative face area < " << warnRelFaceArea
                << '.' << nl
                << endl;
        }
        else
        {
            Info<< "All faces have relative face area > " << warnRelFaceArea
                << '.' << nl
                << endl;
        }
    }
    ...
    if (nWarnRelFaceArea > 0)
    {
        ...
        {
            WarningInFunction
                << nWarnRelFaceArea << " faces with face areas < "
                << warnRelFaceArea
                << " found.\n"
                << endl;
        }
...
bool Foam::polyMeshGeometry::checkRelativeEdgeLength
(
    const bool verbose,
    const scalar warnRelEdgeLength,
    const hexRef8& meshCutter,
    const labelList& checkFaces,
    labelHashSet* setPtr
)
{
    ...
    scalar minRelEdgeLength = GREAT;
    scalar sumRelEdgeLength = 0.0;
    label nSumRelEdgeLength = 0;
    label nWarnRelEdgeLength = 0;
    ...
    forAll(checkFaces, i)
    {
        ...
        if (flevel > -1)
        {
            scalar len = edge0Length / (1<<flevel);
            const labelList& fEdges = mesh.faceEdges()[facei];
            scalar mineratio = GREAT;
            forAll(fEdges, fei)
            {
                scalar eLen = mesh.edges()[fEdges[fei]].mag(pts);
                scalar eratio = min(len,eLen)/max(len,eLen);
                mineratio = min(eratio, mineratio);
            }

            minRelEdgeLength = min(minRelEdgeLength, mineratio);
            sumRelEdgeLength += mineratio;
            nSumRelEdgeLength++;
            if (mineratio < warnRelEdgeLength)
            {
                if (setPtr)
                {
                   setPtr->insert(facei);
    ...
    if (verbose)
    {
        if (nSumRelEdgeLength > 0)
        {
            Info<< "Face relative edge length (1 = unchanged) : average = "
                << sumRelEdgeLength / nSumRelEdgeLength
                << "  min = " << minRelEdgeLength << endl;
        }

        if (nWarnRelEdgeLength > 0)
        {
            Info<< "There are " << nWarnRelEdgeLength
                << " faces with relative edge length < " << warnRelEdgeLength
                << '.' << nl
                << endl;
        }
        else
        {
            Info<< "All faces have relative edge length > " << warnRelEdgeLength
                << '.' << nl
                << endl;
        }
    }
    ...
    if (nWarnRelEdgeLength > 0)
    {
        ...
        {
            WarningInFunction
                << nWarnRelEdgeLength << " faces with relative edge length < "
                << warnRelEdgeLength
                << " found.\n"
                << endl;
        }
