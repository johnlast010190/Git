...
void Foam::boundaryMesh::read(const polyMesh& mesh)
{
    ...
    if (debug)
    {
        ...
        forAll(patches_, patchi)
        {
            const boundPatch& bp = patches_[patchi];
...
void Foam::boundaryMesh::readTriSurface(const fileName& fName)
{
    ...
    forAll(regions, i)
    {
        ...
        {
            regionToBoundaryPatch.insert(regions[i], bPatch);
    ...
    forAll(indices, indexI)
    {
        ...
        if (tri.region() != surfRegion)
        {
            ...
            boundPatch& bp = patches_[foamRegion];
...
) const
{
    ...
    {
        const boundPatch& bp = patches_[bPatchi];
    ...
    forAll(patches_, bPatchi)
    {
        ...
        else
        {
            ...
            const directPolyPatch& dpp =
                refCast<const directPolyPatch>(oldPatch);

            newPatchPtrList[newPatchi] = dpp.clone
            (
                newMesh.boundaryMesh(),
                newPatchi,
                facesToBeDone,
...
void Foam::boundaryMesh::addPatch(const word& patchName)
{
    ...
    boundPatch* bpPtr = new boundPatch
    (
        patchName,
        patchi,
        0,
...
void Foam::boundaryMesh::deletePatch(const word& patchName)
{
    ...
    PtrList<boundPatch> newPatches(patches_.size() - 1);
...
)
{
    ...
    PtrList<boundPatch> newPatches(patches_.size());
    ...
    forAll(patches_, patchi)
    {
        if (patchi == changeI)
        {
            ...
            const boundPatch& oldBp = patches_[patchi];
...
) const
{
    ...
    for (label i = 0; i < nFaces; i++)
    {
        ...
        triEngine.triangulate(UIndirectList<point>(mesh().points(), f));

        // Copy into triVerts
        forAll(triEngine.triPoints(), triI)
        {
            const face& triF = triEngine.triPoints(triI, f);
...
) const
{
    ...
    for (label i = 0; i < nFaces; i++)
    {
        ...
        triEngine.triangulate(UIndirectList<point>(patch.localPoints(), f));
