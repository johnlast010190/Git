...
) const
{
    ...
    if (detJ < 1e-5)
    {
        collapseAxis = f.faceEdge(f.longestEdge(pts)).vec(pts);
...
//    }
    ...
    reduce(
        std::tie(nPoints, nUnvisited, nUncollapsed, nCollapsed),
...
) const
{
    ...
    while (true)
    {
        ...
        label totalNEdges = mesh_.nEdges();
        reduce(
            std::tie(nUncollapsed, totalNEdges),
