...
) const
{
    ...
    //check that there aren't any faces that would be left with only two edges
    label countUnflagged = 0;
    forAll(mesh_.faces(),fI)
    {
        label countDeleted = 0;
        forAll(mesh_.faces()[fI],j)
        {
            if (pointCanBeDeleted[mesh_.faces()[fI][j]])
            {
                countDeleted++;
            }
        }
        if (countDeleted > mesh_.faces()[fI].size()-3)
        {
            countUnflagged++;
            forAll(mesh_.faces()[fI],j)
            {
                pointCanBeDeleted[mesh_.faces()[fI][j]] = false;
            }
        }
    }
    if (countUnflagged)
    Info<< "Unflagged nodes previously marked for deletion in "
         << countUnflagged << " faces\n";
