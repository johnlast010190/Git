...
#include "polyTopoChange/refinementDistanceData/refinementDistanceData.H"
...
// master face now unset for internal faces because of problem identifying refinement
// faces
...
Foam::scalar Foam::hexRef8::getLevel0EdgeLength() const
{
    ...
    Pstream::listCombineReduce(typEdgeLenSqr, minOp<scalar>());
    ...
    Pstream::listCombineReduce(maxEdgeLenSqr, maxOp<scalar>());
    ...
    savedCellLevel_(0)
{
    ...
    {
       history_.writeOpt() = IOobject::NO_WRITE;
    }
...
) const
{
    ...
    // Calculate coupled cell centre and level
    vectorField neiCc(mesh_.nFaces()-mesh_.nInternalFaces());
    labelList nLevel(mesh_.nFaces()-mesh_.nInternalFaces());

    for (label faceI = mesh_.nInternalFaces(); faceI < mesh_.nFaces(); faceI++)
    {
        label own = faceOwner[faceI];
        neiCc[faceI-mesh_.nInternalFaces()] = cc[own];
        nLevel[faceI-mesh_.nInternalFaces()] =
                (
    ...
    syncTools::swapBoundaryFaceList(mesh_, nLevel);
    ...
    forAll(facesToCheck, i)
    {
        ...
        if (!mesh_.isInternalFace(facei))
        {
            ...
            if (patchi != -1 && isA<processorPolyPatch>(patches[patchi]))
            {
                ...
                label neiLevel = nLevel[facei-mesh_.nInternalFaces()];
    ...
    if (debug)
    {
...

        {
            ...
            (
                mesh_,
                "cellsToRefineOut2",
                labelHashSet(newCellsToRefine.size())
            );
            forAll(refineCell, celli)
            {
                if (refineCell.get(celli))
...
)
{
    ...
    labelList faceAnchorLevel(mesh_.nFaces(),-1);
    boolList refinedCellFaces(mesh_.nFaces(),false);
    ...
    forAll(cellLabels, i)
    {
        ...
        forAll(cFaces, cFI)
        {
            ...
            refinedCellFaces[facei] = true;
            faceAnchorLevel[facei] = 0;
        }
    }

    // Synchronize faceAnchorLevel across coupled patches. (logical or)
    ...
    for (label facei = 0; facei < mesh_.nInternalFaces(); facei++)
    {
        if (faceAnchorLevel[facei] >= 0 && refinedCellFaces[facei])
        {
            label own = mesh_.faceOwner()[facei];
                ...
                )
                {
                    if (nAnchorPoints[celli] == 8)
                    {
                        ...
                        FatalErrorInFunction
                            ...
                            << " lower level. Check case has been cleaned"
                            << " and starting mesh is valid" << nl
                            << abort(FatalError);
                    }
            ...
            if (cellMidPoint[celli] >= 0)
            {
                if (nAnchorPoints[celli] != 8)
                {
                    ...
                    FatalErrorInFunction
                        ...
                        << " lower level. Check case has been cleaned"
                        << " and starting mesh is valid" << endl
