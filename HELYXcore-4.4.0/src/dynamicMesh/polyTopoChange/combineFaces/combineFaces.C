...
)
{
    ...
    forAll(f, fp0)
    {
        ...
        if (magEPrev > doubleScalarSMALL && magE10 > doubleScalarSMALL)
        {
    ...
    forAll(cEdges, i)
    {
        ...
        (
            (
             ...
             && !patches[p0].coupled() && !patches[p1].coupled()
             && !fixedEdges_[edgeI])
        )
        {
            vector f0Normal, f1Normal;
            bool foundEdge = true;

            if (surfNormalsPtr_)
            {
                const vectorField& sn = *surfNormalsPtr_;
                if (sn[f0] != point::max && sn[f1] != point::max)
                {
                    f0Normal = sn[f0];
                    f0Normal /= mag(f0Normal) + VSMALL;
                    f1Normal = sn[f1];
                    f1Normal /= mag(f1Normal) + VSMALL;
                }
                else
                {
                    foundEdge = false;
                    f0Normal = mesh_.faceAreas()[f0];
                    f0Normal /= mag(f0Normal) + VSMALL;
                    f1Normal = mesh_.faceAreas()[f1];
                    f1Normal /= mag(f1Normal) + VSMALL;
                }
            }
            else
            ...
            vector f0Area = mesh_.faceAreas()[f0];
            vector f1Area = mesh_.faceAreas()[f1];
            scalar minArea = min(mag(f0Area), mag(f1Area));
            scalar maxArea = max(mag(f0Area), mag(f1Area));
            bool largeAreaRatio = minArea/maxArea < maxAreaRatio ? true : false;

            if (foundEdge && (((f0Normal&f1Normal) > minCos) || largeAreaRatio))
            {
...
(
    const polyMesh& mesh,
    const labelList& includePatches,
    const List<labelList> &featureShadows,
    const bool undoable,
    const bool samePatch,
    const vectorField* surfNormals
)
:
    mesh_(mesh),
    undoable_(undoable),
    ...
    savedPointLabels_(0),
    savedPoints_(0),
    samePatch_(samePatch),
    surfNormalsPtr_(surfNormals)
{

    fixedEdges_.setSize(mesh_.edges().size(),false);

    //loop through all the featureShadows finding mesh edges that need
    // to be preserved
    forAll(featureShadows,sI)
    {
        for (label j = 0; j < featureShadows[sI].size()-1; ++j)
        {
            const labelList &edgeMfd =
                mesh_.pointEdges()[featureShadows[sI][j]];

            label nextVertex = featureShadows[sI][j+1];

            forAll(edgeMfd,k)
            {
                if
                (
                    mesh_.edges()[edgeMfd[k]].otherVertex(featureShadows[sI][j])
                    == nextVertex
                 )
                {
                    fixedEdges_[edgeMfd[k]] = true;
                    break;
                }
            }
        }
    ...
    //exclude edges not in include patch set
    if (includePatches.size() > 0)
    {
        labelHashSet includePatchSet(includePatches);
        const polyBoundaryMesh& patches = mesh_.boundaryMesh();
        forAll(patches, patchi)
        {
            if (!patches[patchi].coupled() && !includePatchSet.found(patchi))
            {
...
) const
{
    ...
    forAllConstIter(labelHashSet, boundaryCells, iter)
    {
        ...
        regioniseFaces(featureCos, maxAreaRatio, celli, cEdges, faceRegion);
...
) const
{
    ...
    return getMergeSets(featureCos, minConcaveCos, maxAreaRatio, boundaryCells);
}
