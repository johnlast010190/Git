...
#include "regionCoupled/fvPatchFields/regionCoupled/regionCoupledFvPatchField.H"
#include "monolithicSolve/monolithicSolve.H"
...
#include "solverObjects/fvMesh/fvMeshSolver.H"
...
defineTypeNameAndDebug(helyxSolve, 0);
...
void Foam::helyxSolve::createOptions(const dictionary& configOptionsDict)
{
    ...
    forAll(fvOptions_, i)
    {
        ...
        // Create list of solver object to have available during initialisation
        solverObjects_.set
        (
            i,
            new solverObjectList(opts, false)
        );
    }
    ...
    forAll(fvOptions_, i)
    {
        ...
        // Re-create list of solver objects now that some may have been deleted
        solverObjects_.set
        (
            i,
            new solverObjectList(opts, false)
...
void Foam::helyxSolve::orderFvOptions(fv::optionList& options)
{
    ...
    forAll(options, j)
    {
        ...
        {
            bool isAdjoint =
                refCast<fv::solverOption>(options[j]).solverObject().adjoint();
            if (!isAdjoint)
            {
                oldToNew[j] = newIdx++;
            }
...
void Foam::helyxSolve::gatherDependencyInfo()
{
    ...
    forAll(solverObjects_, i)
    {
        ...
        // As a convenience, automatically nest outerCorrector in the time loop
        corrMembers.insert(solverObject::timeLoopCorrectorName, wordList());
        corrMembers[solverObject::timeLoopCorrectorName].append
        (
            solverObject::outerCorrectorName
        );
...
void Foam::helyxSolve::checkSolveInfo()
{
...
void Foam::helyxSolve::substituteCorrectorNames
(
...
void Foam::helyxSolve::addInitLoopDependencies()
{
    // Add a dependency for every solve in time loop on every solve in init
    // loop to ensure init loop is done first
    label timeCorrector =
        correctorNames_.find(solverObject::timeLoopCorrectorName);
    label initCorrector =
        correctorNames_.find(solverObject::initialisationLoopName);
    if (timeCorrector >= 0 && initCorrector >= 0)
    {
        forAll(correctorMembers_[timeCorrector], solvei)
        {
            solveID solveIDi = correctorMembers_[timeCorrector][solvei];
            forAll(correctorMembers_[initCorrector], solvej)
            {
                allSolves_[solveIDi]->dependencies.append
                (
                    correctorMembers_[initCorrector][solvej]
                );
            }
        }
    }
...
template<class Type>
void Foam::helyxSolve::getCoupledFields
(
    const fvMesh& mesh,
...
void Foam::helyxSolve::gatherCoupledGroups
(
    solveInfoTable& tree,
...
void Foam::helyxSolve::nestCorrectors()
{
    ...
    forAll(correctorNames_, corri)
    {
        ...
        forAllConstIter(HashSet<word>, correctorCorrectors[corri], iter)
        {
            ...
            allCorrectors_[iter.key()]->parentCorrectorName =
...
void Foam::helyxSolve::addCoupledGroupDependencies
(
    correctorInfo& corr
)
{
...
void Foam::helyxSolve::addCoupledGroupNestingDependencies
(
    correctorInfo& corr
)
{
...
void Foam::helyxSolve::constructSolveSchedule
(
    correctorInfo& corr
)
{
    ...
    forAll(corr.schedule, i)
    {
        ...
        if (!foundOne)
        {
            FatalErrorInFunction
                ...
                << "among the following items or their contents:" << nl;

            scheduleItem* item = nullptr;
            forAll(sortSolveSchedule, j)
            {
                if (!sortSolveSchedule[j]->flag)
                {
                    item = sortSolveSchedule[j];
                    break;
                }
            }
            ...
            do
            {
                ...
                {
                    // Visited, but not printed an error yet
                    // Only print when we visit this again, to ensure we only
                    // print the part which is a loop
                    item->flag = 2;
                }

                scheduleItem* newItem = nullptr;
                for (solveID& depItem : item->localDependencies)
                {
                    if
                    (
                        !corr.items[depItem]->flag
                     || corr.items[depItem]->flag == 2
                    )
                    {
                        newItem = corr.items[depItem];
                        break;
                    }
                }
                item = newItem;
...
void Foam::helyxSolve::setScheduleOrder(correctorInfo& corr)
{
...
void Foam::helyxSolve::processSolveSchedule(correctorInfo& corr)
{
...
bool Foam::helyxSolve::isLTS()
{
    ...
    // Check if all the regions have local time stepping enabled
    if (LTS)
    {
        forAll(solverObjects_, regioni)
        {
        ...
        {
            FatalErrorInFunction
                << "Local time stepping is enabled in some regions "
                << "but not in others. This is not supported."
                << exit(FatalError);
        }
    }
    return LTS;
...
void Foam::helyxSolve::runSchedule(correctorInfo& corr)
{
    ...
    else if (correctorName == solverObject::initialisationLoopName)
    {
        Info<< "\nStarting initialisation schedule\n" << endl;
    }

    #if !defined( WIN32 ) && !defined( WIN64 )
    addProfiling(correctorProf, "helyxSolve::runCorrector "+correctorName);
    #endif

    // Clear the solverPerformance info for this corrector
    solverPerformance_.set(correctorName, dictionary());
    ...
    )
    {
        ...
        if (correctorName == solverObject::timeLoopCorrectorName)
        {
            ...
            if (isLTS())
            {
                Info<< "LTS time scales - min/max deltaT:" << endl;

                // Set local time step from all solver object and smooth
                // globally.
                forAll(solverObjects_, i)
                {
                    volScalarField* rDeltaT =
                        &solverObjects_[i][0].lookupOrConstructRDeltaT();

                    // Store previous timestep as old
                    const volScalarField rDeltaT0("rDeltaT0", *rDeltaT);

                    // Make sure that after we store the old value we reset
                    // the value of the rDeltaT, since in old solvers the
                    // values is recalculated in any case
                    rDeltaT->primitiveFieldRef() = VSMALL;

                    // When no function object changes rDeltaT set dt to 1
                    if (!solverObjects_[i].setRDeltaT())
                    {
                        rDeltaT->primitiveFieldRef() = 1.0;
            ...
            else if (adjustTimeStep)
            {
                ...
                deltaTFactor =
                    min(min(deltaTFactor, 1.0 + 0.1*deltaTFactor), 1.2);
            ...
            Info<< "Outer Loop Iteration = " << time_.timeIndex() << endl;
        }
        else if (correctorName == solverObject::outerCorrectorName)
        {
            // Backward compatibility: set the corrector number in the solution
            // control (1-based)
            forAll(solnControls_, i)
            {
                solnControls_[i].setCorr(corrector+1);
            }
        }
        ...
        forAll(solverObjects_, i)
        {
            ...
            satisfied &=
                (corrector >= solnControls_[i].nIter(correctorName)-1);
            bool minItersSatisfied =
                (corrector >= solnControls_[i].minIter(correctorName)-1);
            satisfied &= minItersSatisfied;
            ...
            if (correctorName != solverObject::timeLoopCorrectorName)
            {
                // Time loop convergence is done separately at loop end
                conv = convergenceSatisfied(i, correctorName, corrector);
            }

            // Check the legacy convergence controls as well
            if
            (
            ...
            )
            {
                ...
                {
                    conv |=
                        convergenceSatisfied(i, correctorName, corrector, true);
                }
            }

            conv &= minItersSatisfied;
        ...
        if (converged && finalIter)
        {
            ...
            Info<< "Residual convergence tolerance reached. "
                << "Performing final iteration of " << correctorName << endl;
        }
        if
        (
            correctorName == solverObject::timeLoopCorrectorName
        ...
        finalCorrector_.set(correctorName, finalIter);
        ...
        forAll(solverObjects_, regioni)
        {
            ...
            if (correctorName == solverObject::outerCorrectorName)
            {
                ...
                else if (isA<helyxCoupledControl>(solnControls_[regioni]))
                {
                    const helyxCoupledControl& hcc =
                        refCast<helyxCoupledControl>(solnControls_[regioni]);
                ...
                // Reset update flags
                coordinateFrame::resetUpdates
                (
                    solutionRegistries_[regioni].mesh().thisDb(),
                    corrector+1
                );
        ...
        for (stepi = 0; stepi < corr.schedule.size(); stepi++)
        {
            if (corr.schedule[stepi].isCorrector())
            {
                correctorInfo& nestedCorrector =
                    dynamic_cast<correctorInfo&>(corr.schedule[stepi]);
                runSchedule(nestedCorrector);
            }
            else
            {
                solveInfo& info =
                ...
                if (threadTeamLen > 0)
                {
                    ...
                    addProfiling
                    (
                        assembleProf, "helyxSolve::assemble " + info.solveName
                    );
                    #endif
                ...
                // Check if need to wait for further matrices before
                // region-coupled solve

                // No coupled dependencies: solve regions segregated
                ...
                {
                    {
                        ...
                        addProfiling
                        (
                            solveProf, "helyxSolve::solve " + info.solveName
                        );
                        #endif
                    ...
                    }
                    {
                        ...
                        addProfiling
                        (
                            corrProf, "helyxSolve::correct " + info.solveName
                        );
                        #endif
                        solver.correctBase
                        (
                    ...
                    if (info.readyToSolve)
                    {
                        ...
                        (
                            ...
                            (
                                corr,
                                info,
                                smxs, vmxs, stmxs, tmxs, v4mxs, v5mxs,
                                solverDictName
                            )
                        )
                        {
                        ...
                        (
                            solveProf,
                            "helyxSolve::solveRegionCoupled " + solverDictName
                        );
                        #endif
                        ...
                        callRegionCoupledSolver
                        (
                            smxs, correctorName, corrector, solverDictName
                        );
                        callRegionCoupledSolver
                        (
                            vmxs, correctorName, corrector, solverDictName
                        );
                        callRegionCoupledSolver
                        (
                            stmxs, correctorName, corrector, solverDictName
                        );
                        callRegionCoupledSolver
                        (
                            tmxs, correctorName, corrector, solverDictName
                        );
                        ...
                        forAll(info.coupledGroup, i)
                        {
                            ...
                            else
                            {
                                ...
                                (
                                    corrProf,
                                    "helyxSolve::correct " + info.solveName
                                );
                                #endif
                        ...
                        else if (nConsolidated > 1)
                        {
                            ...
                            (
                                corrProf,
                                "helyxSolve::correctConsolidated "
                              + groupNames_[regionGroup_[regioni]]
                            );
                            #endif
                ...
                // Capture solver performance data for any solves that
                // were not done by helyxSolve directly
                const fvMesh& meshi = solutionRegistries_[regioni].mesh();
                setSolverPerformanceFromDict
                (
                    correctorName,
                    meshi.solverPerformanceDict(),
                    solutionRegistries_[regioni].db()
                );
                setSolverPerformanceFromDict
                (
                    correctorName,
                    meshi.blockSolverPerformanceDict(),
                    solutionRegistries_[regioni].db()
                );
            }
        }
        ...
        if (correctorName == solverObject::timeLoopCorrectorName)
        {
            ...
            // Check residualControls for time loop - done separately from the
            // other convergence checks as we don't want to do a final loop
            // after convergence; just write and exit.
            bool converged = true;
            forAll(solnControls_, regioni)
            {
                bool conv =
                    convergenceSatisfied(regioni, correctorName, corrector);

                // Look at legacy convergence controls as well
                if (isA<simpleControl>(solnControls_[regioni]))
                {
                    conv |=
                        convergenceSatisfied
                        (
                            regioni, correctorName, corrector, true
                        );
                }
...
)
{
    ...
    if (isA<helyxCoupledControl>(solnControls_[regionj]))
    {
        const helyxCoupledControl& hcc =
        refCast<helyxCoupledControl>(solnControls_[regionj]);
        if (hcc.nCorr() == 1)
        {
    ...
    tmp<fvBlockMatrix<vector5>> v5mx = solverj.assembleVector5MatrixBase
        (
            infoj.solveName, finalSolve, dictName
        );
    ...
    (
        (
          ...
          + label(infoj.v4mx.valid()) + label(infoj.v5mx.valid())
        ) > 1
    )
    {
        FatalErrorInFunction
...
void Foam::helyxSolve::correctNext
(
    correctorInfo& corr,
    label& groupIdx,
...
bool Foam::helyxSolve::collectCoupledMatrices
(
    correctorInfo& corr,
    solveInfo& info,
    DynamicList<tmp<fvScalarMatrix>>& smxs,
...
)
{
    ...
    forAll(info.coupledGroup, i)
    {
        ...
        bool found =
         ...
         || infoi.v4mx.valid() || infoi.v5mx.valid();

        solverObject& solver =
            solverObjects_[regionNames_[infoi.regionName]][infoi.solverOrder];

        missingOnlyInactive &= (found || !solver.active());
    ...
    if (!allFound)
    {
        return !anyFound || missingOnlyInactive;
    }
    else
    {
        ...
        forAll(info.coupledGroup, i)
        {
            ...
            if (infoi.v5mx.valid())
            {
                v5mxs.append
                (
                    tmp<fvBlockMatrix<vector5>>(infoi.v5mx, finalIters)
                );
            }
...
template<class Type>
void Foam::helyxSolve::callSegregatedSolver
(
...
)
{
    ...
    SolverPerformance<Type> perf =
        mx->solve
        (
            mx->psi().mesh().solution().solverDict(solverDictName)
        );

    setSolverPerformance(perf, correctorName, mx->psi().name());
...
)
{
    ...
    printRegionName(regioni);
    BlockSolverPerformance<Type> perf =
        mx->solve
        (
            mx->psi().mesh().solution().solverDict(solverDictName)
        );
...
template<class Type>
void Foam::helyxSolve::callRegionCoupledSolver
(
...
)
{
    ...
    forAll(mxs, i)
    {
        ...
        topoChanged |= mxs[i]->psi().mesh().topoChanged();
    ...
    forAll(mxs, i)
    {
        setSolverPerformance(perf, correctorName, mxs[i]->psi().name());
    }
}


template<class SolverPerformanceType>
void Foam::helyxSolve::setSolverPerformance
(
...
{
    // Add the solver performance data to this corrector and recursively to
    // all correctors that nest this one

    if (correctorName == word::null)
    {
        return;
    }

    solverPerformance_.insert(correctorName, dictionary());
    Pair<SolverPerformanceType> perfs(perf, perf);
    solverPerformance_[correctorName].readIfPresent(fieldName, perfs);
    // If it wasn't read (first iteration), perfs.first() will remain set
    // to perf
    perfs.second() = perf;
    solverPerformance_[correctorName].set(fieldName, perfs);

    setSolverPerformance
    (
        perf,
        allCorrectors_[correctorName]->parentCorrectorName,
        fieldName
    );
}
...
void Foam::helyxSolve::setSolverPerformanceFromEntry
(
    const entry& data,
    const word& correctorName,
    const word& fieldName,
...
{
    // Parse and set solver performance data if field of corresponding type
    // found

    if (obr.foundObject<VolField<Type>>(fieldName))
    {
        List<SolverPerformanceType<Type>> perf(data.stream());
        setSolverPerformance
        (
            perf.last(), correctorName, fieldName
        );
    }
}


void Foam::helyxSolve::setSolverPerformanceFromDict
(
    const word& correctorName,
...
{
    // Add all the solver performance data from dictionary

    forAllConstIter(dictionary, dict, iter)
    {
        const word& fieldName(iter().keyword());
        setSolverPerformanceFromEntry<scalar, SolverPerformance>
        (
            iter(), correctorName, fieldName, obr
        );
        setSolverPerformanceFromEntry<vector, SolverPerformance>
        (
            iter(), correctorName, fieldName, obr
        );
        setSolverPerformanceFromEntry<sphericalTensor,  SolverPerformance>
        (
            iter(), correctorName, fieldName, obr
        );
        setSolverPerformanceFromEntry<symmTensor, SolverPerformance>
        (
            iter(), correctorName, fieldName, obr
        );
        setSolverPerformanceFromEntry<tensor, SolverPerformance>
        (
            iter(), correctorName, fieldName, obr
        );
        setSolverPerformanceFromEntry<vector4, BlockSolverPerformance>
        (
            iter(), correctorName, fieldName, obr
        );
    }
...
bool Foam::helyxSolve::convergenceSatisfied
(
    label regioni,
    const word& correctorName,
    const label corrector,
...
{
    if (!solverPerformance_.found(correctorName))
    {
        return false;
    }
    ...
    const dictionary& perfDict =
      ...
      : solverPerformance_[correctorName];
    ...
    forAllConstIter(dictionary, perfDict, iter)
    {
        ...
        const label fieldi =
            solnControls_[regioni].applyToField
            (
                variableName, true, legacy ? word::null : correctorName
            );
        if (fieldi != -1)
        {
            foundAtLeastOneField = true;
            solutionControl::fieldData& resControl =
                solnControls_[regioni].residualControl
                (
                    legacy ? word::null : correctorName
                )[fieldi];
    ...
    {
        // Residual Control Dict does not contain any fields of this region
        return false;
    }
    return allConverged;
}
...
void Foam::helyxSolve::printSolutionSchedule
(
...
void Foam::helyxSolve::printRegionName
(
    const label regioni
) const
{
...
Foam::label Foam::helyxSolve::getMaxTeamLen(correctorInfo& corr)
{
    label maxTeamLen = 0;
...
void Foam::helyxSolve::topoChange(const correctorInfo& corr)
{
    for (label i = 0; i < corr.schedule.size(); i++)
    {
        if (corr.schedule[i].isCorrector())
        {
            const correctorInfo& nestedCorr =
                dynamic_cast<const correctorInfo&>(corr.schedule[i]);

            // Look for 'fvMesh' solver inside outerCorrector loop
            if (nestedCorr.correctorName == solverObject::outerCorrectorName)
            {
                for (label j = 0; j < nestedCorr.schedule.size(); j++)
                {
                    if (!nestedCorr.schedule[j].isCorrector())
                    {
                        const solveInfo& info =
                            dynamic_cast<const solveInfo&>
                            (
                                nestedCorr.schedule[j]
                            );

                        if (info.solveName == "fvMesh")
                        {
                            const label regioni = regionNames_[info.regionName];
                            fv::fvMeshSolver& solver =
                                dynamic_cast<fv::fvMeshSolver&>
                                (
                                    solverObjects_[regioni][info.solverOrder]
                                );
...
Foam::helyxSolve::helyxSolve(Time& time)
:
    regIOobject
    (
        IOobject
    ...
    time_(time),
    topLevelCorrector_(solverObject::topLevelLoopName)
{}


// * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * * //

bool Foam::helyxSolve::isPostProcessActive(argList& args)
{
...
Foam::dictionary Foam::helyxSolve::getSolverOptionConfiguration
(
...
)
{
    ...
    fileName etcFileName =
        "caseDicts/solvers/helyxSolve"/configName/"options.cfg";
    ...
    else if (configName != "helyxSolve")
    {
        FatalErrorInFunction
            << "helyxSolve configuration " << configName
            << " was not found" << nl
            << "at " << "$HELYX_ETC"/etcFileName << nl
            << exit(FatalError);
    }
    fileName defaultEtcFileName =
        "caseDicts/solvers/helyxSolve/options.cfg";
...
void Foam::helyxSolve::runPostProcess
(
    argList& args,
    const word& configName
)
{
    ...
    while (scheduler_->loop())
    {
        ...
        {
            runTime.setTime(timeDirs[timei].value(), timei);
            ...
            try
            {
                ...
                forAll(solutionRegistries_, meshi)
                {
                    ...
                    do
                    {
                        ...
                        auto& regI = const_cast<objectRegistry&>
                        (
                            solutionRegistries_[meshi].registry()
                        );
                        HashTable<regIOobject*> objs = regI.lookupClass<regIOobject>();
...
int Foam::helyxSolve::runSolver(argList& args, const word& configName)
{
    ...
    while (scheduler_->loop())
    {
        ...
        // Add dependencies to ensure the initialisation loop is scheduled
        // before the time loop
        addInitLoopDependencies();
        ...
        // Run the schedule from the top level
