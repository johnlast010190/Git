...
{
    if (isA<mappedLduInterface>(interface))
    {
        const mappedLduInterface& rcInterface =
            refCast<const mappedLduInterface>(interface);

        if
        (
            rcInterface.isInterface()
         && isA<lduInterfaceField>
            (
                matrices[matrixI].psi().boundaryField()[patchI]
            )
        )
        {
            // Coupled interface and implicitly coupled field (either within
        ...
        {
            // Don't couple through interfaces where the patch is coupled but
            // not the field being solved (allows for some fields to be
            // region-coupled and others not)
            foreignOffset = 0;
            foreignMatrix = -1;
            return false;
        }
    ...
    {
        // Not a mapped interface. Must be coupled within the same mesh.
        foreignOffset = 0;
        forAll(matrices, j)
        {
            if (j == matrixI)
...
void Foam::monolithicSolve<Type>::superMeshTopoChanged(objectRegistry& obr)
{
    ...
    <
        ...
        MoveableMeshObject
    >(obr);
    // Keep meshObjects that have an topoChange
    // callback
    meshObject::clearUpto
    <
        lduMesh,
...
template<class Type>
Foam::SolverPerformance<Type> Foam::monolithicSolve<Type>::solve
(
    UPtrList<fvMatrix<Type>>& matrices,
...
)
{
    ...
    UPtrList<VolField<Type>> psi(matrices.size());
    forAll(matrices, i)
    {
        psi.set
        (
    ...
    for (direction cmpt=0; cmpt<Type::nComponents; cmpt++)
    {
        ...
        forAll(matrices, i)
        {
            ...
            forAll(interfaceFields, j)
            {
                ...
                {
                    // See if the interface couples different regions
                    label foreignOffset(0); // offset of the other region mesh
                    label foreignMatrix(-1);
                    getCoupledMatrixAndOffset
                    (
                    ...
                    if (foreignMatrix > -1)
                    {
                        ...
                        superInterfaceFields.set
                        (
                            k,
                            &rawSuperInterfaceFields.last()
                        );
                    }
                }
                k++;
