...
#include "regionCoupled/fvPatchFields/specieTransferMassFraction/specieTransferMassFractionFvPatchScalarField.H"
#include "regionCoupled/fvPatchFields/specieTransferVelocity/specieTransferVelocityFvPatchVectorField.H"
...
void Foam::specieTransferTemperatureFvPatchScalarField::updateCoeffs()
{
    ...
    // Get the diffusivity
    const scalarField& Cp = thermoPtr_->Cp().boundaryField()[patchi];
...
Foam::specieTransferTemperatureFvPatchScalarField::snGrad() const
{
    ...
    (
        thermoPtr_->Cp().boundaryField()[patch().index()].internalField()
    );
