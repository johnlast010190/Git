...
#include "compactGaussLaplacianScheme.H"
...
tmp<surfaceScalarField> overRelaxedDeltaCoeffs
(
    const SurfaceField<TensorType>& gamma,
    const VolField<Type>&
)
{
...
)
{
    ...
    (
        ...
        (
            "laplacian(" + vf.name() + ')',
            mesh,
            mesh.deltaCoeffs().dimensions()*SfGammaNorm.dimensions()
           *vf.dimensions()/dimVolume,
            zeroGradientFvPatchField<scalar>::typeName
        )
    );
    tmp<SurfaceField<Type>> tsg =
...
)
{
    ...
    tmp<SurfaceField<Type>> tFluxCorr
    (
    ...
    autoPtr<SurfaceField<Type>> tangCorr;
