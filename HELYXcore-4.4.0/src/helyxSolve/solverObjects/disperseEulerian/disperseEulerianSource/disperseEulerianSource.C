...
:
    ...
    des_
    (
        obr_.lookupObject<disperseEulerian>(dict.lookup<word>("dispersePhase"))
    ),
    heatTrans_(),
    massTrans_(),
    ...
    disableBoundaryForces_
    (
        coeffs_.lookupOrDefault<bool>("disableBoundaryForces", false)
    ),
    patches_
    (
        coeffs_.lookupOrDefault<wordList>
        (
            "patches",
...
void Foam::fv::disperseEulerianSource::sourceFields(wordList& fieldNames)
{
...
)
{
    if (eqn.psi().name() == "T")
    {
        if (debug)
        {
            Info<< "Inserting thermal source terms into "
        ...
        for (label i=0; i < phases.size(); i++)
        {
            ...
            if (debug)
            {
                Info<< "min/max(Kh): " << min(Kh).value() << " , "
                    << max(Kh).value() << nl
                    << "min/max(Tc): " << min(Tc).value() << " , "
                    << max(Tc).value() << nl
                    << "Td: " << phases[i].Td().value() << endl;
            }
            eqn +=
                Kh*phases[i].Td()
              - fvm::Sp(Kh, Tc)
              + massTrans_[i].mDot()*phases[i].dHevap()
               /(
                    max(1. - phases[i].alphad(), scalar(0.01))*phases[i].rhoc()
                );
        }
    }
    else if (std::string(eqn.psi().name()).find("alpha") != std::string::npos)
    {
        ...
        for (label i=0; i < phases.size(); i++)
        {
            if (phases[i].alphad().name() == eqn.psi().name())
            {
                ...
                if (debug)
                {
                    Info<< "Inserting mass source term into transport "
                        << "Equation for field " << eqn.psi().name() << nl
                        << "min mass source: "
                        << min(massTrans_[i].mDot()/phases[i].rhod()) << "\n"
                        << "max mass source: "
                        << max(massTrans_[i].mDot()/phases[i].rhod()) << endl;
                }
            }
        }
    }
...
)
{
    if (eqn.psi().name() == "U")
    {
        if (debug)
        {
            Info<< "Inserting momentum source terms into transport "
                << "Equation for field " << eqn.psi().name() << endl;
        }
        ...
        for (label i=0; i < phases.size(); i++)
        {
            ...
            (
                phases[i].alphad()*phases[i].rhod()/phases[i].rhoc()
            );
            volScalarField Kd(forcesD_[i].K() * scaleFactor);

            volVectorField liftForces(forcesL_[i].F()*scaleFactor);
            volVectorField turbulentDispersionForces
            (
                forcesTD_[i].F()*scaleFactor
            );
            ...
            if (disableBoundaryForces_)
            {
                forAll(patches_, pI)
                {
                    ...
                    forAll(mesh_.boundaryMesh(), patchI)
                    {
                        ...
                        {
                            Info<< "found patch " << patchName
                                << " in list " << patches_ << endl;
