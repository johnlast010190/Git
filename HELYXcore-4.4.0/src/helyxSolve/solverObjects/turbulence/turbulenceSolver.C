...
bool Foam::fv::turbulenceSolver::initialise()
{
    ...
    if (phaseSystemPtr_)
    {
        Info<< "Detected phasic turbulence models" << endl;
        return true;
    }
...
)
{
    ...
    helyxCoupledControl* coupledCtrl =
        dynamic_cast<helyxCoupledControl*>(solnControlPtr_);
    ...
    )
    {
        ...
        else if (phaseSystemPtr_)
        {
            phaseSystemPtr_->correctTurbulence();
        }
    }
}
