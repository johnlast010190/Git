...
: runTime_(runTime),
  mesh_(mesh)
{
    adapterInfo("Loaded the HELYX-preCICE adapter", "info");
...
bool preciceAdapter::Adapter::configFileRead()
{
    ...
    preciceConfigFilename_ = preciceDict.lookup<fileName>("preciceConfig");
    DEBUG(adapterInfo("  precice-config-file : " + preciceConfigFilename_));

    // Read and display the participant name
    participantName_ = preciceDict.lookup<word>("participant");
    ...
    wordList modules = preciceDict.lookup<wordList>("modules");
    ...
    else
    {
        for (const entry& interfaceDictEntry : interfaceDict)
        {
            if (interfaceDictEntry.isDict())
            {
                ...
                word meshName = interfaceDict.lookup<word>("mesh");
                ...
                wordList patches = interfaceDict.lookup<wordList>("patches");
                ...
                wordList writeData =
                    interfaceDict.lookup<wordList>("writeData");
                ...
                wordList readData = interfaceDict.lookup<wordList>("readData");
...
preciceAdapter::Adapter::~Adapter()
{
    ...
    adapterInfo("The HELYX preCICE adapter is based on the OpenFOAM-preCICE adapter. "
