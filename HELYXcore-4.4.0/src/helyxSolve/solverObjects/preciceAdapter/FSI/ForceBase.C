...
preciceAdapter::FSI::ForceBase::devRhoReff() const
{
    // For turbulent flows
...
Foam::tmp<Foam::volScalarField> preciceAdapter::FSI::ForceBase::rho() const
{
    ...
    else if (solverType_.compare("incompressible") == 0)
    {
        const dictionary& FSIDict =
            mesh_.lookupObject<IOdictionary>("preciceDict").subOrEmptyDict
            (
                "FSI"
            );
...
Foam::tmp<Foam::volScalarField> preciceAdapter::FSI::ForceBase::mu() const
{
    ...
    {
        return mesh_.lookupObject<volScalarField>("mu");
    }
    else
    {
        FatalErrorInFunction
...
void preciceAdapter::FSI::ForceBase::writeToBuffer
(
    double* buffer,
...
) const
{
    ...
    for (const label patchID : patchIDs_)
    {
        ...
        // Pressure forces
        // FIXME: We need to subtract the reference pressure
