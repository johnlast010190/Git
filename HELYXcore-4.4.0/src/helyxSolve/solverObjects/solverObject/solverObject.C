...
#include "solverObjects/reactions/reactionsSource.H"
...
namespace Foam
{
    ...
    const word solverObject::topLevelLoopName = "topLevelLoop";
    const word solverObject::initialisationLoopName = "initialisation";
...
bool Foam::solverObject::isReactingFoam()
{
    if (!isReactingFoamPtr_.valid())
    {
        const wordList reactionsNames
        (
            obr_.names<fv::reactionsSource>()
        );
        if (reactionsNames.size() == 1)
        {
            isReactingFoamPtr_.set
            (
                new bool
                (
                    obr_.lookupObject<fv::reactionsSource>
                    (
                        reactionsNames.first()
                    ).reactingFoam()
                )
            );
        }
        ...
        {
            isReactingFoamPtr_.set(new bool(false));
        }
    }
    return isReactingFoamPtr_();
}
...
:
    ...
    name_(name),
    obr_(obr),
    mesh_(const_cast<fvMesh&>(fvSolutionRegistry::getMesh(obr))),
...
Foam::tmp<Foam::fvBlockMatrix<Foam::vector5>>
Foam::solverObject::assembleVector5MatrixBase
(
    const word& solveName,
    bool& finalSolve,
    word& dictName
)
{
    if (active())
    {
        return this->assembleVector5Matrix(solveName, finalSolve, dictName);
    }
    else
    {
        return tmp<fvBlockMatrix<vector5>>();
    }
}
...
void Foam::solverObject::solve()
{
    ...
    // Simulate the process that helyxSolve would use so that the object
    ...
    do   // Repeated iterations for correctors
    {
        ...
        forAll(solveNames, solvei)
        {
            ...
            if (currentCorrector == activeCorrectors.last())
            {
                ...
                tmp<fvBlockMatrix<vector5>> vm5 =
                    this->assembleVector5MatrixBase
                    (
                        solveName, finalSolver, dictName
                    );
                if (finalSolver)
