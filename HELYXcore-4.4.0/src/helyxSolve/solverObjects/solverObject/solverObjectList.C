...
bool Foam::solverObjectList::setRDeltaT()
{
    // We need to check if all the solver objects in this list
    // have the same mesh
    solverObject* lastToUpdate = nullptr;
    bool changedDeltaT = false;
    forAll(*this, i)
    {
        solverObject* objecti = &this->operator[](i);
        if (objecti->isLastRDeltaTCorrector())
        {
            if (lastToUpdate)
            {
                FatalErrorInFunction
                    << "Solver objects with the same mesh can't have multiple "
                    << "different object marked to be updated last."
                    << exit(FatalError);
            }
            lastToUpdate = objecti;
        }
        else
        {
            changedDeltaT = objecti->setRDeltaT() ? true : changedDeltaT;
        }
    }

    // Flow solvers do sweeps and smooths so need to be done on the end
    if (lastToUpdate)
    {
        changedDeltaT = lastToUpdate->setRDeltaT() ? true : changedDeltaT;
    }

    return changedDeltaT;
}
...
)
{
    ...
    forAll(*this, i)
    {
        ...
        // Always add the predefined correctors
