...
)
{
    ...
    position_ = ppp.transform().transformPosition(position_);
...
void Foam::particle::readFields(CloudType& c)
{
    ...
    IOField<label> origProcId(procIO, read && haveFile);
...
template<class TrackData>
Foam::label Foam::particle::track
(
...
)
{
    ...
    {
        stepFraction_ += trackToFace
        (
            endPosition, td, backwards
        )*(1.0 - stepFraction_);

        if (nTracks++ > 1000)
        {
            Pout<< "Possible stuck particle. Removing "
                 << endl;
...
)
{
    ...
    do
    {
        ...
        if (lambdaMin < doubleScalarSMALL)
        {
            ...
            if (mesh_.moving())
            {
                ...
                tetPointRef tet00 = (backwards ? tetIs.tet(mesh_) :  tetIs.oldTet(mesh_));

                // Tet at timestep end
                tetPointRef tet = (backwards ? tetIs.oldTet(mesh_) :  tetIs.tet(mesh_));
                ...
                position_ += trackingCorrectionTol*(tet0.centre() - position_);
            }
            else
            {
        ...
        // found to hit.
        if ((tris.empty() && facei_ < 0) || noBasePt_)
        {
        ...
        if (lambdaMin > doubleScalarSMALL)
        {
            if (lambdaMin <= 1.0)
            {
                ...
                vector newPosition = position_
                    + lambdaMin*(endPosition - position_);
                if (mag(newPosition-position_) > VSMALL)
                {
    ...
    if (lambdaMin < doubleScalarSMALL)
    {
        ...
        )
        {
            ...
            if (isA<wallPolyPatch>(patches[patchi]))
            {
                ...
                const vector nHat = wallTri.unitNormal();
    ...
    forAll(thisCell, cFI)
    {
        ...
        if (isA<wallPolyPatch>(patches[patchi]))
        {
            ...
            forAll(faceTetIs, tI)
            {
                ...
                triPointRef tri =
                (
                    (mesh_.moving() && backwards) ?
                    tetIs.oldFaceTri(mesh_) : tetIs.faceTri(mesh_)
                );

                const vector n = tri.areaNormal();
                const vector nHat = tri.unitNormal();
                ...
                if (mesh_.moving())
                {
                    ...
                    (
                        backwards ?
                        tetIs.faceTri(mesh_) : tetIs.oldFaceTri(mesh_)
                    );
...
)
{
    ...
    // Have patch transform the position
    position_ = receiveCpp.transform().transformPosition(position_);
...
{
    bool dAMIs = td.cloud().distributedAMIs();

    if (dAMIs)
    {
        td.keepParticle = false;
        Pout<< "Hitting a particle at a distributed cyclicAMI "
             << "is not possible currently. "
             << "If possible, try to preserve them during decomposition. "
             << "Exiting..."
             << endl;

        Pout<< abort(FatalError);
    }
