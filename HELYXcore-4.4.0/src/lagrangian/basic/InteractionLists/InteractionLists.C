...
void Foam::InteractionLists<ParticleType>::buildInteractionLists()
{
    ...
    treeBoundBox procBbRndExt(treeBoundBox(mesh_.points()).extend(1e-4));
...
)
{
    ...
    extendedProcBbsInRange = std::move(tmpExtendedProcBbsInRange);
    extendedProcBbsTransformIndex = std::move(tmpExtendedProcBbsTransformIndex);
    extendedProcBbsOrigProc = std::move(tmpExtendedProcBbsOrigProc);
}
...
)
{
    ...
    particle->position() =
        transform.invTransformPosition(particle->position());

    if (transform.transformsPosition())
    {
        particle->transformProperties(inv(transform));
    }
}
