...
)
{
    ...
    pc_ =
        td.pInterp().interpolate(this->position(), this->currentTetIndices())
      + td.cloud().thermo().thermo().pRefValue();
...
)
{
    ...
    forAll(td.cloud().rhoTrans(), i)
    {
        ...
        CpEff +=
            Y*const_cast<SLGThermo&>(td.cloud().thermo()).Cp
            (
                i,
                this->pc_,
                this->Tc_
...
)
{
    ...
    SLGThermo& thermo = const_cast<SLGThermo&>(td.cloud().thermo());
    ...
    forAll(Ys, i)
    {
        ...
        rhos += Xs[i]*W;
        mus += Ys[i]*sqrtW*thermo.mu(i, pc_, T);
        kappas += Ys[i]*cbrtW*thermo.kappa(i, pc_, T);
        Cps += Xs[i]*thermo.Cp(i, pc_, T);
...
)
{
    ...
    this->U_ =
        this->calcVelocity(td, dt, celli, Res, mus, mass1, Su, dUTrans, Spu, pForces);
    ...
    if (td.cloud().solution().coupled())
    {
        ...
        forAll(dMass, i)
        {
            ...
            label gid = composition.localToCarrierId(0, i);
            scalar hs =
                const_cast<SLGThermo&>(composition.thermo()).hs(gid, pc_, T0);
...
#include "parcels/Templates/ReactingParcel/ReactingParcelIO.C"
