...
:
    ...
    decoupleSmallCells_(cs.decoupleSmallCells_),
...
{
    transient_ = dict_.lookup<Switch>("transient");
    coupled_ = dict_.lookup<Switch>("coupled");
    cellValueSourceCorrection_ =
        dict_.lookup<Switch>("cellValueSourceCorrection");
    dict_.readIfPresent("decoupleSmallCells", decoupleSmallCells_);
    dict_.readIfPresent("maxCo", maxCo_);
    dict_.readIfPresent("deltaTMax", deltaTMax_);

    if (steadyState())
    {
        calcFrequency_ = dict_.lookup<label>("calcFrequency");
        maxTrackTime_ = dict_.lookup<scalar>("maxTrackTime");
    ...
    if (coupled_)
    {
        const dictionary& schemesDict =
            dict_.subDict("sourceTerms").subDict("schemes");
