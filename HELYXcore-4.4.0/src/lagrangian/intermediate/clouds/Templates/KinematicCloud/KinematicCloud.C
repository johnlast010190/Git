...
#include "clouds/Templates/KinematicCloud/KinematicCloud.H"
...
) const
{
    ...
    const vector n = tetIs.faceTri(mesh_).unitNormal();
    ...
    else
    {
        ...
        vector n00 = tetIs.oldFaceTri(mesh_).areaNormal();
...
template<class CloudType>
void Foam::KinematicCloud<CloudType>::topoChange()
{
    updateCellOccupancy();
...
template<class CloudType>
void Foam::KinematicCloud<CloudType>::autoMap(const polyTopoChangeMap& mapper)
{
...
template<class CloudType>
void Foam::KinematicCloud<CloudType>::autoMap()
{
    typedef typename particle::TrackingData<KinematicCloud<CloudType>> tdType;

    tdType td(*this);

    Cloud<parcelType>::template autoMap<tdType>(td);
}
