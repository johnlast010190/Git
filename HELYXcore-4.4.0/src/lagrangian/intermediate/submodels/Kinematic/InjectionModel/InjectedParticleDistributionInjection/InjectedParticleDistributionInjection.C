...
:
    ...
    binWidth_(this->coeffDict().template lookup<scalar>("binWidth")),
    sizeDistribution_(),
    parcelsPerInjector_
    (
        ceil(this->coeffDict().template lookup<scalar>("parcelsPerInjector"))
    ),
    resampleSize_
    (
    ...
    (
        dict.lookup<bool>("applyDistributionMassTotal")
    ),
    ignoreOutOfBounds_
    (
...
template<class CloudType>
void Foam::InjectedParticleDistributionInjection<CloudType>::topoChange()
{}
