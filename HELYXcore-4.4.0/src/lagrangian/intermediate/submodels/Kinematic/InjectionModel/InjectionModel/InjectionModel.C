    ...
    injectorID_(this->coeffDict().lookupOrDefault("injectorID", -1))
{
    ...
    if (owner.solution().active())
    {
        ...
        {
            massTotal_ =
                this->coeffDict().template lookup<scalar>("massTotal");
            SOI_ = this->coeffDict().template lookup<scalar>("SOI");
        }
        else
        {
    ...
    if (this->coeffDict().found("parcelID"))
    {
        parcelID_ = this->coeffDict().template lookup<label>("parcelID");
    }
...
void Foam::InjectionModel<CloudType>::inject(TrackData& td)
{
    ...
    if (prepareForNextTimeStep(time, newParcels, newVolumeFraction))
    {
        ...
        for (label parcelI = 0; parcelI < newParcels; parcelI++)
        {
            ...
            //- attach the parcel identifier for injection
            pPtr->parcelID() = parcelID_;
                }
            }
        }
    }
...
)
{
    ...
    for (label parcelI = 0; parcelI < newParcels; parcelI++)
    {
        ...
        if (celli > -1)
        {
            ...
            // Apply correction to velocity for 2-D cases
            meshTools::constrainDirection(mesh, mesh.geometricD(), pPtr->U());
...
#include "submodels/Kinematic/InjectionModel/InjectionModel/InjectionModelNew.C"
