...
#include "submodels/Kinematic/CollisionModel/PairCollision/WallModel/WallLocalJKRSSD/WallLocalJKRSSD.H"

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

template<class CloudType>
void Foam::WallLocalJKRSSD<CloudType>::findMinMaxProperties
(
    scalar& rMin,
    scalar& rhoMax,
    scalar& UMagMax
...
template<class CloudType>
void Foam::WallLocalJKRSSD<CloudType>::evaluateWall
(
...
) const
{
    ...
    scalar etaN = alpha*sqrt(p.mass()*kN)*pow025(penDepth);

    vector fN_PW =
        rHat_PW
       *(kN*pow(penDepth, 3.0/2.0) - etaN*(U_PW & rHat_PW));
    ...
    {
        scalar kT = 8.0*sqrt(pREff*penDepth)*Gstar;
...
template<class CloudType>
Foam::WallLocalJKRSSD<CloudType>::WallLocalJKRSSD
(
...
:
    ...
    verbose_
    (
        this->coeffDict().template lookupOrDefault<Switch>("verbose", false)
    )
{
    if (useEquivalentSize_)
    {
    ...
    forAll(wallPatchIndices, wPI)
    {
        ...
        scalar nu = patchCoeffDict.lookup<scalar>("poissonsRatio");

        scalar E = patchCoeffDict.lookup<scalar>("youngsModulus");

        Estar_[wPI] = 1/((1 - sqr(pNu))/pE + (1 - sqr(nu))/E);

        Gstar_[wPI] = 1/(2*((2 + pNu - sqr(pNu))/pE + (2 + nu - sqr(nu))/E));

        alpha_[wPI] = patchCoeffDict.lookup<scalar>("alpha");

        mu_[wPI] = patchCoeffDict.lookup<scalar>("mu");

        scalar patchGamma = patchCoeffDict.lookup<scalar>("patchGamma");

        scalar parcelGamma = patchCoeffDict.lookup<scalar>("parcelGamma");
...
template<class CloudType>
Foam::scalar Foam::WallLocalJKRSSD<CloudType>::pREff
(
...
template<class CloudType>
bool Foam::WallLocalJKRSSD<CloudType>::controlsTimestep() const
{
    return true;
}
...
template<class CloudType>
Foam::label Foam::WallLocalJKRSSD<CloudType>::nSubCycles() const
{
    if (!(this->owner().size()))
    {
        return 1;
