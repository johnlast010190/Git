...
#include "submodels/Reacting/PhaseChangeModel/LiquidEvaporationBoil/LiquidEvaporationBoil.H"
...
) const
{
    ...
    scalar Cpc = 0.0;
    scalar kappac = 0.0;
    SLGThermo& thermo = const_cast<SLGThermo&>(this->owner().thermo());
    forAll(thermo.carrier().Y(), i)
    {
        scalar Yc = thermo.carrier().Y()[i][celli];
        Hc += Yc*thermo.ha(i, pc, Tc);
        Hsc += Yc*thermo.ha(i, ps, Ts);
        Cpc += Yc*thermo.Cp(i, ps, Ts);
        kappac += Yc*thermo.kappa(i, ps, Ts);
    }
