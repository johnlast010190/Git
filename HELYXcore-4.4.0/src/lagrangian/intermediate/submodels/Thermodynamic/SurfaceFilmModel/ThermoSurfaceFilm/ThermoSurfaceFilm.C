...
#include "submodels/Thermodynamic/SurfaceFilmModel/ThermoSurfaceFilm/ThermoSurfaceFilm.H"
    ...
    nParcelsSplashed_(0)
{
    ...
    {
        deltaWet_ = this->coeffDict().template lookup<scalar>("deltaWet");
        splashParcelType_ =
            this->coeffDict().lookupOrDefault("splashParcelType", -1);
        parcelsPerSplash_ =
            this->coeffDict().lookupOrDefault("parcelsPerSplash", 2);
        Adry_ = this->coeffDict().template lookup<scalar>("Adry");
        Awet_ = this->coeffDict().template lookup<scalar>("Awet");
        Cf_ = this->coeffDict().template lookup<scalar>("Cf");
    }
}
