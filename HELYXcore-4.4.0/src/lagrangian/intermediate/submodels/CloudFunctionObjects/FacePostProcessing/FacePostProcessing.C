    ...
    timeOld_(owner.mesh().time().value())
{
    ...
    forAll(faceZoneNames, i)
    {
        ...
        if (zoneI != -1)
        {
            ...
            mass_[nZones].setSize(fz.size(), 0.0);
            massTotal_[nZones].setSize(fz.size(), 0.0);
            massFlowRate_[nZones].setSize(fz.size(), 0.0);
            ...
            makeLogFile(zoneName, nZones, nFaces, totArea);
            nZones++;
        }
        else
        {
