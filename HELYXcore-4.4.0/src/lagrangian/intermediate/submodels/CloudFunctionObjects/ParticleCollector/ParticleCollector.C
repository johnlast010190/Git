...
)
{
    ...
    forAll(faces_, facei)
    {
        ...
        triEngine.triangulate(UIndirectList<point>(points_, f));

        forAll(triEngine.triPoints(), triI)
        {
            tris.append(triEngine.triPoints(triI, f));
        }
...
void Foam::ParticleCollector<CloudType>::initConcentricCircles()
{
    ...
    radius_ = this->coeffDict().template lookup<List<scalar>>("radius");
    ...
    coordSys_ = cylindricalCS(origin, normal_[0], refDir);
