...
std::size_t Foam::molecule::sizeofFields() const
{
    return reinterpret_cast<const char *>(&siteForces_) - reinterpret_cast<const char *>(&Q_);
}
...
{
    const bool read = mC.size();

    particle::readFields(mC);

    IOField<tensor> Q(mC.fieldIOobject("Q", IOobject::MUST_READ), read);
    mC.checkFieldIOobject(mC, Q);

    IOField<vector> v(mC.fieldIOobject("v", IOobject::MUST_READ), read);
    mC.checkFieldIOobject(mC, v);

    IOField<vector> a(mC.fieldIOobject("a", IOobject::MUST_READ), read);
    mC.checkFieldIOobject(mC, a);

    IOField<vector> pi(mC.fieldIOobject("pi", IOobject::MUST_READ), read);
    mC.checkFieldIOobject(mC, pi);

    IOField<vector> tau(mC.fieldIOobject("tau", IOobject::MUST_READ), read);
    ...
    IOField<label> id(mC.fieldIOobject("id", IOobject::MUST_READ), read);
