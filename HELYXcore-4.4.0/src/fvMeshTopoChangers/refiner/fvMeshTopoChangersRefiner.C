...
{
    if (protectedCells_.size() == 0 || protectedCells_.size() != mesh().nCells())
    {
        protectedCells_.clear();
        protectedCells_.setSize(mesh().nCells(), 0);
    }
    ...
    // since the coupled one is refined.
    {
            ...
            forAll(patches, patchi)
            {
                ...
                if (isType<wallFvPatch>(currPatch))
                {
                    forAll(currPatch, facei)
                    {
                        ...
                        if (!protectedCells_.get(faceCelli))
                        {
                            if (protectedCells_.set(faceCelli, 1))
                            {
                                nWallCellsProtected++;
                                nProtected++;
                            }
        ...
        if (protectedCellZones_.size() > 0)
        {
            ...
            forAll(protectedCellZones_, cZI)
            {
                ...
                forAll(cZone, cI)
                {
                    if (!protectedCells_.get(cZone[cI]))
                    {
                        if (protectedCells_.set(cZone[cI], 1))
                        {
    ...
    if (protectedCellsHexMesh_.valid())
    {
        forAll(protectedCellsHexMesh_(), cellI)
        {
            if (protectedCellsHexMesh_()[cellI] > 0)
            {
                if (protectedCells_.set(cellI, 1))
                {
                    nProtected++;
    ...
    {
        cellSet protectedCells(mesh(), "protectedCellSet", nProtected);
        ...
        if (outputProtected_)
        {
            // add protectedCells and write protectedCellsHexMesh_
            if (!protectedCellsHexMesh_.valid())
            {
                protectedCellsHexMesh_.reset
                (
                    new volScalarField
                    (
                        IOobject
            ...
            forAll(protectedCellsHexMesh_(), cellI)
            {
                if (protectedCellsHexMesh_()[cellI] < 1)
                {
                    if (protectedCells_.get(cellI))
                    {
                        protectedCellsHexMesh_()[cellI] = 1;
                    }
                }
            }

            protectedCellsHexMesh_->write();
        }
    }
}
...
{
    bool zeroProtected(protectedCells_.empty());
    ...
    // Check cells for 8 corner points
    checkEightAnchorPoints(unrefineableCells, nInvalidAnchors);
...
void Foam::fvMeshTopoChangers::refiner::readDict()
{
    ...
    protectLayers_ = dict_.lookupOrDefault<Switch>("protectLayers", true);

    if (dict_.found("protectedPatches"))
        protectedPatches_ = wordList(dict_.lookup("protectedPatches"));

    if (dict_.found("protectedCellZones"))
        protectedCellZones_ =
            wordList(dict_.lookup("protectedCellZones"));

    if (dict_.found("refineStartTime"))
        refineStartTime_ = dict_.lookup<scalar>("refineStartTime");

    if (dict_.found("refineStopTime"))
        refineStopTime_ = dict_.lookup<scalar>("refineStopTime");

    danglingRefine_ =
        dict_.lookupOrDefault<Switch>("danglingRefine", false);

    if (dict_.found("mapSurfaceFields"))
    {
        List<word> surfFlds = List<word>
        (
            dict_.lookup("mapSurfaceFields")
        );
    ...
    outputProtected_ =
        dict_.lookupOrDefault<Switch>("outputProtected", false);
}
...
)
{
    ...
    UPtrList<surfaceScalarField> fluxes(mesh().fields<surfaceScalarField>());
    ...
    forAll(fluxes, i)
    {
        ...
        const word& Uname = correctFluxes_[flux.name()];
        ...
        surfaceScalarField::Boundary& phiBf = flux.boundaryFieldRef();
...
) const
{
    ...
    forAll(pointLevel, pointi)
    {
        ...
        forAll(pCells, pCelli)
        {
            ...
            if (pointLevel[pointi] <= cellLevel[celli])
            {
                ...
                {
                    if (protectedCells.set(celli, true))
                    {
                        nProtected++;
                    }
                }
    ...
    {
        if (!protectedCells[celli] && nAnchorPoints[celli] != 8)
        {
            protectedCells.set(celli, true);
            nProtected++;
        }
    }
}
...
{
    typedef SurfaceField<T> GeoField;

    const UPtrList<GeoField> sFields(mesh().fields<GeoField>());
    ...
    forAll(sFields, i)
    {
        const GeoField& sField = sFields[i];

        if (mapSurfaceFields_.found(sField.name()))
        {
            if (debug)
            {
            ...
            Field<T> tsField(mesh().nFaces(), pTraits<T>::zero);
            forAll(sField.internalField(), iFace)
            {
                tsField[iFace] = sField.internalField()[iFace];
            }
            forAll(sField.boundaryField(), iPatch)
            {
                label globalIdx = mesh().boundaryMesh()[iPatch].start();
                forAll(sField.boundaryField()[iPatch], faceI)
                {
                    tsField[faceI+globalIdx] =
                    sField.boundaryField()[iPatch][faceI];
                }
            }
            ...
            for (label facei = 0; facei < mesh().nInternalFaces(); facei++)
            {
                ...
                if (oldFacei == -1)
                {
                    ...
                    forAll(faceOwner, iFace)
                    {
                        ...
                        {
                            tmpValue += tsField[faceOwner[iFace]];
                            magFld += mag(tsField[faceOwner[iFace]]);
                            counter++;
                        }
                    }
                    forAll(faceNeighbour, iFace)
                    {
                        ...
                        {
                            tmpValue += tsField[faceNeighbour[iFace]];
                            magFld += mag(tsField[faceNeighbour[iFace]]);
                            counter++;
                        }
                    }
                    if (counter > 0)
...
:
    ...
    protectedCellsHexMesh_(),
    refineMeshOuterCorr_(true),
    isFirstIter_(true),
    outputProtected_(false)
{
...
bool Foam::fvMeshTopoChangers::refiner::update()
{
    ...
    // on-the-fly.
    //const dictionary& dict_(dict()); // check if this still works!

    label refineInterval = dict_.lookup<label>("refineInterval");
    ...
    )
    {
        ...
        {
            label maxCells = dict_.lookup<label>("maxCells");
            ...
            const scalar lowerRefineLevel =
                dict_.lookup<scalar>("lowerRefineLevel");
            const scalar upperRefineLevel =
                dict_.lookup<scalar>("upperRefineLevel");
            const scalar unrefineLevel =
                dict_.lookup<scalar>("unrefineLevel");
            const label nBufferLayers =
                dict_.lookup<label>("nBufferLayers");
