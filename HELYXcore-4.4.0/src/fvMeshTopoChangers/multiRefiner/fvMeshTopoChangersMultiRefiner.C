...
#include "multiRefiner/fvMeshTopoChangersMultiRefiner.H"
...
{
    defineTypeNameAndDebug(multiRefiner, 0);
    addToRunTimeSelectionTable(fvMeshTopoChanger, multiRefiner, fvMesh);
}
}
...
void Foam::fvMeshTopoChangers::multiRefiner::selectRefineCandidates
(
...
) const
{
    ...
    {
        refiner::selectRefineCandidates
        (
            lowerRefineLevel,
            upperRefineLevel,
            vFld,
...
Foam::scalarField
Foam::fvMeshTopoChangers::multiRefiner::minCellField
(
...
void Foam::fvMeshTopoChangers::multiRefiner::updateRefinementField()
{
    ...
    intRefFld = dimensionedScalar(dimless, 0);

    // Get the cell level field from refiner
    const labelList& cellLevel = meshCutter().cellLevel();

    // Initialize list for target refinement level per cell
    labelList targetLevel (mesh().nCells(), 0);
    ...
    else
    {
        ...
        Field<scalar> refFld(mesh().nCells(), 0.0);
        ...
        forAll(fieldNames, i)
        {
            ...
            const scalar minValue =
                fields_[dictName].lookup<scalar>("minValue");
            const scalar maxValue =
                fields_[dictName].lookup<scalar>("maxValue");
            const label refineLevel =
                fields_[dictName].lookup<label>("refineLevel");

            const volScalarField& fld =
                mesh().lookupObject<volScalarField>(fldName);
            ...
            forAll(fld, cellI)
            {
                if ((fld[cellI] >= minValue) && (fld[cellI] <= maxValue))
                {
                    ...
                    // But do not decrease it if cell already marked for higher
                    // refinement level by previous criterion.
        ...
        Field<scalar> cubeRtV = Foam::pow(mesh().V(), 1.0/3.0);
        ...
        forAll(curlFieldNames, i)
        {
            ...
            const volVectorField& fld =
                mesh().lookupObject<volVectorField>(fldName);
        ...
        forAll(interfaceRefineField, i)
        {
            ...
            // Read region of maximum refinement levels inside and outside
            // of interface indicator field (does not need to be alpha,
            // can also be a concentration field).
            scalar innerRefLayers =
                interface_[fldName].lookup<scalar>("innerRefLayers");
            scalar outerRefLayers =
                interface_[fldName].lookup<scalar>("outerRefLayers");
            ...
            {
                interfaceLayers =
                    Switch(interface_[fldName].lookup("interfaceLayers"));
                nAddLayers =
                    interface_[fldName].lookup<scalar>("nAddLayers");
            }
            ...
            {
                addLayerMethod =
                    interface_[fldName]["addLayerMethod"][0].wordToken();
            ...
            {
                refineLevel =
                    interface_[fldName].lookup<scalar>("maxRefineLevel");

                // To avoid user input mistakes, limit the value with the
                // maximum allowed.
                refineLevel = min(refineLevel, maxRefinement_);
            }

            // TODO: add a min max value of the specified field
            // to be marked for refinement.
            // NOW: hard-coded method snGradAlpha:
            // checks mag(alpha_N - alpha_P) > value ? 1:0

            const volScalarField& fld =
                mesh().lookupObject<volScalarField>(fldName);

            volScalarField isInterface(intRefFld*0.0);
            isInterface = dimensionedScalar("isInterface", dimless, 0);

            surfaceScalarField deltaAlpha
            (
                mag(fvc::snGrad(fld)/mesh().deltaCoeffs())
            );
            ...
            //        neg
            //        (
            //          - fvc::average(fvc::interpolate(isInterface))
            //           *pos0(fld - fldInterfaceValue)
            //        );
            ...
            // New version using volPointInterpolation
            ...
            for (label i = 0; i < innerRefLayers; i++)
            {
                ...
                (
                    isInterface*pos0(fld - fldInterfaceValue)
                );
            ...
            for (label i = 0; i < outerRefLayers; i++)
            {
                ...
                (
                    isInterface*pos0(fldInterfaceValue - fld)
                );
            ...
            if (nAddLayers > 0)
            {
                ...
                for (label i = 1; i < refineLevel; i++)
                {
                    ...
                    {
                        isInterface +=
                            neg(- fvc::average(fvc::interpolate(isInterface)));
                        isInterface = neg(- isInterface);
                    }
                    ...
                    forAll(isInterface, cellI)
                    {
                        ...
                        {
                            targetLevel[cellI] =
                                max(targetLevel[cellI], (refineLevel - i));
                        }
                    }
                }
            }
        ...
        forAll(refinedRegions_, regionI)
        {
            ...
            (
                mesh(),
                "cellSet",
                mesh().nCells()/10 + 1 // estimate
            );

            source->applyToSet(topoSetSource::NEW, selectedCellSet);

            const labelList cells = selectedCellSet.toc();

            label minLevel = region.dict().lookup<label>("minLevel");
        ...
        //-DD: Add buffer layer based on targetLevel field to prevent
        // 2-to-1 refinement
        if (nBufferLayers_ > 1)
        {
                ...
                for (label i = 1; i < nBufferLayers_; i++)
                {
                    blockedLevel =
                        ...
                        (
                            ...
                            pos0
                            (
                                fvc::average(fvc::interpolate(blockedLevel))
                              - SMALL
                            )*(currLayer-1)
                        );
                }
        ...
        {
            // Create a field with non-default boundary conditions in order to
            // solve relaxation
            ...
            {
                targetLevelField.primitiveFieldRef()[cellI] =
                    targetLevel[cellI];
            }
    ...
    {
        intRefFld.primitiveFieldRef()[cellI] =
            targetLevel[cellI] - cellLevel[cellI];
...
void Foam::fvMeshTopoChangers::multiRefiner::smoothTargetLevel
(
...
)
{
    ...
    surfaceScalarField diffCoeff
    (
        pow(diffCoeff_/targetLevelField.mesh().deltaCoeffs(), 2)
    );
    ...
    surfaceScalarField hasRefLevelJump
    (
        neg(-mag(fvc::snGrad(targetLevelField)))
    );
    ...
    {
        hasRefLevelJump =
            neg(-mag(fvc::interpolate(fvc::average(hasRefLevelJump))));
    }
    ...
    if (allowOnlyLevelGrowth_)
    {
        ...
        forAll(targetLevelDiscrete, cellI)
        {
            targetLevelDiscrete[cellI] =
                max(targetLevelDiscrete[cellI], targetLevel[cellI]);
        }
    }
...
void Foam::fvMeshTopoChangers::multiRefiner::readDict()
{
    ...
    dictionary& dict = dynamicMeshDict.subDict("topoChanger");

    nBufferLayers_ = dict.lookupOrDefault<label>("nBufferLayers",0);
    refineInterval_ = dict.lookup<label>("refineInterval");
    maxRefinement_ = dict.lookup<label>("maxRefinement");

    if (dict.isDict("refinementControls"))
    {
        dictionary& refineControlDict = dict.subDict("refinementControls");
        ...
        targetLevelSmoothing_ =
            refineControlDict.lookupOrDefault<Switch>
            (
                "targetLevelSmoothing",
                false
            );
        ...
        if (enableRefinementControl_)
        {
            ...
            // Overwrite field name entry in topoChanger dictionary if necessary
            if (fieldName != "internalRefinementField")
            {
                Info<< "Overwriting dictionary entries: \n"
                    << "'field', 'lowerRefineLevel' and 'upperRefineLevel' \n"
                    << " in topoChanger dictionary!" << endl;

                dict.set("field", word("internalRefinementField"));
                dict.set("lowerRefineLevel", scalar(0.5));
                dict.set("upperRefineLevel", scalar(maxRefinement_ + 0.5));
                dict.set("unrefineLevel", scalar(-0.5));
...
Foam::fvMeshTopoChangers::multiRefiner::multiRefiner
(
    fvMesh& mesh,
    ...
    maxRefinement_(0)
{
    ...
    if (enableRefinementControl_)
    {
        internalRefinementFieldPtr_.set
        (
            new volScalarField
            (
                IOobject
...
bool Foam::fvMeshTopoChangers::multiRefiner::update()
{
    ...
    // Part 2: Call normal update from fvMeshTopoChangers::refiner
    bool hasChanged = refiner::update();

    return hasChanged;
}


void Foam::fvMeshTopoChangers::multiRefiner::topoChange
(
    const polyTopoChangeMap& map
)
{
    refiner::topoChange(map);
}


void Foam::fvMeshTopoChangers::multiRefiner::distribute
(
    const polyDistributionMap& mdpm
)
{
    refiner::distribute(mdpm);
}
