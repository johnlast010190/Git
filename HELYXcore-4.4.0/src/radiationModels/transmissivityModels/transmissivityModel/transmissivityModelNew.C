...
Foam::autoPtr<Foam::radiationModels::transmissivityModel>
Foam::radiationModels::transmissivityModel::New
(
...
)
{
    ...
    const auto ctor =
        ctorTableLookup
        (
            "transmissivityModel type",
            dictionaryConstructorTable_(),
            modelType
        );
    return autoPtr<transmissivityModel>(ctor(dict, mesh));
}
