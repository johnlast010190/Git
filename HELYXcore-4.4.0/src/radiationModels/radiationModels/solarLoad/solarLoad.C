...
const Foam::word Foam::radiationModels::solarLoad::viewFactorWalls = "viewFactorWall";


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

bool Foam::radiationModels::solarLoad::updateHitFaces()
{
    if (hitFaces_.empty())
    {
         hitFaces_.reset
         (
            new faceShading(mesh_, solarCalc_.direction(), TRef_.value())
        );
         return true;
    }
    else
...
void Foam::radiationModels::solarLoad::updateAbsorptivity
(
...
void Foam::radiationModels::solarLoad::updateDirectHitRadiation
(
...
void Foam::radiationModels::solarLoad::updateSkyDiffusiveRadiation
(
    const labelHashSet& includePatches,
    const labelHashSet& includeMappedPatchBasePatches
)
...
void Foam::radiationModels::solarLoad::initialise(const dictionary& coeffs)
{
    if (coeffs.found("gridUp"))
    {
        verticalDir_ = coeffs.lookup<vector>("gridUp");
    ...
    useVFbeamToDiffuse_ = coeffs.lookup<bool>("useVFbeamToDiffuse");
...
void Foam::radiationModels::solarLoad::calculateQdiff
(
    const labelHashSet& includePatches,
    const labelHashSet& includeMappedPatchBasePatches
)
...
Foam::radiationModels::solarLoad::solarLoad
(
...
Foam::label  Foam::radiationModels::solarLoad::nBands() const
{
    return nBands_;
}


bool Foam::radiationModels::solarLoad::read()
{
...
void Foam::radiationModels::solarLoad::calculate()
{
    const polyBoundaryMesh& patches = mesh_.boundaryMesh();
    ...
    {
        // Reset Ru and qr
        Ru_ = dimensionedScalar("Ru", dimMass/dimLength/pow3(dimTime), 0);
        qrBf = 0.0;

        solarCalc_.correctSunFlux();
...
Foam::tmp<Foam::volScalarField> Foam::radiationModels::solarLoad::Rp() const
{
...
Foam::radiationModels::solarLoad::Ru() const
{
    return Ru_;
}
