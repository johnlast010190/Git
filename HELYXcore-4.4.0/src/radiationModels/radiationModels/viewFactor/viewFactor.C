...
const Foam::word Foam::radiationModels::viewFactor::viewFactorWalls =
    "viewFactorWall";
...
void Foam::radiationModels::viewFactor::initialise()
{
    ...
    if (debug)
    {
        Pout<< "radiationModels::viewFactor::initialise() Selected patches:"
            << selectedPatches_ << endl;
        Pout<< "radiationModels::viewFactor::initialise() Number of coarse faces:"
    ...
    if (Pstream::master())
    {
        ...
        bool smoothing = coeffs_.lookup<bool>("smoothing");
        if (smoothing)
        {
            if (debug)
            {
        ...
        constEmissivity_ = coeffs_.lookup<bool>("constantEmissivity");
        if (constEmissivity_)
        {
            CLU_.reset
            (
    ...
    {
        applySolarLoad_ = this->template lookup<bool>("useSolarLoad");
    }
...
void Foam::radiationModels::viewFactor::calculate()
{
    ...
    Pstream::listCombineReduce(T, maxOp<scalar>());
    Pstream::listCombineReduce(E, maxOp<scalar>());
    Pstream::listCombineReduce(qrExt, maxOp<scalar>());
