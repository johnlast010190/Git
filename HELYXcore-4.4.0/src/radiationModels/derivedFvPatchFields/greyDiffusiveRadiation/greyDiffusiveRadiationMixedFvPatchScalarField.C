...
Foam::radiationModels::greyDiffusiveRadiationMixedFvPatchScalarField::
...
:
    ...
    (
        dict.lookupOrDefault<bool>("externalRadiationLoad", true)
    )
{
...
void Foam::radiationModels::greyDiffusiveRadiationMixedFvPatchScalarField::
updateCoeffs()
{
    if (this->updated())
    {
    ...
    if (!isA<fvDOM>(radiation))
    {
        FatalErrorInFunction
            << "In field \"" << internalField().name()
            << "\" the boundary \"" << patch().name() << "\" has \"" << typeName
            << "\", which requires fvDOM radiation model."
            << nl << exit(FatalError);
    }
    ...
    //note: the contribution to q_absorbed is added in top level radiation solver
    if
    (
        externalRadiationLoad_
    ...
    if (dom.useExternalBeam())
    {
        refValue() = io*Iexternal*transmissivity*
            (
...
coupledRadiantTransmission() const
{
    ...
    if (isA<mappedFvPatch>(this->patch()))
    {
        ...
        const mappedFvPatch& mpp =
            refCast<const mappedFvPatch>(patch());
        ...
        if (isA<fvDOM>(radiation))
        {
            ...
            if
            (
                db().foundObject<volScalarField>(this->internalField().name())
            )
            {
                scalarField nbrFlux =
                ...
                Isource = mpp.interpolate(Isource, defaultValues);
            }
        }
    ...
    else if (isA<cyclicAMIFvPatch>(this->patch()))
    {
        ...
        const cyclicAMIFvPatch& mpp =
            refCast<const cyclicAMIFvPatch>(patch());
        const fvPatch& nbrPatch = mpp.nbrPatch();
        const label nbrPatchI = nbrPatch.index();

        const radiationModel& radiation =
            nbrPatch.boundaryMesh().mesh().lookupObject<radiationModel>
            (
                "radiationProperties"
            );
...
) const
{
    ...
    if (!db().foundObject<fvDOM>("radiationProperties"))
    {
        FatalErrorInFunction
            << "In field \"" << internalField().name()
            << "\" the boundary \"" << patch().name() << "\" has \"" << typeName
...
void Foam::radiationModels::greyDiffusiveRadiationMixedFvPatchScalarField::write
(
    Ostream& os
) const
{
