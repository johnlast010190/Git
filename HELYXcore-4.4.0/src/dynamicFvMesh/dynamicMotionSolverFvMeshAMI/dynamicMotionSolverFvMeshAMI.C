...
bool Foam::dynamicMotionSolverFvMeshAMI::update()
{
    ...
    for (polyPatch& pp : pbm)
    {
        DebugInformation
            ...
            << " size:" << returnReduceToMaster(pp.size(), sumOp<label>())
            << " mag(faceAreas):" << gSum(pp.magFaceAreas()) << endl;
    ...
    if (changeRequired)
    {
        ...
        //   trigger polyBoundaryMesh::topoChange (init and update) and
        //   ::calcGeometry (with topoChanged = false)
        // - BUT: mesh still corresponds to original (non-extended mesh) so
        //   we want to bypass these calls...
        // - after changes topoChanged = true
        ...
        // - polyMesh::topoChange
        //   - fires initTopoChange and topoChange in AMI BCs - called before
        //     mapFields
        // - AMI addressing must be up-to-date - used by, e.g. FaceCellWave
        // - will trigger again polyBoundaryMesh::topoChange (init and update)
        topoChange(map());

        // Move points and update derived properties
        // Note:
        // - resets face areas based on raw point locations!
        // - polyBoundaryMesh::topoChange (init and update)
        // Note:
