...
void Foam::dynamicRefineFvMesh::readDict()
{
    ...
    if (refineDict.found("refineStartTime"))
        refineStartTime_ = refineDict.lookup<scalar>("refineStartTime");

    if (refineDict.found("refineStopTime"))
        refineStopTime_ = refineDict.lookup<scalar>("refineStopTime");
...
)
{
    ...
    autoPtr<polyTopoChangeMap> map = meshMod.changeMesh(*this, false);
        ...
        if (mapSurfaceFields_.found(iter.key()))
        {
            ...
            for (label facei = 0; facei < nInternalFaces(); facei++)
            {
                ...
                if (oldFacei == -1)
                {
                    ...
                    if (counter > 0)
                    {
                        if (SurfaceField<T>::typeName == "surfaceScalarField")
                        {
                            tmpValue /= counter;
                        }
                        else if (SurfaceField<T>::typeName == "surfaceVectorField")
                        {
                            magFld /= counter;
