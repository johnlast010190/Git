...
#include "patchSwitchFvMesh/patchSwitches/tableSwitch/tableSwitch.H"
...
bool tableSwitch::enableCondition()
{
    ...
    Info<< this->type()
         ...
         << ": open, disableCondition - boolean table: "
         << tableValue << endl;

    return condition;
}

bool tableSwitch::disableCondition()
{
    bool tableValue = boolTable_->value(mesh_.time().value());
    ...
    Info<< this->type()
         << " for GIB faceZone "
         << mesh_.faceZones()[zoneID_].name()
         << ": closed, enableCondition - boolean table: "
         << tableValue << endl;
