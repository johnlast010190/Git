...
void Foam::mapGIB::mapNonOverlappingTargetFaces()
{
...

    {
        ...
        if (nNonOverFaces)
        {
            ...
            const indirectPolyPatch& slaveGIB
            (
                refCast<const indirectPolyPatch>(mesh_.boundary()[sId].patch())
            );
...
void Foam::mapGIB::makeMasterAMIInterpolation() const
{
    if (mAMIInterPtr_)
    {
...
) const
{
    ...
    forAll(dNonOverFaces, fI)
    {
        ...
        else
        {
            ...
            {
                WarningInFunction
                    << "Source patch size for mapping zero" << endl;

            }

            WarningInFunction
                << "Unable to map face " << fII << " at "
                << targetPatchm_->faceCentres()[fII] << endl;
        }
    }
    if (debug)
    {
...
void Foam::mapGIB::makeSlaveAMIInterpolation() const
{
    if (sAMIInterPtr_)
    {
        FatalErrorInFunction << abort(FatalError);
    ...
    sAMIInterPtr_ = new AMIInterpolation
        (
            *sourcePatchs_,
            *targetPatchs_,
            faceAreaIntersect::tmMesh,
