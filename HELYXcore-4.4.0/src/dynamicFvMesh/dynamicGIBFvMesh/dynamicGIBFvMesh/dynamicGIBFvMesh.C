...
void Foam::dynamicGIBFvMesh::initialize()
{
    ...
    baseSf_ = new vectorField(this->faces().size(), Zero);
    baseCf_ = new vectorField(this->faces().size(), Zero);
    vectorField& baseSf = *baseSf_;
    vectorField& baseCf = *baseCf_;

    scalarField magfAreas(this->faces().size());

    baseCC_ = new vectorField(this->cells().size(), Zero);
...
void Foam::dynamicGIBFvMesh::modifyRegion0Patch()
{
    if (region0Patch_.size() == 0)
    {
        ...
        do
        {
            ...
            patchi ++;
        } while (patchi < this->boundary().size());
    }
}
...
void Foam::dynamicGIBFvMesh::makeFl() const
{
    ...
    pointField neiCc(this->nFaces() - this->nInternalFaces(), Zero);
    pointField ownCc(this->nFaces() - this->nInternalFaces(), Zero);
        ...
        if (!isA<indirectPolyPatch>(pp))
        {
            ...
            if (pp.coupled())
            {
                forAll(pp, pFacei)
                {
                    ...
                    neiCc[facei - this->nInternalFaces()] =
                        baseCc[faceCells[pFacei]];
                    ownCc[facei - this->nInternalFaces()] =
                        baseCc[faceCells[pFacei]];
                }
            }
            else
            {
                ...
                {
                    const label facei = pp.start() + pFacei;
                    neiCc[facei - this->nInternalFaces()] = baseCf[facei];
                    ownCc[facei - this->nInternalFaces()] =
                        baseCc[faceCells[pFacei]];
                }
            }
    ...
    forAll(interestFaces, facei)
    {
        ...
        {
            const label own = this->owner()[interestFacesI];
            const label nei = this->neighbour()[interestFacesI];
        ...
        {
            const label patchi =
                this->boundaryMesh().whichPatch(interestFacesI);
            const polyPatch& pp = this->boundary()[patchi].patch();
            if (!isA<indirectPolyPatch>(pp))
            {
                onCC[facei] = ownCc[interestFacesI - this->nInternalFaces()];
                nbCC[facei] = neiCc[interestFacesI - this->nInternalFaces()];
            }
        }
    }
    ...
    if (false)
    {
        const word timeName(this->time().timeName());
...
void Foam::dynamicGIBFvMesh::makeFlipMap() const
{
    if (fmPtr_)
    {
        FatalErrorInFunction << abort(FatalError);
...
void Foam::dynamicGIBFvMesh::makeInterPoints0() const
{
    ...
    forAll(fll, facei)
    {
        const face& f = this->faces()[fll[facei]];
        forAll(f, pFacei)
        {
...
void Foam::dynamicGIBFvMesh::makePhiB() const
{
    ...
    {
        sweptVols[facei] = this->faces()[facei].sweptVol(cP0, cP);
    }
    phiBPtr_ =
    ...
    forAll(fmNew, facei)
    {
        ...
        {
            const label patchi = this->boundaryMesh().whichPatch(fII);
...
void Foam::dynamicGIBFvMesh::makePhiTrS() const
{
    ...
    boolList ppUp(this->points().size(), false);
        ...
        if (popUpC[celli] == 1)
        {
            ...
            forAll(cFaces, cFacei)
            {
                const label facei = cFaces[cFacei];
                {
                    ...
                    {
                        const labelList& f = this->faces()[facei];
                        forAll(f, pointi)
                        {
                            ppUp[f[pointi]] = false;
                        }
    ...
    syncTools::syncPointList(*this, ppUp, plusEqOp<bool>(), true);
    ...
    boolList fPopIndi(this->faces().size(), false);
    forAll(this->points(), pointi)
    {
        const labelList& pFacesi = this->pointFaces()[pointi];
        if (ppUp[pointi])
        {
    ...
    forAll(fPopIndi, facei)
    {
        if (fPopIndi[facei] == true)
        {
            sweptVols[facei] = this->faces()[facei].sweptVol(oldPs, newPs);
        }
    }
...
void Foam::dynamicGIBFvMesh::makePhiTrG() const
{
    ...
    forAll(fPopIndi, facei)
    {
        if (fPopIndi[facei] == true)
        {
            sweptVols[facei] = this->faces()[facei].sweptVol(newPs, oldPs);
        }
    }
    ...
    forAll(fmNew, facei)
    {
        ...
        else
        {
            ...
            if (!isA<indirectPolyPatch>(this->boundaryMesh()[patchi]))
            {
                const label lpfI = fII - this->boundaryMesh()[patchi].start();
                const labelList& fcs = this->boundary()[patchi].faceCells();
...
void Foam::dynamicGIBFvMesh::makeFullSnapCellPoints() const
{
    ...
    forAll(fsc, celli)
    {
        const labelList& cPoints = this->cellPoints()[fsc[celli]];
        forAll(cPoints, cPointi)
        {
    ...
    syncTools::syncPointList(*this, markPoints, plusEqOp<bool>(), true);
...
void Foam::dynamicGIBFvMesh::makePopCellPoints() const
{
    ...
    syncTools::syncPointList(*this, popCellPoints, plusEqOp<bool>(), true);
}
...
void Foam::dynamicGIBFvMesh::makePopSPoints() const
{
    ...
    syncTools::syncPointList(*this, popSPoints, plusEqOp<bool>(), true);
}
...
void Foam::dynamicGIBFvMesh::makePopGPoints() const
{
    ...
    syncTools::syncPointList(*this, popGPoints, plusEqOp<bool>(), true);
}
...
void Foam::dynamicGIBFvMesh::makeRecPoints0() const
{
    ...
    boolList oldnewintPoints(points().size(), false);
    boolList marknewFaces(faces().size(), false);
    boolList markoldFaces(faces().size(), false);
    ...
    forAll(this->faces(), facei)
    {
        ...
        {
            forAll(this->faces()[facei], fPointi)
            {
                const label pointi = this->faces()[facei][fPointi];
...
void Foam::dynamicGIBFvMesh::makeRecAllPoints0() const
{
    ...
    pointField disp (this->points().size(), Zero);
    ...
    if (true)
    {
        ...
        const labelList& addr =
            this->boundary()[masterId()].patch().meshPoints();
        ...
        syncTools::syncPointList(*this, pointMoved, plusEqOp<bool>(), true);

        const labelList& addrS =
            this->boundary()[slaveId()].patch().meshPoints();
        ...
        syncTools::syncPointList(*this, disp, maxMagEqOp(), vector::zero);
        recAllPoints0 += disp;
...
void Foam::dynamicGIBFvMesh::makeBoundaryPoints() const
{
    ...
    for (label facei = this->nInternalFaces(); facei < this->nFaces(); facei++)
    {
        const label patchi = this->boundaryMesh().whichPatch(facei);
        if
        (
    ...
    syncTools::syncPointList(*this, boundaryPoints, plusEqOp<bool>(), true);
}
    ...
    forAll(interestFaces, facei)
    {
        ...
        if (interestFacesI >= this->nInternalFaces())
        {
            ...
            if (!isA<indirectPolyPatch>(pp))
            {
                if (!pp.coupled())
                {
                    const label own = this->faceOwner()[interestFacesI];
    ...
    for (int i = 1; i < 4; i++)
    {
        ...
        forAll(this->neighbour(), facei)
        {
            if (interestedFace[facei])
            {
                ...
                {
                    const labelList& cFaces = this->cells()[own];
                    forAll(cFaces, cFacei)
                    {
                ...
                {
                    const labelList& cFaces = this->cells()[nei];
                    forAll(cFaces, cFacei)
                    {
                        const label cfII = cFaces[cFacei];
                        if (!constraintFace[cfII])
            ...
            if (!isA<indirectPolyPatch>(pp))
            {
                ...
                forAll(pp, pFacei)
                {
                    ...
                    if (interestedFace[facei])
                    {
                        ...
                        const labelList& cFaces = this->cells()[fCelli];
                        forAll(cFaces, cFacei)
                        {
                            const label cfII = cFaces[cFacei];
                            if (!constraintFace[cfII])
...
) const
{
    ...
    forAll(fList, facei)
    {
        ...
        const face& f = this->faces()[fListi];
        forAll(f, fPointi)
        {
...
) const
{
    ...
    forAll(dIntPoints, pointi)
    {
        ...
        {
            dIntPointLoc[pointi] = this->points()[gpI];
        }
    }
    ...
    hitPointPtr_ = new vectorField(this->points().size(), Zero);
    ...
    pointField fromPoints(this->points().size(), Zero);
    forAll(dIntPoints, pointi)
    {
        const label gpI = dIntPoints[pointi];
...
) const
{
    ...
    for (unsnapStep = 0; unsnapStep < maxUnsnapIntervals; unsnapStep++)
    {
        ...
        vectorField d(this->faces().size(), Zero);
        forAll(this->faces(), facei)
        {
            if (hasInterPoint[facei])
            {
                label celli = this->faceOwner()[facei];
                const face& f = this->faces()[facei];
                point ownCentre
                (
                ...
                if (facei < this->faceNeighbour().size())
                {
                    label cellj = this->faceNeighbour()[facei];
                    point neiCentre
                    (
...
void Foam::dynamicGIBFvMesh::calculateFlipMap() const
{
    ...
    boolList markCells(this->cells().size(), false);
    const labelList& fList = fl();
    ...
    forAll(fList, facei)
    {
        const label own = this->faceOwner()[fList[facei]];
        if (markCells[own])
        {
            fm[facei] = true;
        }
...
void Foam::dynamicGIBFvMesh::popShrinkFields()
{
    ...
    boolList fPopIndi(this->faces().size(), false);
    boolList popIndi(this->cells().size(), false);
    forAll(this->points(), pointi)
    {
        const labelList& pFacesi = this->pointFaces()[pointi];
...
void Foam::dynamicGIBFvMesh::popGrowFields()
{
    ...
    scalarField sV(this->cells().size(), 0.0);
...
void Foam::dynamicGIBFvMesh::modifyVolSqueezedCells() const
{
    ...
    forAll(dcV0, celli)
    {
        ...
        const labelList& cFaces = this->cells()[gcI];
        forAll(cFaces, cFacei)
        {
            const label facei = cFaces[cFacei];
    ...
    forAll(this->boundary(), patchi)
    {
        ...
        if (isA<indirectPolyPatch>(poly))
        {
            ...
            const indirectPolyPatch& inPoly =
                refCast<const indirectPolyPatch>(this->boundary()[patchi].patch());
...
void Foam::dynamicGIBFvMesh::correctBCs()
{
    ...
    if (correctBCs_)
    {
        ...
        {
            volScalarField& k = lookupObjectRef<volScalarField>("k");
            forAll(k, celli)
            {
                if (k[celli] < SMALL)
                {
        ...
        forAll(limitNegFieldNames_, fieldI)
        {
            ...
            if (this->foundObject<volScalarField>(substring))
            {
                volScalarField& field =
                    lookupObjectRef<volScalarField>(substring);
...
:
    ...
    masterFCells_(this->cells().size(), false),
    slaveFCells_(this->cells().size(), false),
    region0Patch_
    (
