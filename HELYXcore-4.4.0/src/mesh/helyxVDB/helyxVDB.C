...
Foam::labelListList Foam::helyxVDB::calcVoxelVoxels
(
                ...
                (
                    ...
                    [&](const IndexGrid::ValueOnCIter& iter)
                    {
                        ...
                        (
                            ...
                            globalIDGrids[cellLevel - int(cellLevel > 0)]->getConstAccessor()
                        );

                        IndexGrid::ConstAccessor fineAcc
                        (
                            //globalIDGrids[cellLevel+1]->getConstAccessor()
                            globalIDGrids[cellLevel + int(cellLevel < maxCellLevel_)]->getConstAccessor()
                        );
...
void Foam::helyxVDB::refineDanglingVoxels
(
...
Foam::label Foam::helyxVDB::syncGrids
(
    std::vector<FloatGrid::Ptr>&  cellLevelGrids,
    std::vector<IndexGrid::Ptr>&  globalIDGrids,
...
Foam::labelList Foam::helyxVDB::countVoxels
(
...
Foam::labelList Foam::helyxVDB::countVoxelsLeaf
(
    const std::vector<FloatGrid::Ptr>&  cellLevelGrids,
    std::vector<IndexGrid::Ptr>&  globalIDGrids,
    pointField& voxelCentres
...
const std::vector<openvdb::Coord> Foam::helyxVDB::getVoxelVertices
(
...
void Foam::helyxVDB::addMidPoint
(
    openvdb::Coord p,
    IndexGrid::Accessor& pointIDAcc,
    IndexGrid::Accessor& pointLevelAcc,
...
void Foam::helyxVDB::addEmptyPatch
(
    fvMesh& mesh,
...
Foam::labelListList Foam::helyxVDB::groupSameLevelRegions
(
...
openvdb::Index64 Foam::helyxVDB::onVoxelsInCoordBbox
(
    FloatGrid::Ptr grid,
...
std::vector<FloatGrid::Ptr> Foam::helyxVDB::sliceGrids
(
...
)
{
    ...
    scalar minDelta = 1 / (1 << maxCellLevel_);
...
void Foam::helyxVDB::doDecomposition
(
    const std::vector<FloatGrid::Ptr>& grids,
...
void Foam::helyxVDB::fillProcGridsOrDecompose
(
    const std::vector<FloatGrid::Ptr>& grids,
    std::vector<std::vector<FloatGrid::Ptr>>& decomposedGrids,
    const Vector<label>& n,
...
Foam::dictionary Foam::helyxVDB::createDecompDict
(
    const Time& runTime,
    const word method
)
...
Foam::dictionary Foam::helyxVDB::checkDecomposeParDict
(
    const Time& runTime,
...
)
{
    ...
    const label nDomains = decompositionDict.lookup<label>("numberOfSubdomains");
...
Foam::triSurface Foam::helyxVDB::distributeSurface
(
...
)
{
    ...
    forAll(faceMap, i)
    {
        ...
        const label mul = 1 << refinementLevels[regioni];
...
Foam::triSurface Foam::helyxVDB::featureTriSurface
(
...
void Foam::helyxVDB::topDownAdvect
(
    FloatGrid::Ptr grid,
    const label cellLevel,
    label maxLevel /*= -1*/
)
{
    ...
    for (label i = maxLevel; i > cellLevel; i--)
    {
        ...
        delta -= 0.5 * (1 << (maxLevel - i));
    }

    //delta *= voxelSize_ * Foam::pow(2, maxCellLevel_ - maxLevel);
    delta *= voxelSize_ * (1 << (maxCellLevel_ - maxLevel));

    advectLevelSet(grid, delta, true);
}

void Foam::helyxVDB::bottomUpAdvect
(
    FloatGrid::Ptr grid,
    const label cellLevel
)
{
    ...
    scalar delta = 0.5 * (1 << (maxCellLevel_ - cellLevel));
...
void Foam::helyxVDB::advectLevelSet
(
    FloatGrid::Ptr grid,
    const scalar delta,
...
const openvdb::Coord Foam::helyxVDB::toGlobal
(
...
const openvdb::Coord Foam::helyxVDB::toLocal
(
...
void Foam::helyxVDB::splitVoxelFace
(
...
//void Foam::helyxVDB::checkPatchVoxelsProximity
//(
...
void Foam::helyxVDB::sliceBoundBox
(
    treeBoundBox& bb,
...
)
{
    ...
    scalar surfVoxelSize = voxelSize_ * (1 << (maxCellLevel_ - coarserCellLevel));
...
const Foam::triSurface Foam::helyxVDB::getVDBCuttingPlane
(
...
void Foam::helyxVDB::clipClosedSurface
(
    triSurface& surf,
    treeBoundBox& surfBb,
    const label coarserCellLevel
...
void Foam::helyxVDB::decomposeVDBSurface
(
    triSurface& surf,
    const label coarserCellLevel
)
...
//const Foam::List<Foam::triSurface> Foam::helyxVDB::boundBoxToPlanes
const Foam::List<Foam::plane> Foam::helyxVDB::boundBoxToPlanes
(
...
const Foam::triSurface Foam::helyxVDB::boundBoxToTriSurface
(
    treeBoundBox& box,
...
const Foam::List<Foam::triSurface> Foam::helyxVDB::splitSurface
(
    const triSurface& surf,
...
const Foam::triSurface Foam::helyxVDB::addSurfaces
(
...
)
{
    ...
    forAll(surfs, surfi)
    {
        ...
        List<labelledTri> addfaces(addsurf.stdMoveFaces());
        List<point> addpoints(addsurf.stdMovePoints());
...
const Foam::triSurface Foam::helyxVDB::gatherTriSurface
(
...
openvdb::math::BBox<openvdb::Vec3s> Foam::helyxVDB::getBoundingBox
(
    const std::vector<FloatGrid::Ptr>& grids
)
{
...
Foam::labelListList Foam::helyxVDB::decomposeGrids
(
    const std::vector<FloatGrid::Ptr>& cellLevelGrids,
    std::vector<IndexGrid::Ptr>&  globalIDGrids,
    std::vector<std::vector<FloatGrid::Ptr>>& decomposedGrids,
...
void Foam::helyxVDB::decomposeGrids
(
    const std::vector<FloatGrid::Ptr>& grids,
    std::vector<std::vector<FloatGrid::Ptr>>& decomposedGrids,
    const dictionary& decompositionDict,
...
std::vector<FloatGrid::Ptr> Foam::helyxVDB::getInteriorGrids
(
...
std::vector<FloatGrid::Ptr> Foam::helyxVDB::generateLODSequence
(
...
void Foam::helyxVDB::multiResUDF
(
    const word& name,
...
BoolGrid::Ptr Foam::helyxVDB::partialInterior
(
...
FloatGrid::Ptr Foam::helyxVDB::createCurvatureMask
(
...
//void Foam::helyxVDB::createPartialNarrowBandLevelSetGrids
//(
//    const triSurface& inputSurf,
...
std::vector<FloatGrid::Ptr> Foam::helyxVDB::multiResSDFandDispl
(
    const triSurface& inputSurf,
    const label minLevel,
    const label maxLevel,
...
std::vector<FloatGrid::Ptr> Foam::helyxVDB::multiResSDF
(
    const triSurface& inputSurf,
    const label minLevel,
    const label maxLevel,
...
const Foam::triSurface Foam::helyxVDB::volumeToMesh
(
    const scalar iso,
...
std::vector<FloatGrid::Ptr> Foam::helyxVDB::createNarrowBandLevelSetGridsPar
(
    const triSurface& inputSurf
)
{
    ...
    for (label cellLevel = maxCellLevel_; cellLevel >= 0; --cellLevel)
    {
        ...
        // as voxel is located at min corner of a cell
        //const label edgeLength = Foam::pow(2, maxCellLevel_ - cellLevel);
        const label edgeLength = 1 << (maxCellLevel_ - cellLevel);
...
std::vector<openvdb::GridPtrVecPtr> Foam::helyxVDB::sendReceiveGrids
(
...
template<typename GridType>
std::vector<std::vector<typename GridType::Ptr>> Foam::helyxVDB::sendReceiveGrids
(
    std::vector<typename GridType::Ptr>& procGrids,
    const word& gridsName
)
...
void Foam::helyxVDB::intersectProcGrids
(
...
template<typename GridType>
void Foam::helyxVDB::gatherProcGrids
(
    std::vector<typename GridType::Ptr>& procGrids
)
{
...
void Foam::helyxVDB::removeSolidVoxels
(
    FloatGrid::Ptr SDFGrid,
    FloatGrid::Ptr grid
)
...
//std::vector<FloatGrid::Ptr> Foam::helyxVDB::createNarrowBandMultiResGrids
//(
//    const triSurface& inputSurf
//)
//{
...
void Foam::helyxVDB::combineGrids
(
    const word name,
...
std::vector<FloatGrid::Ptr> Foam::helyxVDB::distanceRefinementGrading
(
    const word& name,
    const triSurface& inputSurf,
...
std::vector<FloatGrid::Ptr> Foam::helyxVDB::refinementRegionGrading
(
    const word& name,
    const triSurface& inputSurf,
...
void Foam::helyxVDB::topologyDifference
(
...
void Foam::helyxVDB::topologyUnion
(
    const std::vector<FloatGrid::Ptr>& resultGrids,
    const std::vector<FloatGrid::Ptr>& grids
)
...
std::vector<FloatGrid::Ptr> Foam::helyxVDB::deepCopy
(
    const std::vector<FloatGrid::Ptr>& grids
)
{
...
openvdb::CoordBBox Foam::helyxVDB::getLevelNBoundingBox
(
...
openvdb::Index64 Foam::helyxVDB::activeVoxelCountInBBox
(
    const std::vector<FloatGrid::Ptr>& grids,
    const label& maxSurfLevel,
    const openvdb::math::BBox<openvdb::Vec3s>& bBox,
...
std::vector<FloatGrid::Ptr> Foam::helyxVDB::surfaceGrading
(
    const word& name,
...
void Foam::helyxVDB::calculateBufferGrids
(
    const word& name,
...
)
{
    ...
    for (int cellLevel = bufferLevel; cellLevel >= 0; --cellLevel)
    {
        ...
        if (cellLevel > 0)
        {
            ...
            transformOffset += 0.5 * (1.0/ (1 << (bufferLevel - cellLevel + 1)));
...
FloatGrid::Ptr Foam::helyxVDB::searchableBoxToLevelSet
(
    const point min,
    const point max,
    const label cellLevel
)
{
    ...
    scalar edgeLength = 1 << (maxCellLevel_ - cellLevel);
...
FloatGrid::Ptr Foam::helyxVDB::meshToLevelSet
(
    const triSurface& inputSurf,
    const label cellLevel,
...
)
{
    ...
    const scalar delta = voxelSize_* (1 << (maxCellLevel_ - cellLevel));
...
//void Foam::helyxVDB::offsetLevelSetValues
//(
//    const FloatGrid::Ptr& grid,
//    const scalar& offset
//)
...
bool Foam::helyxVDB::isOpenSurface
(
    const FloatGrid::Ptr& grid
)
{
...
FloatGrid::Ptr Foam::helyxVDB::topologyUnionLevelSets
(
    const std::vector<std::vector<FloatGrid::Ptr>>& boundaryCellLevels,
    const label cellLevel
)
...
FloatGrid::Ptr Foam::helyxVDB::csgUnionLevelSets
(
    const std::vector<std::vector<FloatGrid::Ptr>>& boundaryCellLevels,
    const label cellLevel,
...
)
{
    ...
    if (convertToSDF)
    {
        ...
        label edgeLength = 1 << (maxCellLevel_ - cellLevel);
