...
void Foam::VDBGridsConverter::convert()
{
    ...
    struct UpdatePointListOp
    {
        ...
        inline void operator()(const IndexGrid::ValueOnCIter& iter) const
        {
            ...
            const openvdb::Coord xyzGlobal = helyxVDB::toGlobal(xyz, indexTransform_);
    ...
    struct UpdateFinerIndexGridOp
    {
        ...
        inline void operator()(const FaceGrid::ValueOnCIter& iter) const
        {
            ...
            for (unsigned pointI=0; pointI < 4; pointI++)
            {
                ...
                const openvdb::Coord xyzCoord = helyxVDB::toLocal(xyzGlobal, coarseIndexGrid_.transformPtr());

                const openvdb::Coord xyzFineCoord = helyxVDB::toLocal(xyzGlobal, fineIndexGrid_.transformPtr());
    ...
    for (label cellLevel = 0; cellLevel <= maxCellLevel; cellLevel++)
    {
        ...
        label edgeLength = 1 << (maxCellLevel - cellLevel); //in (smallest) voxel units
    ...
    cellLevelField_ = std::move(cellLevelField);
    distanceField_ = std::move(distanceField);
    patchIDField_ = std::move(patchIDField);
    procIDField_ = std::move(procIDField);
} // convert()
