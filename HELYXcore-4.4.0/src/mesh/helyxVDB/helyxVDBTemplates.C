...
struct SendGridOp
{
    ...
    void operator()(const tbb::blocked_range<size_t>& range) const
    {
        for (size_t proci = range.begin(); proci < range.end(); ++proci)
        {
            ...
            {
                Foam::helyxVDB::sendGrid<GridType>
                (
                    grid_,
                    pBufSize_,
                    pBufGrid_,
        ...
        for (size_t proci = range.begin(); proci < range.end(); ++proci)
        {
            ...
            )
            {
                typename GridType::Ptr gridFromProcI =
                    Foam::helyxVDB::receiveGrid<GridType>
                    (
                        pBufSize_,
                        pBufGrid_,
                        proci
...
template<class T>
Foam::label Foam::helyxVDB::activeCount
(
    std::vector<T>& list
)
{
...
template<typename GridType>
std::vector<typename GridType::Ptr> Foam::helyxVDB::receiveGrids
(
    PstreamBuffers& pBufSize,
    PstreamBuffers& pBufGrid,
    const label node
...
template<typename GridType>
typename GridType::Ptr Foam::helyxVDB::receiveGrid
(
    PstreamBuffers& pBufSize,
    PstreamBuffers& pBufGrid,
    const label node
...
template<class T>
void Foam::helyxVDB::allGatherList
(
    List<T>& list
)
{
...
template<typename GridType>
Foam::List<Foam::List<Foam::boundBox>> Foam::helyxVDB::sendReceiveBoundBoxes
(
    const std::vector<typename GridType::Ptr>& grids
)
{
...
template<class Type>
void Foam::helyxVDB::createVolField
(
    fvMesh& mesh,
...
template<typename GridType>
Foam::label Foam::helyxVDB::getActiveVoxels
(
    const std::vector<typename GridType::Ptr>& grids
)
{
