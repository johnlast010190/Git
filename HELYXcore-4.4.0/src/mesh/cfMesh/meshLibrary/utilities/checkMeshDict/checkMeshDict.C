...
#include "utilities/checkMeshDict/checkMeshDict.H"
...
void checkMeshDict::checkBasicSettings() const
{
    ...
    const scalar maxCellSize = meshDict_.lookup<scalar>("maxCellSize");
    ...
    {
        const scalar bcs = meshDict_.lookup<scalar>("boundaryCellSize");
    ...
    {
        const scalar mcs = meshDict_.lookup<scalar>("minCellSize");
    ...
    {
        const bool keep =
            meshDict_.lookup<bool>("keepCellsIntersectingBoundary");

        if (keep && meshDict_.found("checkForGluedMesh"))
        {
            meshDict_.lookup<bool>("checkForGluedMesh");
        }
    }
    ...
    {
        meshDict_.lookup<bool>("enforceGeometryConstraints");
    }
}
    ...
    if (meshDict_.found("localRefinement"))
    {
        if (meshDict_.isDict("localRefinement"))
        {
            ...
            forAll(entries, dictI)
            {
                ...
                {
                    const scalar cs = dict.lookup<scalar>("cellSize");
                ...
                {
                    const label nLevels =
                        dict.lookup<label>("additionalRefinementLevels");
                ...
                {
                    const scalar s =
                        dict.lookup<scalar>("refinementThickness");
...
void checkMeshDict::checkBoundaryLayers() const
{
    if (meshDict_.found("boundaryLayers"))
    {
        ...
        {
            bndLayers.lookup<label>("nLayers");
        }
        if (bndLayers.found("thicknessRatio"))
        {
            bndLayers.lookup<scalar>("thicknessRatio");
        }
        if (bndLayers.found("maxFirstLayerThickness"))
        {
            bndLayers.lookup<scalar>("maxFirstLayerThickness");
        }
        ...
        if (bndLayers.isDict("patchBoundaryLayers"))
        {
            ...
            forAll(patchNames, patchI)
            {
                ...
                if (patchBndLayers.isDict(pName))
                {
                    ...
                    {
                        patchDict.lookup<label>("nLayers");
                    }
                    if (patchDict.found("thicknessRatio"))
                    {
                        patchDict.lookup<scalar>("thicknessRatio");
                    }
                    if (patchDict.found("maxFirstLayerThickness"))
                    {
                        patchDict.lookup<scalar>("maxFirstLayerThickness");
                    }
                    if (patchDict.found("allowDiscontinuity"))
                    {
                        patchDict.lookup<bool>("allowDiscontinuity");
                    }
                }
                else
                {
        ...
        {
            bndLayers.lookup<bool>("optimiseLayer");
        }
        ...
        if (bndLayers.found("optimisationParameters"))
        {
            ...
            {
                optParams.lookup<bool>("reCalculateNormals");
            }

            if (optParams.found("nSmoothNormals"))
            {
                const label nSmoothNormals =
                    optParams.lookup<label>("nSmoothNormals");
            ...
            if (optParams.found("featureSizeFactor"))
            {
                const scalar featureSizeFactor =
                    optParams.lookup<scalar>("featureSizeFactor");
            ...
            if (optParams.found("relThicknessTol"))
            {
                const scalar relThicknessTol =
                    optParams.lookup<scalar>("relThicknessTol");
            ...
            if (optParams.found("maxNumIterations"))
            {
                const label maxNumIterations =
                    optParams.lookup<label>("maxNumIterations");
    ...
    if (meshDict_.found("patchCellSize"))
    {
        ...
        if (meshDict_.isDict("patchCellSize"))
        {
            ...
            forAll(patchNames, patchI)
            {
                ...
                const scalar cellSize = pDict.lookup<scalar>("cellSize");
