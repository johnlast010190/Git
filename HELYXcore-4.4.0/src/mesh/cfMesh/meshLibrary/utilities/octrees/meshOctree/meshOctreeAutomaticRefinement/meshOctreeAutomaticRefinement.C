...
void meshOctreeAutomaticRefinement::setMaxRefLevel()
{
    ...
    {
        const scalar maxSize(meshDict_.lookup<scalar>("maxCellSize"));
        scalar cs(meshDict_.lookup<scalar>("minCellSize"));
        cs *= (1.0 + SMALL);
    ...
    maxRefLevel_(0)
{
    ...
    {
        useDATABoxes_ = dict.lookup<bool>("keepCellsIntersectingBoundary");
    }
