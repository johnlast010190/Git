...
void meshOctreeCreator::setRootCubeSizeAndRefParameters()
{
    ...
    (
        scalingFactor_ *
        meshDictPtr_->lookup<scalar>("maxCellSize")
    );
    ...
    if (meshDictPtr_->found("boundaryCellSize"))
    {
        ...
        scalar cs(meshDictPtr_->lookup<scalar>("boundaryCellSize"));
    ...
    if (meshDictPtr_->found("patchCellSize"))
    {
        ...
        if (meshDictPtr_->isDict("patchCellSize"))
        {
            ...
            forAll(patchNames, patchI)
            {
                ...
                const scalar cs = patchDict.lookup<scalar>("cellSize");
    ...
    if (meshDictPtr_->found("localRefinement"))
    {
        if (meshDictPtr_->isDict("localRefinement"))
        {
            ...
            forAll(entries, dictI)
            {
                ...
                {
                    const scalar cs =
                        patchDict.lookup<scalar>("cellSize");
                ...
                {
                    refinementThickness =
                        patchDict.lookup<scalar>("refinementThickness");
                }
