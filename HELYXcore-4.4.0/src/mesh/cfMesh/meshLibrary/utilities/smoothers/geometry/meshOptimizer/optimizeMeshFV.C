    ...
    if (mesh_.returnTime().foundObject<IOdictionary>("meshDict"))
    {
        ...
        if (meshDict.found("boundaryLayers"))
        {
            ...
            if (layersDict.found("optimiseLayer"))
                smoothLayer = layersDict.lookup<bool>("optimiseLayer");
        }
...
void meshOptimizer::untangleBoundaryLayer()
{
    ...
    if (mesh_.returnTime().foundObject<IOdictionary>("meshDict"))
    {
        ...
        if (meshDict.found("boundaryLayers"))
        {
            ...
            {
                untangleLayer =
                    layersDict.lookup<bool>("untangleLayers");
            }
        }
    }
...
)
{
    ...
    do
    {
        ...
        Info<< "Iteration " << nIter
            << ". Max Orthogonality : "
            << (Foam::acos(minCos) * 180.0 / M_PI)
            << " Number of high orthogonality faces is : " << nBadFaces << endl;
        ...
        else if (nBadFaces < minNumBadFaces && minCos >= initialOrthoCos)
        {
            minErrorPts = mesh_.points();
            minNumBadFaces = nBadFaces;
    ...
    if (minNumBadFaces != 0)
    {
        Info<<"Using mesh from orthogonality iteration : " << minIter <<endl;
        mesh_.points() = minErrorPts;
        mesh_.clearAddressingData();
    }
}
