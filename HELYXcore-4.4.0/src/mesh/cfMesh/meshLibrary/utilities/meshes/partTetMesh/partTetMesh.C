...
void partTetMesh::updateVerticesSMP(const List<LongList<labelledPoint>>& np)
{
    ...
    pointField centre(points_.size(), vector::zero);
    scalarField denom(points_.size(), 0.0);
    ...
    # endif
    {
        ...
        forAll(updateType, pI)
        {
            if ((updateType[pI] & CELLCENTRE) || (updateType[pI] & FACECENTRE))
            {
                points_[pI] = centre[pI] / denom[pI];
            }
        }
    }
}
