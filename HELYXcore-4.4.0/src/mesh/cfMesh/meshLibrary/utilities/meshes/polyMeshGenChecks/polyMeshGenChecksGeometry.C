...
)
{
    ...
    forAll(cells, cI)
    {
        ...
        {
            Info<< "Cell " << cI << " contains face labels out of range: "
    ...
    if (nOpen > 0)
    {
        Info<< nOpen << " open cells found. Max cell openness: "
            << maxOpenCell << endl;
    ...
    if (nAspect > 0)
    {
        Info<< nAspect << " high aspect ratio cells found.  "
...
)
{
    ...
    # ifdef USE_OMP
    # pragma omp parallel for if (own.size() > 100) \
    ...
    if (nNegVolCells != 0)
    {
        Info<< nNegVolCells << " zero or negative cell volume detected."
            << endl;
...
)
{
    ...
    if (minArea < VSMALL)
    {
        Info<< "Zero or negative face area detected.  Minimum negative area: "
...
)
{
    ...
    if (nNegVolCells != 0)
    {
        Info<< nNegVolCells << " zero or negative part tetrahedra detected."
            << endl;
...
)
{
    ...
    const scalar severeNonorthogonalityThreshold =
        Foam::cos(nonOrthWarn/180.0*M_PI);
    ...
    if (report)
    {
        if (counter > 0)
        {
            Info<< "Mesh non-orthogonality Max: "
                << Foam::acos(minDDotS)/M_PI*180.0
                << " average: " <<
                   Foam::acos(sumDDotS/counter)/M_PI*180.0
                << endl;
        }
    }
    ...
    if (errorNonOrth > 0)
    {
        Info<< "Error in non-orthogonality detected" << endl;
...
)
{
    ...
    forAll(faces, faceI)
    {
        ...
        const scalar pyrVol = -Foam::primitiveMeshTools::pyramidVol
        (
            ctrs[owner[faceI]],
            fCentres[faceI],
            fAreas[faceI]
    ...
    {
        if (Pstream::master())
            Info<< "Error in face pyramids: " << nErrorPyrs
...
)
{
    ...
    for (label faceI=0;faceI<nInternalFaces;++faceI)
    {
        ...
        const point faceIntersection =
            centres[own[faceI]]*dNei/(dOwn+dNei+VSMALL)
          + centres[nei[faceI]]*dOwn/(dOwn+dNei+VSMALL);
        ...
        forAll(procBoundaries, patchI)
        {
            ...
            # endif
            {
                ...
                forAll(otherCentres, fI)
                {
                    ...
                    const point faceIntersection =
                        cOwn*dNei/(dOwn+dNei+VSMALL)
                      + cNei*dOwn/(dOwn+dNei+VSMALL);
...
)
{
    ...
    if (nWarnSkew > 0)
    {
        Info<< "Large face skewness detected.  Max skewness = " << maxSkew
...
)
{
    ...
    if (nConcave > 0)
    {
        Info<< nConcave  << " face points with severe concave angle (> "
            << maxDeg << " deg) found.\n"
            << endl;
...
)
{
    ...
    if (nWarped > 0)
    {
        Info<< nWarped  << " faces with severe warpage (flatness < "
            << warnFlatness << ") found."
            << endl;
...
)
{
    ...
    nCriticalErrors = returnReduce(badFaces.size(), sumOp<label>());
...
(
    ...
    const bool /*report*/,
    const boolList* activeFacePtr,
    const scalar orthogonalityThreshold
)
{
    badFaces.clear();
    ...
    forAll(checkValues, faceI)
    {
        if (activeFacePtr && activeFacePtr->operator[](faceI))
        {
            minCos = min(minCos,checkValues[faceI]);
            if (checkValues[faceI] < orthogonalityThreshold)
            {
                badFaces.insert(faceI);
            }
        }
    ...
    const label nBadFaces = returnReduce(badFaces.size(), sumOp<label>());
    reduce(minCos, minOp<scalar>());

    Info<< "Selected " << nBadFaces
         << " out of " << returnReduce(checkValues.size(), sumOp<label>())
         << " faces with high orthogonality" << endl;
