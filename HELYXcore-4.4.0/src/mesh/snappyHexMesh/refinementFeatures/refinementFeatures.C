   ...
   //Check for global geometry dict
    ...
    forAll(featDicts, featI)
    {
        ...
        if (globalGeomDict.found("lines"))
        {
            ...
            {
                tdict = ldict.subDict(featFileName);
        ...
        eMesh.doTransforms(dict, io, tdict);
...
void Foam::refinementFeatures::calcFeatureEdges
(
   const label surfI,
   const triSurface& s,
   const refinementSurfaces& refineSurfaces,
   DynamicList<Tuple2<label, edge>>& featureEdges
)
{
    ...
    forAll(pointFaces, pointi)
    {
        ...
        forAll(pFaces, i)
        {
            ...
            const scalar featureAngle = refineSurfaces.featureRefineAngle
            (
                surfI,
                region
            );
            ...
            if (nextPointI > pointi)
            {
                ...
                forAll(pFaces, j)
                {
                    ...
                    if (nPointI == nextPointI || pPointI == nextPointI)
                    {
                        nNext++;
                    }
                }
                ...
                if (manifold)
                {
                    forAll(pFaces, j)
                    {
                        const labelledTri& fnbr = s.localFaces()[pFaces[j]];
                        label fpnbr = findIndex(fnbr, pointi);
                        label nPointI = fnbr[fnbr.fcIndex(fpnbr)];
                        label pPointI = fnbr[fnbr.rcIndex(fpnbr)];
                        ...
                        if (nPointI == nextPointI || pPointI == nextPointI)
                        {
                            ...
                            if (!visited[j])
                            {
                                ...
                                const label& nRegion = s[pFaces[j]].region();
                                ...
                                if (!wrongOrient)
                                {
                                    ...
                                    )
                                    {
                                        ...
                                        {
                                            pointFeatureEdges.insert(e,nRegion);
                                        }
                                        else
                                        {
                                            pointFeatureEdges.insert(e,region);
                                        }
                                    }
                                }
                                break;
            ...
            if (prevPointI > pointi)
            {
                ...
                forAll(pFaces, j)
                {
                    ...
                    label nPointI = fnbr[fnbr.fcIndex(fpnbr)];
                    label pPointI = fnbr[fnbr.rcIndex(fpnbr)];
                    if (nPointI == prevPointI || pPointI == prevPointI)
                    {
                        nNext++;
                    }
                }
                    ...
                    forAll(pFaces, j)
                    {
                       ...
                       if (nPointI == prevPointI || pPointI == prevPointI)
                       {
                           ...
                           if (!visited[j])
                           {
                               ...
                               scalar dotProd = (fNorm & nNorm);
                               //check for wrongly oriented surface
                               if (dotProd < -0.9659 && (eVecOwn & eVecNei) < 0)
                               {
                                  wrongOrient = true;
                               }
                               const label& nRegion = s[pFaces[j]].region();
        ...
        {
            featureEdges.append(Tuple2<label, edge>(iter(),iter.key()));
        }
    }
    ...
    minCos_(-1)
{
    ...
    forAll(surf, surfI)
    {
        ...
        if (isA<triSurfaceMesh>(geom))
        {
            ...
            DynamicList<Tuple2<label, edge>> featureEdges(s.nPoints()/10);
            ...
            else
            {
                ...
                if (returnReduce(featureEdges.size(), sumOp<label>()) > 0)
                {
                    Pstream::scatter(featureEdges);
                }
            }
            s.clearOut();
            ...
            forAll(s.patches(), patchI)
            {
                ...
                {
                    label edgeRegion = featureEdges[edgei].first();
                    if (edgeRegion == patchI)
                    {
                        const edge e = featureEdges[edgei].second();
                        label v0 = e[0];
                        label v1 = e[1];
                        edg[nEdges] = e;
                        nEdges++;
                ...
                if (returnReduce(pts.size(), sumOp<label>()))
                {
                    ...
                    //- Check if feaureLevels refinement keyword set
                    // and use that for distance based feature refinement
                    List<Tuple2<scalar,label>> refineLevels =
                        refineSurfaces.featureRefineLevels(surfI,patchI);
                    if (refineLevels.size() > 1 || refineLevels[0].second() > 0)
                    {
                        List<scalar> refDistance(refineLevels.size());
                        List<label> refLevel(refineLevels.size());
                        forAll(refineLevels, leveli)
                        {
                            refDistance[leveli] = refineLevels[leveli].first();
                            refLevel[leveli] = refineLevels[leveli].second();
                        }
                        levels_[sz] = refLevel;
                        distances_[sz] = refDistance;
                    }
                    else
                    {
                        levels_[sz] =
    ...
    forAll(featDicts, featI)
    {
        ...
        //Perform optional transforms
        eMesh.doTransforms(dict, mesh.thisDb(), tdict);
        ...
        {
            // Look up 'level' for single level
            levels_[sz] = labelList(1, dict.lookup<label>("level"));
            distances_[sz] = scalarField(1, 0.0);
        }
...
void Foam::refinementFeatures::trim(const polyMesh& mesh)
{
    ...
    {
        meshBb[0] = treeBoundBox(boundBox(mesh.points(), false)).extend(1E-3);
    }
