...
)
{
    ...
    reduce(std::tie(sum, n), ParallelOp<sumOp<T>, sumOp<label>>{});
...
)
{
    ...
    forAll(edges, edgeI)
    {
        if (isMasterEdge[edgeI])
        {
            ...
            if (isBoundaryEdge[meshEdges[edgeI]])
            {
            ...
            {
                if (!isBoundaryPoint[meshPoints[e[0]]])
                {
                    sum[e[0]] += eWeight*pointData[v1];
                }
                if (!isBoundaryPoint[meshPoints[e[1]]])
                {
                    sum[e[1]] += eWeight*pointData[v0];
                }
            }
        }
    }
