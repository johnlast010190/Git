...
)
{
    ...
    if (returnReduce(nBaffles, sumOp<label>()))
    {
        ...
        topoChange(map, baffledFacesSet.toc(), updateIntersection);
    }
...
)
{
    ...
    if (returnReduce(couples.size()+faceToPatch.size(), sumOp<label>()))
    {
        ...
        topoChange(map, newExposedFaces, updateIntersection);
    }
...
Foam::autoPtr<Foam::polyTopoChangeMap> Foam::meshRefinement::mergeZoneBaffles
(
...
) const
{
    ...
    bool singleNamedRezone = refineParams.singleNamedRezone();
    ...
    while (true)
    {
        ...
        forAll(mesh_.faces(), facei)
        {
            ...
            else if (foundNamed && namedSurfaceIndex[facei] > -1)
            {
               blockedFaces[facei] = true;
            }
        }
        ...
        DynamicList<labelPair> nNeiZones(mesh_.nFaces()/10);
        labelList regionPID(globalRegion.nRegions(),-1);
        forAll(cellToZone, celli)
        {
            label globalRegionI = globalRegion[celli];
            if (excludeRegions.found(globalRegionI))
            {
                ...
                forAll(cellFaces, j)
                {
                    ...
                    if (patchi == -1)
                    {
                        ...
                        {
                            neiZone = cellToZone[nei];
                            neiRegion = globalRegion[nei];
                        }
                        else
                        {
                            neiZone = cellToZone[own];
                            neiRegion = globalRegion[own];
                        }
                    }
                    else if (patches[patchi].coupled())
                    {
                        label bfacei = meshFaceI - mesh_.nInternalFaces();
                        neiZone = neiCellZone[bfacei];
                        neiRegion = neiGlobalRegion[bfacei];
                    }
                    else
                    {
                        regionPID[globalRegionI] = max
                        (
                            regionPID[globalRegionI],
                            patchi
                        );
                    }
                    ...
                    if
                    (
                        globalRegionI != neiRegion
                        && !excludeRegions.found(neiRegion)
                    )
                    {
                        nNeiZones.append(labelPair(globalRegionI,neiZone));
                    }
                }
            }
        }
        Pstream::listCombineReduce(regionPID, maxOp<label>());

        // Set neighbour zone by maximum number of connected faces
        List<label> newZoneID(globalRegion.nRegions(), -2);
        List<List<labelPair>> gatheredNbrs(Pstream::nProcs());
        gatheredNbrs[Pstream::myProcNo()] = nNeiZones;
        Pstream::gatherList(gatheredNbrs);
        if (Pstream::master())
        {
            List<labelList> nbrZones(globalRegion.nRegions(),labelList(nZones+2,0));
            forAll(gatheredNbrs, proci)
            {
                const List<labelPair>& nbrPairs = gatheredNbrs[proci];
                forAll(nbrPairs, pairi)
                {
                    label regioni = nbrPairs[pairi].first();
                    label zonei = nbrPairs[pairi].second()+2;
                    nbrZones[regioni][zonei]++;
                }
            }
            forAll(nbrZones, regioni)
            {
                const labelList& nbrCount = nbrZones[regioni];
                label maxNbr = 0;
                label nbrZone = -2;
                forAll(nbrCount, i)
                {
                    if (nbrCount[i] > maxNbr)
                    {
                        maxNbr = nbrCount[i];
                        nbrZone = i-2;
                    }
                }
                newZoneID[regioni] = nbrZone;
            }
        }
        ...
        forAll(cellToZone, celli)
        {
            ...
            if (foundExcluded)
            {
                if (newZoneID[globalRegionI] != cellToZone[celli])
                {
        ...
        if (foundNamed)
        {
            ...
            forAll(mesh_.cells(), celli)
            {
                if (markedCells[celli])
                {
                    ...
                    label globalReg = max(regionPID[regioni], label(0));
                    if (namedRegion1 != -1 || cellToZone[celli] == -2)
                    {
                        forAll(cFaces, cFI)
                        {
                            label facei = cFaces[cFI];
                            ...
                            if (ownZone == -2 && neiZone != ownZone)
                            {
                                namedSurfaceIndex[facei] = -1;
                                globalRegion1[facei] = globalReg;
                                globalRegion2[facei] = globalReg;
                            }
...
) const
{
    ...
    forAll(start, i)
    {
        ...
        else
        {
            ...
            if (mag(hitNormals[facei]) > VSMALL)
            {
    ...
    forAll(start, i)
    {
        ...
        if (nNonHitsPerFace[facei] > 3 && nHitsPerFace[facei] > 0)
        {
            ...
            hitNormals[facei] /= (mag(hitNormals[facei]) + VSMALL);
            ...
            forAll(fCells, fCI)
            {
                ...
                forAll(c, cFI)
                {
                    ...
                    if (faceType[cfacei] == 0)
                    {
                        const point& cc = mesh_.cellCentres()[celli];
                        scalar hDist = mag(cc-hitMid);
                        if (hDist < (minHitDistance[facei]+VSMALL))
                        {
                            nClosestCells[facei]++;
                        }
                        break;
                ...
                if (topFace != -1)
                {
                    ...
                    if (keepCell)
                    {
                        ...
                        forAll(sideEdges, sei)
                        {
                            if
                            (
                                nEdgeBoundaryFaces[sideEdges[sei]] == 2
                                || nEdgeBoundaryFaces[sideEdges[sei]] == 4
                            )
                            {
                                nSideMan++;
                            }
...
) const
{
    ...
    forAll(cellLevel, celli)
    {
        ...
        if (cellToZone[celli] == -2 || !foundNamed)
        {
            continue;
        }
    ...
    //Check for cutoff boundary cells
    ...
    forAll(mesh_.cells(), celli)
    {
        ...
        forAll(cPoints, i)
        {
            ...
            if
            (
                isBoundaryPoint[pointI]
                && pointLevel[pointI] <= cellLevel[celli]
            )
            {
                nBoundaryAnchors++;
            }
        ...
        const labelList& cFaces = mesh_.cells()[celli];
        label namedi = -1;
        label globalregioni = -1;
        bool removedNei = false;
        label nNonBaffled = 0;
        forAll(cFaces, cFI)
        {
            label facei = cFaces[cFI];
            label namedFaceIndex = namedSurfaceIndex[facei];
            label globalFaceIndex = globalRegion1[facei];
            namedi = max(namedi,namedFaceIndex);
            globalregioni = max(globalregioni,globalFaceIndex);
            if (namedFaceIndex  == -1 && globalFaceIndex == -1)
            {
                nNonBaffled++;
            }
        ...
        )
        {
            ...
            {
                label facei = cFaces[cFI];
                globalRegion1[facei] = globalregioni;
                globalRegion2[facei] = globalregioni;
            }
        }
    }
...
void Foam::meshRefinement::getNamedIntersections
(
    const pointField& cellCentres,
    const labelList& surfacesToTest,
    const labelList& testFaces,
...
) const
{
    ...
    forAll(testFaces, i)
    {
        ...
        if (surface1[i] != -1)
        {
            // If both hit should probably choose lowest ID
            if (surface2[i] != -1)
            {
                if (surface2[i] < surface1[i])
                {
...
) const
{
    ...
    if
    (
        moveCentroidSurfaces.size()
        || checkForCracks()
        || closureSurfaces.size()
    )
    {
    ...
    if (debug&MESH)
    {
        ...
        if (namedSurfaces.size())
        {
            faceSet namedFaces(mesh_, "namedFaces", mesh_.nFaces()/100);
            forAll(globalRegion1, facei)
            {
                if
                (
                ...
                {
                    namedFaces.insert(facei);
                }
            }
            namedFaces.instance() = timeName();
            Pout<< "Dumping " << namedFaces.size()
                << " named intersected faces to "
                << namedFaces.objectPath() << endl;
            namedFaces.write();
        }
...
)
{
    ...
    while (true)
    {
        ...
        if (returnReduce(cellsToRemove.size(), sumOp<label>()) != 0)
        {
            ...
            autoPtr<polyTopoChangeMap> map = doRemoveCells
            (
                cellsToRemove,
                exposedFaces,
                exposedPatches,
...
Foam::autoPtr<Foam::polyTopoChangeMap> Foam::meshRefinement::dupNonManifoldPoints
(
...
Foam::autoPtr<Foam::polyTopoChangeMap> Foam::meshRefinement::mergePoints
(
...
)
{
    ...
    // Check face for no named zone intersection between zones and re-allocate
    // to closest neighbour on named faceZone
    if (namedSurfaces.size())
    {
        labelList namedEdges(mesh_.nEdges(), -1);
        forAll(namedEdges, edgei)
        {
            const labelList& eFaces = mesh_.edgeFaces()[edgei];
            forAll(eFaces, efi)
            {
                label facei = eFaces[efi];
                namedEdges[edgei] =
                   max(namedEdges[edgei],namedSurfaceIndex[facei]);
            }
        }
        ...
        forAll(mesh_.faces(), facei)
        {
            ...
            {
                label geomSurfI = findIndex(surfaceToCellZone, ownZone);
                if (geomSurfI == -1)
                {
                   geomSurfI = findIndex(surfaceToCellZone, neiZone);
                }

                if (geomSurfI != -1)
                {
                    const labelList& fEdges = mesh_.faceEdges()[facei];
                    label edgeNamedSurf = -1;
                    forAll(fEdges, fei)
                    {
                       label edgei = fEdges[fei];
                       edgeNamedSurf = max(edgeNamedSurf,namedEdges[edgei]);
                    }
                    if (edgeNamedSurf > -1)
                    {
                       namedSurfaceIndex[facei] = edgeNamedSurf;
                    }
                    else
                    {
                       namedSurfaceIndex[facei] = geomSurfI;
    ...
    if (returnReduce(cellsToRemove.size(), sumOp<label>()) != 0)
    {
        ...
        forAll(cellsToRemove, i)
        {
            ...
            forAll(c, cFI)
            {
                ...
                {
                    maxPatchID = max(maxPatchID, patchi);
                }
            }
            forAll(c, cFI)
            {
                ...
                if (patchi != -1 && !patches[patchi].coupled())
                {
                    exposedFaceID[facei] = patchi;
                }
                else
                {
                    exposedFaceID[facei] = maxPatchID;
                }
            }
        }
            ...
            forAll(outerShellFaces, i)
            {
                if (nearestInfo[i].hit())
                {
                    ...
                    if (outNormMag > VSMALL)
                    {
                        ...
                        if ((outNorm & nearestNormal[i]) > VSMALL)
                        {
                            namedSurfaceIndex[facei] = -1;
                        }
                    }
...
void Foam::meshRefinement::mergeCouples
(
    const refinementParameters& refineParams
)
{
    ...
    bool extraRemoval = refineParams.mergeSingleCouplesExtra();

    if (extraRemoval)
    {
        Info<< "Merging extra couples" << endl;
    }
    else
    {
    ...
    labelList nEdgeCoupledFaces(pp.nEdges(), 0);
    labelList nEdgePatchFaces(pp.nEdges(), 0);
    forAll(meshEdges, edgei)
    {
        const labelList& edgeFaces = pp.edgeFaces()[edgei];
        nEdgePatchFaces[edgei] = edgeFaces.size();
        forAll(edgeFaces, j)
        {
    ...
    boolList baffleFaces(pp.size(), false);
    forAll(pp, i)
    {
        label nExtBaffleEdges = 0;
        label nIntBaffleEdges = 0;
        label meshfacei = pp.addressing()[i];
        label ownLevel = cellLevel[mesh_.faceOwner()[meshfacei]];
        ...
        forAll(fEdges, fei)
        {
            ...
            if (pLevel0 > ownLevel || pLevel1 > ownLevel)
            {
                splitEdge = true;
            }

            label nCoupledFaces = nEdgeCoupledFaces[edgei];
            label nPPFaces = nEdgePatchFaces[edgei];

            if
            (
                (nCoupledFaces == 2) && (nPPFaces == 2)
            )
            {
                if (nPPFaces == nCoupledFaces)
                {
                    if (splitEdge)
                    {
                        nExtBaffleEdges++;
        ...
        if (extraRemoval)
        {
            if (nExtBaffleEdges > 0)
            {
                baffleFaces[i] = true;
            }
        }
        else if (nExtBaffleEdges == 4 && nIntBaffleEdges == 0)
        {
            baffleFaces[i] = true;
        }
    }

    boolList intBaffleEdges(meshEdges.size(), true);
    ...
    forAll(meshEdges, edgei)
    {
        ...
        forAll(eFaces, efi)
        {
            if (!baffleFaces[eFaces[efi]])
            {
                intBaffleEdges[edgei] = false;
            }
        }
    }
    ...
    pointField coupledFC(couples.size());
    scalarField coupledDistSqr(couples.size());
    labelList coupledIndex(couples.size());
    label nCouples = 0;
    forAll(pp, i)
    {
        if (!baffleFaces[i])
        ...
        forAll(fEdges, fei)
        {
            ...
            if
            (
                !intBaffleEdges[edgei]
                && nEdgeCoupledFaces[edgei] == 4
                && nEdgePatchFaces[edgei] == 4
            )
            {
                foundNonBaffleNbr = true;
                break;
            }
        }
        label couplei = coupled[meshfacei];
        if (!foundNonBaffleNbr && !foundCouple[couplei])
        {
            foundCouple[couplei] = true;
            coupledFC[nCouples] = mesh_.faceCentres()[meshfacei];
            coupledIndex[nCouples] = couplei;
            label ownLevel = cellLevel[mesh_.faceOwner()[meshfacei]];
            scalar clengthSqr = sqr(5*edge0Len / pow(2., ownLevel));
            coupledDistSqr[nCouples] = clengthSqr;
            nCouples++;
        }
    }
    coupledFC.setSize(nCouples);
    coupledIndex.setSize(nCouples);
    coupledDistSqr.setSize(nCouples);
    ...
    (
        allSurfaces,
        coupledFC,
        coupledDistSqr,    // sqr of attraction
        nearestSurface,
        nearestInfo
    );
    ...
    forAll(coupledIndex,i)
    {
        if (nearestSurface[i] != -1)
        {
            vector hNorm = nearestNormal[i]
               / (mag(nearestNormal[i])+VSMALL);
            label meshface0 = couples[coupledIndex[i]].first();
            vector fNorm = mesh_.faceAreas()[meshface0]
               / (mesh_.magFaceAreas()[meshface0] + VSMALL);

            if (mag(hNorm & fNorm) > 0.342)
            {
                foundCouple[coupledIndex[i]] = false;
            }
        }
    }
    ...
    label nCouplesMerged = returnReduce(nMerge, sumOp<label>());

    Info<<"Merging " << nCouplesMerged <<" coupled faces"<<endl;
...
Foam::autoPtr<Foam::polyTopoChangeMap>
Foam::meshRefinement::removeDisconnectedRegions()
{
...
) const
{
    ...
    else
    {
        ...
        {
            if (regionVolume[regioni] > VSMALL)
            {
                areaToVol[regioni] =
        ...
        forAll(mesh_.cells(), celli)
        {
            ...
            (
                regionVolume[regioni] > VSMALL && wrapArea[regioni] > VSMALL
    ...
    forAll(mesh_.cells(), celli)
    {
        ...
        if (ownRegion == 2)
        {
            if (removeArea[regioni] > VSMALL && keepArea[regioni] < VSMALL)
            {
                regionType[celli] = 1;
            }
            else if (removeArea[regioni] < VSMALL && keepArea[regioni] > VSMALL)
            {
                regionType[celli] = 0;
            }
            else if (removeArea[regioni] > VSMALL && keepArea[regioni] > VSMALL)
        ...
        if (cNorms.size() > 1)
        {
            for (label i = 0; i < cNorms.size(); i++)
            {
                for (label j = i+1; j < cNorms.size(); j++)
                {
                    ...
                    separationVec /= mag(separationVec) + VSMALL;
            ...
            forAll(locationsInMesh, locj)
            {
                ...
                )
                {
                    ...
                    (
                        ...
                        labelList(1,insideCells[loci]),
                        pointField(1,locationsInMesh[locj]),
                        labelList(1,insideCells[locj]),
                        leakPathFormatter
                    );
                    nLeaks++;
                }
