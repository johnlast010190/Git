...
) const
{
    ...
    VolField<vector>& U = UPtr();
    VolField<scalar>& p = pPtr();
    ...
    while (solve)
    {
        ...
        if (nIter % checkStage == 0)
        {
            ...
            if (excludePoints.size() == 0)
            {
                ...
                {
                    Info<< nl << "Removing "
                        << returnReduce(nRemoved, sumOp<label>())
                        << " cells from mesh after iteration " << nIter
                        << nl << endl;
                }
            }
            else
    ...
    if (excludePoints.size() > 0)
    {
            ...
            if (cellI != -1)
            {
                ...
                forAll(cFaces, cFaceI)
                {
                    ...
                    if (!allFaceInfo[faceI].valid(dummyTrackData))
                    {
                        ...
                        (
                            fieldData(pf[faceI], pf[faceI], true)
                        );
                        allFaceInfo[faceI] =
        ...
        for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
        {
            if (!allFaceInfo[faceI].valid(dummyTrackData))
            {
                allFaceInfo[faceI] = fieldData(pf[faceI], pf[faceI], false);
            }
        }
        forAll(p.boundaryField(), patchI)
        {
            ...
            forAll(bField, faceI)
            {
                ...
                if (!allFaceInfo[meshFaceI].valid(dummyTrackData))
                {
                    ...
                    allFaceInfo[meshFaceI] =
                        fieldData(boundFaceP, boundFaceP, false);
                }
            }
        }
