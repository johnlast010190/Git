...
)
{
    //split manifold face before splitting cells
    {
        ...
        topoChange(map, getChangedFaces(map, labelList(0)));
                ...
                if (cut4.size() == 4)
                {
                    ...
                    forAll(possibleFaces, faceI)
                    {
                        ...
                        if (nCut > 0)
                        {
                            ...
                            aveSurfaceNormal /= (mag(aveSurfaceNormal) + VSMALL);
    ...
    forAll(surfacesToBaffle, sI)
    {
        ...
        forAll(surface1, pointI)
        {
            if (surface1[pointI] == surfI)
            {
                ...
                normals67[pointI] /= (mag(normals67[pointI]) + VSMALL);
                localI++;
            }
        }
    }
    ...
    while (nIter < maxIter)
    {
            ...
            for (direction j = 0; j < vector::nComponents; j++)
            {
                ...
                forAll(mesh_.edges(), meshedgei)
                {
                    ...
                    if (hit1[meshedgei].hit() && hit2[meshedgei].hit())
                    {
                        ...
                        vector unitEdgeVec = eVec / (eLength + VSMALL);
        ...
        forAll(mesh_.points(), pointI)
        {
            ...
            forAll(pEdges, pEI)
            {
                ...
                if
                (
                    mag(edgeDispVec[meshedgei]) > VSMALL
        ...
        */
        ...
        else if (nCut == 4 && nConEdges == 2)
        {
            ...
            if (basePt1 != -1 && basePt2 != -1)
            {
                ...
                {
                    vector cNorm1 =
                        cutNormals[basePt1]/(mag(cutNormals[basePt1]) + VSMALL);
                    vector cNorm2 =
                        cutNormals[basePt2]/(mag(cutNormals[basePt2]) + VSMALL);
