...
(
    const Switch regionsInitRefine,
    const label nAllowRefine,
...
) const
{
    ...
    label totalNumRegions = surfaces_.minLevel().size();
    DynamicList<label> regionMinLevel(totalNumRegions);
    DynamicList<scalar> regionBBMag(totalNumRegions);
    DynamicList<point> regionBBCentre(totalNumRegions);

    scalar scaledRefLength = 2*edge0Len;

    if (regionsInitRefine)
    {
        forAll(surfaces, surfI)
        {
            ...
            if (isA<triSurfaceMesh>(s))
            {
                ...
                const List<boundBox> triBBs = tri.regionBoundBoxes(meshBB);
                forAll(s.regions(), regionI)
                {
                    const boundBox& bb = triBBs[regionI];
                    point limitedMin = ::Foam::max(bb.min(), meshBB.min());
                    point limitedMax = ::Foam::min(bb.max(), meshBB.max());
                    const boundBox meshLimitedBB(limitedMin,limitedMax);
                    if (meshLimitedBB.mag() < scaledRefLength)
                    {
                        regionBBCentre.append(meshLimitedBB.midpoint());
                        regionBBMag.append(meshLimitedBB.mag());
                        label mLevel = surfaces_.minLevel(surfI,regionI);
                        regionMinLevel.append(mLevel);
                    }
                }
            }
            else if
            ...
            )
            {
                ...
                if (meshLimitedBB.mag() < scaledRefLength)
                {
                    regionBBCentre.append(meshLimitedBB.midpoint());
                    regionBBMag.append(meshLimitedBB.mag());
                    label mLevel = surfaces_.minLevel(surfI,0);
                    regionMinLevel.append(mLevel);
                }
            }
        }
    ...
    else
    {
        forAll(surfaces, surfI)
        {
            ...
            )
            {
                ...
                bbCentre[geomI] = meshLimitedBB.midpoint();
                bbMag[geomI] = meshLimitedBB.mag();
    ...
    if
    (
        max(minLevel) == -1 && regionMinLevel.size() == 0
    )
    {
    ...
    forAll(mesh_.cells(), cellI)
    {
        ...
        if (regionsInitRefine && markedMinLevel == -1)
        {
            forAll(regionMinLevel, i)
            {
                if (regionBBMag[i] > VSMALL)
                {
                    if (cLevel >= regionMinLevel[i])
                    {
                        continue;
                    }
                    scalar ccToBoxCC = mag(cc - regionBBCentre[i]);
                    if (ccToBoxCC < spacing && regionBBMag[i] < spacing)
                    {
                        markedMinLevel = regionMinLevel[i];
                        break;
                    }
                }
            }
...
) const
{
    ...
    {
        const boolList& modifiedCells = modifiedCellsPtr_();
        forAll(surfaceIndex_, faceI)
        {
            if (surfaceIndex_[faceI] != -1)
            {
                ...
                if (mesh_.isInternalFace(faceI))
                {
                    ...
                    if
                    (
                        (refineCell[own] == -1 || refineCell[nei] == -1)
                        && (modifiedCells[own] ||  modifiedCells[nei])
                    )
                    {
                ...
                {
                    if (refineCell[own] == -1 && modifiedCells[own])
                    {
                        testFaces[nTest++] = faceI;
                    }
                }
        ...
        for (int k = 0; k < (j == 0 ? 1 : nRays); k++)
        {
            ...
            forAll(updatedIntersectionCells, i)
            {
                ...
                if (allHits[i].size() > 1)
                {
                    ...
                    rayVector /= (mag(rayVector) + VSMALL);
...
)
{
    ...
    boolList checkCells(mesh_.nCells(), false);
    {
        ...
        forAll(surfaceHitInfo, i)
        {
            ...
            {
                label global = surfaces_.globalRegion
                (
                    proxSurfaces[surfaceHit[i]],
                    pRegions[i]
                );
...
)
{
    ...
    const Switch regionsInitRefine = refineParams.regionsInitRefine();
