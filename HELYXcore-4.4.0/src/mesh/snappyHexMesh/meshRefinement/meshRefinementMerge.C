...
{
//#ifdef HELYX_USE_TBB
//    Timer timer("meshRefinement::mergePatchFaces");
//#endif
    // Patch face merging engine
    combineFaces faceCombiner(mesh_, patchIDs, List<labelList>(), false);
...
(
    ...
    const vectorField* surfNormal,
    List<labelPair>* baffles
)
{
//#ifdef HELYX_USE_TBB
//    Timer timer("meshRefinement::mergePatchFacesUndo");
//#endif
    // Patch face merging engine
    combineFaces faceCombiner
    (
        mesh_,
        patchIDs,
        featureShadows,
    ...
    (
        ...
        (
            minCos,
            concaveCos,
            maxAreaRatio, //try merge below this value
            boundaryCells
        )
    );
    ...
    if (nFaceSets > 0)
    {
        ...
        topoChange(map, growFaceCellFace(retestFaces), updateIntersection);
        ...
        for (label iteration = 0; iteration < 100; iteration++)
        {
            ...
            labelList checkFaces(selectInnerFaces());
            ...
            // Mark zoned faces as in error if causing
            // possible snap issues
            if (baffles && iteration == 0)
            {
                boolList baffleFaces(mesh_.nFaces(), false);
                const List<labelPair>& zb = *baffles;
                forAll(zb, i)
                {
                    baffleFaces[zb[i].first()] = true;
                    baffleFaces[zb[i].second()] = true;
                }
                ...
                forAll(patches, patchi)
                {
                    ...
                    if (!patch.coupled())
                    {
                        ...
                        forAll(patch, i)
                        {
                            ...
                            labelHashSet cFaceSet(mesh_.cells()[own]);
                            const labelList fEdges = mesh_.faceEdges()[facei];
                            DynamicList<label> zoneFaces(fEdges.size());
                            label nMarked = 0;
                            label nZoned = 0;
                            forAll(fEdges, fEI)
                            {
                                ...
                                bool hangingEdge = false;
                                if
                                (
                                    mesh_.pointEdges()[e[0]].size() == 2
                                    || mesh_.pointEdges()[e[1]].size() == 2
                                )
                                {
                                    hangingEdge = true;
                                    nMarked += label(1);
                            ...
                            if (zoneFaces.size() > 1 && nZoned >= nMarked-2)
                            {
                                forAll(zoneFaces, zFI)
                                {
                                    errorFaces.insert(zoneFaces[zFI]);
                                }
                            }
                        }
                    }
                }
                errorFaces.sync(mesh_);
            }

            label nErrorFaces = errorFaces.size();
...
{
//#ifdef HELYX_USE_TBB
//    Timer timer("meshRefinement::doRemovePoints");
//#endif
    // Topology changes container
    polyTopoChange meshMod(mesh_);
...
{
//#ifdef HELYX_USE_TBB
//    Timer timer("meshRefinement::doRestorePoints");
//#endif
    // Topology changes container
    polyTopoChange meshMod(mesh_);
...
{
//#ifdef HELYX_USE_TBB
//    Timer timer("meshRefinement::mergeEdgesUndo");
//#endif
    ...
    if (nRemove > 0)
    {
        ...
        autoPtr<polyTopoChangeMap> map =
         doRemovePoints(pointRemover, pointCanBeDeleted, updateIntersection);
        ...
        for (label iteration = 0; iteration < 100; iteration++)
        {
            ...
            autoPtr<polyTopoChangeMap> map =
            doRestorePoints(pointRemover, masterErrorFaces, updateIntersection);
