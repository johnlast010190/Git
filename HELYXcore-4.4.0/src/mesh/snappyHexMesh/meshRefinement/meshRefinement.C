...
Foam::scalar Foam::meshRefinement::extTol_ = 3e-8;
...
Foam::scalar Foam::meshRefinement::minIsoEdgeLength()
{
    ...
    return edge0Len/(1<<maxLevel);
}

//- Estimate of crack tolerance
void Foam::meshRefinement::setCrackTol()
{
    crackTol_ = crackDetection_.second() * minIsoEdgeLength();
...
//#endif
    ...
    // During zoneing checkCracks maybe active so don't entend twice
    if (extendRay)
    {
        const vectorField smallVec(extTol_*(end-start));
        start -= smallVec;
        end += smallVec;
    }
}
...
)
{
    ...
    topoChange(map, newExposedFaces, updateIntersections);
...
)
{
...

    {
...

        {
            ...
            topoChange(map, growFaceCellFace(changedFaces));
        }
    }
...
)
{
    ...
    PtrList<polyTopoChangeMap> myProcMap(nMeshesInProc[Pstream::myProcNo()]);
    ...
    forAll(myProcMeshes, i)
    {
        ...
        if (balanceSurfaceCells)
        {
            ...
            // Change the mesh.
            autoPtr<polyTopoChangeMap> map = reorderMesh(meshi, cellOrder, faceOrder);
        ...
        //if (debug)
        {
            ...
            (
                ...
                (
                    ...
                    meshi,
                    dimensionedScalar(dimless, meshNo),
                    zeroGradientFvPatchScalarField::typeName
                )
            );
            procID().write();
...
{
    // add required dummy values
    // overwritten in polyPatch clone function call
    dictionary patchDict(patchInfo);
    patchDict.set("nFaces", 0);
    patchDict.set("startFace", 0);

    return fvMeshTools::addPatch
    (
        mesh,
        patchName,
        patchDict,
    ...
    if (gCellI == -1)
    {
        ...
        if (nearestCellI != -1)
        {
            ...
            forAll(f, facei)
            {
                ...
                vector normal = Sf[nFace]/(mag(Sf[nFace]) + VSMALL);
        ...
        if (Pstream::myProcNo() == minProcID && minDist < 0.1)
        {
            cellI = nearestCellI;
        }
    }
...
)
{
    ...
    {
        // Update layer cells
        map.distributeCellData(layerCells()());
    }

    if (modifiedCellsPtr_.valid())
    {
        // Update proximity refinement new cell marker
        map.distributeCellData(modifiedCellsPtr_());
    }
...
)
{
    ...
    if (layerCells().valid())
    {
        labelIOList& lcells = layerCells()();
        // Map data
        const labelList& cellMap = map.cellMap();

        labelList newLayerCells(cellMap.size());
        forAll(cellMap, newCelli)
        {
            label oldCelli = cellMap[newCelli];
            if (oldCelli == -1)
            {
                newLayerCells[newCelli] = -1;
            }
            else
            {
               newLayerCells[newCelli] = lcells[oldCelli];
            }
        }
        lcells.transfer(newLayerCells);
    }
    ...
    //Mark any modified cells
    if (modifiedCellsPtr_.valid())
    {
        boolList& modifiedCells = modifiedCellsPtr_();
        modifiedCells.setSize(mesh_.nCells());
        modifiedCells = false;
        boolList modifiedFaces(mesh_.nFaces(), false);
        // mark any updated cell (point addition)
        forAll(mesh_.cells(), celli)
        {
            ...
            forAll(cPts,cpi)
            {
                ...
                if (pointi >= map.nOldPoints())
                {
                   modifiedCells[celli] = true;
                   break;
                }
            }
            if (modifiedCells[celli])
            {
                const cell& c = mesh_.cells()[celli];
                forAll(c,cfi)
                {
                    label facei = c[cfi];
                    modifiedFaces[facei] = true;
                }
            }
        }
        syncTools::syncFaceList(mesh_, modifiedFaces, orEqOp<bool>());
        //Mark any cell where neighbour has changed
        forAll(mesh_.cells(), celli)
        {
            if (!modifiedCells[celli])
            {
    ...
    // Update faceToCoupledPatch_
    {
        ...
        faceToCoupledPatch_ = std::move(newFaceToPatch);
    }
...
{
    Info<< nl
        << "Repatching faces based on input surfaces" <<endl;

    const polyBoundaryMesh& patches = mesh_.boundaryMesh();

    scalar tol =
        surfaceToPatchDict.lookupOrDefault<scalar>("tolerance",1.0);
    scalar align =
        surfaceToPatchDict.lookupOrDefault<scalar>("alignment",0.707);
    ...
    Info<< "Finished reading Geomery." <<endl;
    ...
    forAll(patches, patchi)
    {
        ...
        if (!pp.coupled())
        {
            ...
            forAll(pp, i)
            {
                ...
                start[nChecked] = fc - tol*projDistCC*fA;
                end[nChecked] = fc + tol*projDistFC*fA;
    ...
    forAll(geom, geomi)
    {
        ...
        regionToPatch[geomi].setSize(regNames.size());
        forAll(regNames, i)
        {
    ...
    Info<< "Added new mesh patches." <<endl;
    ...
    forAll(geom, geomi)
    {
        ...
        forAll(info, hiti)
        {
            if (nearestSurfaces[hiti] == geomi)
            {
                ...
                if (patchi != closestPatchID && mag(dProd) > align)
                {
                    label own = mesh_.faceOwner()[facei];
...
Foam::edgeMesh Foam::meshRefinement::calcFeatureEdgeMesh
(
    const polyMesh& mesh,
    const indirectPrimitivePatch& pp,
    const edgeClassification& eClass
)
{
    ...
    if (Pstream::master())
    {
        allPoints = ListListOps::combine<pointField>
        (
            gatheredPoints,
            accessOp<pointField>()
        );
        ...
        allEdges = ListListOps::combine<edgeList>
        (
            gatheredEdges,
            accessOp<edgeList>()
        );
    ...
    Pstream::scatter(allPoints);
    Pstream::scatter(allEdges);
    ...
    if (maxWarpage != GREAT)
    {
        ...
        if (includedPatchesIDs.size())
        {
            ...
            forAll(pp.localFaces(), fI)
            {
                ...
                vector fn = fAreas[meshFaceI] / (mag(fAreas[meshFaceI]) + VSMALL);
...
(
    ...
    const boolList& blockedFace,
    const pointField& locationsInMesh,
    const labelList& cellsInMesh,
    const pointField& locationsOutsideMesh,
    const labelList& cellsOutsideMesh,
    const writer<scalar>& leakPathFormatter
)
{
    const polyBoundaryMesh& pbm = mesh.boundaryMesh();
    ...
    if (Pstream::master())
    {
        fileName fName("nopath");
        if (allLeakPaths.size() == 0)
        {
            WarningInFunction
                <<"Detected leak but no leak path could be calculated." <<endl;
            return;
        }
