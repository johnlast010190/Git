...
)
{
    ...
    while (true)
    {
        ...
        labelList boundaryCellType(mesh_.nCells(), -1);
        boolList markedForRemoval(mesh_.nCells(), false);
        ...
        forAll(mesh_.cells(), celli)
        {
            ...
            // Check for grown up internal faces with all boundary points
            const cell& c = mesh_.cells()[celli];

            if (nBoundaryPts > 3)
            {
                DynamicList<label> allBoundaryIntFaces(c.size());
                forAll(c,cFI)
                {
                    label facei = c[cFI];
                    label patchi = patches.whichPatch(facei);
                    if (patchi == -1)
                    {
                        ...
                        if (nBdyPts == f.size())
                        {
                           allBoundaryIntFaces.append(facei);
                        }
                    }
                }
                if (allBoundaryIntFaces.size() > 0)
                {
                    forAll(allBoundaryIntFaces, ifi)
                    {
                        label facei = allBoundaryIntFaces[ifi];
                        label cLevel = cellLevel[celli];
                        const labelList& fEdges = mesh_.faceEdges()[facei];
                        label nGrownUpEdges = 0;
                        forAll(fEdges, fEI)
                        {
                            label meshedgei = fEdges[fEI];
                            const labelList& eFaces =
                                mesh_.edgeFaces()[meshedgei];
                            forAll(eFaces, eFI)
                            {
                                label patchi = patches.whichPatch(facei);
                                if (patchi != -1 && grownUpPatches[patchi])
                                {
                                    const edge& e = mesh_.edges()[meshedgei];
                                    if
                                    (
                                        pointLevel[e[0]] > cLevel
                                        || pointLevel[e[1]] > cLevel
                                    )
                                    {
                                        nGrownUpEdges++;
                                    }
            ...
            if (nBoundaryPts == cPts.size())
            {
                ...
                forAll(c,cFI)
                {
                    ...
                    if (boundaryFaces[facei] != -1)
                    {
                        ...
                        bool refineFace
                        (
                            meshCutter_.faceLevel(facei) > clevel ? true : false
                        );
                ...
                {
                    boundaryCellType[celli] = label(2);
                }
                else if (nAnchorBoundFaces >= 12)
                {
                    boundaryCellType[celli] = label(1);
                }
                else
                {
                    boundaryCellType[celli] = label(0);
                }
            }
            else if (nBoundaryPts > 6)
            {
                const cell& c = mesh_.cells()[celli];
                label clevel = cellLevel[celli];
                label nAnchorBoundFaces = 0;
                ...
                forAll(c,cFI)
                {
                    ...
                    if (boundaryFaces[facei] != -1 || nBdyPts == f.size())
                    {
                        if (refineFace)
                        {
                            nAnchorBoundFaces++;
        ...
        labelList neiBoundaryCellType(mesh_.nFaces()-mesh_.nInternalFaces());
        for
        (
            label facei = mesh_.nInternalFaces();
            facei < mesh_.nFaces();
        ...
        {
            neiBoundaryCellType[facei-mesh_.nInternalFaces()] =
                boundaryCellType[owners[facei]];
        }
        syncTools::swapBoundaryFaceList(mesh_, neiBoundaryCellType);

        boolList singleBoundaryNei(mesh_.nCells(), false);
        //Remove boundary cells which could cause snap problems
        forAll(mesh_.cells(), celli)
        {
            if (boundaryCellType[celli] < 1)
            {
                continue;
            }
            ...
            forAll(c, cFI)
            {
                ...
                {
                    if (neiBoundaryCellType[facei-mesh_.nInternalFaces()] > -1)
                    {
                        nBoundNei++;
                    }
                }
            ...
            if ((boundaryCellType[celli] == 2 && nBoundNei == 0) || singleGap)
            {
                markedForRemoval[celli] = true;
            }
        }
            ...
            forAll(mesh_.edges(), edgei)
            {
                if (edgeType[edgei] == 1)
                {
                    ...
                    forAll(e, ei)
                    {
                        ...
                        {
                            if (mag(eC-pECentres[peci]) < VSMALL)
                            {
                                nEdgesFound++;
                            }
                        }
        ...
        else
        {
            ...
            // Remove boundary face cells with two removed neighbours
            boolList neiMarkedRemoval(mesh_.nFaces()-mesh_.nInternalFaces());
            for
            (
                label facei = mesh_.nInternalFaces();
                facei < mesh_.nFaces();
            ...
            {
                neiMarkedRemoval[facei-mesh_.nInternalFaces()] =
                    markedForRemoval[owners[facei]];
            }
            syncTools::swapBoundaryFaceList(mesh_, neiMarkedRemoval);

            labelList nEdgeRemovedInterfaces(mesh_.nEdges(), 0);
            forAll(mesh_.edges(), edgei)
            {
                if (boundaryEdges[edgei] != -1)
                {
                ...
                forAll(eFaces,efi)
                {
                    ...
                    label ownRemoved = false;
                    label neiRemoved = false;
                    label own = owners[facei];
                    if (markedForRemoval[own])
                    {
                        ownRemoved = true;
                    }
                    label patchi = patches.whichPatch(facei);
                    ...
                    if
                    (
                        (ownRemoved && !neiRemoved)
                        || (!ownRemoved && neiRemoved)
                    )
                    {
                        nEdgeRemovedInterfaces[edgei]++;
                    }
                }
            }
            ...
            (
                mesh_,
                nEdgeRemovedInterfaces,
                plusEqOp<label>(),
                label(0)               // null value
            );
            DynamicList<label> boundaryMarked(mesh_.nCells()/100);
            forAll(mesh_.faces(),facei)
            {
                if (boundaryFaces[facei] != -1)
                {
                    ...
                    if (!markedForRemoval[own])
                    {
                        const labelList& cEdges = mesh_.cellEdges()[own];
                        const face& f = mesh_.faces()[facei];
                        labelHashSet fSet(f);
                        forAll(cEdges, cei)
                        {
                            label edgei = cEdges[cei];
                            if (nEdgeRemovedInterfaces[edgei] == 4)
                            {
                                const edge& e = mesh_.edges()[edgei];
                                if (fSet.found(e[0]) || fSet.found(e[1]))
                                {
                                    boundaryMarked.append(own);
                                }
                            }
                        }
                    }
            ...
            forAll(boundaryMarked, i)
            {
                markedForRemoval[boundaryMarked[i]] = true;
            }
        }
