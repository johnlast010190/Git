...
dictionary Foam::helyxMeshGenerator::finalOptimizeSetup
(
...
void Foam::helyxMeshGenerator::cfOptimize
(
    fvMesh& mesh,
...
)
{
    ...
    if (optimType != "none" && optimType == "helyxOptimize")
    {
...
void Foam::helyxMeshGenerator::extractTriSurface
(
    const polyMesh& mesh,
...
)
{
    ...
    gatheredZones[Pstream::myProcNo()] = std::move(compactZones);
...
Foam::scalar Foam::helyxMeshGenerator::getMergeDistance
(
    const polyMesh& mesh,
...
)
{
    ...
    if (mesh.time().writeFormat() == IOstream::ASCII)
    {
        ...
        scalar writeTol = Foam::pow
#else
        const scalar writeTol = Foam::pow
#endif
        (
            scalar(10.0),
...
void Foam::helyxMeshGenerator::writeMesh
(
    const string& msg,
...
)
{
    ...
    if (removeZeroSized)
    {
        ...
        mesh.faceZones().clearZeroSized();
        mesh.cellZones().clearZeroSized();
    }
...
void Foam::helyxMeshGenerator::blockMeshTriangulate
(
    const polyMesh& mesh,
...
)
{
    ...
    // Update refinement surfaces with new block mesh surfaces
    dictionary& refSurf =
    ...
    forAll(bMesh, patchI)
    {
        ...
        if (!isA<processorPolyPatch>(pp))
        {
            ...
            if (refSurf.found(patchName))
            {
                //explictly check for corresponding active geometry
                forAllConstIter(dictionary, geometryDict, iter)
                {
                    const word& key = iter().keyword();
                    dictionary gdict = geometryDict.subDict(key);
                    if (!gdict.lookupOrDefault("active", true))
                    {
                        continue;
                    }
                    ...
                    (
                        gdict.found("name")
                        ? gdict.lookup<word>("name") : key
                    );
                    if (patchName == gname)
                    {
                        FatalErrorInFunction
                            << "BlockMesh patch : " << patchName
                            << " already exists as a geometry entry : " << gdict
                            << exit(FatalError);
                    }
                }
            }
...
void Foam::helyxMeshGenerator::addVDBpatches
(
...
Foam::dictionary Foam::helyxMeshGenerator::calcReSnapNonBoundaryZoneDict
(
...
)
{
    ...
    //Explicitly added zoned surfaces (when using named region zoning)
    if (layerParams.addedZoneReSnapSurfaces().size() > 0)
    {
        const wordReList& explicitSurfaces =
           layerParams.addedZoneReSnapSurfaces();
        const searchableSurfaces& geometry = surfaces.geometry();
        forAll(explicitSurfaces, i)
        {
            const wordRe& esurf  = explicitSurfaces[i];
            labelList geomIDs = findStrings(esurf, geometry.fileNames());
            forAll(geomIDs, geomi)
            {
                word fName = geometry.fileNames()[geomIDs[geomi]];
                if (!reSnapZoneGeometryDict.found(fName))
                {
                    reSnapZoneGeometryDict.add
                    (
                        fName,
                        geometryDict.subDict(fName)
                    );
                }
            }
        }
...
Foam::dictionary Foam::helyxMeshGenerator::calcReSnapGeometryDict
(
    const layerParameters& layerParams,
    const refinementSurfaces& surfaces,
...
void Foam::helyxMeshGenerator::autoCreateDecomposition
(
    const label nProcs,
...
void Foam::helyxMeshGenerator::addProtectedCellsExtrude
(
    const fvMesh& mesh,
...
)
{
    ...
    const labelIOList& layerCells  = meshRefiner.layerCells()();
...
bool Foam::helyxMeshGenerator::addLayers
(
...
void Foam::helyxMeshGenerator::runBlockMesh
(
    const dictionary& meshDict,
...
)
{
   ...
   if (Pstream::parRun())
   {
       ...
       meshDistribute distributor(meshPtr(), mergeDist);
...
Foam::helyxMeshGenerator::helyxMeshGenerator(Time& time)
:
    runTime_(time)
{}
...
Foam::autoPtr<Foam::fvMesh> Foam::helyxMeshGenerator::generateMesh
(
    const word& regionName
)
{
...
{
    word dictName("helyxHexMeshDict");
    ...
    if (!dictIO.typeHeaderOk<IOdictionary>(true))
    {
        if (Pstream::master())
        {
            ...
            FatalErrorInFunction << "Cannot find meshing dictionary "
                << "helyxHexMeshDict in folder : " << failedPath
                << exit(FatalError);
        }
    }
    ...
    if (Pstream::parRun() || useVDB)
    {
       ...
       if (checkHeaderOK)
       {
           ...
           numSubDomains = dictPtr->lookup<label>("numberOfSubdomains");
    ...
    {
        writeDict = meshDict.lookup<bool>("writeDict");
    }
    ...
    // transform inside/outside locations based on current reference frame
    refineParams.doTransforms(mesh,meshDict);

    const word oldInstance
    (
        controller.block() == meshControl::BLOCK
        ? "constant" : mesh.pointsInstance()
    );
    ...
    // Mesh distribution engine (uses tolerance to reconstruct meshes)
    meshDistribute distributor(mesh, mergeDist);
    ...
    else
    {
        ...
        {
           crackDetection.first() = meshDict.lookup<bool>("crackDetection");
        }
        if (meshDict.found("crackTol"))
        {
           crackDetection.second() = meshDict.lookup<scalar>("crackTol");
        }
        if (meshDict.found("extraRays"))
        {
           addedRays = meshDict.lookup<bool>("extraRays");
        }
    ...
    // Perform check of min cell size and pyramid volume constraint
    scalar minVol = motionDict.lookupOrDefault<scalar>("minVol", -1, true);
    if (minVol > VSMALL)
    {
        scalar minEdgLength = meshRefiner.minIsoEdgeLength();
        scalar pyrVol = 0.1666*Foam::pow(minEdgLength,3.0);
        if (pyrVol < 20*minVol)
        {
            Info<< nl;
            WarningInFunction
                << nl << "    The minimum isotropic mesh edge length is : "
                << minEdgLength
                << " and meshQualityControl minVol is : " << minVol << nl
                << "    Because the undistorted pyramid volume is : "
                << pyrVol
                << ". Try reducing minVol keyword to avoid snap issues."
                << nl << endl;
        }
    }
    ...
    if (!useVDB)
    {
        ...
        if (controller.block() == meshControl::AUTO)
        {
            ...
            (
                "blockMesh",
                patchInfo,
                false //whether to warn if patch already exists
            );
        }
    ...
    if (controller.refine())
    {
        ...
        else if (useVDB)
        {
            ...
            if (numSubDomains != Pstream::nProcs())
            {
                ...
                Info<< "\nPlease restart helyxHexMesh for snap and "
    ...
    {
        // setup layer field if not generated
        if (!meshRefiner.layerCells().valid())
        {
            meshRefiner.createLayerStack();
        }
        ...
        //Create layer cell field for extrude/dual with layer cells (>-1)
        labelIOList& layerCells = meshRefiner.layerCells()();
    ...
    meshRefiner.topoChange(map, labelList(0));
    ...
    )
    {
        ...
        if (outputLayerCells && meshRefiner.layerCells().valid())
        {
            volScalarField layerCellsVolFld
            (
                IOobject
                (
                    "layerStacks",
            ...
            const labelIOList& layerCells = meshRefiner.layerCells()();
            forAll(layerCellsVolFld, celli)
            {
                layerCellsVolFld[celli] = scalar(layerCells[celli]);
            }
            layerCellsVolFld.write();
        }
    }
