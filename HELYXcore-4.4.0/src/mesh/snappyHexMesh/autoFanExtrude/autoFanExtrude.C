...
)
{
    ...
    autoPtr<polyDistributionMap> distMap = meshRefiner_.balance
    (
        marked,
...
)
{
    ...
    autoPtr<polyTopoChangeMap> mapPts = meshRefiner_.dupNonManifoldPoints();
...
)
{
    ...
    reduce(valid, orOp<bool>());
    if (valid)
    {
       label nFanElements = 0;
       forAll(marked, facei)
       {
           if (!marked[facei])
           {
       ...
       reduce(nFanElements, sumOp<label>());
       if (nFanElements == 0)
       {
       ...
       {
           Info<<"Fan being constructed from : "<<nFanElements<<" faces."
               <<endl;
       }
    }
    else
    {
        Info<<"Not a valid fan as number of layers is changing across the fan."
             <<endl;
    }
...
Foam::autoPtr<Foam::polyTopoChangeMap> Foam::autoFanExtrude::extrudeLayer
(
    const indirectPrimitivePatch& bafflePP,
    const labelList& bafflePtType,
...
)
{
    ...
    autoPtr<polyTopoChangeMap> extrudeMap = extrudeMod().changeMesh(mesh, false);

    // Update fields
    mesh.topoChange(extrudeMap);
    ...
    meshRefiner_.topoChange(extrudeMap, labelList(0));
...
)
{
    ...
    if (nPointPairs > 0)
    {
        ...
        autoPtr<polyTopoChangeMap> map = meshRefiner_.mergePoints(pointToMaster);
    }

    autoPtr<polyTopoChangeMap> mapMergePtr = meshRefiner_.mergeBaffles
    (
...
void Foam::autoFanExtrude::setRefinement()
{
    ...
    autoPtr<polyTopoChangeMap> extrudeMap = extrudeLayer
    (
        bafflePP(),
        bafflePtType,
        disp
