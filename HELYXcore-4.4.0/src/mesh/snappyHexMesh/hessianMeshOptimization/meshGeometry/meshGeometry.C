...
{
    faceCentreDerivatives_.setSize(mesh_.faces().size());
    forAll(mesh_.faces(), fI)
    {
...
void Foam::meshGeometry::splitProcessorPoints()
{
    ...
    labelList procMeshPts(mesh_.nPoints(), -1);
    forAll(processorPoints_, pI)
    {
        procMeshPts[processorPoints_[pI]] = pI;
    }
                ...
                forAll(pointFaces, fI)
                {
                    ...
                    if (fL >= mesh_.nInternalFaces())
                    {
                        ...
                        forAll(fc, pI)
                        {
                            ...
                            if (meshP!=gP)
                            {
                                label localLabel = procMeshPts[meshP];
...
const Foam::List<Foam::DiagTensor<scalar>>&
Foam::meshGeometry::faceCentreDerivatives() const
{
