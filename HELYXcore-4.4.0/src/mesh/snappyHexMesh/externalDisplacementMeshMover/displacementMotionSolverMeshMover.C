...
{
    const label nRelaxIter = moveDict.lookup<label>("nRelaxIter");
...
:
    ...
    fieldSmoother_(mesh())
{
    // initialise points0 with mesh points
    solverPtr_->points0() = pointDisplacement.mesh()().points();
}
...
//            resid += mag(pointDisplacement()[pointI]-displacement[patchPointI]);
    ...
    {
        solverPtr_->pointDisplacement().forceAssign(pointDisplacement());
        // Force solving and constraining - just so its pointDisplacement gets
        // the correct value
        (void)solverPtr_->newPoints();
        pointDisplacement().forceAssign(solverPtr_->pointDisplacement());
    }
