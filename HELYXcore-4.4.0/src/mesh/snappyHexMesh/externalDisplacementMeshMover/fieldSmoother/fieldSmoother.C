...
) const
{
    ...
    const boolList isBoundaryEdge(mesh_.nEdges(), false);
    const boolList isBoundaryPoint(mesh_.nPoints(), false);
...
(
    ...
    const boolList& isExternalEdge,
    const boolList& isExternalPoint,
...
) const
{
    ...
    for (label iter = 0; iter < nIter; iter++)
    {
        ...
        forAll(average, pointI)
        {
            if (!isCornerPoint[meshPoints[pointI]])
            {
                // full smoothing neighbours + point value
                average[pointI] = 0.5*(normals[pointI]+average[pointI]);
                normals[pointI] = average[pointI];
