...
void Foam::medialAxisMeshMover::topoChangeGeometric
(
    const polyMesh& mesh,
    ...
    // Surface points that are to remain fixed.
    {
        ...
        if (maxAR > 0)
        {
            ...
            forAll(mesh.cells(), cellI)
            {
                ...
                scalar ratio = maxSA/(minSA+VSMALL);
    ...
    for (label i = 0; i < nDispSmooth; i++)
    {
        ...
        if (i < nDispSmooth/2)
        {
            ...
            forAll(mesh.points(), pointI)
            {
                ...
                {
                    weightedAverage =
                        volCentre[pointI]/ (volSum[pointI] + VSMALL);
...
void Foam::medialAxisMeshMover::update(const dictionary& coeffDict)
{
    ...
    boolList cornerPts(mesh().nPoints(), false);
    {
        ...
        forAll(pointNormals, pointI)
        {
            ...
            else if (grownUpPoints[meshPointI])
            {
                ...
                pointNormals[pointI] /= (mag(pointNormals[pointI]) + VSMALL);
            }
        }
    }
    ...
    if (!smoothBaffleEdgeNormals)
    {
        ...
        forAll(baffleDir, i)
        {
            if (nPointFaces[i] > 0 && mag(baffleDir[i]) > VSMALL)
            {
                    ...
                    forAll(eFaces, i)
                    {
                        ...
                        if (mag(edgeNormal[meshEdgeI] &  dir) < 0.9848)
                        {
                            ...
                            )
                            {
                                ...
                                disp0 /= (mag(disp0) + VSMALL);
                                vector disp1 = pointWallDist[e[1]].v();
                                disp1 /= (mag(disp1) + VSMALL);
...
)
{
    ...
    {
        scalar angle = coeffDict.lookup<scalar>("featureAngleGrowAround");
...
)
{
    ...
    {
        if (mag(patchDisp[pointI]) <= minThickness[pointI]+VSMALL)
        {
