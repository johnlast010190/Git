...
)
{
    ...
    if (revertErrors_)
    {
        ...
        (
            motionDict_.lookupOrDefault<scalar>("minArea", VSMALL, true)
        );
        const scalar minVol
        (
            motionDict_.lookupOrDefault<scalar>("minVol", -VSMALL, true)
        );
        const scalar maxNonOrtho
        (
            motionDict_.lookupOrDefault<scalar>("maxNonOrtho", 70.0, true)
        );
        if (mesh.checkCellVolumes(true, &errorCells)) noFailedChecks++;
        if (mesh.checkFaceAreas(true, minArea, &errorFaces)) noFailedChecks++;
        if (mesh.checkFaceOrthogonality(true, &errorFaces, maxNonOrtho))
            noFailedChecks++;
        if (mesh.checkFacePyramids(true, minVol, &errorFaces)) noFailedChecks++;
                    ...
                    forAll(f, fp)
                    {
                        ...
                        if ((pN&fA) > VSMALL)
                        {
                            ...
                            if (lHit.hit())
                            {
                                ...
                                {
                                    if (eLength1 > VSMALL && eLength2 > VSMALL)
                                    {
...
void Foam::autoSplitCells::flattenWarpedFaces
(
...
)
{
    ...
    pointField disp(mesh.nPoints(), vector::zero);
    labelList nDisp(mesh.nPoints(), label(0));
    label nMoved = 0;
    forAll(pp, i)
    {
        ...
        vectorField projectDir(lf.size());
        forAll(lf,fp)
        {
            label lfp = lf[fp];
            ...
            vProj[fp] = (n & fn);
            projectDir[fp] = mesh.points()[meshpointi] - vProj[fp]*fn;
        }
        ...
        if (warpedFace)
        {
            forAll(lf,fp)
            {
                label lfp = lf[fp];
                label meshpointi = meshPoints[lfp];
                disp[meshpointi] += (projectDir[fp]-mesh.points()[meshpointi]);
                nDisp[meshpointi]++;
                nMoved++;
            }
        }
    ...
    if (returnReduce(nMoved, sumOp<label>()) == 0)
    {
        return;
    }

    syncTools::syncPointList(mesh, disp, plusEqOp<point>(), vector::zero);
    syncTools::syncPointList(mesh, nDisp, plusEqOp<label>(), label(0));
...
)
{
    ...
    const scalar minAreaSplit
    (
        motionDict_.lookupOrDefault<scalar>("minSplitRatio", scalar(0.25))
    );
    ...
    forAll(cutFaces, i)
    {
        ...
        if (minAreaSplit > 0)
        {
            ...
            scalar areaA = cutFace0.mag(mesh.points())+VSMALL;
            scalar areaB = cutFace1.mag(mesh.points())+VSMALL;
            scalar splitRatio = min(areaA,areaB) / max(areaA,areaB);

            if (splitRatio < minAreaSplit)
            {
                continue;
            }
        }
    ...
    if (nCellsToSplit > 0)
    {
...

        {
            ...
            labelIOList& layerCells = meshRefiner_.layerCells()();
...
)
{
    ...
    cellSet errorCells(mesh, "errorCells", mesh.nCells()/100+1);
    cellSet aspectRatioCells(mesh, "aspectRatioCells", mesh.nCells()/100+1);
    faceSet errorFaces(mesh, "errorFaces", mesh.nFaces()/100+1);

    label noFailedChecks(0);
    if (mesh.checkClosedCells(true, &errorCells, &aspectRatioCells))
        noFailedChecks++;
    if (revertErrors_)
    {
        const scalar minArea
    ...
    if (noFailedChecks > 0)
    {
        ...
        forAllConstIter(labelHashSet, aspectRatioCells, iter)
        {
            errors[iter.key()] = true;
        }
                ...
                else
                {
                    ...
                    if (nHits == f.size())
                    {
...

                        {
                            ...
                            scalar areaA = mag(fAA)+VSMALL;
                            scalar areaB = mag(fAB)+VSMALL;
                            ...
                            areaA = mag(fAA)+VSMALL;
                            areaB = mag(fAB)+VSMALL;
...
)
{
    ...
    // As final step flatten all warped faces
    flattenWarpedFaces
    (
        warpage,
        fchwarpage,
...
)
{
    ...
    List<labelPair> splitCells = split(cutFaces);
...
(
    ...
    const labelList& excludePatches,
    bool extruded,
    bool preMerged,
    bool revertErrors
)
