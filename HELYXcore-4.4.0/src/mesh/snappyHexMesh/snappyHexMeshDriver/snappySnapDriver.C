...
)
{
    ...
    if (preSmoothBaffles)
    {
        ...
        forAll(pointFaces, i)
        {
            if (wtSum[i] > 0 && mag(aveNorm[i]) > VSMALL)
            {
                label meshPointi = meshPoints[i];
...
) const
{
    ...
    scalarField edgeGamma(1.0/(edgePatchDist(meshMover.pMesh(), pp) + VSMALL));
...
Foam::autoPtr<polyTopoChangeMap> Foam::snappySnapDriver::surfaceToPatch
(
    const snapParameters& snapParams,
    ...
    forAll(pp, i)
    {
        ...
        if (!isZonedFace.get(faceI))
        {
            ...
            else if (hPoint1[i].hit() && hitInfo[i].hit())
            {
               ...
               surfNorm /= (mag(surfNorm) + VSMALL);

               vector faceArea = mesh.faceAreas()[faceI];
               faceArea /= (mag(faceArea) + VSMALL);
...
//#endif
    ...
    if (calcNormals)
    {
        ...
        findSurfaceNormals
        (
            word("facemerge"), //name of interrogation stage
            -1, //set dummy iteration count
            allSurfaces,
            pp,
            snapDist,
            mesh.points(),
...
)
{
    ...
    if (nSplits > 0)
    {
        ...
        //Repatch surface
        autoPtr<polyTopoChangeMap> map = surfaceToPatch
        (
            snapParams,
            refineParams
        );
...
)
{
    ...
    scalarField minArea(meshPoints.size(), GREAT);
    scalarField maxArea(meshPoints.size(), -GREAT);
    ...
    forAll(meshPoints, pointi)
    {
        ...
        forAll(pFaces, pFI)
        {
            ...
            label  level = cellLevel[own];
            scalar len = edge0Len / pow(2., level);
            scalar fArea = mesh.magFaceAreas()[meshFaceI]/(len*len);
    ...
    forAll(meshPoints, pointi)
    {
        if (!stationaryPoints[pointi])
        {
            ...
            forAll(pFaces, pFI)
            {
                face f = pp.localFaces()[pFaces[pFI]];
                label start = findIndex(f,pointi);

                label next = f.fcIndex(start);
                label prev = f.rcIndex(start);

                point ptStart = mesh.points()[meshPoints[f[start]]];
                point ptNext = mesh.points()[meshPoints[f[next]]];
                point ptPrev = mesh.points()[meshPoints[f[prev]]];
                vector edgeVec1 = ptNext - ptStart;
                vector edgeVec2 = ptStart - ptPrev;
                edgeVec1 /= (mag(edgeVec1) + VSMALL);
                edgeVec2 /= (mag(edgeVec2) + VSMALL);
    ...
    reduce(nSliver, sumOp<label>());

    Info<<"Number of sliver points to smooth : " << nSliver << endl;
    ...
    for (label i = 0; i < snapParams.nSliverSmooths(); i++)
    {
        ...
        forAll(meshPoints, i)
        {
            ...
            if (!stationaryPoints[i] && sumAreas[i] > VSMALL)
            {
    ...
    forAll(pp.localPoints(), pointi)
    {
        ...
        pointDisp[meshpointi] = newPoints[meshpointi]
...
//#endif
    ...
    while (nCollapseIter < 3)
    {
        ...
        forAll(pp.edges(), edgeI)
        {
            ...
            const labelList& eFaces = pp.edgeFaces()[edgeI];
            vector eV = e.vec(pp.localPoints())/(e.mag(pp.localPoints())+VSMALL);
        ...
        //check whole mesh
        label nInitErrors = motionSmoother::checkMesh
        (
            false,
            mesh,
            motionDict,
        ...
        {
            if (mag(displacement[pointI]) > VSMALL || !midPointCollapse[pointI])
            {
        ...
        bool foundMerge(nNew != pointMap.size() ? true : false);

        if (!returnReduce(foundMerge, orOp<bool>()))
        {
            break;
        }

        polyTopoChange collapseMod(mesh);
        Map<label> pointToMaster(pointMap.size() - nNew);
        const labelList& pointLevel = meshRefiner_.meshCutter().pointLevel();

        labelListList newToOld(invertOneToMany(nNew, pointMap));
        forAll(newToOld, i)
        {
            const labelList& oldPts = newToOld[i];
            if (oldPts.size() == 2)
            {
                label pointToRemove = -1;
                label pointToKeep = -1;
                label mpt0 = meshPointMap[oldPts[0]];
                label mpt1 = meshPointMap[oldPts[1]];
                if (pointLevel[mpt0] > pointLevel[mpt1])
                {
                    pointToRemove = mpt1;
                    pointToKeep = mpt0;
                }
        ...
        // Change the mesh (no inflation)
        autoPtr<polyTopoChangeMap> map = collapseMod.changeMesh(mesh, false, true);
        ...
        forAll(pp.meshPoints(), pointI)
        {
            ...
            if (origPointI != -1)
            {
                ...
                (
                    ...
                    && (magDisp < VSMALL && midPointCollapse[origPointI])
                )
                {
    ...
    forAll(pp.edges(), edgeI)
    {
        ...
        if (nPPEdgeFaces[meshEdgeI] == 2 && pp.edgeFaces()[edgeI].size() == 2)
        {
            ...
            if (f0.size() == 3 && f1.size() == 3)
            {
                ...
                norm0 /= (mag(norm0) + VSMALL);

                vector norm1 = mesh.faceAreas()[face1];
                norm1 /= (mag(norm1) + VSMALL);
...
)
{
    ...
    //remove layers already generated on grown up layer patches
    {
        ...
        if (returnReduce(ppCellRemoval.size(), sumOp<label>()) == 0)
        {
            ...
            if (meshOptimDict.found("helyxOptimizeCoeffs"))
            {
                dictionary& coeffsDict =
                    meshOptimDict.subDict("helyxOptimizeCoeffs");
        ...
        const List<labelList> gUpEdgePatches = eClass.uniqueEdgePatches();
        //Set boundary points
        forAll(ppCellRemoval.edges(), edgeI)
        {
            ...
            {
                const labelList& nbrPatches = gUpEdgePatches[edgeI];
                forAll(nbrPatches, npi)
                {
                    if (regionSnap[nbrPatches[npi]])
                    {
                        pointFtrType[e[0]] = label(3);
                        pointFtrType[e[1]] = label(3);
                        break;
                    }
                }
            }
    ...
    //Project and smooth
    {
        ...
        for (label nSmooth=0; nSmooth < 2; ++nSmooth)
        {
            sumAreas = 0;
            sumDisp =  vector::zero;
            forAll(ppCellRemoval, i)
        ...
        vectorField bNormal(meshPoints.size(), vector::zero);
        labelList nEdges(meshPoints.size(), label(0));
        forAll(ppCellRemoval, i)
        {
            const face& f = ppCellRemoval.localFaces()[i];
            forAll(f, fp)
            {
                label next = f.fcIndex(fp);
                label lpt0 = f[fp];
                label lpt1 = f[next];
                label pt0 = meshPoints[lpt0];
                label pt1 = meshPoints[lpt1];
                if (pointFtrType[pt0] == 0 && pointFtrType[pt1] == 0)
                {
                    vector eVec = newPoints[pt1]-newPoints[pt0];
                    eVec /= (mag(eVec) + VSMALL);

                    vector pDir0 = pointFtrDir[pt0]
                       / (mag(pointFtrDir[pt0]) + VSMALL);
                    vector pDir1 = pointFtrDir[pt1]
                       / (mag(pointFtrDir[pt1]) + VSMALL);

                    vector pDir = 0.5*(pDir0+pDir1);
                    vector gDir = (eVec ^ pDir);
                    gDir /= (mag(gDir) + VSMALL);
                    bNormal[lpt0] += gDir;
                    bNormal[lpt1] += gDir;
                    nEdges[lpt0]++;
                    nEdges[lpt1]++;
        ...
        forAll(ppCellRemoval.edges(), edgei)
        {
            ...
            label moveIndex =  -1;
            label wallIndex =  -1;
            if (nEdges[le[0]] > 0 && nEdges[le[1]] == 0)
            {
                wallIndex = le[0];
                moveIndex = le[1];
            }
            else if (nEdges[le[1]] > 0 && nEdges[le[0]] == 0)
            {
                wallIndex = le[1];
                moveIndex = le[0];
            }
            ...
            if (wallIndex != -1)
            {
                scalar eLen = e.mag(newPoints);
                vector gDir = bNormal[wallIndex]/nEdges[wallIndex];
                gDir /= (mag(gDir) + VSMALL);
                vector outerPt = newPoints[meshPoints[wallIndex]]
                    - 0.5*eLen*gDir;
                sumDisp[moveIndex] +=
                   (outerPt - newPoints[meshPoints[moveIndex]]);
                sumAreas[moveIndex] += 1.0;
            }
        }
        ...
        //Final re-snap
        {
            ...
            if (snapParams.mergeGrownUp())
            {
                meshRefiner_.mergePatchFacesUndo
                (
                    Foam::cos(degToRad(10.0)),
                    Foam::cos(degToRad(10.0)),
                ...
                //Perform additional smoothing as result of merging
                resetPrimitivePatchAddressing(noLayerPatchIDs,ppCellRemoval);

                //Smooth boundary edges after merge
                const labelList& mergedMeshPoints = ppCellRemoval.meshPoints();
                const labelList mergedMeshEdges
                (
                    ppCellRemoval.meshEdges(mesh.edges(), mesh.pointEdges())
                );
                edgeClassification eClass
                ...
                labelList mergedBoundaryType(mergedMeshPoints.size(), -1);

                List<List<point>> ppPtEdgeCentres
                (
                    mergedMeshPoints.size(),
                    List<point>(0)
                );

                forAll(mergedMeshEdges, edgei)
                {
                    if (eType[edgei].first() == edgeClassification::BOUNDARY)
                    {
                        const edge& le = ppCellRemoval.edges()[edgei];
                        point eC = le.centre(ppCellRemoval.localPoints());
                        forAll(le,lei)
                        {
                            label pointi = le[lei];
                            mergedBoundaryType[pointi] = 0;
                            List<point>& ptEdgeCtrs = ppPtEdgeCentres[pointi];
                            label sz = ptEdgeCtrs.size()+1;
                            ptEdgeCtrs.setSize(sz);
                            ptEdgeCtrs[sz-1] = eC;
                        }
                    }
                }
                ...
                forAll(mergedMeshPoints, pointi)
                {
                    if (ppPtEdgeCentres[pointi].size() > 1)
                    {
                        const List<point>& ptECs = ppPtEdgeCentres[pointi];
                        const point lp = ppCellRemoval.localPoints()[pointi];
                        vector eDir0 = lp - ptECs[0];
                        vector eDir1 = ptECs[1] - lp;
                        eDir0 /= (mag(eDir0) + VSMALL);
                        eDir1 /= (mag(eDir1) + VSMALL);
                        if ((eDir0 & eDir1) < 0.939)
                        {
                            mergedBoundaryType[pointi] = 1;
                        }
                    }
                }
                ...
                scalarField sumMergedLen(mergedMeshPoints.size(), 0.0);
                vectorField sumMergedPt
                (
                    mergedMeshPoints.size(), vector::zero
                );

                newPoints =  mesh.points();
                pointDisp.setSize(mesh.nPoints(), vector::zero);

                for (label nSmooth=0; nSmooth < 4; ++nSmooth)
                {
                    sumMergedLen = 0;
                    sumMergedPt =  vector::zero;
                    forAll(ppCellRemoval.edges(), edgei)
                    {
                        if (eType[edgei].first() == edgeClassification::BOUNDARY)
                        {
                            const edge& le = ppCellRemoval.edges()[edgei];
                            const edge& e =
                                mesh.edges()[mergedMeshEdges[edgei]];
                            point eC = e.centre(newPoints);
                            scalar eLen = e.mag(newPoints);
                            label pLevel = 0;
                            forAll(le,lei)
                            {
                                label meshpointi = mergedMeshPoints[le[lei]];
                                pLevel = max(pLevel, pointLevel[meshpointi]);
                            }
                            forAll(le,lei)
                            {
                                label pointi = le[lei];
                                if (mergedBoundaryType[pointi] != 0)
                                {
                                    continue;
                                }
                                scalar pLen = edge0Len / pow(2., pLevel);
                                scalar weight = eLen / pLen;
                                sumMergedPt[pointi] += eC * weight;
                                sumMergedLen[pointi] += weight;
                            }
                        }
                   }
                   ...
                   forAll(mergedMeshPoints, pointi)
                   {
                        label meshpointi = mergedMeshPoints[pointi];
                        if (sumMergedLen[pointi] > VSMALL)
                        {
                            pointDisp[meshpointi] =
                            (
                                sumMergedPt[pointi] / sumMergedLen[pointi]
                            )
                            - newPoints[meshpointi];
                        }
                    }
...
)
{
    ...
    //remove layers already generated on zero layer patches
    {
        ...
        while (true)
        {
            ...
            forAll(mesh.faces(), faceI)
            {
                ...
                label layerOwnID = layerCells[own];
                label layerNeiID = -1;
...

    {
        ...
        {
            autoPtr<polyTopoChangeMap> map = meshRefiner_.mergeZoneBaffles
            (
                true,   // internal zones
                true   // baffle zones
            );
...
void Foam::snappySnapDriver::snapAndMerge(snapParameters& snapParams)
{
    fvMesh& mesh = meshRefiner_.mesh();
    const polyBoundaryMesh& patches = mesh.boundaryMesh();
    ...
    // Get the labels of added patches.
    labelList adaptPatchIDs;
    bool preMergeAll = snapParams.globalPreMerge();
    if (const dictionary* preMergeDictPtr = snapDict.subDictPtr("preFaceMergePatches"))
    {
        const labelList& meshedPatches = meshRefiner_.meshedPatches();
        DynamicList<label> preMergePatches(meshedPatches.size());
        forAll(meshedPatches, i)
        {
            const label patchi = meshedPatches[i];
            const word& patchName = patches[patchi].name();
            if (preMergeDictPtr->found(patchName))
            {
                const dictionary& preMergeDict =
                    preMergeDictPtr->subDict(patchName);
                if (preMergeDict.found("active"))
                {
                    if (preMergeDict.lookup<bool>("active"))
                    {
                        preMergePatches.append(patchi);
                    }
                }
                else if (preMergeAll)
                {
        ...
        adaptPatchIDs = preMergePatches;
    }
    else if (preMergeAll)
    {
        adaptPatchIDs = meshRefiner_.meshedPatches();
    }

    if (adaptPatchIDs.size() == 0)
    {
        return;
    }

    Info<<"Pre snapping and merging faces.. " << nl <<endl;
...
)
{
    ...
    bool internalOrBaffleZone = createBaffles();
    ...
    labelHashSet wrongFaces(mesh.nFaces()/100);
    label errorsFound = motionSmoother::checkMesh
    (
        false,
        mesh,
        motionDict,
...
//#endif
    ...
    const labelList directPatchIDs =  snapParams.directSnappedIDs();
    autoPtr<indirectPrimitivePatch> ppDirectPtr
    (
        meshRefinement::makePatch
        (
           mesh,
    ...
    indirectPrimitivePatch& ppDirect = ppDirectPtr();

    // Calculate direct sizing and also trigger addressing calculation
    label directSz = returnReduce
    (
        ppDirect.meshPoints().size(),
        sumOp<label>()
    );
    ...
    bool directRepartition = false;
    if
    (
        snapParams.directFeatureSnapping() &&
        snapParams.directSnapRepartition() && !dryRun &&
        regionSnappedPatches.size() > 0 &&
        directSz > 0
    )
    {
        if (nInternalBaffles > 0)
        {
            autoPtr<polyTopoChangeMap> map = meshRefiner_.mergeZoneBaffles
            (
                true,  // internal zones
            ...
            resetPrimitivePatchAddressing(directPatchIDs,ppDirect);
        }

        directRepartition = repartitionDirectSnap
        (
            directPatchIDs,
            regionSnap,
            ppDirect
        ...
        if (createBaffles())
        {
            internalBaffles = meshRefiner_.subsetBaffles
            (
                mesh,
                internalFaceZones,
                localPointRegion::findDuplicateFacePairs(mesh)
    ...
    if (snapParams.directFeatureSnapping() && directSz > 0 && !dryRun)
    {
        ...
        (
            ...
            ppDirect,
            directPatchIDs,
            meshRefinement::makeDisplacementField(pMesh, directPatchIDs),
            motionDict
        );
        ...
        // reset pp addressing after all topo changes
        resetPrimitivePatchAddressing(adaptPatchIDs,pp);
    ...
    )
    {
        ...
        resetPrimitivePatchAddressing(adaptPatchIDs,pp);
        updateDirectSnap(map(),mesh,directPatchIDs,ppDirect,flSnap);
    }
    ...
    else if (directRepartition)
    {
        repartitionDirectSnap
        (
            adaptPatchIDs,
            boolList(),
            pp,
            true // force a rebalance
        );
    }
    ...
    {
        autoPtr<polyTopoChangeMap> map = surfaceToPatch(snapParams,refineParams);
        //update pp addressing
        resetPrimitivePatchAddressing(adaptPatchIDs, pp);
    }
...
bool Foam::snappySnapDriver::createBaffles()
{
    ...
    return internalOrBaffleZone;
}


bool Foam::snappySnapDriver::repartitionDirectSnap
(
    const labelList& adaptPatchIDs,
    const boolList& regionSnap,
    indirectPrimitivePatch& pp,
    const bool forceRebalance
)
{
    if (Pstream::parRun())
    {
        ...
        DynamicList<label> regionFaces(pp.size());
        if (regionSnap.size() != 0)
        {
            List<labelList> edgeRegions(meshEdges.size());
            forAll(meshEdges, edgei)
            {
                const labelList& eFaces = pp.edgeFaces()[edgei];
                labelList& eRegion = edgeRegions[edgei];
                DynamicList<label> ePIDs(eFaces.size());
                forAll(eFaces, i)
                {
                    label facei = pp.addressing()[eFaces[i]];
                    label patchi = patches.whichPatch(facei);
                    if
                    (
                        patchi != -1
                    ...
                    {
                        if (findIndex(ePIDs, patchi) == -1)
                        {
                            ePIDs.append(patchi);
                        }
                    }
                }
                eRegion.transfer(ePIDs);
            }
            ...
            boolList regionPts(meshPoints.size(), false);
            forAll(pp.edges(), edgei)
            {
                const labelList& ePIDs = edgeRegions[edgei];
                if (ePIDs.size() > 1)
                {
                   forAll(ePIDs, epi)
                   {
                       label patchi = ePIDs[epi];
                       if (regionSnap[patchi])
                       {
                           const edge& e = pp.edges()[edgei];
                           regionPts[e[0]] = true;
                           regionPts[e[1]] = true;
            ...
            forAll(pp,i)
            {
                ...
                {
                    label meshfacei = pp.addressing()[i];
                    if (blockedFaces[meshfacei])
                    {
                        regionFaces.append(i);
                    }
                }
            }
        }
        ...
        (
            forceRebalance
            || returnReduce(regionFaces.size(), sumOp<label>()) != 0
        )
        {
            Info<< nl
                << "Direct snap repartition" << nl
                << "-----------------------" << nl
                << endl;

            PtrList<labelList> specifiedProcessorFaces(0);
            labelList specifiedProcessor(0);

            if (!forceRebalance)
            {
                boolList markedFaces(pp.size(), false);
                forAll(regionFaces, i)
                {
                    markedFaces[regionFaces[i]] = true;
                }

                regionSplit2D regSplit
                (
                    mesh,
                    pp,
                    markedFaces
                );
                label nRegions = regSplit.nRegions();

                boolList activeRegions(nRegions, false);
                forAll(markedFaces, i)
                {
                    if (markedFaces[i])
                    {
                        label regioni = regSplit[i];
                        if (regioni > -1)
                        {
                            activeRegions[regioni] = true;
                        }
                    }
                }
                Pstream::listCombineGather(activeRegions, orEqOp<bool>());
                Pstream::listCombineScatter(activeRegions);
                labelList activeProc(nRegions,-1);
                label incrProc = 0;
                forAll(activeRegions, i)
                {
                    if (activeRegions[i])
                    {
                        label proci = incrProc % Pstream::nProcs();
                        activeProc[i] = proci;
                        incrProc++;
                    }
                }

                label nProcsNeeded = max(activeProc)+1;
                // Faces that move as block onto single processor
                List<DynamicList<label>> procDist(nProcsNeeded);
                forAll(markedFaces, i)
                {
                    if (markedFaces[i])
                    {
                       ...
                       if (regioni != -1)
                       {
                           label proci =  activeProc[regioni];
                           if (proci != -1)
                           {
                               procDist[proci].append(pp.addressing()[i]);
                           }
                       }
                    }
                }
                specifiedProcessorFaces.setSize(nProcsNeeded);
                specifiedProcessor.setSize(nProcsNeeded);
                forAll(specifiedProcessor, i)
                {
                    specifiedProcessorFaces.set(i,new labelList(procDist[i]));
                    specifiedProcessor[i] = i;
                }
            }
    ...
    if (smoothSurf)
    {
        ...
        for (label i = 0; i < 5; i++)
        {
            ...
            forAll(ppLay.meshPoints(), i)
            {
                ...
                if (!stationaryPoints[i] && sumAreas[i] > VSMALL)
                {
                    snapPts[nSet] = (sumCentres[i] / sumAreas[i]);
                    snapDist[nSet] =
            ...
            forAll(ppLay.meshPoints(), i)
            {
                ...
                if (!stationaryPoints[i] && sumAreas[i] > VSMALL)
                {
                    ...
                    {
                        pointDisp[meshPointI] = (sumCentres[i]/sumAreas[i])
                           - newPoints[meshPointI];
                    }
                    nSet++;
                }
...
)
{
    ...
    while (true)
    {
        ...
        forAll(pp, i)
        {
            ...
            if (nDegenerateEdges > 0)
            {
                ...
                scalar ac1 =  modF.mag(mesh.points())
                    / (f.mag(mesh.points())+ VSMALL);
                scalar ac2 =  intF.mag(mesh.points())
                    / (mesh.faces()[degenerateFace].mag(mesh.points())+ VSMALL);
        ...
        else
        {
            ...
            labelHashSet wrongFaces(mesh.nFaces()/100);
            labelList checkFaces(meshRefiner_.selectInnerFaces());
            label nErrors = motionSmoother::checkMesh
            (
                false,
                mesh,
                motionDict,
            ...
            if (nErrors > 0)
            {
                ...
                // Change the mesh (no inflation)
                autoPtr<polyTopoChangeMap> resetMap =
                    meshReset.changeMesh(mesh, false, true);

                // Update fields (problem when face added to zero sized patch)
                mesh.topoChange(resetMap);
                ...
                meshRefiner_.topoChange(resetMap,labelList(0));
...
void Foam::snappySnapDriver::updateDirectSnap
(
    const polyTopoChangeMap &map,
    const fvMesh &mesh,
    const labelList &adaptPatchIDs,
