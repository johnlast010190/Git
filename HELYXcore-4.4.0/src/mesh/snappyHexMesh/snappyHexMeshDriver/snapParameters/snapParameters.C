...
)
{
    ...
    directSnapPatches_.setSize(boundaryMesh.size(), false);
    ...
    if (globalDirectSnap())
    {
        ...
        {
            label patchI = meshedPatches[i];
            directSnapPatches_[patchI] = true;
        }
        ...
        forAll(mesh.faceZones(), zoneI)
        {
            ...
            if (hasInfo)
            {
                if (zoneFeatureSnapping())
                {
                    if (fzType == surfaceZonesInfo::BOUNDARY)
                    {
                        directSnapPatches_[mpI] = true;
                        directSnapPatches_[spI] = true;
                    }
                    else
                    {
                        directSnapPatches_[mpI] = true;
                        directSnapPatches_[spI] = false;
                    }
                }
                else
                {
                    directSnapPatches_[mpI] = false;
                    directSnapPatches_[spI] = false;
                }
            }
        }
            ...
            if (!pp.coupled() && featureEdgesDictPtr->found(patchName))
            {
                ...
                if (featureEdgeDict.found("featureSnap"))
                {
                    featureEdgePatches_[patchI] =
                        featureEdgeDict.lookup<bool>("featureSnap");
                }
                ...
                if (featureEdgeDict.found("directSnapBoundaries"))
                {
                    directSnapPatches_[patchI] =
                        readBool
                        (
                            featureEdgeDict.lookup("directSnapBoundaries")
                        );
                }
            }
        }
...
:
    ...
    directSnapPatches_(boundaryMesh.size(), false),
    directFeatureSnapping_
    (
        dict.lookupOrDefault<bool>("directFeatureSnapping",false)
    ),
    directSnapRepartition_
    (
        dict.lookupOrDefault<bool>("directSnapRepartition",false)
    ),
    ...
    globalDirectSnap_(dict.lookupOrDefault<bool>("globalDirectSnap",true)),
    globalSnapRegions_(dict.lookupOrDefault<bool>("globalRegionSnap",false)),
    globalPreMerge_(dict.lookupOrDefault<bool>("globalPreMerge",true)),
    ...
    snapFlipIter_(dict.lookupOrDefault<label>("snapFlipIter", labelMax)),
    snapFlipTol_(dict.lookupOrDefault<scalar>("snapFlipTol", scalar(0.25))),
    mergeGrownUp_(dict.lookupOrDefault<bool>("extrudeMergeGrownUp", false))
{
    Info<< nl
        << "Snap parameters" << nl
        << "---------------" << nl
