...
:
    ...
    mergeSingleCouplesExtra_
    (
        dict.lookupOrDefault<Switch>
        (
            "mergeSingleCouplesExtra",
            false
        )
    ),
    balanceRefine_
    ...
    singleNamedRezone_
    (
        dict.lookupOrDefault<Switch>("singleNamedRezone",false)
    ),
    mergePreExtrude_
    (
    ...
    finalSurfaceRefine_
    (
        dict.lookupOrDefault<Switch>("finalSurfaceRefine",false)
    ),
    regionsInitRefine_
    (
        dict.lookupOrDefault<Switch>("regionsInitialRefine",false)
    ),
    surfaceRefineRemoval_
    (
        dict.lookupOrDefault<Switch>("surfaceRefineRemoval",true)
    )
{
    if (dict.found("proximityAngle"))
    {
        proximityAngle_ = mag
        (
            Foam::cos(degToRad(dict.lookup<scalar>("proximityAngle")))
        );
    }
    else
    {
...
void Foam::refinementParameters::doTransforms
(
    const fvMesh& mesh,
...
)
{
    ...
    {
        const coordinateFrame* coordFramePtr =
            coordinateFrame::lookupNew(mesh, meshDict);
        const coordinateSystem& coord = coordFramePtr->coorSys();
        // Perform transforms
        locationsInMesh_ = coord.transformPoint(locationsInMesh_);
        locationsOutsideMesh_ = coord.transformPoint(locationsOutsideMesh_);
    }
    return;
}
