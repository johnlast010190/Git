...
//#endif
    ...
    bool shiftEndPt = false;
    if
    (
        iter == 0 &&
        (ftrSnapType == word("presnap") || ftrSnapType == word("mainsnap"))
    )
    {
        shiftEndPt = true;
    }
    ...
    bool extrude
    (
       controller_.algorithm() == meshControl::EXTRUDE ? true : false
    );
    ...
    if (!fastFeatureSnap)
    {
        ...
        forAll(pp, facei)
        {
            ...
            vector fN = ppFaceAreas[facei]/(mag(ppFaceAreas[facei])+VSMALL);
            vector ccDir = ((ppCellCentres[facei]-ppFaceCentres[facei])&fN)*fN;
            forAll(lf,fp)
            {
                label pointi = lf[fp];
                pointCellCentres[pointi] += ccDir;
                pointFaceNormals[pointi] += fN;
...

    {
        ...
        forAll(hitPoint1, pointI)
        {
            ...
            else
            {
               ...
               (
                   extrude && shiftEndPt
                   && !isZonedFace.get(pp.addressing()[pointI])
               )
               {
                   // extrude method CC maybe be wrong side of geometry
                   endPoints[pointI] =
                       0.5*(ppFaceCentres[pointI]+ppCellCentres[pointI]);
               }
               else
               {
                   endPoints[pointI] = ppFaceCentres[pointI];
        ...
        forAll(surf1, i)
        {
            if (hPoint1[i].hit())
            {
                vector areaNorm = ppFaceAreas[pointCentreIndex[i]]
                    / (mag(ppFaceAreas[pointCentreIndex[i]]) + VSMALL);
                ...
                scalar dot1 = -(projSurfNorm
                                / (mag(projSurfNorm) + VSMALL)) & areaNorm;
                scalar dot2 = -(surfNorm
                                / (mag(surfNorm) + VSMALL)) & areaNorm;
        ...
        forAll(checkSurfaces, sI)
        {
            ...
            forAll(hitSurface, i)
            {
                if (hitSurface[i] == surfI)
                {
                    ...
                    vector vec1 =
                        ppFaceAreas[i] / (mag(ppFaceAreas[i]) + VSMALL);
    ...
    if (fastFeatureSnap)
    {
        forAll(pp, i)
        {
            if (!stationaryFaces[i])
            {
                ...
                point avePoint = (patchFaceDisp[i] + fc);
    ...
    else
    {
        ...
        {
            const face& f = pp.localFaces()[i];
            vector fArea = -ppFaceAreas[i];
            fArea = fArea / (mag(fArea) + VSMALL);

            // check face centres
            vector sNorm = surfHitPatchCentreNormals[i];
            scalar magSNorm = mag(sNorm);
            scalar maxDotProd = 0.0;
            if (magSNorm > VSMALL)
            {
               sNorm = sNorm / mag(sNorm);
               maxDotProd = (fArea & sNorm);
            }
            label maxFp = -1;
            // check face points
            forAll(f, fp)
            {
                vector sPNorm = surfHitPatchPointNormals[f[fp]];

                scalar magSPNorm = mag(sPNorm);
                if (magSPNorm > VSMALL)
                {
                    sPNorm = sPNorm / mag(sPNorm);
                    scalar fpDotProd = (fArea & sPNorm);
                    if (fpDotProd > maxDotProd)
                    {
                       maxFp = fp;
                       maxDotProd = fpDotProd;
                    }
                }
            }
            if (maxFp == -1)
            ...
            {
                alignedSurfaceNormal[i] = surfHitPatchPointNormals[f[maxFp]];
            }

            if ((fArea & alignedSurfaceNormal[i]) > alignmentAngle)
            {
               forAll(f, fp)
               {
                  label pointi = f[fp];
                  avePointNormals[pointi] += alignedSurfaceNormal[i];
                  nPointCounter[pointi]++;
               }
            }
        }
        ...
        forAll(pp, i)
        {
            if (!stationaryFaces[i])
            {
                ...
                vector fArea = -ppFaceAreas[i];
                fArea = fArea / (mag(fArea) + VSMALL);
                vector alignedNormal = alignedSurfaceNormal[i];
                // if not well aligned use average of neighbouring face
                //normals to find closest alignment

                if ((fArea & alignedNormal) < alignmentAngle)
                {
                    vector fAveNorm = vector::zero;
                    label nCount = 0;
                    forAll(f, fp)
                    {
                        fAveNorm += avePointNormals[f[fp]];
                        nCount += nPointCounter[f[fp]];
                    }

                    if (nCount > 0 && mag(fAveNorm) > 0)
                    {
                        fAveNorm /= nCount;
                    }
                    else
                    {
                        fAveNorm = alignedSurfaceNormal[i];
                    }

                    scalar magFAveNorm = mag(fAveNorm);
                    if (magFAveNorm > VSMALL)
                    {
                        fAveNorm = fAveNorm / magFAveNorm;
                        // check face centres
                        vector sCentreNorm = surfHitPatchCentreNormals[i];
                        sCentreNorm = sCentreNorm / (mag(sCentreNorm) + VSMALL);
                        scalar maxDotProd = (fAveNorm & sCentreNorm);
                        label maxFp = -1;
                        // check face points
                        forAll(f, fp)
                        {
                            vector sPNorm = surfHitPatchPointNormals[f[fp]];
                            sPNorm = sPNorm / (mag(sPNorm) + VSMALL);
                            scalar fpDotProd = (fAveNorm & sPNorm);
                            if (fpDotProd > maxDotProd)
                            {
                                maxFp = fp;
                                maxDotProd = fpDotProd;
                        ...
                        {
                            alignedNormal = surfHitPatchPointNormals[f[maxFp]];
                        }
                    }
                }
                scalar magAlignedNorm = mag(alignedNormal);
                vector avePoint = vector::zero;
                if (magAlignedNorm > SMALL)
                {
                    alignedNormal /= magAlignedNorm;
                    ...
                    forAll(f, fp)
                    {
                        ...
                        if (magSPNorm > VSMALL)
                        {
                            sPNorm = sPNorm / magSPNorm;
                            if ((alignedNormal & sPNorm) > alignmentAngle)
                            {
                                point updatedPt = newPoints[pp.meshPoints()[f[fp]]]
                                    + patchPointDisp[f[fp]];
                                avePoint += updatedPt;
                                nPointsFound++;
                            }
                    ...
                    vector sCentreNorm = surfHitPatchCentreNormals[i];
                    sCentreNorm = sCentreNorm / (mag(sCentreNorm) + VSMALL);
                    scalar dotProd = alignedNormal & sCentreNorm;
    ...
    if (initialSnap)
    {
        ...
        for (label iter = 0; iter < 2; iter++)
        {
            ...
            forAll(ppInit, i)
            {
                ...
                aveNormal /= f.size();
                aveNormal /= (mag(aveNormal) + VSMALL);
            ...
            forAll(start, i)
            {
                if (hitPoint1[i].hit())
                {
                    ...
                    if (hitDist > VSMALL && hitDist > maxProjectionDistance)
                    {
                        hitVec /= hitDist;
        ...
        {
            if (sumAreasMesh[meshPointI] > VSMALL && !gapBoundaryPts[meshPointI])
            {
...
) const
{
    ...
    )
    {
        ...
        )
        {
            ...
            //helyx optimisation step
            if (opt & optStages::SMOOTH)
            {
                dictionary dummyOptimDict;
                dummyOptimDict.add("type","helyxOptimize",true);
                dummyOptimDict.add
                    ("helyxOptimizeCoeffs",dictionary(), true);
                dummyOptimDict.subDict("helyxOptimizeCoeffs").add
                (
                    "optimizationMethod",
            ...
            )
            {
                ...
                if (optimType == "helyxOptimize")
                {
                    meshOptimDict.subDict("helyxOptimizeCoeffs").add
                    (
                        "optimizationMethod",
                        "fullMeshOptimization",
                        true
        ...
        )
        {
            ...
            pointField appPts(std::move(gapSnapLoc));
...
) const
{
    ...
    forAll(pp.edges(), edgei)
    {
        ...
        if (eN.size() == 2)
        {
            ...
            const vector fNorm0 = fArea0/(mag(fArea0)+ VSMALL);
            const vector fNorm1 = fArea1/(mag(fArea1)+ VSMALL);
            ...
            if (magSNorm0 < VSMALL || magSNorm1 < VSMALL)
            {
                continue;
            }
            sNorm0 /= magSNorm0;
            ...
            if (weakSnapCheck || tightAngleCheck)
            {
                ...
                forAll(e, ei)
                {
                    ...
                    if (magD < maxDisp)
                    {
                        ...
                        sumDispPP[e[ei]] += d / (weight + VSMALL);
                        sumAreasPP[e[ei]] += 1.0 / (weight + VSMALL);
                    }
                }
            }
        }
...
) const
{
    ...
    forAll(finalHitInfo, fI)
    {
        ...
        point aveNorm = pNormals[fI] / (mag(pNormals[fI]) + VSMALL);
        label nHit = 0;
        vector fArea = ppFaceAreas[i] / (mag(ppFaceAreas[i]) + VSMALL);

        scalar dotProd = mag(fArea & aveNorm);
        vector maxNorm = pNormals[fI] / (mag(pNormals[fI]) + VSMALL);
...
) const
{
    ...
    bool postMergeSnap = false;
    if (ftrSnapType == "removalmergesnap")
    {
        postMergeSnap = true;
    }
    ...
    forAll(pp.edgeFaces(), edgeI)
    {
        ...
        (
            ...
            && (zonedEdges[edgeI] || postMergeSnap)
        )
        {
            featureEdge[meshEdgeI] = true;
            featurePoint[e[0]] = max(label(2),featurePoint[e[0]]);
            ...
            forAll(f, fp)
            {
                ...
                if (nFeatureEdges[pointi] == 2)
                {
                    ...
                    vector nextEdge = newPoints[f[nextFp]]
                       - currPt;
                    nextEdge /= (mag(nextEdge) + VSMALL);
                    vector prevEdge = currPt
                       - newPoints[f[prevFp]];
                    prevEdge /= (mag(prevEdge) + VSMALL);
    ...
    if (controller_.algorithm() == meshControl::EXTRUDE && iter > 20)
    {
        ...
        labelList nEdgeFlipFaces(pp.nEdges(), 0);
        ...
        forAll(meshPoints, pointi)
        {
            ...
            forAll(pEdges, pei)
            {
                label edgei = pEdges[pei];
                label meshedgei = meshEdges[edgei];
                if (!featureEdge[meshedgei] && nEdgeFlipFaces[edgei] != 2)
                {
                    pEdgeCentre[pointi] +=
            ...
            forAll(lf, lfi)
            {
                ...
                )
                {
                    ...
                    )
                    {
                        ...
                        if
                        (
                            nEdgeFlipFaces[nextEdge] > 1
                            || nEdgeFlipFaces[prevEdge] > 1
                        )
                        {
                            continue;
                        }
        ...
        forAll(pp.edges(), edgei)
        {
            ...
            if
            (
                nEdgeFlipFaces[edgei] == 2 && !featureEdge[meshedgei]
            )
            {
                const labelList& eFaces = pp.edgeFaces()[edgei];
                forAll(eFaces, eFI)
    ...
    {
        label meshPointI = meshPoints[i];
        if (!stationaryPoints[meshPointI] && sumAreasPP[i] > VSMALL)
        {
            pointDisp[meshPointI] = (sumDispPP[i] / sumAreasPP[i])
                - newPoints[meshPointI];
        }
    }
    ...
    if (iter > 10)
    {
        ...
        forAll(mesh.points(), meshPointI)
        {
            if (boundaryPoints[meshPointI] && featurePoint[meshPointI] == -2)
            {
                if (sumAreasMesh[meshPointI] > VSMALL)
                {
...
void Foam::snappySnapDriver::flipNonConvergedFaces
(
   const pointField& newPoints,
   const indirectPrimitivePatch& pp,
   const pointField& ppFaceCentres,
   ...
   const List<DynamicList<Tuple2<label,point>>>& allHits,
   const scalar& flipTol,
   pointField& sumDispPP,
   scalarField& sumAreasPP,
   pointField& pointDisp
)  const
{
    const fvMesh& mesh = meshRefiner_.mesh();
    const scalar edge0Len = meshRefiner_.meshCutter().level0EdgeLength();
    const labelList& cellLevel = meshRefiner_.meshCutter().cellLevel();
    scalarField ppMaxDisp(pp.size(), -GREAT);
    scalarField ppPtMaxDisp(meshPoints.size(), -GREAT);

    forAll(meshPoints, pointi)
    {
        label meshpointi = meshPoints[pointi];
        const List<Tuple2<label,point>>& allHitPts = allHits[pointi];
        forAll(allHitPts, hiti)
        {
            label facei = allHitPts[hiti].first();
            point hitPt = allHitPts[hiti].second();
            label own = mesh.faceOwner()[pp.addressing()[facei]];
            label level = cellLevel[own];
            scalar len = edge0Len / pow(2., level);
            vector disp = hitPt - newPoints[meshpointi];
            scalar relDisp = mag(disp)/len;
            ppMaxDisp[facei] = max(ppMaxDisp[facei], relDisp);
            ppPtMaxDisp[pointi] = max(ppPtMaxDisp[pointi],relDisp);
        }
    }
    syncTools::syncPointList
    (
    ...
    // check edge alignement with edge centre to face centre vector
    scalarField eAlign(pp.edges().size(), 0.0);
    forAll(pp.edges(), edgei)
    {
        const labelList& eFaces = pp.edgeFaces()[edgei];
        const edge& e = mesh.edges()[meshEdges[edgei]];
        point eC = e.centre(newPoints);
        point eUV = e.unitVec(newPoints);
        forAll(eFaces, efi)
        {
            label facei = eFaces[efi];
            point fC = ppFaceCentres[facei];
            vector fcToEc = fC-eC;
            fcToEc /= (mag(fcToEc) + VSMALL);
            eAlign[edgei] = max(eAlign[edgei], mag(fcToEc&eUV));
        }
    }
    syncTools::syncEdgeList
    (
    ...
    boolList smoothPts(meshPoints.size(), false);
    forAll(pp, i)
    {
        const face& f = pp.localFaces()[i];
        scalar maxFaceDisp = ppMaxDisp[i];
        ...
        if (maxFaceDisp > flipTol)
        {
            scalar maxPtDisp = -GREAT;
            label maxLoc = -1;
            forAll(f,fp)
            {
               if (ppPtMaxDisp[f[fp]] > maxPtDisp)
               {
                   maxPtDisp = ppPtMaxDisp[f[fp]];
                   maxLoc = fp;
               }
            }

            if (maxFaceDisp > 0.99*maxPtDisp)
            {
                label nextFp = f.fcIndex(maxLoc);
                label prevFp = f.rcIndex(maxLoc);
                label oppFp = f.fcIndex(nextFp);
                label meshpointi = meshPoints[f[oppFp]];
                if (!featurePts[meshpointi])
                {
                    point movePt = 0.5*
                    (
                        newPoints[meshPoints[f[nextFp]]]
                        + newPoints[meshPoints[f[prevFp]]]
                    );
                    vector disp = movePt - newPoints[meshPoints[f[oppFp]]];
                    sumDispPP[f[oppFp]] =  disp;
                    sumAreasPP[f[oppFp]] = 1.0;
                    smoothPts[f[maxLoc]] = true;
                }
            }
        }
        else
        {
            ...
            forAll(fEdges, fei)
            {
                ...
                maxEdgeAngle = max(maxEdgeAngle,eAlign[edgei]);
            }

            if (nonFtrPt != -1 && nFtrEdges == 2 && maxEdgeAngle > 0.939)
            {
                excludedFaces[i] = true;
                label nextFp = f.fcIndex(nonFtrPt);
                label prevFp = f.rcIndex(nonFtrPt);
                point movePt = 0.5*
                (
                    newPoints[meshPoints[f[nextFp]]]
                    + newPoints[meshPoints[f[prevFp]]]
                );
                vector disp = movePt - newPoints[meshPoints[f[nonFtrPt]]];
                sumDispPP[f[nonFtrPt]] =  disp;
                sumAreasPP[f[nonFtrPt]] = 1.0;
                nextFp = f.fcIndex(nextFp);
                smoothPts[f[nextFp]] = true;
            }
        }
    }
    syncTools::syncPointList
    ...
    forAll(meshPoints, pointi)
    {
        if (smoothPts[pointi])
        {
            ...
            forAll(pFaces, pfi)
            {
                ...
                if (excludedFaces[pfacei])
                {
                    continue;
                }
                sumDispPP[pointi] +=
                    (ppFaceCentres[pfacei]-newPoints[meshPoints[pointi]]);
                sumAreasPP[pointi]++;
            }
        }
    }
    ...
    forAll(meshPoints, pointi)
    {
        if (sumAreasPP[pointi] > VSMALL)
        {
            label meshpointi = meshPoints[pointi];
            pointDisp[meshpointi] = sumDispPP[pointi]
               / sumAreasPP[pointi];
        }
    }
...
) const
{
    ...
    for (label iter = 0; iter < maxIter; iter++)
    {
        ...
        //Whether to activate face flippping at specified iteration
        label flipIter = snapParams.snapFlipIter();
        scalar flipTol = snapParams.snapFlipTol();

        // Smooth surface points (feature edge aware)
        if
        (
           smoothSurface && (iter % 2 == 0) && iter > smoothStartIter
           && (iter <= flipIter || iter > flipIter+6)
        )
        {
            nSurfSmooth++;
            edgeClassification eClass
        ...
        if (iter > 5)
        {
            forAll(pp, i)
            {
                ...
                scalar aRatio = minLength/(maxLength + VSMALL);

                scalar areaRatio = mag(ppFaceAreas[i])
                    /(mag(ppOrigFaceAreas[i]) + VSMALL);
        ...
        List<DynamicList<Tuple2<label,point>>> allHits(pp.meshPoints().size());
        forAll(pointIndex, fI)
        {
           label i = pointIndex[fI];
           const face& f = pp.localFaces()[i];
           const point& fc = ppFaceCentres[i];
           const  vector fArea = ppFaceAreas[i]
               / (mag(ppFaceAreas[i]) + VSMALL);
           ...
           if
           (
               iter == 0 && ftrSnapType == word("mainsnap")
               && mag(dProd) < 0.1736
           )
           {
               vector fcToCC = surfPointToSnap - ppCellCentres[i];
               if ((n1n2[1] & fcToCC) > 0 && (n1n2[0] & fcToCC) > 0)
               {
                   n1n2[1] = -n1n2[1];
               }
           }
           ...
           forAll(f, fp)
           {
               ...
               allHits[f[fp]].append(Tuple2<label,point>(i,p));
               scalar weight = mag(newFaceArea);

               sumDispPP[f[fp]] += fWeight * disp / (weight + VSMALL);
               sumAreasPP[f[fp]] += fWeight / (weight + VSMALL);
           }
        }
        ...
        forAll(pp, i)
        {
            if (stationaryFaces[i])
            {
                ...
                forAll(f, fp)
                {
                    sumAreasPP[f[fp]] += 1. /(mag(ppFaceAreas[i]) + VSMALL);
                }
            }
        }
        ...
        forAll(meshPoints, i)
        {
            ...
            {
                label meshPointI = meshPoints[i];
                pointDisp[meshPointI] = sumDispPP[i] / (sumAreasPP[i] + VSMALL);
            }
            else
            {
        ...
        forAll(meshPoints, i)
        {
            ...
            if (!stationaryPoints[meshPointI] && allHits[i].size()>0)
            {
                ...
                {
                    allHitsDisp[i] += mag(allHits[i][hitI].second()-projPt);
                    nHits[i]++;
                }
            }
        }
        ...
        if (iter == flipIter)
        {
            Info<<"Flipping non-converged faces at iteration " << iter <<endl;
            sumDispPP = vector::zero;
            sumAreasPP = 0.0;
            flipNonConvergedFaces
            (
                newPoints,
                pp,
                ppFaceCentres,
...
) const
{
    ...
    labelList layerPts(mesh.nPoints(), -1);
    forAll(layerCells, celli)
    {
        label layerCellID = layerCells[celli];
    ...
    forAll(pp, i)
    {
        ...
        forAll(f, fp)
        {
            ...
            if (!nextFE && !prevFE && !featurePts[meshpointi])
            {
                otherPt = meshpointi;
            }
        ...
        if (nFtrEdges == 2 && valid && otherPt != -1)
        {
            ...
            if (edgeLen > VSMALL)
            {
                ...
                scalar dispLen = hypLen*hypLen-tanLen*tanLen;
                if (dispLen <= 0)
                {
                    continue;
                }
                scalar normLen = sqrt(dispLen);
    ...
    // Check for collapsing edges
    labelList nMovedUpdate(nMoved);
    forAll(mesh.edges(), edgei)
    {
        const edge& e = mesh.edges()[edgei];
        label i0 = e[0];
        label i1 = e[1];
        if (nMoved[i0] > 0 || nMoved[i1] > 0)
        {
            point pt0 = newPoints[i0];
            point pt1 = newPoints[i1];
            scalar eLen0 = mag(pt1-pt0);
            if (nMoved[i0] > 0)
            {
                pt0 += (newDisp[i0]/nMoved[i0]);
            }
            if (nMoved[i1] > 0)
            {
                pt1 += (newDisp[i1]/nMoved[i1]);
            }
            scalar eLen1 = mag(pt1-pt0);
            scalar eRatio = eLen1/(eLen0+VSMALL);
            if (eRatio < 0.4)
            {
                nMovedUpdate[i0] = 0;
                nMovedUpdate[i1] = 0;
    ...
    forAll(nMovedUpdate, pointi)
    {
        if (nMovedUpdate[pointi] == 0)
        {
            nMoved[pointi] = 0;
        }
    }
    ...
    // move interior points after surface motion
    dictionary dummyOptimDict;
    dummyOptimDict.add("type","helyxOptimize",true);
    dummyOptimDict.add
        ("helyxOptimizeCoeffs",dictionary(), true);
    ...
    //look for colliding edges
    labelHashSet collisionPointsSet(mesh.nPoints()/100);
    forAll(meshEdges, edgei)
    {
        ...
        vector uev = (newPoints[e[1]] - newPoints[e[0]])/eLen;
        scalar ePrj = 0;
        forAll(e,ei)
        {
            label meshpointi = e[ei];
            const labelList& pEdges = mesh.pointEdges()[meshpointi];
            forAll(pEdges, pEI)
            {
                label pedgei = pEdges[pEI];
                if (layerEdges[pedgei])
                {
                    const edge& le = mesh.edges()[pedgei];
                    label otherPt
                    (
                        le[0] == meshpointi ? le[1] : le[0]
                    );
                    point startPt = newPoints[meshpointi];
                    point outerPt = newPoints[otherPt];
                    vector layerVec = outerPt - startPt;
                    scalar pDist = (layerVec & uev);
                    if (ei == 0)
                    {
                        ePrj += pDist;
                    }
        ...
        if (ePrj > eLen)
        {
            collisionPointsSet.insert(e);
        }
    }

    pointSet collisionPoints
    (
        mesh,
        "collisionPoints",
        collisionPointsSet
    );
    collisionPoints.sync(mesh);
    ...
    forAll(surfNormals, pti)
    {
        ...
        forAll(pEdges, pEI)
        {
            ...
            if (layerEdges[edgei])
            {
                ...
                if
                (
                    !featurePts[meshpointi] && !gapPts[otherPt]
                    && !collisionPoints.found(meshpointi)
                )
                {
                    continue;
                }
                ...
                else
                {
                    ...
                    if (layerHeight > VSMALL)
                    {
                        ...
                        if (dProd >VSMALL)
                        {
                            ...
                            staticPts[otherPt] = true;
                            nMoved[otherPt]++;
                            newDisp[otherPt] += 0.5*(endPt-outerPt);
                        }
                    }
                }
             }
