    ...
    if (f.size() > 3)
    {
        ...
        if (f.size() - nExtruded >= 3 && nExtruded > 0)
        {
            ...
            scalar areaRatio = faceArea /
                ( mesh.magFaceAreas()[masterFaceI] + VSMALL);
...
)
{
    ...
    if (nAdded > 0)
    {
        ...
        for (label patchi = nOldPatches; patchi < nPatches; patchi++)
        {
            ...
            label procPatchi = meshRefiner_.addPatch
            (
                mesh,
                name,
                patchDict,
    ...
    forAll(patchIDs, i)
    {
        ...
        if (order.found("numLayers") && patchToNLayers[patchi])
        {
            if (order.found("expansionRatio"))
            {
                if
                (
                    order.found("finalLayerThicknessAbs")
                    || order.found("finalLayerThickness")
                )
                {
                    forAll(meshPoints, pointi)
                    {
                ...
                else if (order.found("fch") || order.found("rfch"))
                {
                    ...
                    bool relativeFCH(order.found("rfch") ? true : false);
                ...
                (
                    order.found("maxLayerThicknessAbs")
                    || order.found("maxLayerThickness")
                )
                {
                    bool relativeMLT = true;
                    if (order.found("maxLayerThicknessAbs"))
                    {
                        relativeMLT = false;
                    }
                    forAll(meshPoints, pointi)
            ...
            )
            {
                ...
                if (order.found("finalLayerThicknessAbs"))
                {
                    relativeFLT = false;
                }
                ...
                if (order.found("fch") || order.found("rfch"))
                {
                    ...
                    forAll(meshPoints, pointi)
                    {
                        ...
                        if (relativeFCH)
                        {
                            ratio = flt / (edgeLen*patchFCH[patchi]);
                        }
                        else
                        {
                            ratio = flt / (patchFCH[patchi]);
                        }
...
)
{
    ...
    label nAllowableErrors = motionSmoother::checkMesh
    (
        false,
        mesh,
        motionDict,
...

    {
        ...
        autoPtr<polyTopoChangeMap> map = meshRefiner_.dupNonManifoldPoints
        (
            regionSide
        );
    ...
    bool containsZoneLayers = false;
    if (returnReduce(pointToMaster.size(), sumOp<label>()))
    {
        containsZoneLayers = true;
    }
    ...
    // Print a bit
    {
        ...
        forAll(layerPatchIDs(), i)
        {
            ...
            if (totNPoints > 0)
            {
                std::tuple<scalar, scalar, label>
                   sums{sumThickness, sumNearWallThickness, sumNumLayers};
                reduce
                (
                    sums,
                    ParallelOp<sumOp<scalar>,
                    sumOp<scalar>,
                    sumOp<label>>{}
                );
...

    {
        ...
        while (true)
        {
            ...
            // Sync collapsed points to prevent issues with geometric
            // merging of zone baffles
            if (syncPts)
            {
                boolList resetPts(mesh.nPoints(), false);
                label nSyncReset = 0;
                forAll(pp(), facei)
                {
                    ...
                    if (zoneid != -1)
                    {
                        ...
                        forAll(f, fp)
                        {
                            ...
                            point oldPt = oldPoints[meshpointi];
                            point newPt = mesh.points()[meshpointi];
                            if
                            (
                                nPatchPointLayers[ptI] == 0
                                && mag(newPt-oldPt) > SMALL
                            )
                            {
                                nSyncReset++;
                                resetPts[meshpointi] = true;
                            }
                        }
                    }
                }

                if (returnReduce(nSyncReset, sumOp<label>()) > 0)
                {
                    syncTools::syncPointList
                    (
                        mesh,
                    ...
                    forAll(resetPts, pointi)
                    {
                        if (resetPts[pointi])
                        {
                            newPts[pointi] = oldPoints[pointi];
                        }
                    }
                    splitMod.movePoints(newPts);
            ...
            autoPtr<polyTopoChangeMap> splitMap = splitMod.makeMesh
            (
                newMeshPtr,
                IOobject
                (
            ...
            #if !defined( WIN32 ) && !defined( WIN64 )
            addProfiling(grow, "snappyHexMesh::layers::topoChange");
            #endif
            ...
            autoPtr<polyTopoChangeMap> map = meshMod.changeMesh(newMesh, false);
            ...
            if (finaliseLayers && containsZoneLayers && !syncPts)
            {
                syncPts = true;
            }
            else if (finaliseLayers)
            {
                ...
                if
                (
                    (layerParams.writeVTK() || layerParams.writeExtraVTK())
                    && controller_.mode() != meshControl::DRYRUN
                )
                {
                    ...
                    if (layerParams.writeExtraVTK())
                    {
                        const polyBoundaryMesh& patches =
                           newMesh.boundaryMesh();
                        scalarField facePatchPID(nPatchFaceSplit.size(), -1);
                        forAll(fchFaceSplit, i)
                        {
                            label patchi =
                               patches.whichPatch(ppSplit.addressing()[i]);
...

    {
        ...
        autoPtr<polyTopoChangeMap> mapSplit =
            savedSplitMod.changeMesh(mesh, false);

        // Update fields
        mesh.topoChange(mapSplit);
        ...
        meshRefiner_.topoChange(mapSplit, labelList(0));
...

    {
        ...
        autoPtr<polyTopoChangeMap> map = savedMeshMod.changeMesh
        (
            mesh,
            false,
            true,
        ...
        if (containsZoneLayers)
        {
            ...
            scalar mergeTol = 0.1*minEdgeLength;
            labelList oldToNew;
...
) const
{
    if (nLay == 1)
    {
        ...
        else if (a1 < (a2 + VSMALL) && a1 > (a2 - VSMALL))
        {
            return scalar(1);
        }
    }
