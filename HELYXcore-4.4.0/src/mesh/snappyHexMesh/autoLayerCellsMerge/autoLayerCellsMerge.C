...
)
{
    ...
    if (mesh.checkFacePyramids(true, -VSMALL, &errorFaces)) noFailedChecks++;
...
label Foam::autoLayerCellsMerge::topoChange
(
...
)
{
    ...
    {
        label own = owners[facei];
        label ownLayerID = layerCells[own];
...
)
{
    ...
    for (label i = 0; i < nMergeIter; i++)
    {
        ...
        {
            label nMerged = topoChange(markedCells);
