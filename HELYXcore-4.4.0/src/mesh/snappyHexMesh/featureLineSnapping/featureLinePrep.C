    ...
    dump_(false)
{
    ...
    {
       geometryFLs_ = dict.lookup<bool>("geometryFeatureLines");
    }
    if (dict.found("verboseFeatureLines"))
    {
       verbose_ = dict.lookup<bool>("verboseFeatureLines");
    }
    if (dict.found("dumpFeatureLineData"))
    {
       dump_ = dict.lookup<bool>("dumpFeatureLineData");
    }
}
...
void Foam::featureLinePrep::extractRegionBdries()
{
    ...
    forAll(surf, surfI)
    {
        ...
        if (isA<triSurfaceMesh>(geom))
        {
            ...
            regionFeatures_.set(sz, new edgeMesh(pts, edg));
            sz++;
        }
    }
    regionFeatures_.setSize(sz);
            ...
            if (pointEdges[pointI].size() != 2)
            {
                forAll(pointEdges[pointI], i)
                {
                    ...
                    if (edgeVisited.set(edgeI, 1u))
                    {
                        ...
                        featureLines_.append(featureLine(rPoints, -2.0));
