...
(
    ...
    nNoValidEndPts_(0),
    nFailedConstr_(0),
    nFilterCorrect_(snapDict.lookupOrDefault<label>("nFilterCorrect", 5)),
    excludedRegionPairs_(List<Tuple2<word, word>>(0))
{
    if (snapDict.found("verboseFeatureLines"))
    {
        verbose_ = snapDict.lookup<bool>("verboseFeatureLines");
    }
    if (snapDict.found("dumpFeatureLineData"))
    {
        dump_ = snapDict.lookup<bool>("dumpFeatureLineData");
    }
    if (snapDict.found("excludedRegionPatchPairs"))
    {
        snapDict.readIfPresent
        (
            "excludedRegionPatchPairs",
            excludedRegionPairs_
        );
    }
}
...
)
{
    ...
    for (int i = 0; i < nFilterCorrect_; i++)
    {
        if (!filterDisplacementField())
        {
            //Make sure zero point displacement synced
            syncTools::syncPointList
            (
                meshRefiner_.mesh(),
                pp_.meshPoints(),
...
) const
{
    labelList  nodeMfd;
    {
        if (allowSplits)
        {
            ...
            forAll(faceMfd,fI)
            {
                label facei = faceMfd[fI];
                const face &f = pp_.localFaces()[facei];
                label own = mesh.faceOwner()[pp_.addressing()[facei]];
                label cLevel = cellLevel[own];
                DynamicList<label> anchorPts(4);
                label pLevel = pointLevel[meshPoints[curPt]];
                if (pLevel <= cLevel)
                {
                    label nextFp = findIndex(f, curPt);
                    forAll(f,fp)
                    {
                        pLevel = pointLevel[meshPoints[f[nextFp]]];
                        if (pLevel <= cLevel)
                        {
                            anchorPts.append(f[nextFp]);
                        }
                        nextFp = f.fcIndex(nextFp);
                    }
                }
                label excludeAnchor1 = -1;
                label excludeAnchor2 = -1;
                if (anchorPts.size() == 4)
                {
                    label edgei = meshTools::findEdge
                    (
                        pp_.edges(),
                        pp_.pointEdges()[curPt],
                        curPt,
                        anchorPts[1]
                    );
                    if (edgei == -1)
                    {
                       excludeAnchor1 = anchorPts[1];
                    }
                    edgei = meshTools::findEdge
                    (
                        pp_.edges(),
                        pp_.pointEdges()[curPt],
                        curPt,
                ...
                forAll(f,fp)
                {
                    ...
                    (
                        pointi != curPt
                        && pointi != excludeAnchor1
                        && pointi != excludeAnchor2
                    )
                    {
                        rawNodes.append(pointi);
                    }
                }
            }
            ...
            nodeMfd.setSize
            (
                label(std::unique(nodeMfd.begin(),nodeMfd.end())
                                  - nodeMfd.begin())
            );
        }
        else
...
(
    ...
    const boolList& excludedRegionPts,
            ...
            while (found)
            {
                ...
                if (currentHit.hit())
                {
                    ...
                    forAll(projResults,pI)
                    {
                        ...
                        scalar ratio = ftrLength / (edgeLength + VSMALL);
...
void Foam::featureLineSnapper::buildShadowChains()
{
    ...
    boolList excludedRegionPts(pp_.meshPoints().size(), false);
    if (excludedRegionPairs_.size() > 0)
    {
        Info<< "Excluding inter region patches "<<endl;
        const polyBoundaryMesh& patches = mesh.boundaryMesh();
        List<labelList> edgePatches(mesh.nEdges(),labelList(0));
        ...
        boolList excludedPts(pp_.meshPoints().size(), false);
        forAll(meshEdges, edgei)
        {
            label meshedgei = meshEdges[edgei];
            labelList ePID = edgePatches[meshedgei];

            if (ePID.size() == 2)
            {
                word patchname0 = patches[ePID[0]].name();
                word patchname1 = patches[ePID[1]].name();
                forAll(excludedRegionPairs_,pairi)
                {
                    word exclname0 = excludedRegionPairs_[pairi].first();
                    word exclname1 = excludedRegionPairs_[pairi].second();
                    if
                    (
                        (exclname0 == patchname0 && exclname1 == patchname1)
                        || (exclname0 == patchname1 && exclname1 == patchname0)
                    )
                    {
                        const edge& e = pp_.edges()[edgei];
                        excludedPts[e[0]] = true;
                        excludedPts[e[1]] = true;
                    }
                }
            }
        }
        ...
        forAll(excludedPts, pti)
        {
            if (excludedPts[pti])
            {
                const labelList& pFaces = pp_.pointFaces()[pti];
                forAll(pFaces, pfi)
                {
                    label facei = pFaces[pfi];
                    const face& lf = pp_.localFaces()[facei];
                    forAll(lf,fp)
                    {
                       excludedRegionPts[lf[fp]] = true;
                    }
                }
            }
        }
        ...
        if (minStartPt != -1 && minEndPt != -1)
        {
            ...
            scalar curFtrDist = 0.0;
            {
                ...
                if (hitf.hit())
                {
                    ...
                    label nearestFtrEdge = hitf.index();
                    scalar ftrDist = mag(flp - featurel.point(nearestFtrEdge))
                       + flParams[nearestFtrEdge];
                    curFtrDist = ftrDist;
                }
                else
            ...
            if (foundNext == 1)
            {
...

                {
                    ...
                    if (mag(allDisp[minEndPt]) > VSMALL)
                    {
            ...
            else if (foundNext == 2)
            {
...

                {
                    ...
                    if (hitf.hit())
                    {
                        ...
                        label nearestFtrEdge = hitf.index();
                        scalar ftrDist =
                           mag(flp - featurel.point(nearestFtrEdge))
                           + flParams[nearestFtrEdge];
                        curFtrDist = ftrDist;
                    }
                    else
    ...
    forAll(rawShadows, chainI)
    {
       ...
       forAll(nchain, i)
       {
          if (mag(allDisp[nchain[i]]) > VSMALL)
          {
...
) const
{
    ...
    forAll(nodeMfd,i)
    {
        if (nodeMfd[i] == endPt)
        {
            ...
            scalar length = flParams[flParams.size()-1] + VSMALL;
    ...
    DynamicList<nodeMfdProjResult> projResults(nodeMfd.size());
    ...
    //line
    pointIndexHit curHitRaw = ftrLineTree.findNearest
    (
        pp_.localPoints()[curPt],GREAT
    );
    ...
    forAll(nodeMfd,i)
    {
        ...
        pointIndexHit nbrHitRaw = ftrLineTree.findNearest
        (
            cPoint,GREAT
        );
        ...
        if
        (
           mag(curHitRaw.hitPoint()-nbrHitRaw.hitPoint())
           < 0.15*lengthAve[curPt]
        )
        {
            continue;
        }

        if (mag(allDisp[nodeMfd[i]]) > VSMALL)
        {
        ...
        point curPtHit  = pp_.localPoints()[curPt] + allDisp[curPt];
        if (mag(curPtHit-hit.hitPoint()) < 0.15*lengthAve[curPt])
        {
            continue;
        }
        ...
        projResults.append(nodeMfdProjResult(i,distSqr,hit));
    }
                ...
                )
                {
                    ...
                    else
                    {
                         ...
                         )
                        {
                            ...
                            {
                                fNorm /= (faceArea + VSMALL);
                                nNorm /= (newFaceArea + VSMALL);
                                ...
                                if ((fNorm & nNorm) > 0.965)
                                {
                                    ...
                                    (
                                        mag
                                        (ftrHitPoint - pp_.localPoints()[curPt])
                                        < lengthAve[curPt]
                                    )
                                    {
                                        allDisp[curPt] =  ftrHitPoint
    ...
    forAll(shadowChains_,scI)
    {
        if (shadowChains_[scI].size() > 0)
        {
            ...
            if (!chainIsLoop || chain.size() > 5)
            {
                ...
                //node n-1 is basically just a dummy that's equal to node 0
                //so it's skipped in these checks).
                if (chainIsLoop)
                {
...
void Foam::featureLineSnapper::smoothChains()
{
    //Look at neighboring edges sharing a face and average central point
    forAll(shadowChains_, scI)
    {
        if (!shadowChainSurfBound_[scI])
        {
            continue;
        }
        const labelList &chain = shadowChains_[scI];
        for (label pI=1; pI < chain.size()-1; ++pI)
        {
            const label v0 = chain[pI-1];
            const label v1 = chain[pI];
            const label v2 = chain[pI+1];

            label edge0 = meshTools::findEdge
            (
                pp_.edges(),
                pp_.pointEdges()[v0],
                v0,
            ...
            label edge1 = meshTools::findEdge
            (
                pp_.edges(),
                pp_.pointEdges()[v1],
                v1,
            ...
            if (edge0 != -1 && edge1 != -1)
            {
                const labelList& eFaces0 = pp_.edgeFaces()[edge0];
                const labelList& eFaces1 = pp_.edgeFaces()[edge1];
                std::list<label> intx;
                ...
                if (numCommon > 0)
                {
                    point pt0 = pp_.localPoints()[v0] + pointDisp_[v0];
                    point pt1 = pp_.localPoints()[v1] + pointDisp_[v1];
                    point pt2 = pp_.localPoints()[v2] + pointDisp_[v2];

                    vector eVec0 = pt1 - pt0;
                    vector eVec1 = pt2 - pt1;

                    scalar magVec0 = mag(eVec0);
                    scalar magVec1 = mag(eVec1);

                    if (magVec0 > VSMALL && magVec1 > VSMALL)
                    {
                        eVec0 /= magVec0;
                        eVec1 /= magVec1;
                        scalar minEdgeLen = min(magVec0,magVec1);
                        scalar maxEdgeLen = max(magVec0,magVec1);

                        scalar eRatio = minEdgeLen/maxEdgeLen;
                        scalar dProd = (eVec0 & eVec1);
                        if (dProd > 0.939 && eRatio < 0.66)
                        {
                            point avePoint = 0.5*(pt0+pt2);
                            pointDisp_[v1] = avePoint - pp_.localPoints()[v1];
                        }
                     }
                }
            }
 ...
 */
bool Foam::featureLineSnapper::filterDisplacementField()
{
    ...
    // which are not mesh region boundaries
    {
        ...
        boolList nonFtrPts(meshPoints.size(), false);
        forAll(eType, edgei)
        {
            if (eType[edgei].first() == edgeClassification::MANIFOLD)
            {
                edge e = pp_.edges()[edgei];
                nonFtrPts[e[0]] = true;
                nonFtrPts[e[1]] = true;
            }
        }
        ...
        boolList resetDisp(meshPoints.size(), false);
        ...
        forAll(shadowChains_, scI)
        {
           ...
           for (label pI=0; pI < chain.size()-1; ++pI)
           {
              ...
              if (edgei != -1)
              {
                  ...
                  scalar magv0v1m = mag(v0v1m) + VSMALL;
                  v0v1m /= magv0v1m;

                  if ((v0v1 & v0v1m) < 0.9396)
                  {
            ...
            for (label pI=0; pI < chain.size()-1; ++pI)
            {
                ...
                if (edgei == -1)
                {
                    ...
                    if (numCommon == 1)
                    {
                        ...
                        forAll(fEdges, fp)
                        {
                            label fedgei = fEdges[fp];
                            const edge& e = pp_.edges()[fedgei];
                            const label mp0 = meshPoints[e[0]];
                            const label mp1 = meshPoints[e[1]];
                            ...
                            if (edgeRegions[faceMeshEdgeI].size() > 1)
                            {
                ...
                else
                {
                    ...
                    forAll(eFaces, efi)
                    {
                        label meshfacei = pp_.addressing()[eFaces[efi]];
                        const labelList faceEdges =
                            mesh.faceEdges()[meshfacei];
                        forAll(faceEdges, fei)
                        {
                            if (edgeRegions[faceEdges[fei]].size() > 1)
                            {
                               foundMultiRegion = true;
                            }
                        }
                ...
                if (shadowChainSurfBound_[scI])
                {
                    if (!foundMultiRegion || convexEdge)
                    {
                        resetEdge = true;
                    }
                }
                ...
                if (resetEdge)
                {
                    resetDisp[chain[pI]] = true;
                    resetDisp[chain[pI+1]] = true;
                }
            }
        }

        scalar validScale = 0.8;
        if (!dualMesh_ && allowQuadSplits_)
        {
            validScale = 0.6;
        }
        const scalar validConvexScale = 1.5;
        ...
        boolList convexPts(meshPoints.size(), false);
        ...
        forAll(meshPoints, pointi)
        {
            if (!convexPts[pointi])
            {
                bool validDisp = mag(pointDisp_[pointi]) <
                    validScale*lengthAve[pointi];
                if (!validDisp)
                {
                    resetDisp[pointi] = true;
                }
            }
        }
        ...
        forAll(pp_, facei)
        {
            ...
            vector fDir0 = pp_.faceAreas()[facei];
            scalar fMag0 = mag(fDir0);
            ...
            vector fDir1 = fNew.faceArea(newFacePts);
            scalar fMag1 = mag(fDir1);
            scalar aRatio = fMag1 / (fMag0 + VSMALL);

            if (aRatio < 0.2 || (fDir0 & fDir1) < 0)
            {
                forAll(f,fp)
                {
                    label pointi = f[fp];
        ...
        //loop over points looking for any points that have moved
        //through a surface
        ...
        forAll(pp_.localPoints(),pI)
        {
            if (mag(pointDisp_[pI]) > VSMALL)
            {
                const labelList& pFaces = pp_.pointFaces()[pI];
                // choose one cc in the domain
                label facei = pp_.addressing()[pFaces[0]];
                const label owningCell = meshRefiner_.mesh().faceOwner()[facei];
        ...
        forAll(start, pI)
        {
            ...
            if (surface1[pI] != -1)
            {
                ...
                scalar d1 = mag(vec1);
                scalar d2 = mag(vec2);
                scalar otherSideTol(nonFtrPts[i] ? 0.94 : 0.98);
                if ((d2 / (d1 + VSMALL)) < otherSideTol)
                {
                    resetDisp[i] = true;
                }
            }
        ...
        //loop over edges looking for any that are too short as
        ...
        forAll(pp_.edges(),eI)
        {
            ...
            {
                nShortEdges++;
                resetDisp[pp_.edges()[eI][0]] = true;
                resetDisp[pp_.edges()[eI][1]] = true;
            }
        }

        if (nShortEdges && verbose_)
        {
            Pout<< " Reset " << nShortEdges
                << " edges due to excessive shrinking" << endl;
        }
        ...
        forAll(resetDisp, i)
        {
            if (resetDisp[i] && mag(pointDisp_[i]) > VSMALL)
            {
        ...
        Info<<"Filtering " << nReset << " points."  <<endl;
...
)
{
    ...
    forAll(points,j)
    {
        ...
        unitEdges[j] /= (mag(unitEdges[j]) + VSMALL);
    }
    //find smallest dp
    scalar minDp = 1.0;
    forAll(unitEdges,j)
...
)
{
    ...
    forAll(shadowChains_,scI)
    {
        ...
        for (label pI=0; pI < chain.size()-1; ++pI)
        {
            ...
            if (allowQuadSplits_ && !isEdge(lPt1,lPt2))
            {
                ...
                if
                (
                    mag(pointDisp_[lPt1]) < VSMALL
                    || mag(pointDisp_[lPt2]) < VSMALL
                )
                {
                    displacedEdge = false;
                }
                ...
                )
                {
                    ...
                    )
                    {
                        ...
                        scalar areaRatio = faceArea /
                            ( mesh.magFaceAreas()[origID] + VSMALL);
    ...
    autoPtr<polyTopoChangeMap> mapSplit = firstMeshMod.changeMesh
    (
        mesh,
        false,
        true
    ...
    autoPtr<polyTopoChangeMap> mapReverse = meshMod.changeMesh
    (
        meshRefiner_.mesh(),
        false,
        true
...
) const
{
    forAll(shadowChains_,scI)
    {
        ...
        forAll(chain,pI)
        {
            ...
            if (mag(pointDisp_[chain[pI]]) > VSMALL)
            {
...
void Foam::featureLineSnapper::topoChange
(
