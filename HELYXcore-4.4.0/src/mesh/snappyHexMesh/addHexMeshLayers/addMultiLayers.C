...
|   o   O   o    |  Version : 4.4.0
|    o     o     |  Copyright © 2016 ENGYS Ltd. <http://engys.com/>
|       o        |
\*---------------------------------------------------------------------------
...
) const
{
    ...
    const List<Switch> medialCollapse = layerParams_.medialCollapse();
    ...
    scalar medialCollapseAngle = layerParams_.medialCollapseCos();
    scalar medialCollapseRatio = layerParams_.medialCollapseRatio();

    bool tryMedialCollapse = false;
    if (medialCollapseAngle > -1)
    {
        forAll(medialCollapse, patchi)
        {
            if (medialCollapse[patchi])
            {
                tryMedialCollapse = true;
                break;
            }
        }
    }

    if (tryBaffleCollapse || tryCornerCollapse || tryMedialCollapse)
    {
        boolList collapsedPts(pp.meshPoints().size(),false);
        ...
        {
            Info<<"Checking for corner/baffle edges to collapse layers on"
                <<endl;

            boolList checkEdges(meshEdges.size(), false);
            ...
            forAll(meshEdges, edgei)
            {
               ...
               )
               {
                   ...
                   collapsedPts[e[0]] = true;
                   collapsedPts[e[1]] = true;
               }
            }
            ...
            forAll(meshPoints, pointi)
            {
                if
                (
                    (nConvexEdges[pointi] == 2 && nConcaveEdges[pointi] == 2)
                    ||
                    (
                        nConvexEdges[pointi] == 1 && nConcaveEdges[pointi] > 0
                        && nBaffleEdges[pointi] == 1
                    )
                )
                {
                    const labelList& pEdges = pp.pointEdges()[pointi];
        ...
        // Collapse based on proximity to medial axis
        if (tryMedialCollapse)
        {
            Info<<"Checking for medial points to collapse layers on"
               <<endl;
            ...
            pointField unsmoothedPointNormals =
            ...
            forAll(meshPoints, pointi)
            {
                ...
                forAll(pFaces, pfi)
                {
                    ...
                    ppPtLevel[pointi] = max(cellLevel[own],ppPtLevel[pointi]);
                }
            }
            ...
            forAll(meshPoints, pointi)
            {
                ...
                scalar pLen = edge0Length / (1<<ppPtLevel[pointi]);
            ...
            forAll(edges, edgei)
            {
                ...
                )
                {
                    ...
                    if (eMag > VSMALL)
                    {
                        ...
                        scalar dist0 =
                            (p0-pointWallDist[e[0]].origin()) & eVec;
                        scalar dist1 =
                            (pointWallDist[e[1]].origin()-p1) & eVec;
            ...
            forAll(meshPoints, pointi)
            {
                ...
                {
                    scalar pLen = pointMedialDist[meshpointi].s();
                    scalar mDist =
                        Foam::sqrt(pointMedialDist[meshpointi].distSqr());
                    if (mDist < medialCollapseRatio*pLen)
                    {
                        collapsedPts[pointi] = true;
                    }
                }
            }
        }
...
void Foam::addMultiLayers::handleWrongOrientedFaces()
{
    ...
    mesh.checkFacePyramids(true, -VSMALL, &errorFaces);

    labelList neiLayerCells;
    ...
    forAll(mesh.faces(), facei)
    {
        ...
        label ownLayerCell = layerCells[owners[facei]];
        ...
        forAll(cutEdges, edgei)
        {
            if (cutEdges[edgei])
            {
                ...
                forAll(eFaces, eFI)
                {
                    ...
                    label ownLayerCell = layerCells[own];
                    label neiLayerCell = -1;
...
void Foam::addMultiLayers::reorientateFaces()
{
    ...
    if (nOpen > 1)
    {
        ...
        forAll(mesh.faces(), faceI)
        {
            ...
            label ownLayerCell = layerCells[own[faceI]];
            bool neiOpen = false;
            label neiLayerCell = -1;
                            ...
                            for (label stackI=0; stackI < nLay; ++stackI)
                            {
                                ...
                                if (zoneID >= 0)
                                {
                                   const faceZone& fZone =
                                      mesh.faceZones()[zoneID];
                                   zoneFlip =
                                      fZone.flipMap()[fZone.whichFace(sfacei)];
                                }
...
)
{
    ...
    while (true)
    {
        ...
        forAll(pp, facei)
        {
            if (ppLayers[facei] > 1 || (ppLayers[facei] == 1 && incrementLower))
            {
