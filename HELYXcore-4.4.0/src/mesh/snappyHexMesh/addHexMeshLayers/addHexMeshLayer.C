        ...
        forAll(edgeLoop, eLI)
        {
            ...
            forAll(pEdgesStart, pEI)
            {
                ...
                if (boundaryPts[otherPt])
                {
                    ...
                    eDirStart /= (mag(eDirStart) + VSMALL);
                    break;
                }
            }
...
void Foam::addHexMeshLayer::removeProblemCells()
{
    ...
    autoPtr<polyTopoChangeMap> map = meshRefiner_.doRemoveCells
    (
        cellsToRemove,
        exposedFaces,
        exposedPatchIDs,
        ...
        //Cut cells
        {
            ...
            if (trackLayerCells_)
            {
                ...
                forAll(map().cellMap(), cellI)
                {
                    ...
                    {
                        label originalCell = map().cellMap()[cellI];
                        label layerID = layerCells[originalCell];
                        if (layerID > -1)
                        {
...
void Foam::addHexMeshLayer::topoChange(const polyTopoChangeMap& map)
{
