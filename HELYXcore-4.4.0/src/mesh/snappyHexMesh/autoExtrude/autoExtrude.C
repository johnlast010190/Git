...
)
{
    ...
    if (edict.found("fch"))
    {
        scalar extrudeFCH = edict.lookup<scalar>("fch");
...
)
{
    ...
    {
        useInputFCH = true;
        extrudeFCH  = edict.lookup<scalar>("fch");
    }
...
)
{
    ...
    if (nSourceRegions == nTargetRegions)
    {
        ...
        treeCombinedBb = treeCombinedBb.extend(1e-4);
...
void Foam::autoExtrude::removeUnusedPoints()
{
    ...
    forAll(mesh.points(), pointI)
    {
        if (nPointEdges[pointI] == 2 && nPointFaces[pointI] > 2)
        {
            removedPts[pointI] = true;
        }
    }
    ...
    forAll(mesh.faces(), facei)
    {
        ...
        {
            Pout
                << "Trying to remove unused points but face "
                << " at location : " << mesh.faceCentres()[facei]
                << " after removal will leave a face with less than 3 points."
                << nl << endl;
            forAll(f, fp)
            {
                removedPts[f[fp]] = false;
            }
        }
    }
    ...
    forAll(mesh.points(), pointi)
    {
        if (removedPts[pointi])
        {
            nPointsRemoved++;
        }
    }

    if (returnReduce(nPointsRemoved, sumOp<label>()) > 0)
    {
...
)
{
    ...
    //Use existing cellZones if required
    Switch existingCellZones =
        edict.lookupOrDefault<Switch>("useExistingCellZones", false);
    if (existingCellZones)
    {
        forAll(extrudePatch, i)
        {
            label celli = mesh.faceOwner()[extrudePatch.addressing()[i]];
            label zonei = mesh.cellZones().whichZone(celli);
            cellZoneSource[i] = zonei;
        }
    }
    else if (edict.found("cellZone"))
    {
...
)
{
    ...
    forAllConstIter(dictionary, extrudeDict, iter)
    {
        ...
        if (action != extAction::ADD)
        {
            ...
            // Change the mesh. No inflation.
            autoPtr<polyTopoChangeMap> map = swapMeshMod.changeMesh(mesh, false);
        ...
        addZones(extrudePatch, edict, cellZoneSource, faceZoneSource);
...
void Foam::autoExtrude::cleanupFaceZones()
{
    ...
    if (nFaceZones > 0)
    {
        ...
        {
            // Change the mesh. No inflation.
            autoPtr<polyTopoChangeMap> zoneMap =
                zoneMeshMod.changeMesh(mesh, false);

            // Update fields
            mesh.topoChange(zoneMap);
