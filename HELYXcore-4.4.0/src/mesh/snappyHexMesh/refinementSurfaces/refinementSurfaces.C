    ...
    reorient_(refineDict.lookupOrDefault<Switch>("reorient",false))
{
    ...
    if (refineDict.found("featureRefineAngle"))
    {
        scalar featRefineAngle =
            refineDict.lookup<scalar>("featureRefineAngle");
    ...
    List<Tuple2<scalar,label>> featureLevels = List<Tuple2<scalar,label>>
    (
       1,
       Tuple2<scalar,label>(0.0,-1)
    );

    if (refineDict.found("featureLevels"))
    {
        featureLevels =
            refineDict.lookup<List<Tuple2<scalar,label>>>("featureLevels");
    }
    List<List<Tuple2<scalar,label>>> globalFeatureLevels
    (
        surfI,
        featureLevels
    );
    ...
    {
        scalar angle =
            refineDict.lookup<scalar>("minBaffleAngle");
    ...
    List<Map<List<Tuple2<scalar,label>>>> regionFeatureLevels(surfI);
    ...
    forAll(allGeometry_.names(), geomI)
    {
        ...
        if (ePtr)
        {
            ...
            {
                scalar featRefineAngle =
                    dict.lookup<scalar>("featureRefineAngle");
                if (featRefineAngle < 0 || featRefineAngle > 180)
                {
            ...
            if (dict.found("featureLevels"))
            {
                List<Tuple2<scalar,label>> featLevels =
                    dict.lookup<List<Tuple2<scalar,label>>>("featureLevels");
                globalFeatureLevels[surfI] = featLevels;
            }
            ...
            {
                scalar angle =
                    dict.lookup<scalar>("minBaffleAngle");
                if (angle < 0 || angle > 180)
                {
            ...
            if (dict.found("regions") && !singleRegion_[surfI])
            {
                ...
                forAll(regionNames, regionI)
                {
                    if (regionsDict.found(regionNames[regionI]))
                    {
                        ...
                        if (regionDict.found("featureLevels"))
                        {
                            List<Tuple2<scalar,label>> featLevels =
                               regionDict.lookup<List<Tuple2<scalar,label>>>
                               ("featureLevels");

                            regionFeatureLevels[surfI].insert
                            (
                                regionI,
                                featLevels
                            );
                        ...
                        if (regionDict.found("curvature"))
                        {
                            ...
                            (
                                regionDict.lookup<scalar>("curvature")
                            );
    ...
    featureRefineLevels_.setSize(nRegions);
    featureRefineLevels_ = List<Tuple2<scalar,label>>
    (
        1,Tuple2<scalar,label>(0.0,-1)
    );
    ...
    forAll(globalMinLevel, surfI)
    {
        ...
        for (label i = 0; i < nRegions; i++)
        {
            ...
            featureRefineLevels_[globalRegionI] = globalFeatureLevels[surfI];
        ...
        typedef List<Tuple2<scalar,label>> featLevels;
        forAllConstIter(Map<featLevels>, regionFeatureLevels[surfI], iter)
        {
            label globalRegionI = regionOffset_[surfI] + iter.key();

            featureRefineLevels_[globalRegionI] =
                regionFeatureLevels[surfI][iter.key()];
        }
...
void Foam::refinementSurfaces::findHigherCurvature
(
    const label surfI,
    const searchableSurface& geom,
    const List<pointIndexHit>& intersectionInfo,
...
) const
{
    ...
    {
        pointField hitPts(intersectionInfo.size());
        scalarField hitRadSqr(intersectionInfo.size());
        labelList hitMap(intersectionInfo.size());
        label nSet = 0;
        forAll(intersectionInfo, i)
        {
            if (intersectionInfo[i].hit())
            {
                scalar cellLength = edge0Len/(1<<currentLevel[i]);
                scalar dTheta = curvatureField[i] * cellLength;
                scalar regionCurvature =
                   curvature(surfI, region[i]);
                if (regionCurvature > 0)
                {
                    if (dTheta > regionCurvature)
                    {
                    ...
                    else if (currentLevel[i] > surfaceOnlyLevel[i])
                    {
                        scalar radius = 0.866*cellLength;
                        hitPts[nSet] = intersectionInfo[i].hitPoint();
                        hitRadSqr[nSet] = radius*radius;
                        hitMap[nSet] = i;
                        nSet++;
                    }
                }
        ...
        hitPts.setSize(nSet);
        hitRadSqr.setSize(nSet);
        hitMap.setSize(nSet);
        scalarField maxCurvature(hitPts.size(), -GREAT);
        if (isA<triSurfaceMesh>(geom))
        {
            triSurfaceMesh& triSurf = const_cast<triSurfaceMesh&>
            (
                refCast<const triSurfaceMesh>(geom)
            );

            triSurf.findHighCurvature
            (
                hitPts,
                hitRadSqr,
                maxCurvature
            ...
            forAll(hitMap, hiti)
            {
                ...
                scalar regionCurvature =
                    curvature(surfI, region[i]);
                scalar cellLength = edge0Len/(1<<currentLevel[i]);
                if (cellLength*maxCurvature[hiti] > regionCurvature)
                {
                    label maxCurvatureLevel = min
                    (
                        currentLevel[i]+1,
        ...
        labelList surfaceOnlyLevel(start.size(), -1);
        {
            ...
            if (checkCurvature) //TODO threaded
            {
                ...
                (
                    ...
                    intersectionInfo,
                    region,
                    currentLevel,   // current cell refinement level
                    edge0Len,
                    surfaceOnlyLevel
                );
            } //if checkCurvature
        ...
        labelList surfaceOnlyLevel(intersectionInfo.size(), -1);
        {
            ...
            if (checkCurvature) //TODO threaded
            {
                ...
                (
                    ...
                    intersectionInfo,
                    region,
                    UIndirectList<label>(currentLevel, intersectionToPoint)(),
                    edge0Len,
                    surfaceOnlyLevel
                );
            } // if (checkCurvature)
...
) const
{
    ...
    forAll(nearest, pointI)
    {
        if (hit1[pointI].hit())
        {
            point hPt = hit1[pointI].hitPoint();
            nearest[pointI] = hPt;
            if (namedIntersections)
            {
                nearest[pointI] += 1e-3*(start[pointI] - hPt);
            }
        }
        else
        {
