...
namespace Foam
{
    defineTypeNameAndDebug(GIBAreaSmoothing, 0);
}


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

Foam::labelList Foam::GIBAreaSmoothing::labelListPatch(const label& masterID)
{
...
void Foam::GIBAreaSmoothing::smooth()
{
    scalar GsmoothedMin;
    scalar GsmoothedMax;
    ...
    reduce
    (
        std::tie(GsmoothedMax, GsmoothedMin),
        ParallelOp<maxOp<scalar>, minOp<scalar>>{}
    );
    ...
    if (debug)
    {
        ...
        simpleVTKWriter bla = simpleVTKWriter(pp, pp.points());
        ...
        bla.write(word("Test_yy") + Gsource.mesh().time().timeName() + ".vtk");
    }
}


void Foam::GIBAreaSmoothing::createFAFields()
{
...
void Foam::GIBAreaSmoothing::createFAMesh()
{
    ...
    labelList meshEdges =
        patch.meshEdges(mesh_.edges(), mesh_.cellEdges(), faceCells);
    ...
    const indirectPolyPatch& gibPolyPatch =
        refCast<const indirectPolyPatch>(mesh_.boundary()[patchID_].patch());
    ...
    // Sync edges that belong to proc patch but in the finite area mesh
    // are boundary edges
    ...
    // Reorder processorFaPatch using ordering of neighbour processorPolyPatch
        ...
        if (faPatches[patchI].type_ == processorFaPatch::typeName)
        {
            ...
            for (label i = 0; i < reorderedEdgeLabels.size(); i++)
            {
                reorderedEdgeLabels[i] =
                    faPatches[patchI].edgeLabels_[sortedNgbFaces.indices()[i]];
            }
    ...
    forAll(faPatchLst, pI)
    {
        ...
        if (faPatches[pI].type_ == processorFaPatch::typeName)
        {
            const processorPolyPatch& procPatch =
                ...
                (
                    mesh_.boundaryMesh()[faPatchID[pI]]
                );
        ...
        {
            faPatches[pI].dict_.add("ngbPolyPatchIndex", 100001);
        }
        else
        {
            faPatches[pI].dict_.add("ngbPolyPatchIndex", 100000);
        }
...
Foam::labelList Foam::GIBAreaSmoothing::findIsolatedAreas()
{
    ...
    forAll(markedEdges, eI)
    {
        if (markedEdges[eI])
        {
            ...
            initialEdgesInfo.append(patchEdgeFaceBoolInfo(true));
        }
    }
    ...
    // Walk
    PatchEdgeFaceWave<indirectPrimitivePatch, patchEdgeFaceBoolInfo> calc
    (
        mesh_,
        gibPolyPatch,
        initialEdges,
...
void Foam::GIBAreaSmoothing::setSmoothingOptions(const dictionary& dict)
{
    r_ = dimensionedScalar("radius", dimLength, dict.lookup("radius"));
    standardDeviation_ = dict.lookupOrDefault("Sigma_deviation", 3);
    cutOffAngle_ = dict.lookupOrDefault<scalar>("cut_off_angle", 180);
    smoothTransition_ = dict.lookupOrDefault<bool>("smoothTransition", true);
}


void Foam::GIBAreaSmoothing::clearOutData()
{
...
Foam::GIBAreaSmoothing::GIBAreaSmoothing
(
    const fvMesh& mesh,
...
:
    mesh_(mesh),
    areaMesh_(mesh_, labelListPatch(masterID)),
    patchID_(masterID),
    constraintFaces_(constraintFaces),
    r_("radius", dimLength, dict.lookup("radius")),
    standardDeviation_(dict.lookupOrDefault("Sigma_deviation", 3)),
    cutOffAngle_(dict.lookupOrDefault<scalar>("cut_off_angle", 180)),
    smoothTransition_(dict.lookupOrDefault<bool>("smoothTransition", true)),
...
void Foam::GIBAreaSmoothing::update()
{
    smooth();
}


Foam::tmp<Foam::scalarField> Foam::GIBAreaSmoothing::smoothSens() const
{
