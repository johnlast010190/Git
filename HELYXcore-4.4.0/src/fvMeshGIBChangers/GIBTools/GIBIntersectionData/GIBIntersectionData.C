...
void Foam::GIBIntersectionData::faceCellFaceIntersections
(
    intersectionData& iD,
    LIFOStack<label>& lst,
...
{
    while ((lst.size()!=0) && (!iD.hitWall()) && (!iD.hitProc()))
    {
        label fI = lst.top();
        ...
        if (faceInters.hit())
        {
            ...
            if (((dis1 & dis) > 0) && (magDis1 < magDis))
            {
                ...
                {
                    label patchI = mesh_.boundaryMesh().whichPatch(fI);
                    ...
                    else if (mesh_.boundary()[patchI].coupled())
                    {
                        ...
                        label neiProcPatchID = proPolyPatch.neighbProPatchID();
...
void Foam::GIBIntersectionData::intersectionChecking()
{
    while (doLoop())
    {
...
void Foam::GIBIntersectionData::sendPointsToProcessors
(
...
void Foam::GIBIntersectionData::receiveAndUpdateData
(
...
)
{
    ...
    forAll(ginterData, procI)
    {
        ...
        forAll(ginterDataI, pI)
        {
            ...
            if (ginterDataII.fromProc() == Pstream::myProcNo())
            {
...
bool Foam::GIBIntersectionData::doLoop()
{
    int sumPoints = 0;
...
Foam::GIBIntersectionData::GIBIntersectionData
(
    const fvMesh& mesh,
...
void Foam::GIBIntersectionData::modifyPointsPassingBoundary
(
