...
namespace Foam
{
    defineTypeNameAndDebug(GIBMapping, 0);
}


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::GIBMapping::storeFields()
{
    mapSerial();
}


void Foam::GIBMapping::mapSerial()
{
    ...
    StoreOldFieldsToPatch<scalar, fvsPatchField, surfaceMesh>
    (
        gmfvsScalars_,
        mId
    );
    ...
    StoreOldFieldsToPatch<vector, fvsPatchField, surfaceMesh>
    (
        gmfvsVectors_,
        mId
    );
    ...
    StoreOldFieldsToPatch<tensor, fvsPatchField, surfaceMesh>
    (
        gmfvsTensors_,
        mId
    );
    ...
    StoreOldFieldsToPatch<sphericalTensor, fvPatchField, volMesh>
    (
        gmfvSpTensors_,
        mId
    );
    StoreOldFieldsToPatch<sphericalTensor, fvsPatchField, surfaceMesh>
    (
        gmfvsSpTensors_,
        mId
    );
    ...
    StoreOldFieldsToPatch<symmTensor, fvPatchField, volMesh>
    (
        gmfvSymmTensors_,
        mId
    );
    StoreOldFieldsToPatch<symmTensor, fvsPatchField, surfaceMesh>
    (
        gmfvsSymmTensors_,
        mId
    );
...
void Foam::GIBMapping::combinePolyPatch()
{
    ...
    (
        GIBChanger_.mesh().boundary()[GIBChanger_.slaveId()]
    );
...
void Foam::GIBMapping::mapMaster()
{
    const label& mId = GIBChanger_.masterId();
    ...
    (
        ...
        (
            GIBChanger_.mesh().boundary()[mId].patch()
        )
    );
    ...
    const AMIInterpolation& gibmasterIn = masterAMIInter();
...
void Foam::GIBMapping::mapSlave()
{
    ...
    (
        refCast<const indirectPolyPatch>
        (
            GIBChanger_.mesh().boundary()[sId].patch()
        )
    );
    ...
    const AMIInterpolation& gibslaveIn = slaveAMIInter();
...
void Foam::GIBMapping::mapNonOverlappingTargetFaces()
{
    {
...
void Foam::GIBMapping::makeMasterAMIInterpolation() const
{
    if (mAMIInterPtr_)
    {
...
Foam::labelList Foam::GIBMapping::calcNearestPatchFaceMapping
(
...
) const
{
    ...
    (
        treeBoundBox(sourcePm.points(), sourcePm.meshPoints()).extend(1e-4)
    );
...
void Foam::GIBMapping::makeSlaveAMIInterpolation() const
{
    if (sAMIInterPtr_)
    {
        FatalErrorInFunction << abort(FatalError);
...
void Foam::GIBMapping::clearOutGIBData()
{
...
Foam::GIBMapping::GIBMapping
(
    const fvMeshGIBChangersBase& GIBChanger,
...
void Foam::GIBMapping::mapNonOverlapFaces
(
    const labelList& dNonOverFaces,
...
Foam::triSurface Foam::GIBMapping::triS()
{
    pointField gibPoints = gppPoints_;
    faceList gibfaces = gppFaces_;

    labelList allZones(gppFaces_.size(), GIBChanger_.slaveId());
