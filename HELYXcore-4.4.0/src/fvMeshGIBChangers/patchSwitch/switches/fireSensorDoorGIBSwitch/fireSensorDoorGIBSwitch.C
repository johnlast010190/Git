...
#include "patchSwitch/switches/fireSensorDoorGIBSwitch/fireSensorDoorGIBSwitch.H"
...
{
    defineTypeNameAndDebug(fireSensorDoorGIBSwitch, 0);
    addToRunTimeSelectionTable(GIBSwitch, fireSensorDoorGIBSwitch, dictionary);
}
}
...
bool Foam::fvMeshGIBChangers::fireSensorDoorGIBSwitch::enableCondition()
{
    const volScalarField& ps = mesh_.lookupObject<volScalarField>(psName_);
    ...
    if (!found)
    {
        Info<< this->type() << " for GIB faceZone "
            ...
            << " Points: " << points_ << tab << "out of bounds" << endl;
    }
    else
    {
        Info<< this->type() << " for GIB faceZone "
...
Foam::fvMeshGIBChangers::fireSensorDoorGIBSwitch::fireSensorDoorGIBSwitch(const fvMesh& mesh, const dictionary& dict)
:
    GIBSwitch(mesh, dict),
    threshold_(dict.lookup<scalar>("openThreshold")),
    patchNames_(0),
    points_(0),
