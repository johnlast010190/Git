...
) const
{
    ...
    DebugInformation<< FUNCTION_NAME << " : Starting hole flood filling" << endl;

    DebugInformation<< FUNCTION_NAME << " : Starting hole cells : "
    ...
    DebugInformation<< FUNCTION_NAME << " : Marked internal hole boundaries : "
        << nBlocked << endl;
    ...
    DebugInformation<< FUNCTION_NAME << " : Marked all hole boundaries : "
        << nBlocked << endl;
    ...
    //}
    DebugInformation<< FUNCTION_NAME << " : Determined regions : "
        << nRegions << endl;
    ...
    DebugInformation<< FUNCTION_NAME << " : Done local analysis" << endl;
    ...
    distributionMap map(globalCells, compactStencil, compactMap);

    DebugInformation<< FUNCTION_NAME << " : Converted stencil into compact form"
        << endl;
    ...
    while (true)
    {
        ...
        Pstream::listCombineReduce(regionType, maxOp<label>());

        DebugInformation<< FUNCTION_NAME << " : Gathered region type" << endl;
        ...
        DebugInformation<< FUNCTION_NAME << " : Interpolated region type" << endl;
        ...
        DebugInformation<< FUNCTION_NAME << " : Determined regions changed : "
            << nChanged << endl;
    ...
    DebugInformation<< FUNCTION_NAME << " : Finished hole flood filling" << endl;
}
...
) const
{
    ...
    while (returnReduce(isFront.count(), sumOp<unsigned int>()))
    {
...
)
{
    ...
    while (true)
    {
        ...
        distributionMapBase::distribute<point, minMagSqrEqOp<point>, flipOp>
        (
    ...
    // Re-do the distributionMap
    List<Map<label>> compactMap;
    distributionMap map(globalCells, cellStencil_, compactMap);
...
bool Foam::cellCellStencils::inverseDistance::update()
{
    ...
    Pstream::listCombineReduce(nCellsPerZone, plusOp<label>());
