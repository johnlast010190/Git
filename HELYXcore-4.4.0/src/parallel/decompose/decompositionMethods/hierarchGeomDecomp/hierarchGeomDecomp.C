...
)
{
    ...
    if (returnReduce(current.size(), sumOp<label>()) > 0)
    {
        forAll(current, i)
        {
            label pointi = current[indices[i]];
            sortedWeightedSizes[i + 1] = sortedWeightedSizes[i]
                + weights[pointi];
        }
...
)
{
    ...
    while (true)
    {
        label size = mid - minIndex;
        reduce
        (
            std::tie(size, hasNotChanged),
            ParallelOp<sumOp<label>,
            andOp<bool>>{}
        );

        // Never happens on first iteration, of course...
        if (hasNotChanged)
        {
            if (debug)
            {
        ...
        hasNotChanged = (mag(midValue-midValuePrev) < SMALL);
...
)
{
    ...
    while (true)
    {
        scalar weightedSize = sortedWeightedSizes[mid]
            - sortedWeightedSizes[minIndex];
        reduce
        (
            std::tie(weightedSize, hasNotChanged),
            ParallelOp<sumOp<scalar>, andOp<bool>>{}
        );
...
)
{
    ...
    label globalCurrentSize = current.size();
    scalar minCoord = (
        sortedCoord.size()
        ? sortedCoord[0]
        : GREAT
    ...
    reduce(
        std::tie(globalCurrentSize, minCoord, maxCoord),
        ParallelOp<sumOp<label>, minOp<scalar>, maxOp<scalar>>{}
    );
