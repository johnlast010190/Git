...
)
{
    ...
    return autoPtr<decompositionMethod>(ctor(decompositionDict));
}
...
)
{
    ...
    forAll(patches, patchi)
    {
        ...
        else if (isA<cyclicAMIPolyPatch>(pp) && transparentAMIs)
        {
            polyPatch& ppnC = const_cast<polyPatch&>(pp);
            const cyclicAMIPolyPatch& ppAMI =
                static_cast<cyclicAMIPolyPatch&>(ppnC);

            if (ppAMI.owner())
            {
                //if(false)
                if (ppAMI.AMI().singlePatchProc() == -1)
                {
                    const labelListList& srcAddr = ppAMI.AMI().srcAddress();
                    const labelListList& tgtAddr = ppAMI.AMI().tgtAddress();

                    labelList dfcOn(ppAMI.faceCells());
                    forAll(dfcOn, cI)
                    {
                        dfcOn[cI] = globalAgglom.toGlobal(agglom[dfcOn[cI]]);
                    }
                    ppAMI.AMI().srcMap().distribute(dfcOn);

                    labelList dfcNb(ppAMI.nbrPatch().faceCells());
                    forAll(dfcNb, cI)
                    {
                        dfcNb[cI] = globalAgglom.toGlobal(agglom[dfcNb[cI]]);
                    }
                    ppAMI.AMI().tgtMap().distribute(dfcNb);

                    forAll(srcAddr, fI)
                    {
                        const labelList& srcAddrI = srcAddr[fI];
                        forAll(srcAddrI, fII)
                        {
                            label cellI =  globalAgglom.toLocal(dfcOn[fI]);
                            //Pout<< fI << tab << cellI << endl;
                            nFacesPerCell[cellI]++;
                        }
                    }
                    forAll(tgtAddr, fI)
                    {
                        const labelList& tgtAddrI = tgtAddr[fI];
                        forAll(tgtAddrI, fII)
                        {
                            label cellI =  globalAgglom.toLocal(dfcNb[fI]);
                            nFacesPerCell[cellI]++;
                        }
                    }
                }
                ...
                {
                    const labelListList& addr = ppAMI.AMI().tgtAddress();
                    const polyPatch& neiP = ppAMI.nbrPatch();
                    boolList usedFace(pp.size(),false);

                    label ownSt = pp.start();
                    label neiSt = neiP.start();

                    forAll(addr, fI)
                    {
                        //Info<< "Nei cell " << faceOwner[neiSt+fI] << " -> " << endl;

                        forAll(addr[fI],nI)
                        {
                            label& neiF = fI;
                            const label& ownF = addr[fI][nI];

                            if (!usedFace[ownF])
                            {
                                label own = agglom[faceOwner[ownSt+ownF]];
                                label nei = agglom[faceOwner[neiSt+neiF]];
                                usedFace[ownF] = true;
                                nFacesPerCell[own]++;
                                nFacesPerCell[nei]++;
                            }
            ...
            if (ppAMI.owner())
            {
                ...
                if (ppAMI.AMI().singlePatchProc() == -1)
                {
                    ...
                    forAll(srcAddr, fI)
                    {
                        ...
                        forAll(srcAddrI, fII)
                        {
                            const label nbCellI = dfcNb[srcAddrI[fII]];
                            label cellI =  globalAgglom.toLocal(dfcOn[fI]);
                            m[offsets[cellI] + nFacesPerCell[cellI]++] = nbCellI;
                        }
                    }
                    forAll(tgtAddr, fI)
                    {
                        ...
                        forAll(tgtAddrI, fII)
                        {
                            const label onCellI = dfcOn[tgtAddrI[fII]];
                            label cellI =  globalAgglom.toLocal(dfcNb[fI]);
                            m[offsets[cellI] + nFacesPerCell[cellI]++] = onCellI;
                        }
                    }
                }
                else
    ...
    forAll(cellCells, celli)
    {
        ...
        label endIndex = offsets[celli+1];
...
)
{
    ...
    else
    {
        ...
        forAll(specifiedProcessorFaces, setI)
        {
            ...
            if (proci == -1)
            {
                // If no processor specified, pick that from the first element.
                // Do a sanity check for processors with no faces on it.
                if (set.size())
                {
