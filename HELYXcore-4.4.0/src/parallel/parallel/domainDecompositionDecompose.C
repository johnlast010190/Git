...
Foam::labelList Foam::domainDecomposition::distributeCells()
{
    ...
    (
        completeMesh(),
        runTimes_.decomposeDictFile()
    );

    scalarField cellWeights;
    if (method.found("weightField"))
    {
        const word weightName = method.lookup("weightField");
    ...
    const labelList result = method.decomposer().decompose
    (
        completeMesh(),
        cellWeights
    );
...
void Foam::domainDecomposition::decompose()
{
    ...
    // Loop through all internal faces and decide which processor they
    // belong to. First visit all internal faces. If cells at both sides
    // belong to the same processor, the face is an internal face.
    // If they are different, it belongs to both processors.
    ...
    // lists to the number of direct patches in the mesh
    labelListList procPatchSize(nProcs());
    labelListList procPatchStartIndex(nProcs());
    forAll(procPatchSize, proci)
    {
        procPatchSize[proci].setSize(patches.nDirectPatches());
        procPatchStartIndex[proci].setSize(patches.nDirectPatches());
    }

    // Patch faces (loop through direct patches only)
    forAll(procPatchSize[0], patchi)
    {
    ...
    for (label proci = 0; proci < nProcs(); proci++)
    {
        ...
        // GIB checking. If 'faceZones' exists in the last timestep,
        // change the polyMesh instance
        IOobject fZonesIO
        (
            "faceZones",
        ...
        if (fZonesIO.typeHeaderOk<faceZoneMesh>(false))
        {
            completeMesh_->setInstance(completeMesh().pointsInstance());
        }
        ...
        (
            curPatchSizes.size()
          + nInterProcPatches
          + completeMesh().boundaryMesh().nIndirectPatches(),
            nullptr
        );
        ...
        forAll(curPatchSizes, patchi)
        {
            ...
            if (isA<directPolyPatch>(meshPatch))
            {
                const directPolyPatch& dpp =
                    refCast<const directPolyPatch>(meshPatch);
        ...
        // Create point zones
        {
            const pointZoneMesh& pz = completeMesh().pointZones();
        ...
        // Create face zones
        {
            const faceZoneMesh& fz = completeMesh().faceZones();
        ...
        // Create cell zones
        {
            const cellZoneMesh& cz = completeMesh().cellZones();
        ...
        // Add indirect patches. Needs to be after zones creation.
        forAll(meshPatches, patchi)
        {
            const polyPatch& meshPatch = meshPatches[patchi];

            if (isA<indirectPolyPatch>(meshPatch))
            {
                const indirectPolyPatch& ipp =
                    refCast<const indirectPolyPatch>(meshPatch);

                // Map GIBs
                procPatches[nPatches] = ipp.clone
                (
                    procMesh.boundaryMesh(),
                    ipp.zoneId()
                ).ptr();
