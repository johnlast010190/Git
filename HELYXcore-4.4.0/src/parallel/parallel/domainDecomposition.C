        ...
        forAll(procMeshes_, proci)
        {
            ...
            forAll(procMesh.boundary(), procNccPatchi)
            {
                ...
                if (isA<nonConformalFvPatch>(fvp))
                {
                    ...
                    Sf.boundaryFieldRef()[completeNccPatchi]
                   .resize(size, Zero);

                    Cf.boundaryFieldRef()[completeNccPatchi]
                   .resize(size, Zero);
...
{
    // If a GIB is set, complete mesh and processor meshes instances must match
    if (hasGIB_) return 0;
...
void Foam::domainDecomposition::writeGIBMeshData
(
    const fvMesh& procMesh,
    const label proci
)
{
    // Manual basePoints decomposition (temporarily, currently inefficient)
    IOobject ioBasePoints
    (
        "basePoints",
        completeMesh().time().findInstance
        (
    ...
    // Check if base point exists in processor zero
    if (ioBasePoints.typeHeaderOk<pointIOField>(false))
    {
        pointIOField basePoints(ioBasePoints);
        pointIOField procBasePoints
        (
            IOobject
            (
                "basePoints",
                procMesh.pointsInstance(),
                polyMesh::meshSubDir,
                procMesh,
                IOobject::NO_READ,
                IOobject::NO_WRITE,
        ...
        const labelList& procPointAddressingI = procPointAddressing_[proci];
        forAll(procPointAddressingI, pointi)
        {
            procBasePoints[pointi] = basePoints[procPointAddressingI[pointi]];
        }

        procBasePoints.write();
    }

    // Reset complete mesh instance to the original faces instance
    // before GIB inclusion, keeping the points instance unchanged
    const fileName oldPointsInstance(completeMesh().pointsInstance());
    ...
    completeMesh_->setInstance(facesInstance);
    completeMesh_->setPointsInstance(oldPointsInstance);
}
...
void Foam::domainDecomposition::readComplete()
{
    ...
    // Look for GIB patches
    forAll(completeMesh_->boundaryMesh(), patchi)
    {
        if (isA<indirectPolyPatch>(completeMesh_->boundaryMesh()[patchi]))
        {
            hasGIB_ = true;
            break;
        }
...
)
{
    ...
    // Do not read-update if decomposing a GIB case
    fvMesh::readUpdateState stat = fvMesh::UNCHANGED;
    if (!isDecomp || !hasGIB_)
    {
        stat = completeMesh_->readUpdate(fvMesh::stitchType::nonGeometric);
    }

    // Do read-update on all processor meshes
    ...
    {
        // If GIB patches are present, clear and then re-read any
        // previously loaded meshes
        if (hasGIB_)
        {
            for (label proci = 0; proci < nProcs(); proci++)
            {
                procMeshes_.set(proci, nullptr);
            }
            readProcs();
        }
...
Foam::domainDecomposition::procFaceAddressingBf() const
{
    ...
    {
        // Construct cell-to-proc addressing (this is a bit wasteful, as it
        // was available before but has been thrown away)
        ...
        else if (!completeMesh().conformal())
        {
            ...
            forAll(completeMesh().boundary(), nccPatchi)
            {
                ...
                forAll(polyFacesBf[nccPatchi], nccPatchFacei)
                {
                    ...
                    nonConformalProcFaceAddressingBf[nbrProci]
                        [nbrProcNccPatchi].append(nccPatchFacei + 1);
                }
            }
        ...
        forAll(procMeshes_, proci)
        {
            ...
            forAll(procMesh.boundary(), procPatchi)
            {
                ...
                if (isA<indirectPolyPatch>(fvp.patch())) continue;
...
void Foam::domainDecomposition::writeProcs(const bool doSets)
{
    // Read sets
    ...
    for (label proci = 0; proci < nProcs(); proci++)
    {
        ...
        // Write GIB-related data
        writeGIBMeshData(procMesh, proci);
