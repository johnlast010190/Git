...
const Foam::NamedEnum<Foam::distributedTriSurfaceMesh::distributionType, 5>
...
) const
{
    ...
    return autoPtr<distributionMap>::New(std::move(sendMap));
}
...
) const
{
    ...
    autoPtr<distributionMap> mapPtr(new distributionMap(std::move(sendMap)));
...
{
    const triSurface& surf = static_cast<const triSurface&>(*this);
    const labelledTri& nearF = surf[nearFacei];
...
)
{
    ...
    scalarField triWeights(s.size(), 1.0);

    labelList surfMaxLevels;

    if (distType_ == WEIGHTED)
    {
        //read refinement levels from helyxHexMeshDict
        ...
        if (geometryDict.isDict(searchableSurface::name()))
        {
            const dictionary& dict = geometryDict.subDict(searchableSurface::name());

            word surfName = searchableSurface::name();
            dict.readIfPresent("name", surfName);

            // refinement parameters
            dictionary& refineDict = meshDict.subDict("castellatedMeshControls");
            dictionary& surfacesDict = refineDict.subDict("refinementSurfaces");

            const entry* ePtr = surfacesDict.lookupEntryPtr(surfName, false, true);
            if (ePtr)
            {
                ...
                surfMaxLevels = labelList(s.patches().size(), refLevel[1]);

                if (dict.found("regions"))
                {
                    const dictionary& regionsDict = dict.subDict("regions");

                    forAll(s.patches(), regionI)
                    {
                        if (regionsDict.found(s.patches()[regionI].name()))
                        {
                            // Get the dictionary for region
                            const dictionary& regionDict = regionsDict.subDict
                            (
                                s.patches()[regionI].name()
                            );

                            const labelPair refLevel(regionDict.lookup("level"));

                            surfMaxLevels[regionI] = refLevel[1];
                        } // if regionDict found
                    } //forAll regions
                } // if regions
            } // if surfName subDict
        ...
        {
            Info<< "WEIGHTED distributedTriSurfaceMesh: "
                << "Cannot find refinement levels for "
                << searchableSurface::name() << endl;

            surfMaxLevels = labelList(s.patches().size(), 0);
        }

        if (debug)
        {
            Info<<"regionNames " << s.patches() << endl;
            Info<<"surfMaxLevels " << surfMaxLevels << endl;
        }

        // Triangle area and refinement
        forAll(s, trii)
        {
            const label refinementLevel = surfMaxLevels[s[trii].region()];

            const label mult = 1 << refinementLevel;

            triWeights[trii] = s[trii].mag(s.points())*(mult*mult);
        }
    } //if WEIGHTED
    ...
    if (masterOnly)
    {
        ...
        if (!isA<geomDecomp>(decomposer_()))
        {
            ...
            distribution = decomposer_().decompose(faceFaces, triCentres, triWeights);
            UPstream::parRun() = oldParRun;
        }
        else
        {
            // Do the actual decomposition
            distribution = decomposer_().decompose(triCentres, triWeights);
        }
    ...
    else if (distType_ == DISTRIBUTED || distType_ == WEIGHTED)
    {
        ...
        labelList distribution = decomposer_().decompose(triCentres, triWeights);
...
//
//        const distributionMap map(std::move(sendMap));
//
...
//        }
//        distributionMap map(globalBorderTris, borderGlobalMaster
//
//        // Send decomposition
//
//
...
//    }
    else
    {
        ...
        {
            const labelList procs(UPstream::procID(UPstream::worldComm));
            forAll(s, trii)
            {
        ...
        labelList distribution(s.size());
        {
            ...
            const labelList procs(UPstream::procID(Pstream::worldComm));
            globalTris().scatter
            (
                UPstream::worldComm,
                procs,
...
)
{
    const boolList include
    (
        createWithValues<boolList>(s.size(), false, newToOldFaces, true)
    );
    ...
    currentDistType_(FROZEN)    // only used to trigger re-distribution
{
    ...
    (
     ...
     && (distType_ == INDEPENDENT || distType_ == DISTRIBUTED || distType_ == WEIGHTED)
    )
    {
        DebugInFunction
    ...
    currentDistType_(FROZEN)    // only used to trigger re-distribution
{
    ...
    {
        // Wanted distribution type
        if (dict.found("distributionType"))
        {
            distType_ = distributionTypeNames_.read(dict.lookup("distributionType"));
        }

        // Merge distance
        if (dict.found("mergeDistance"))
        {
            mergeDist_ = dict.lookup<scalar>("mergeDistance");
        }

        // Distribution type
        if (dict.found("closed"))
        {
            surfaceClosed_ = dict.lookup<bool>("closed");
        }
    ...
    )
    {
        ...
        const Switch redistribute
        (
            dict.lookupOrDefault("redistributeOnConstruct", true)
        );

        //Force not to redistribute on construction
        // so curvature field can be constructed in serial
        if (!redistribute)
        {
            if (debug)
            {
...
) const
{
    if (!Pstream::parRun())
    {
        triSurfaceMesh::findNearest(samples, nearestDistSqr, info, threaded);
        return;
    }
    ...
    autoPtr<distributionMap> map1Ptr;
    ...
    label nOutside = 0;
    {
        ...
        map1Ptr.set(new distributionMap(std::move(sendMap)));
    }
    const distributionMap& map1 = map1Ptr();
    ...
    autoPtr<distributionMap> map2Ptr;
    {
        List<DynamicList<label>> dynSendMap(Pstream::nProcs());
...
//        label nFound = 0;
        ...
        map2Ptr.reset(new distributionMap(std::move(sendMap)));
    }

    const distributionMap& map2 = map2Ptr();
...
) const
{
    if (!Pstream::parRun())
    {
        triSurfaceMesh::findLineAny(start, end, info, threaded);
    }
    else
    {
        findLine
...
) const
{
    ...
    autoPtr<distributionMap> mapPtr;
    {
        ...
        mapPtr.set(new distributionMap(std::move(sendMap)));
    }
    const distributionMap& map = mapPtr();
...
void Foam::distributedTriSurfaceMesh::getCurvatureField
(
    const List<pointIndexHit>& info,
...
) const
{
    if (!Pstream::parRun())
    {
        triSurfaceMesh::getCurvatureField(info, curvature);
        return;
    }
    ...
    if (foundObject<triSurfacePointScalarField>("curvatureValues"))
    {
        ...
        pointField localHits(info.size(), vector::zero);
        forAll(info, infoi)
        {
            if (info[infoi].hit())
            {
                localHits[infoi] = info[infoi].hitPoint();
            }
        }
        map.distribute(localHits);


        // Do my tests
        // ~~~~~~~~~~~

        curvature.setSize(triangleIndex.size(), scalar(-1));
        ...
        map.reverseDistribute(info.size(), curvature);
    }

    if (debug)
    {
        Pout<< "distributedTriSurfaceMesh::getCurvatureField :"
            << " finished retrieving field for "
            << info.size() << " triangles" << endl;
    }
}
...
)
{
    ...
    distributePointFields<label>(pointMap());
    distributePointFields<scalar>(pointMap());
    distributePointFields<vector>(pointMap());
    distributePointFields<sphericalTensor>(pointMap());
    distributePointFields<symmTensor>(pointMap());
    distributePointFields<tensor>(pointMap());
