...
)
{
    ...
    {
        if (tRoots.type(tRooti) != roots::real) continue;
...
)
{
    ...
    const vector tgtN = tgtTri.unitNormal();
...
//- Calculate the non-dimensional offsets of the target points from the source
...
)
{
    ...
    for (label tgtEi = 0; tgtEi < 3; tgtEi++)
    {
        ...
        // Add the first point if it's within or part of the source triangle
        if (tgtInSrcTri0)
        {
...
)
{
    ...
    forAll(pointLocations, pointi)
    {
        ...
        else if (l.isSrcPoint())
        {
            ...
            barycentric2D tgtTs = srcPointTgtTriIntersection(srcP, srcN, tgtPs);
...
)
{
    ...
    simpleVTKWriter triVTK(fs, ps, false);
    triVTK.write(name + ".vtk", false);
}
...
)
{
    ...
    return offset == 0 ? - patchToPatch::vSmall : offset;
}
...
)
{
    ...
    #endif
    {
        ...
        for (label i = 0; i < ceil(std::log2(1/patchToPatch::small)); ++i)
        {
    ...
    forAll(srcPs, srcEi)
    {
        ...
        )
        {
            ...
            #endif
            {
                ...
                (
                    label i = 0;
                    i < ceil(std::log2(1/patchToPatch::small));
                    ++i
                )
                {
                    const scalar srcT = (srcT0 + srcT1)/2;
