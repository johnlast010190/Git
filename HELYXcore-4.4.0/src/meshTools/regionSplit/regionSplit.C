...
) const
{
    ...
    auto globalCompactPtr = autoPtr<globalIndex>(new globalIndex(nCompact));
...
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
void Foam::regionSplit::report() const
{
    labelList nCellsPerRegion(nRegions(), 0);

    forAll(*this, cellI)
    {
        label regionI = this->operator[](cellI);
    ...
    {
        Info<<"Region: "<<regionI<<" contains "
            <<nCellsPerRegion[regionI]<<" cells"<<endl;
    }
}
// ************************************************************************* //
