...
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
bool Foam::meshTools::checkCellEdgeFaces
(
    const polyMesh &mesh
)
{
    forAll(mesh.cellEdges(),cI) {
    const labelList &edges = mesh.cellEdges()[cI];

    forAll(edges,eI) {
        const labelList &edgeFaces = mesh.edgeFaces()[edges[eI]];

        //how many of these faces neighbour the cell?
        label count = 0;
        forAll(edgeFaces,fI) {
        if
                (
                   (
                      mesh.faceOwner()[edgeFaces[fI]] == cI
                   || mesh.isInternalFace(edgeFaces[fI])
                   )
                && mesh.faceNeighbour()[edgeFaces[fI]] == cI
                )
                {
            ++count;
        }
        ...
        if (count != 2) {
        Info<< "WARNING: edge " << edges[eI] << "("
                     << mesh.edges()[edges[eI]][0] << ","
                     << mesh.edges()[edges[eI]][1] << ") has "
             << count << " faces connected to cell " << cI << "\n";
        }
    }
    }
    return true;
