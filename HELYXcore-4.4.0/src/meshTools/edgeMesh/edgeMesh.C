...
const Foam::dictionary& Foam::edgeMesh::readMeshObjectDict
(
...
void Foam::edgeMesh::doTransforms
(
    const dictionary& dict,
    const IOobject& io,
    const dictionary& eMeshSubdict
)
{
    if (eMeshSubdict.found("referenceFrame"))
    {
        const word frameName = eMeshSubdict.lookup<word>("referenceFrame");
        ...
        storedPoints() = frame.transformPoint(storedPoints());
    }
    ...
    if (dict.found("transforms"))
    {
        ...
        forAll(transforms, dictI)
        {
            ...
            if (transformDict.found("referenceFrame"))
            {
                ...
                storedPoints() = frame.invTransformPoint(storedPoints());
            }
