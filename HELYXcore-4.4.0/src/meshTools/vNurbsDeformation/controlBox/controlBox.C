...
)
{
    ...
    nXCPs_ = dict.lookup<label>("nUCPs");
    nYCPs_ = dict.lookup<label>("nVCPs");
    nZCPs_ = dict.lookup<label>("nWCPs");
...
void controlBox::boxToPolyMesh(const IOobject& io)
{
    ...
    pointField p(points_);
    faceList faces(fList);
    cellList cells(cList);
