...
) const
{
    ...
    if (!endCaps_)
    {
        scalar dp = min(max(0., parallel), magDir_);
        ...
        if (magV < ROOTVSMALL)
        {
            ...
            if (mag(n) < SMALL)
            {
                n = vector(0, 1, 0) ^ unitDir_;
            }
            n /= (mag(n) + SMALL);

            info.setPoint(point1_ + n*radius_);
        }
        else
        {
            info.setPoint(point1_ + dp*unitDir_ + radius_*v/magV);
        }
    }
    else
    {
        ...
        else
        {
            if (parallel < 0.5*magDir_)
            {
                if ((radius_ - magV) < parallel)
                {
                    ...
                    else
                    {
                        info.setPoint
                        (
                            point1_ + parallel*unitDir_ + radius_*v/magV
                        );
                    }
                }
                else
            ...
            {
                if ((radius_ - magV) < magDir_- parallel)
                {
                    if (magV < ROOTVSMALL)
                    {
                        info.setPoint(point1_ + parallel*unitDir_);
...
) const
{
    ...
    {
        point nearPt = start+tNear*V;

        if (!endCaps_)
        {
            vector v(nearPt - point1_);
            ...
            if (magV >= (radius_ - SMALL))
            {
                near.setPoint(nearPt);
                near.setHit();
                near.setIndex(0);
            }
        }
        ...
        if (tFar <= magV)
        {
            point farPt = start+tFar*V;
            if (!endCaps_)
            {
                vector v(farPt - point1_);
    ...
    if (dict.found("transforms"))
    {
        ...
        forAll(transforms, dictI)
        {
            ...
            if (type == "scale")
            {
                ...
                const point pt(transformDict.lookupOrDefault("aboutPoint", vector(0, 0, 0)));
                point1_ -= pt;
                point2_ -= pt;
                point1_ = cmptMultiply(scale,point1_);
                point2_ = cmptMultiply(scale,point2_);
                point1_ += pt;
                point2_ += pt;
                radius_ *= scale.x();
            }
...
:
    ...
    endCaps_(dict.lookupOrDefault<Switch>("endCaps", true))
{
    ...
    else if (dict.found("direction"))
    {
        ...
        magDir_ = dict.lookup<scalar>("length");
        unitDir_ = dir/mag(dir);
        point2_ = point1_ + (magDir_*unitDir_);
    }
    else
    {
        FatalErrorInFunction
            << "Searchable cylinder requires either point2"
            << " or a direction and length keyword."
            << exit(FatalError);
    }
