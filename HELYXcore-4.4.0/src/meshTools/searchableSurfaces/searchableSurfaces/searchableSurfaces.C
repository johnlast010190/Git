...
(
    ...
    const List<fileName>& alternativeSurfacePath,
    const label maxNumberRegions
)
:
    PtrList<searchableSurface>(topDict.size()),
    names_(topDict.size()),
    fileNames_(topDict.size()),
    regionNames_(topDict.size()),
    allSurfaces_(),
    singleRegion_(topDict.size())
{
    label surfI = 0;
    forAllConstIter(dictionary, topDict, iter)
    {
        ...
        dictionary dict = topDict.subDict(key);

        if (!dict.lookupOrDefault("active", true))
        {
            continue;
        }

        Info<< "\nReading geometry: " << key << endl;

        names_[surfI] = key;
        dict.readIfPresent("name", names_[surfI]);

        bool appendRegionName = !singleRegionName;
        if (dict.found("appendRegionName"))
        {
            // Whether to append the region name for single region surfaces
            appendRegionName = dict.lookup<bool>("appendRegionName");
        }

        // Setup fileNames for shells that are also surfaces
        fileNames_[surfI] = key;
        ...
        const word sType = dict.lookup("type");

        if (sType == "triSurfaceMesh")
        {
            //If file does not exist in constant/triSurface look elsewhere
            if (namedIO().filePath() == fileName::null)
            {
                forAll(alternativeSurfacePath, pathI)
                {
                    const fileName& sPath = alternativeSurfacePath[pathI];
                    namedIO().instance() = sPath;
                    if (namedIO().filePath() != fileName::null)
                    {
                        Info<<"Reading " << iter().keyword() <<" from "
                            <<"alternative path location : "<< sPath <<endl;
                        break;
                    }
                }
            }
        }
        else if (sType == "distributedTriSurfaceMesh")
        {
            dict.add("redistributeOnConstruct","false",true);
        }
        ...
        if (sType == "triSurfaceMesh" && s.points()().size() == 0)
        {
            FatalErrorInFunction
               << "Zero sized tri surface detected : " << s.name() << nl
               << exit(FatalError);
        }
        ...
        label sz = localNames.size();

        if ((!appendRegionName && sz == 1) || sz >  maxNumberRegions)
        {
            singleRegion_[surfI] = true;
            rNames.setSize(1);
            rNames[0] = names_[surfI];
        }
        else
        {
            singleRegion_[surfI] = false;
            rNames.setSize(sz);
            forAll(localNames, regionI)
            {
        ...
        // See if dictionary provides any global region names.
        if ((!singleRegion_[surfI] || sz == 1) && dict.found("regions"))
        {
            const dictionary& regionsDict = dict.subDict("regions");
    ...
    fileNames_.setSize(surfI);
    allSurfaces_.setSize(surfI);
    allSurfaces_ = identity(surfI);
    singleRegion_.setSize(surfI);
}
...
Foam::label Foam::searchableSurfaces::findFileSurfaceID(const word& wantedName)
 const
{
    return findIndex(fileNames_, wantedName);
}
