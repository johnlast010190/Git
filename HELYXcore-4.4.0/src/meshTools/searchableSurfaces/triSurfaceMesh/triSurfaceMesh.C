    ...
    if (io.readOpt() != IOobject::NO_READ)
    {
        ...
        if (searchGlobal && Pstream::parRun())
        {
            ...
            if (r == masterOnly && (actualFile != localFile))
            {
                ...
                Pstream::scatter(triSurface::patches());
                triSurface::doTransforms(actualFile, io, dict);
...
Foam::tmp<Foam::pointField> Foam::triSurfaceMesh::findRayNearest
(
    const pointField& points,
    const vector& direction,
    const vector& defaultPoint
) const
{
    List<List<pointIndexHit>> info;
    const vectorField start(points + direction*GREAT);
    const vectorField end(points - direction*GREAT);

    // Find all intersections with the surface
    triSurfaceSearch::findLineAll(start, points - direction*GREAT, info);

    tmp<pointField> tNearestPoints
    (
        new pointField(points.size(), defaultPoint)
    );
    pointField& nearestPoints = tNearestPoints.ref();
    forAll(info, pointi)
    {
        List<pointIndexHit>& infoPointi = info[pointi];
        forAll(infoPointi, j)
        {
            if
            (
                infoPointi[j].hit()
             && (
                    mag(nearestPoints[pointi] - points[pointi])
                  > mag(infoPointi[j].hitPoint() - points[pointi])
                )
            )
            {
                nearestPoints[pointi] = infoPointi[j].hitPoint();
            }
        }
    }
    return tNearestPoints;
}
...
void Foam::triSurfaceMesh::findHighCurvature
(
    const pointField& centres,
    const scalarField& radiusSqr,
...
) const
{
    if (foundObject<triSurfacePointScalarField>("curvatureValues"))
    {
        ...
        triSurfaceSearch::findHighCurvature(centres,radiusSqr,fld,maxCurvature);
    }
