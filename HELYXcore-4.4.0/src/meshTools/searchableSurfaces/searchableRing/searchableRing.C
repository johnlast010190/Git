...
) const
{
    ...
    else
    {
        ...
        scalar length = dict.lookup<scalar>("length");
...
:
    ...
    innerCylinder_(io, point1_, point2_, dict.lookup<scalar>("innerRadius"), dict, false),
    outerCylinder_(io, point1_, point2_, dict.lookup<scalar>("outerRadius"), dict, true)
{
    //Need to reset to take account of transforms
    point1_ = innerCylinder_.start();
    point2_ = innerCylinder_.end();
