        ...
        forAll(faces, facei)
        {
            forAll(faces[facei], facePointi)
            {
                pointNormals[faces[facei][facePointi]] +=
                    faces[facei].areaNormal(mesh_.oldPoints());
            }
        }
...
) const
{
    if (!patchPointOwnerOrigBoundaryPointsPtr_.set(patchi))
    {
        ...
        const directPolyPatch& dpp = refCast<const directPolyPatch>(pp);
        ...
        (
            ...
            (
                meshPointOwnerOrigBoundaryPoint(),
                static_cast<const faceList&>(dpp)
            )
        );

        const primitivePatch ownerOrigBoundaryLocalPatch
        (
            SubList<face>(patchOwnerOrigBoundaryLocalFaces, dpp.size()),
        ...
        {
            const labelList& dppPointOwnerOrigBoundaryPoints =
                patchPointOwnerOrigBoundaryPointsPtr_[patchi];

            forAll(dpp.meshPoints(), dppPointi)
            {
                const label ownerOrigBoundaryPointi =
                    dppPointOwnerOrigBoundaryPoints[dppPointi];

                if
                (
                    dpp.meshPoints()[dppPointi]
                 != ownerOrigBoundary_.meshPoints()[ownerOrigBoundaryPointi]
                )
                {
            ...
            const labelList& dppEdgeOwnerOrigBoundaryEdges =
                patchEdgeOwnerOrigBoundaryEdgesPtr_[patchi];

            forAll(dpp.edges(), dppEdgei)
            {
                const label ownerOrigBoundaryEdgei =
                    dppEdgeOwnerOrigBoundaryEdges[dppEdgei];

                if
                (
                    meshEdge(dpp, dppEdgei)
