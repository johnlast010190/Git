...
namespace Foam
{
    ...
    treeBoundBox combineBox(const treeBoundBox& a, const treeBoundBox& b)
    {
...
Foam::floatScalar Foam::patchToPatch::floatSmall =
    numeric_limits<float>::epsilon();

Foam::floatScalar Foam::patchToPatch::floatVSmall =
    numeric_limits<float>::min();

Foam::floatScalar Foam::patchToPatch::floatRootSmall =
    sqrt(floatSmall);

Foam::floatScalar Foam::patchToPatch::floatRootVSmall =
    sqrt(floatVSmall);


Foam::doubleScalar Foam::patchToPatch::doubleSmall =
    numeric_limits<double>::epsilon();

Foam::doubleScalar Foam::patchToPatch::doubleVSmall =
    numeric_limits<double>::min();

Foam::doubleScalar Foam::patchToPatch::doubleRootSmall =
    sqrt(doubleSmall);

Foam::doubleScalar Foam::patchToPatch::doubleRootVSmall =
    sqrt(doubleVSmall);
...
Foam::scalar Foam::patchToPatch::small = floatSmall;
Foam::scalar Foam::patchToPatch::vSmall = floatVSmall;
Foam::scalar Foam::patchToPatch::rootSmall = floatRootSmall;
Foam::scalar Foam::patchToPatch::rootVSmall = floatRootVSmall;

#elif defined(HELYX_DP)

// Define scalar as a double
Foam::scalar Foam::patchToPatch::small = doubleSmall;
Foam::scalar Foam::patchToPatch::vSmall = doubleVSmall;
Foam::scalar Foam::patchToPatch::rootSmall = doubleRootSmall;
Foam::scalar Foam::patchToPatch::rootVSmall = doubleRootVSmall;
...
) const
{
    ...
    if (srcPatch.has0())
    {
        ...
        box = combineBox(box, srcBox(srcFace, srcPoints0, srcPointNormals0));
    }
...
) const
{
    ...
    box.inflate(patchToPatch::rootSmall);
...
) const
{
    ...
    if (tgtPatch.has0())
    {
        ...
        box = combineBox(box, treeBoundBox(tgtPoints0, tgtFace));
    }
...
) const
{
    ...
    {
        box = combineBox(box, tgtBox(tgtPatch, tgtFacei));
    }
...
)
{
    ...
    if (nonConformalPolyPatch::debug)
    {
        ...
        tgtVTK.write(tgtFileName, false);
    ...
    )
    {
        ...
        if (iterationi == 0 || iterationi % 2 == !isSrc)
        {
            ...
            simpleVTKWriter queueVTK
            (
                faceList(IndirectList<face>(patch.localFaces(), first(queue))),
                patch.localPoints(),
                false
            );

            queueVTK.write(queueFileName, false);
        }
        else
        {
            ln
    ...
    )
    {
        ...
        if (nonConformalPolyPatch::debug > 1)
        {
    ...
    if (nonConformalPolyPatch::debug)
    {
        Info<< indent << "Calculating coupling" << endl;
    }
    ...
    while (srcFacei < srcPatch.size() && srcFacei != -1)
    {
        ...
        if (!seedTgtFaces.empty())
        {
            if (nonConformalPolyPatch::debug)
            {
                Info<< incrIndent << indent << "Restart #" << restarti
                    << " from source face with center: "
            ...
            if (nonConformalPolyPatch::debug)
            {
                ...
                Info<< indent << "Completed " << nTgtFaceComplete << '/'
                    << tgtPatch.size() << " target faces" << decrIndent
                    << endl;
            }
        }
...
void Foam::patchToPatch::distributeSrc(const primitiveOldTimePatch& srcPatch)
{
...
)
{
    ...
    else
    {
        Info<< " ..." << endl << indent
            ...
            << "and target patch '" << srcTgtPatchNames.second().c_str()
    ...
    singleProcess_ =
        patchToPatchTools::singleProcess(srcPatch.size(), tTgtPatch.size());
    ...
    else
    {
        ...
        (
            new List<remote>
            (
                distributePatch(tgtMapPtr_(), tTgtPatch, localTTgtPatchPtr)
            )
        );
