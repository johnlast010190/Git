...
namespace patchToPatches
{
    ...
    defineExtraDebugSwitch
    (
        intersection,
        debugSrcFacei,
        (intersection::typeName + "SrcFace").c_str(),
        -1
    );
    ...
    (
        intersection,
        debugTgtFacei,
        (intersection::typeName + "TgtFace").c_str(),
        -1
    );
}
}
...
)
{
    ...
    const scalar l = sqrt(mag(srcFace.areaNormal(srcPoints)));
...
)
{
    ...
    forAll(srcTriPoints_[srcFacei], srcFaceTrii)
    {
        ...
        forAll(tgtTriPoints_[tgtFacei], tgtFaceTrii)
        {
            ...
            if
            (
                mag(ictSrcPart.area) < patchToPatch::small*srcMagA
             || mag(ictTgtPart.area) < patchToPatch::small*tgtMagA
            )
            {
                continue;
            }
...
)
{
    ...
    if (nCouples != 0)
    {
        ...
        if (nonConformalPolyPatch::debug)
        {
            ...
            srcVTK.addFaceData("coverage", scalarField(srcCoverage_));
            srcVTK.addFaceData("openness", srcOpenness);
            srcVTK.addFaceData("error", srcError);
            srcVTK.addFaceData("depth", srcDepth);
            srcVTK.addFaceData("angle", srcAngle);
            srcVTK.addFaceData("normals", srcPointNormals);

            srcVTK.write(name + "_srcPatch" + ".vtk");
            ...
            tgtVTK.addFaceData("coverage", scalarField(tgtCoverage_));

            tgtVTK.write(name + "_tgtPatch" + ".vtk");
            ...
            if (nonConformalPolyPatch::debug > 1)
            {
                ...
                simpleVTKWriter interVTK(debugFaces_, debugPoints_);

                interVTK.addFaceData("srcFace", debugFaceSrcFaces_);
                interVTK.addFaceData("tgtFace", debugFaceTgtFaces_);
                interVTK.addFaceData("side", debugFaceSides_);

                interVTK.write(name + ".vtk");
