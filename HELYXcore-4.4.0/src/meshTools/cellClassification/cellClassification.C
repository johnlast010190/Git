...
) const
{
    ...
    const indexedOctree<treeDataTriSurface>& surfTree = search.tree();
    ...
    forAll(mesh_.edges(), edgeI)
    {
        ...
        pointIndexHit pHit(surfTree.findLineAny(p0, p1));
    ...
    if (debug)
    {
        Pout<< "Intersected edges of mesh with surface in = "
            << clockTimer.timeIncrement() << " s (cpu "
            << timer.cpuTimeIncrement() << ")" << nl << endl;
    }
    ...
    forAll(edges, edgeI)
    {
        ...
        while (true)
        {
            ...
            else
            {
                ...
                //If stuck on a face increase small vector
                if (pHit.index() == prevHitIndex)
                {
                    smallVec *= 10.;
                }
                prevHitIndex = pHit.index();
...
{
    // Use mesh wave to partition mesh, starting from outsidePt
