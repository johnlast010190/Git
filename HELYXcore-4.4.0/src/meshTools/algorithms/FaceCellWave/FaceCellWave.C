...
)
{
    ...
    {
        const Type& nbrWallInfo = changedFacesInfo[changedFacei];
        ...
        if (!currentWallInfo.equal(nbrWallInfo, td_))
        {
            updateFace
            (
                meshFacei,
...
Foam::label Foam::FaceCellWave<Type, TrackingData>::faceToCell()
{
    ...
    forAll(changedFaces_, changedFacei)
    {
        ...
        const Type& nbrWallInfo = allFaceInfo_[facei];
        ...
        if (facei < nInternalFaces)
        {
            ...
            if (!currentWallInfo2.equal(nbrWallInfo, td_))
            {
                updateCell
                (
                    celli,
...
Foam::label Foam::FaceCellWave<Type, TrackingData>::cellToFace()
{
    ...
    forAll(changedCells_, changedCelli)
    {
        ...
        const Type& nbrWallInfo = allCellInfo_[celli];
