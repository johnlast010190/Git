...
bool Foam::polyMeshZipUpCells(polyMesh& mesh)
{
    ...
    do
    {
        ...
        forAll(Cells, celli)
        {
            ...
            {
                for (const edge curEdge : oldFaces[curFaces[facei]].walkEdges())
                {
            ...
            forAll(edgesToInsert, edgeToInsertI)
            {
                ...
                forAll(facesSharingEdge, facei)
                {
                    ...
                    for (const edge e : oldFaces[currentFaceIndex].walkEdges())
                    {
                        if (e == testEdge)
                        {
                            faceChanges = true;
                    ...
                    if (faceChanges)
                    {
                        ...
                        if (!allPointsPresent)
                        {
                            ...
                            for (const edge e : newFace.walkEdges())
                            {
                                ...
                                else
                                {
                                    ...
                                    newFace[nNewFacePoints] = e.start();
                                    nNewFacePoints++;
                                }
                            }
