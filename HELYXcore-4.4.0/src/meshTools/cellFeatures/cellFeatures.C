...
Foam::labelList Foam::cellFeatures::labels() const
{
    label maxVert = 0;
    forAll(faces(), faceI)
    {
        maxVert += faces()[faceI].size();
    }

    DynamicList<label> p(maxVert);
    labelHashSet mp(maxVert);
    ...
    forAll(faces(), faceI)
    {
        ...
        forAll(f, fpI)
        {
            label meshPointI = f[fpI];

            if (mp.insert(meshPointI))
            {
                p.append(meshPointI);
            }
        }
