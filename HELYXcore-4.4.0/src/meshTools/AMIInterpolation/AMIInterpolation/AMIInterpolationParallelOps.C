...
) const
{
    ...
    if (Pstream::parRun())
    {
        ...
        Pstream::allGatherList(facesPresentOnProc);
...
) const
{
    ...
    if (hasMerged)
    {
        ...
        forAll(tgtFaces, i)
        {
            ...
            face tgtFacesCpI(tgtFaces[i]);

            // Remove duplicate labels arising from merged points
            label faceSize = tgtFaces[i].collapse();

            // if the points merged and they are the same in one face,
            // undo the collapse. Otherwise it will create faces with <3 points

            if (faceSize < 3)
            {
                tgtFaces[i] = tgtFacesCpI;
            }
        }
    }
}
...
Foam::autoPtr<Foam::distributionMap> Foam::AMIInterpolation::calcProcMap
(
    const primitivePatch& srcPatch,
    const primitivePatch& tgtPatch
) const
{
    ...
    //-For bad meshes there can be faces that the bound box
    // should include them for the addressing but it is not
    // In that case increase the bound box extension.
    // However computational cost increases as well
    // Now by default we inflate by 1% the bounding box
    // An absolute tolerance may also be specified
    forAll(procBb[Pstream::myProcNo()], bbI)
    {
        treeBoundBox& bb = procBb[Pstream::myProcNo()][bbI];
        bb.inflateAbs(max(relTol_*mag(bb.span()), tol_));
    }
...

    {
        ...
        forAll(faces, facei)
        {
            if (faces[facei].size())
            {
                ...
                if (sum(procBbOverlaps) == 0)
                {
                    dynSendMap[Pstream::myProcNo()].append(facei);
                }
