...
#include "AMIInterpolation/AMIInterpolation/AMIMethod/faceAreaWeightAMI/faceAreaWeightAMI.H"
...
)
{
    ...
    do
    {
        ...
        (
            ...
            this->srcMagSf_[srcFacei] > faceAreaIntersect::tolerance()
        )
        {
            srcAddr[srcFacei].append(tgtFacei);
            srcWght[srcFacei].append(area.first());
            srcSymWght[srcFacei] += area.second();
            srcCtr[srcFacei].append(interCentroid);

            tgtAddr[tgtFacei].append(srcFacei);
            tgtWght[tgtFacei].append(area.first());
            tgtSymWght[tgtFacei] += area.second();
...
) const
{
    ...
    forAll(srcNbrFaces, i)
    {
        ...
        if (mapFlag[faceS] && seedFaces[faceS] == -1)
        {
            forAll(visitedFaces, j)
            {
                ...
                Tuple2<scalar, scalar> areaT = interArea(faceS, faceT, srcCtr);
                scalar area = areaT.first() + areaT.second();
    ...
    else
    {
        ...
        if (errorOnNotFound && debug)
        {
            //FatalErrorInFunction
            //   << "Unable to set source and target faces" << abort(FatalError);
            WarningInFunction
               << "Unable to set source and target faces" << endl;
        }
    }
}


Foam::Tuple2<Foam::scalar, Foam::scalar> Foam::faceAreaWeightAMI::interArea
(
    const label srcFacei,
...
//  scalar area = 0;
    ...
    vector n(-src.areaNormal(srcPoints));
    vector ntar = tgt.areaNormal(tgtPoints);

    scalar cosAng = 0;
    scalar magv = mag(n)*mag(ntar);
    ...
    if (magN > ROOTVSMALL)
    {
        scalar overlapArea =
            inter.calc
            (
                src, tgt, n/magN, this->triMode_, srcCtr
            );

        // bound cosAngle -1 and 1
        cosAng = min(cosAng, 1-SMALL);
        cosAng = max(cosAng, -1+SMALL);

        if (cosAng>cosAngle_ || cosAng<-cosAngle_)
        {
            area.first() = overlapArea;
        }
        else
        {
            area.second() = overlapArea;
        }
    }
    else
    {
...
)
{
    ...
    forAll(srcWght, srcFacei)
    {
        ...
        scalar srcMagSfI = this->srcMagSf_[srcFacei];

        if (srcMagSfI>0)
        {
            scalar t = s/srcMagSfI;
    ...
    cosAngle_(cosMatchAngle)
{
    if (debug)
    {
        ...
        if (version22)
        {
            restartUncoveredSourceFace_ = true;
        }
    }
}
...
bool Foam::faceAreaWeightAMI::conformal() const
{
    return true;
}
...
void Foam::faceAreaWeightAMI::normaliseWeights
(
    const bool verbose,
