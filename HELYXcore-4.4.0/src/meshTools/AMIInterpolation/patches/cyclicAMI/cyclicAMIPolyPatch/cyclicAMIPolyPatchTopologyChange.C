...
void Foam::cyclicAMIPolyPatch::restoreScaledGeometry()
{
    ...
    if (moveFaceCentres_)
    {
        DebugInformation << "Moving face centres" << endl;
...
bool Foam::cyclicAMIPolyPatch::removeAMIFaces(polyTopoChange& topoChange)
{
    ...
    const cyclicAMIPolyPatch& nbr = nbrPatch();
...
void Foam::cyclicAMIPolyPatch::setAMIFaces()
{
    ...
    autoPtr<distributionMap> srcToTgtMap1;
    autoPtr<distributionMap> tgtToSrcMap1;
       ...
       // Parallel running
        ...
        forAll(tgtToSrcAddr0, tgtFacei0)
        {
            ...
            forAll(newTgtFaces, i)
            {
                ...
                transform().invTransformPosition(srcCtr0);
        ...
        // Update the maps
        {
            ...
            srcToTgtMap1.reset
            (
                new distributionMap(globalSrcFaces1, tgtToSrcAddr1, cMap)
            );
        }
        {
            List<Map<label>> cMap;
            tgtToSrcMap1.reset
            (
                new distributionMap(globalTgtFaces1, srcToTgtAddr1, cMap)
            );
        }
    ...
    else
    {
        ...
        forAll(srcToTgtAddr0, srcFacei0)
        {
            ...
            forAll(srcFaceTgtAddr0, addri)
            {
                ...
                transform().invTransformPosition(pt);
