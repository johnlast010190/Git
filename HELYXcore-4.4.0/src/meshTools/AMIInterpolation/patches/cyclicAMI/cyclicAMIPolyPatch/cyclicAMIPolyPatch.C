    ...
    if (owner())
    {
        ...
        (
            ...
            (
                ...
                AMIReverse_,
                AMIInterpolation::relTolDef_,
                AMIInterpolation::tolDef_,
                AMIInterpolation::cosMatchAngleDef_,
                AMIInterpolation::maxAMIWeightScaleDef_,
                false
            )
        );
    ...
    )
    {
        static_cast<cyclicTransform&>(*this) =
            ...
            (
                ...
                *this,
                nbrPatchName(),
                nbrPatch().primitivePatch::faceCentres(),
...
void Foam::cyclicAMIPolyPatch::initTopoChange(PstreamBuffers& pBufs)
{
    directPolyPatch::initTopoChange(pBufs);

    if (createAMIFaces_ && boundaryMesh().mesh().topoChanged() && owner())
    {
        setAMIFaces();
    }
}
...
void Foam::cyclicAMIPolyPatch::topoChange(PstreamBuffers& pBufs)
{
...
:
    ...
    nbrPatchID_(-1),
    AMIPtr_(nullptr),
    AMIMethod_(AMIInterpolation::imFaceAreaWeight),
    AMIReverse_(false),
    ...
    faceCentres0_()
{
    ...
    if (createAMIFaces_)
    {
        srcFaceIDs_.setSize(dict.lookup<label>("srcSize"));
        tgtFaceIDs_.setSize(dict.lookup<label>("tgtSize"));
...
const Foam::AMIInterpolation& Foam::cyclicAMIPolyPatch::AMI() const
{
    if (!owner())
    {
        FatalErrorInFunction
...
void Foam::cyclicAMIPolyPatch::write(Ostream& os) const
{
    ...
    if (AMIMethod_ != AMIInterpolation::imFaceAreaWeight)
    {
        os.writeEntry
        (
            "method",
