...
)
{
    ...
    // edge 0
    {
        ...
        {
            // Check for co-linearity before constructing plane
            if (mag((tgt[1]-tgt[0]) ^ s*n) > VSMALL)
            {
                plane pl0(tgt[0], tgt[1], p3);
    ...
    forAll(trisA, tA)
    {
        ...
        if (!tpA.collocate())
        {
            forAll(trisB, tB)
            {
                ...
                if (!tpB.collocate())
                {
                    totalArea += triangleIntersect(tpA, tpB, n, centroid);
                }
            }
        }
    }
