...
)
{
    ...
    if (Pstream::parRun())
    {
        ...
        Pstream::allGatherList(procHasElements);
...
)
{
    ...
    Pstream::allGatherList(transferSizes);
...
{
    return procSendIndices(labelListList(tgtLocalSrcFaces), localTgtProcFaces);
}
...
)
{
    ...
    forAll(map.subMap(), proci)
    {
        ...
        forAll(map.subMap()[proci], oldi)
        {
            ...
            {
                map.subMap()[proci][newi ++] = map.subMap()[proci][oldi];
            }
        }
...
{
    return localToRemote(labelListList(indices), indexToProcIndex);
}
