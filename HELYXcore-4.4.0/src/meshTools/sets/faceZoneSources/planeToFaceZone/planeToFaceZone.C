...
void Foam::planeToFaceZone::combine(faceZoneSet& fzSet, const bool add) const
{
    ...
    if (include_ == include::closest)
    {
        ...
        label nRegions = -1;
        {
            ...
            nRegions = max(procRegionOffset + procNRegions);
        }
        ...
        labelList regionMap(identity(nRegions));
        {
            ...
            // Flatten the lists
            List<labelList> regionMapMap(nRegions);
            forAll(regionRegions, regioni)
            {
                regionMapMap[regioni].append(regioni);
                regionMapMap[regioni].append(regionRegions[regioni].toc());
            }

            // Create lists that contain all continuous regions
            bool added = true;
            while (added)
            {
                added = false;
                forAll(regionMapMap, regioni)
                {
                    labelList& regionsI = regionMapMap[regioni];
                    forAll(regionsI, regionj)
                    {
                        const labelList& neiRegionI =
                            regionMapMap[regionsI[regionj]];

                        forAll(neiRegionI, regionk)
                        {
                            if (!regionsI.found(neiRegionI[regionk]))
                            {
                                added = true;
                                regionsI.append(neiRegionI[regionk]);
                            }
                        }
                    }
                }
            ...
            // Sort entries in the lists so they can be compared
            // in next step for duplicates
            forAll(regionMapMap, regioni)
            {
                sort(regionMapMap[regioni]);
            }

            // Remove duplicates
            List<labelList> tmpRegionMapMap(0);
            forAll(regionMapMap, i)
            {
                if (!tmpRegionMapMap.found(regionMapMap[i]))
                {
                    tmpRegionMapMap.append(regionMapMap[i]);
                }
            }
            regionMapMap = tmpRegionMapMap;

            // Recompute the number of regions
            nRegions = regionMapMap.size();

            // Remap the all the regiion splits into continuos
            // regions.
            forAll(regionMap, regioni)
            {
                forAll(regionMapMap, regionj)
                {
                    if (regionMapMap[regionj].found(regioni))
                    {
                        regionMap[regioni] = regionj;
                    }
                }
            }
        }
        ...
        labelList regionNFaces;
        {
            ...
            Pstream::listCombineReduce(regionNFaces, plusOp<label>());
        ...
        label selectedRegioni = -1;
        {
            ...
            Pstream::listCombineReduce(regionMagAreas, plusOp<scalar>());
            Pstream::listCombineReduce(regionCentres, plusOp<point>());
