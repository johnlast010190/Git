...
:
    ...
    surfPtr_(new triSurface(mesh,surfName_)),
...
:
    ...
    includeCut_(dict.lookup<bool>("includeCut")),
    includeInside_(dict.lookup<bool>("includeInside")),
    includeOutside_(dict.lookup<bool>("includeOutside")),
    ...
    surfPtr_(new triSurface(mesh,surfName_,dict)),
    querySurfPtr_(new triSurfaceSearch(*surfPtr_)),
    IOwnPtrs_(true)
{
    checkSettings();
