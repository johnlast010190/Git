    ...
    forAllConstIter(labelHashSet, includePatches, iter)
    {
        ...
        forAll(patch, patchFacei)
        {
            ...
            forAll(triEngine.triPoints(), triI)
            {
                triangles.append
                (
                    labelledTri(triEngine.triPoints(triI, f), newPatchi)
                );
...
Foam::triSurface Foam::triSurfaceTools::delaunay2D
(
    const List<vector2D>& pts,
    const bool delabella
)
{
    if (delabella)
    {
        ...
        MyPoint* cloud = new MyPoint[pts.size()];
        forAll(pts, i)
        {
            cloud[i].x = pts[i][0];
            cloud[i].y = pts[i][1];
        ...
        if (verts > 0)
        {
            const DelaBella0_Triangle* dela = idb->GetFirstDelaunayTriangle();
