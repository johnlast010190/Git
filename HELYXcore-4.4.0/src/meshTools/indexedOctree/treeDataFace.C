...
Foam::treeDataFace::findAllIntersectOp::findAllIntersectOp
(
    const indexedOctree<treeDataFace>& tree,
...
bool Foam::treeDataFace::findAllIntersectOp::operator()
(
    const label index,
    const point& start,
    const point& end,
...
) const
{
    ...
    treeDataFace::findIntersectOp interOp(tree_);
    return interOp(index, start, end, intersectionPoint);
}
