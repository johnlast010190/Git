...
) const
{
    ...
    forAll(pp, facei)
    {
        // Use face-centre decomposition consistent with what is done
        // by octree search
...
) const
{
    // Collect all sample points and the faces they come from.
    {
        ...
        Pstream::allGatherList(globalFc);
...

    {
        ...
        Pstream::allGatherList(globalSamples);
        // Rework into straight list
...

    {
        ...
        Pstream::allGatherList(globalFaces);
...

    {
        ...
        Pstream::allGatherList(nPerProc);
...
) const
{
    ...
    switch (mode)
    {
        ...
        case NEARESTPATCHFACE:
        {
            ...
            else
            {
                ...
                //- The mapping can be very expensive
                //  Thus we do the search in bound box which expands with the
                // magSqr 1% of the patch span
                scalar span = 0.0;
                scalar maxSpan = magSqr(patchBb.span());
                bool foundHit = false;
                do
                {
                    span += 0.01*maxSpan;
                    ...
                    forAll(samples, sampleI)
                    {
                        ...
                        if (!nearInfo.hit())
                        {
                            ...
                            else
                            {
                                point fc
                                (
                                    pp[nearInfo.index()].centre(pp.points())
                                );
    ...
    if (debug)
    {
        ...
        forAll(samples, sampleI)
        {
            scalar distSq = magSqr(sampleLocations[sampleI] - samples[sampleI]);
            maxDistSq = max(maxDistSq, distSq);
        }
        reduce(maxDistSq, maxOp<scalar>());
        InfoInFunction
            << "Greatest matching distance: " << sqrt(maxDistSq) << endl;
    }
}
...
void Foam::mappedPatchBase::calcMapping() const
{
    ...
    if (debug)
    {
        // Check if all elements on other side send a value.
        PackedBoolList used(samplePolyPatch().size());
        forAll(subMap, proci)
        {
            ...
            forAll(map, i)
            {
                ...
                {
                    InfoInFunction
                        << "On patch " << samplePolyPatch().name()
                        << " patchface " << facei
                        << " is used more than once"
                        << " (i.e. mapping is not 1-1)." << endl;
                }
            }
        }
...
:
    ...
    mapPtr_(nullptr),
    AMIPtr_(nullptr),
    AMIInterpolationMethod_(AMIInterpolation::imFaceAreaWeight),
    AMILowWeightCorrection_(0.01),
    AMIReverse_(false),
    AMIRelTol_(debug::floatOptimisationSwitch("AMIRelTol", 0.01)),
    AMITol_(0.0),
    surfPtr_(nullptr),
...
:
    ...
    mode_
    (
        sampleModeNames_.lookupOrDefault("sampleMode", dict, NEARESTPATCHFACE)
    ),
    ...
    AMILowWeightCorrection_
    (
        dict.lookupOrDefault("AMILowWeightCorrection", 0.01)
    ),
    ...
    (
        ...
        (
            "relTol", scalar(debug::floatOptimisationSwitch("AMIRelTol", 0.01))
        )
    ),
    AMITol_(dict.lookupOrDefault("tol", scalar(0))),
    surfPtr_(nullptr),
...
:
    ...
    AMIReverse_(dict.lookupOrDefault<bool>("flipNormals", false)),
    AMIRelTol_(dict.lookupOrDefault("relTol", scalar(0.01))),
    AMITol_(dict.lookupOrDefault("tol", scalar(0))),
    surfPtr_(nullptr),
    surfDict_(dict.subOrEmptyDict("surface"))
{
...
:
    ...
    mapPtr_(nullptr),
    AMIPtr_(nullptr),
    AMIInterpolationMethod_(mpb.AMIInterpolationMethod_),
    AMILowWeightCorrection_(mpb.AMILowWeightCorrection_),
    AMIReverse_(mpb.AMIReverse_),
    AMIRelTol_(mpb.AMIRelTol_),
    AMITol_(mpb.AMITol_),
    surfPtr_(nullptr),
...
bool Foam::mappedPatchBase::sampleMeshFound() const
{
    return patch_.boundaryMesh().mesh().time().foundObject<polyMesh>
    (
        sampleRegion()
    );
}
...
void Foam::mappedPatchBase::write(Ostream& os) const
{
    ...
    if (mode_ == NEARESTPATCHFACEAMI)
    {
        ...
        os.writeEntry("AMILowWeightCorrection", AMILowWeightCorrection_);
        if (AMIRelTol_ != debug::floatOptimisationSwitch("AMIRelTol", 0.01))
        {
            os.writeEntry("relTol", AMIRelTol_);
        }
        os.writeEntry("tol", AMITol_);
    }
    if (!surfDict_.empty())
    {
