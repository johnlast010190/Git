...
namespace Foam
{
    ...
    scalar meshSearch::centretol_ = 1e-5;
}
...
Foam::vector Foam::meshSearch::centreOffset
(
    const point& bPoint,
    const label bFacei,
...
) const
{
    ...
    vector normDir = (c - bPoint);

    return rescale*centretol_*normDir;
}
    ...
    cellDecompMode_(cellDecompMode)
{
    ...
    // Extend slightly and make sure 3D
    overallBbPtr_() = overallBbPtr_().extend(1e-4);
...
Foam::List<Foam::pointIndexHit> Foam::meshSearch::allIntersection
(
    const point& pStart,
    const point& pEnd
) const
{
    ...
    treeDataFace::findAllIntersectOp allIntersectOp(boundaryTree(), shapeMask);
    ...
    while (true)
    {
        ...
        pointIndexHit curHit = boundaryTree().findLine
        (
            pStart,
            pEnd,
            allIntersectOp
    ...
    return List<pointIndexHit>(hits, true);
}


Foam::pointIndexHit Foam::meshSearch::firstIntersection
(
    const point& pStart,
    const point& pEnd
) const
...
) const
{
    ...
    do
    {
        label oldHit  = currentHit;

        bHit = firstIntersection(pt, pEnd);

        if (bHit.hit())
        {
            nHits++;

            currentHit = bHit.index();
            point currentHitPt = bHit.hitPoint();
            hits.append(bHit);

            //check for overlapping patches and add to hits
            {
                point perturbPt = currentHitPt
                    + offset(currentHitPt, currentHit, edgeVec);
                List<pointIndexHit> mHit =
                    allIntersection(currentHitPt,perturbPt);
                label currentHitPatchI =
                    mesh_.boundaryMesh().whichPatch(currentHit);

                forAll(mHit, hiti)
                {
                    label neiHitPatchI =
                        mesh_.boundaryMesh().whichPatch(mHit[hiti].index());
                    if (currentHitPatchI != neiHitPatchI)
                    {
                        bHit = mHit[hiti];
                        currentHit = bHit.index();
                        currentHitPt = bHit.hitPoint();
                        hits.append(bHit);
                        break;
                    }
                }
            ...
            const vector& area = mesh_.faceAreas()[currentHit];
            ...
            //Particle might be stuck try increasing offset direction
            if (nHits > 1e4)
            {
                //Must be lost so exit
                if (rescale > 10)
                {
                    break;
                }
            ...
            // of the destination. If not moving off a face use a shift
            // towards the cell centre
            if (currentHit == oldHit)
            {
                pt += centreOffset(bHit.hitPoint(), bHit.index(), rescale);
            }
            else
            {
                pt = bHit.hitPoint()
