...
#include "surfaceFilmModels/submodels/thermo/phaseChangeModel/noPhaseChange/noPhaseChange.H"
...
)
{
    ...
    const auto ctor =
        ctorTableLookup
        (
            "phaseChangeModel type",
            dictionaryConstructorTable_(),
            modelType
        );
    return autoPtr<phaseChangeModel>(ctor(model, phaseChangeDict));
}
