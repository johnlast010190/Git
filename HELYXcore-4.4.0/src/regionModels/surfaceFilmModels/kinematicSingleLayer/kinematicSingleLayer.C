...
#include "surfaceFilmModels/submodels/kinematic/filmViscosityModel/filmViscosityModel/filmViscosityModel.H"
...
void kinematicSingleLayer::resetPrimaryRegionSourceTerms()
{
    ...
    rhoSpPrimary_.forceAssign(dimensionedScalar(dimMass, 0));
    USpPrimary_.forceAssign(dimensionedVector(dimMass*dimVelocity, Zero));
    pSpPrimary_.forceAssign(dimensionedScalar(dimMass*dimVelocity, 0));
}
...
{
    coverage_.forceAssign(pos(delta_ - deltaSmall_));
}
...
void kinematicSingleLayer::predictDelta()
{
    ...
    delta_.forceAssign(alpha_*VbyA());

    // Update continuity error caused by the "delta" bounding
    updateContinuityErr();
}
...
)
{
    ...
    while (pimple_.correctNonOrthogonal())
    {
        ...
        {
            phi_.forceAssign(alphaEqn.flux());
...
:
    ...
    p_
    (
        IOobject("p", time().timeName(), regionMesh()),
        regionMesh(),
    ...
    Uw_
    (
        IOobject("Uw", time().timeName(), regionMesh()),
        U_
    ),
    phi_
    (
    ...
    phiU_
    (
        IOobject("phiU", time().timeName(), regionMesh()),
        regionMesh(),
        dimensionedScalar(dimVolume/dimTime, 0)
    ),
    continuityErr_
    (
        IOobject("continuityErr", time().timeName(), regionMesh()),
        regionMesh(),
    ...
    primaryMassTrans_
    (
        IOobject("primaryMassTrans", time().timeName(), regionMesh()),
        regionMesh(),
    ...
    cloudMassTrans_
    (
        IOobject("cloudMassTrans", time().timeName(), regionMesh()),
        regionMesh(),
        dimensionedScalar(dimMass, 0),
        zeroGradientFvPatchScalarField::typeName
    ),
    cloudDiameterTrans_
    (
        IOobject("cloudDiameterTrans", time().timeName(), regionMesh()),
        regionMesh(),
    ...
    (
        IOobject("primaryMomentumTrans", time().timeName(), regionMesh()),
        regionMesh(),
    ...
    rhoSp_
    (
        IOobject("rhoSp", time_.timeName(), regionMesh()),
        regionMesh(),
        dimensionedScalar(dimDensity/dimTime, 0)
    ),
    USp_
    (
        IOobject("USp", time().timeName(), regionMesh()),
        regionMesh(),
        dimensionedVector(dimDensity*dimVelocity/dimTime, Zero)
    ),
    pSp_
    (
        IOobject("pSp", time_.timeName(), regionMesh()),
        regionMesh(),
        dimensionedScalar(dimPressure, 0)
    ),
    rhoSpPrimary_
    (
        IOobject(rhoSp_.name(), time().timeName(), primaryMesh()),
        primaryMesh(),
        dimensionedScalar(dimMass, 0)
    ),
    USpPrimary_
    (
        IOobject(USp_.name(), time().timeName(), primaryMesh()),
        primaryMesh(),
        dimensionedVector(dimMass*dimVelocity, Zero)
    ),
    pSpPrimary_
    (
        IOobject(pSp_.name(), time().timeName(), primaryMesh()),
        primaryMesh(),
        dimensionedScalar(dimMass*dimVelocity, 0)
    ),
    UPrimary_
    (
        IOobject("U", time().timeName(), regionMesh()),
        regionMesh(),
        dimensionedVector(dimVelocity, Zero),
    ...
    rhoPrimary_
    (
        IOobject("thermo:rho", time().timeName(), regionMesh()),
        regionMesh(),
    ...
    muPrimary_
    (
        IOobject("mu", time().timeName(), regionMesh()),
        regionMesh(),
        dimensionedScalar(dimPressure*dimTime, 0),
        this->mappedFieldAndInternalPatchTypes<scalar>()
    ),
    ...
    turbulence_(turbulenceModel::New(*this, coeffs_)),
    forces_(*this, coeffs_),
    // fvOptions_(fv::options::New(regionMesh())),
    addedMassTotal_(0)
{
    alpha_.forceAssign(delta_/VbyA());
...
void kinematicSingleLayer::preEvolveRegion()
{
    ...
    availableMass_ = mass();
    cloudMassTrans_.forceAssign(dimensionedScalar(dimMass, 0));
    cloudDiameterTrans_.forceAssign(dimensionedScalar(dimLength, 0));
    primaryMassTrans_.forceAssign(dimensionedScalar(dimMass, 0));
    primaryMomentumTrans_.forceAssign
    (
        dimensionedVector(dimMass*dimVelocity, Zero)
    );
}
