...
#include "thermalBaffle/thermalBaffle/thermalBaffle.H"
...
tmp<volScalarField::Internal> thermalBaffle::calcDelta() const
{
    ...
    DynamicList<LayerInfoData<point>> initialFaceInfoPoints;
    {
        ...
        const vectorField::subField pfc = pp.faceCentres();

        forAll(pp, ppFacei)
        {
            const point& c = pfc[ppFacei];
    ...
    (
        volScalarField::Internal::New("thickness", regionMesh(), dimLength)
    );
...
void thermalBaffle::solveEnergy()
{
    ...
    const volScalarField alpha("alpha", (thermo_->kappa()/thermo_->Cp())*dByT);

    fvScalarMatrix hEqn
    (
        fvm::ddt(rho, he_)
      - fvm::laplacian(alpha, he_)
     ==
        Q_ + Qs_/thickness_
    );
...
const volScalarField& thermalBaffle::Cp() const
{
    return thermo_->Cp();
}


const tmp<volScalarField> thermalBaffle::kappaRad() const
{
...
const tmp<volScalarField> thermalBaffle::kappa() const
{
    ...
    forAll(coupledPatches, i)
    {
        ...
        Info<< indent << "Q : " << patchName << indent
            ...
            (
               ...
               *phe.snGrad()
               *(
                    thermo_->kappa().boundaryField()[patchi]
                   /thermo_->Cp().boundaryField()[patchi]
                )
            )
            << endl;
    }
