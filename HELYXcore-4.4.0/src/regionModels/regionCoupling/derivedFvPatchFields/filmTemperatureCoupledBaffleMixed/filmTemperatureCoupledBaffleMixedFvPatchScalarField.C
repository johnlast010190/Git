...
#include "regionCoupling/derivedFvPatchFields/filmTemperatureCoupledBaffleMixed/filmTemperatureCoupledBaffleMixedFvPatchScalarField.H"
...
void Foam::filmTemperatureCoupledBaffleMixedFvPatchScalarField::updateCoeffs()
{
    ...
    scalarField TFilm(film.thermo().T().boundaryField()[filmPatch]);
    scalarField deltaFilm(film.delta().boundaryField()[filmPatch]);
    scalarField filmKappa
    (
        film.thermo().kappa().boundaryField()[filmPatch]
    );

    filmTemperatureCoupledBaffleMixedFvPatchScalarField nbrField =
        refCast<const filmTemperatureCoupledBaffleMixedFvPatchScalarField>
        (
    ...
    if (mpp.sampleRegion() != "region0")
    {
        ...
        tmp<scalarField> nbrIntFld(new scalarField(nbrField.size(), 0));
        tmp<scalarField> nbrKDelta(new scalarField(nbrField.size(), 0));
    ...
    else
    {
        ...
        nbrKDelta.ref() =
            2.0*alphaFilm*filmKappa/(mag(deltaFilm) + ROOTVSMALL)
