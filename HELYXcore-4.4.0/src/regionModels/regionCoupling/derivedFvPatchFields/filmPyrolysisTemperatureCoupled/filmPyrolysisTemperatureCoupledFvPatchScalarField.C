...
#include "regionCoupling/derivedFvPatchFields/filmPyrolysisTemperatureCoupled/filmPyrolysisTemperatureCoupledFvPatchScalarField.H"
...
void Foam::filmPyrolysisTemperatureCoupledFvPatchScalarField::updateCoeffs()
{
    ...
    typedef regionModels::surfaceFilmModels::thermoSingleLayer
        filmModelType;
    typedef regionModels::pyrolysisModel pyrModelType;
    ...
    scalarField coverageFilm(filmModel.coverage().boundaryField()[filmPatchi]);
    filmModel.toPrimary(filmPatchi, coverageFilm);

    scalarField TFilm(filmModel.thermo().T().boundaryField()[filmPatchi]);
    filmModel.toPrimary(filmPatchi, TFilm);

    // Retrieve pyrolysis model
    pyrModelType& pyrModel =
        db().time().lookupObjectRef<pyrModelType>(pyrolysisRegionName_);

    const label pyrPatchi = pyrModel.regionPatchID(patchi);

    scalarField TPyr(pyrModel.sThermo().T().boundaryField()[pyrPatchi]);
    pyrModel.toPrimary(pyrPatchi, TPyr);


    // Evaluate temperature
    Tp = coverageFilm*TFilm + (1.0 - coverageFilm)*TPyr;
