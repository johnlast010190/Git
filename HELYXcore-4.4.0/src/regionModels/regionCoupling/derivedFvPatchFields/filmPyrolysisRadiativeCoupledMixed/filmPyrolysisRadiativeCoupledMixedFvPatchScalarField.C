...
#include "regionCoupling/derivedFvPatchFields/filmPyrolysisRadiativeCoupledMixed/filmPyrolysisRadiativeCoupledMixedFvPatchScalarField.H"
...
:
    ...
    filmDeltaDry_(dict.lookup<scalar>("filmDeltaDry")),
    filmDeltaWet_(dict.lookup<scalar>("filmDeltaWet"))
{
...
void filmPyrolysisRadiativeCoupledMixedFvPatchScalarField::updateCoeffs()
{
    ...
    const scalarField nbrK(nbrField.kappa());
    ...
    if (pyrolysisRegionName_ == mesh.name())
    {
        ...
        if (qrName_ != "none")
        {
            qr = nbrPatch.lookupPatchField<volScalarField, scalar>
            (
                qrName_
            );
            mpp.distribute(qr);
    ...
    else if (pyrolysis.primaryMesh().name() == mesh.name())
    {
        ...
        if (qrName_ != "none")
        {
            qr = patch().lookupPatchFieldInDb<volScalarField, scalar>
            (
                this->db(),
                qrName_
            );
    ...
    tmp<volScalarField::Internal> thecw(film.htcw().h());
    const scalarField htcw
    (
        UIndirectList<scalar>
        (
            thecw.ref().field(),
            film.thermo().T().boundaryField()[filmPatchi].patch().faceCells()
        )
    );
    ...
    Tfilm = film.thermo().T().boundaryField()[filmPatchi];
