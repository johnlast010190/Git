...
:
    ...
    isMatModels_(false)
{
    if (isType<dummyThermo>(specieThermos_[0]))
    {
        isMatModels_ = true;
        hfModels_.resize(nSpecie_);
        CpModels_.resize(nSpecie_);
        dCpdTModels_.resize(nSpecie_);
        rhoModels_.resize(nSpecie_);
        alphavModels_.resize(nSpecie_);
        Ws_.resize(nSpecie_);
        haModels_.resize(nSpecie_);
        const word phaseName(Yvf_.first().group());
        forAll(Yvf_, speciei)
        {
            const word specieName(Yvf_[speciei].member());
            ...
            dCpdTModels_.set(speciei, speciesModels[dCpdTModel::typeName]);
            rhoModels_.set(speciei, speciesModels[rhoModel::typeName]);
            alphavModels_.set(speciei, speciesModels[alphavModel::typeName]);

            // Molar weights are the same throughout the run
            Ws_[speciei] = (*speciesModels[WModel::typeName])[0];
    ...
    if (reduction_)
    {
        const speciesMassFractions& composition = this->thermo().composition();
...
template<class ThermoType>
Foam::scalar Foam::chemistryModel<ThermoType>::alphav
(
    label i,
    scalar p,
    scalar T
) const
{
    return
        isMatModels_
      ? alphavModels_[i].value(p, T)
      : specieThermos_[i].alphav(p, T);
}


template<class ThermoType>
Foam::scalar Foam::chemistryModel<ThermoType>::Cp
(
    label i,
    scalar p,
    scalar T
) const
{
    return
        isMatModels_
      ? CpModels_[i].value(p, T)
      : specieThermos_[i].Cp(p, T);
}


template<class ThermoType>
Foam::scalar Foam::chemistryModel<ThermoType>::ha
(
    label i,
    scalar p,
    scalar T
) const
{
    return
        isMatModels_
      ? haModels_[i].value(p, T)
      : specieThermos_[i].ha(p, T);
}


template<class ThermoType>
Foam::scalar Foam::chemistryModel<ThermoType>::dCpdT
(
    label i,
    scalar p,
    scalar T
) const
{
    return
        isMatModels_
      ? dCpdTModels_[i].value(p, T)
      : specieThermos_[i].dCpdT(p, T);
}


template<class ThermoType>
Foam::scalar Foam::chemistryModel<ThermoType>::rho
(
    label i,
    scalar p,
    scalar T
) const
{
    return
        isMatModels_
      ? rhoModels_[i].value(p, T)
      : specieThermos_[i].rho(p, T);
}


template<class ThermoType>
void Foam::chemistryModel<ThermoType>::initMolarFractions() const
{
    Ws_.resize(nSpecie_);
    if (isMatModels_)
    {
        const objectRegistry& matObj =
        Yvf_.first().db().subRegistry("materialModels");
        materialTables& mat =
            matObj.lookupObjectRef<materialTables>("materialTables");

        forAll(Yvf_, speciei)
        {
            const word specieName(Yvf_[speciei].member());
            const word phaseName(Yvf_[speciei].group());

            // Molar weights are the same throughout the run
            Ws_[speciei] =
                (*mat.sTable(phaseName, specieName)[WModel::typeName])[0];
        }
    }
    else
    {
        ...
        {
            Ws_[speciei] = specieThermos_[speciei].W();
        }
    }
}
...
) const
{
    ...
    {
        const scalar WiByrhoM = W(sToc(i))/rhoM;
...
) const
{
    ...
    for (label i=0; i<nSpecie_; i++)
    {
        const scalar rhoMByWi = rhoM/W(sToc(i));
    ...
    for (label i=0; i<Y_.size(); i++)
    {
        alphavM += Y_[i]*rhoM*v[i]*alphav(i, p, T);
    }
...
Foam::chemistryModel<ThermoType>::reactionQdot(const label reactioni) const
{
    tmp<volScalarField::Internal> tQdot
    (
    ...
    PtrList<volScalarField::Internal> RR = reactionRR(reactioni);
    ...
    forAll(Yvf_, i)
    {
        const scalar hi
        (
            isMatModels_ ? hfModels_[i][0] : specieThermos_[i].hf()
        );
        forAll(Qdot, celli)
        {
...
) const
{
    ...
    const scalarField Tvf(this->thermo().TAbs());
    const scalarField pvf(this->thermo().pAbs());
...
void Foam::chemistryModel<ThermoType>::calculate()
{
    ...
    const scalarField T(this->thermo().TAbs());
    const scalarField p(this->thermo().pAbs());
    ...
    forAll(rho, celli)
    {
        ...
        {
            RR_[sToc(i)][celli] = dNdtByV[i]*W(sToc(i));
        }
    }
}
...
)
{
    ...
    // Support of legacy solvers
    tmp<volScalarField> trho0vf;
    const word rhoName(this->thermo().phasePropertyName("rho"));
    if (this->mesh().template foundObject<volScalarField>(rhoName))
    {
        trho0vf =
            volScalarField::New
            (
    ...
    else
    {
        trho0vf =
            ...
            (
                rhoName + "0",
                this->thermo().rho()().oldTime()
            );
    }
    const volScalarField& rho0vf = trho0vf.ref();

    const scalarField T0vf
    (
        this->thermo().T().oldTime() + this->thermo().TRef()
    );
    const scalarField p0vf
    (
        this->thermo().p().oldTime() + this->thermo().pRef()
    );
