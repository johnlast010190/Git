...
)
{
    ...
    scalar nodea = ((ownerPoint & n) - hMin)/stabilise(hMax-hMin, SMALL)*(nPlanes-1);
    scalar nodeb = ((nbrPoint & n) - hMin)/stabilise(hMax-hMin, SMALL)*(nPlanes-1);
    if (nodea > nodeb)
    {
        scalar temp = nodea;
        nodea = nodeb;
    ...
    {
        scalar height = nodel*(hMax-hMin)/max(nPlanes-1, 1)+hMin;
...
)
{
    ...
    forAll(f, pi)
    {
        const scalar node =
            ((points[pi] & n) - hMin)
           /stabilise(hMax-hMin, SMALL)*(nPlanes - 1);
...
)
{
    ...
    */
    ...
    volVectorField C("C", f.db(), mesh, mesh.C().dimensions());
