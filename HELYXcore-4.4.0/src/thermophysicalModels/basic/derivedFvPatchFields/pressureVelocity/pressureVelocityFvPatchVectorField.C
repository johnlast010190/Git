...
)
{
    if (isCoupledSolver())
    {
        ...
        if (isA<basePressureFvPatchScalarField>(p.boundaryField()[patchi]))
        {
            ...
            vectorField Up(*this);
            vectorField UpRef(this->size(), Zero);
            if (ppres.coorFramePtr())
            {
                ppres.makeRelative(UpRef);
            }
            ...
            forAll(faceCells, facei)
            {
                ...
                */
                ...
                if (ppres.coorFramePtr())
                {
                    source[faceCelli] -= tCoeff&UpRef[facei];
                }
            }
        }
    }
    ...
    if (isA<basePressureFvPatchScalarField>(p.boundaryField()[patchi]))
    {
        ...
        forAll(faceCells, facei)
        {
            ...
            */
            ...
            if (ppres.coorFramePtr())
            {
                scalar explicitTerm = -(coeffTerm&UpRef[facei]);
                source[faceCelli] += explicitTerm;
                blockB[facei] += explicitTerm;
            }
        }
...
void Foam::pressureVelocityFvPatchVectorField::write(Ostream& os) const
{
    ...
    writeEntryIfDifferent<Switch>(os, "inletFlux", false, inletFlux_);
    os.writeEntry
    (
        "directionSpecification",
