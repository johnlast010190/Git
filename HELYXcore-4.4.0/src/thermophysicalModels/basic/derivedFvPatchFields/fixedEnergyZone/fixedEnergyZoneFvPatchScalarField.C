...
)
{
    ...
    const label& zoneId =
        pMesh.cellZones().findZoneID("inactive_" + faceZoneName);
    ...
    const scalarField pField(zoneCells.size(), pw.cellZoneValue());
    const scalarField TField(zoneCells.size(), Tw.cellZoneValue());
    tmp<scalarField> tE(thermo.he(pField, TField, zoneCells));

    matrix.setValues(zoneCells, tE());
