...
bool Foam::totalPressureFvPatchScalarField::isCoupledSolver() const
{
...
)
{
    ...
    if (internalField().dimensions() == dimPressure)
    {
        if (psiName_ == "none")
        {
            ...
            vectorField UpRel(Up);
            makeRelative(UpRel);
            scalarField phipRel(phip);
            if (!isCoupledSolver())
            {
                scalarField phiRef(this->patch().size(), 0.0);
                makeRelative(phiRef);
                if
                (
                    phip.internalField().dimensions()
            ...
            pp = p0p - 0.5*rhop*(1.0 - pos0(phipRel))*magSqr(UpRel);
        }
        else
        {
    ...
    else if (internalField().dimensions() == dimPressure/dimDensity)
    {
       ...
       if (!isCoupledSolver())
       {
           makeRelative(phipRel);
       }
       pp = p0p - 0.5*neg(phipRel)*magSqr(UpRel);
    }
    else
    {
