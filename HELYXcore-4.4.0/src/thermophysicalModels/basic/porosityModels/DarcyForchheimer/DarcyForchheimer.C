...
void Foam::porosityModels::DarcyForchheimer::computeCylindricalCoeffs()
{
    ...
    scalar rad1 = coeffs_.lookup<scalar>("innerRadius");
    scalar rad2 = coeffs_.lookup<scalar>("outerRadius");
    scalar height = coeffs_.lookup<scalar>("height");
...
void Foam::porosityModels::DarcyForchheimer::updateCorrCoeffs() const
{
    ...
    if (temperatureDependence_ && obr_.foundObject<volScalarField>(TName_) && !updated)
    {
        ...
        forAll(cellZoneIDs_, zoneI)
        {
            ...
            {
                scalar TOffset = basicThermo::TRefIfFound(obr_).value();
                forAll(cells, i)
                {
                    label cellI = cells[i];
                    ddnZones[i] = calcDcorrValue(T[cellI]+TOffset, TRef_+TOffset);
                    ffnZones[i] = calcFcorrValue(T[cellI]+TOffset, TRef_+TOffset);
                }
            }
        }
    }
