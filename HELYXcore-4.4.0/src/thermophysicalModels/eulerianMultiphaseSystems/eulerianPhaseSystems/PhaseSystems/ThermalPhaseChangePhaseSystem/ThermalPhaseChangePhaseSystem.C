...
:
    HeatAndMassTransferPhaseSystem<BasePhaseSystem>(mesh, coeffs),
    volatile_(coeffs.lookup("volatile")),
    saturationModel_(eulerianSaturationModel::New(coeffs.subDict("eulerianSaturationModel"))),
    massTransfer_(coeffs.lookup("massTransfer"))
{
...
template<class BasePhaseSystem>
const Foam::eulerianSaturationModel&
...
Foam::ThermalPhaseChangePhaseSystem<BasePhaseSystem>::massTransfer() const
{
    ...
    eulerianPhaseSystem::massTransferTable& eqns = eqnsPtr();
...
void Foam::ThermalPhaseChangePhaseSystem<BasePhaseSystem>::correctThermo()
{
    ...
    )
    {
        ...
        {
            iDmdtNew.forceAssign(dimensionedScalar(dmdt.dimensions(), 0));
        }
        ...
        H1.boundaryFieldRef() =
            max(H1.boundaryField(), phase1.volFrac().boundaryField()*HLimit);
        H2.boundaryFieldRef() =
            max(H2.boundaryField(), phase2.volFrac().boundaryField()*HLimit);
        ...
        scalar iDmdtRelax(this->mesh().solution().
            fieldRelaxationFactor("iDmdt"));
        ...
        if
        (
            phase2.volFrac().mesh().foundObject<volScalarField>
            (
                "alphat." +  phase2.name()
            )
        )
        {
            const volScalarField& alphat =
                phase2.volFrac().mesh().lookupObject<volScalarField>
                (
                    "alphat." +  phase2.name()
                );
