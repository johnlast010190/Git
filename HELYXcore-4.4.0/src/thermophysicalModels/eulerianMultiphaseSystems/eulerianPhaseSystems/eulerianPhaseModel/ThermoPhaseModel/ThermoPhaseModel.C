...
:
    ...
    (
        &refCast<ThermoType>
        (
            multiphaseThermo::lookupOrCreate(fluid.mesh(), phaseName)
        )
    )
{
    thermo_->validate
    (
        IOobject::groupName(eulerianPhaseModel::typeName, this->name()),
        "h",
        "e"
    );
}
...
template<class BasePhaseModel, class ThermoType>
Foam::rhoThermo& Foam::ThermoPhaseModel<BasePhaseModel, ThermoType>::thermo()
{
    return *thermo_;
}
