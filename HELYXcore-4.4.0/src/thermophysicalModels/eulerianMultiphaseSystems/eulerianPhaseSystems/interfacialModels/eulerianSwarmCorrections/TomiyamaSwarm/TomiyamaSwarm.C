...
Foam::eulerianSwarmCorrections::TomiyamaSwarm::TomiyamaSwarm
(
...
Foam::eulerianSwarmCorrections::TomiyamaSwarm::Cs() const
{
    return
        pow
        (
            max(this->pair_.continuous().volFrac(), residualAlpha_),
            scalar(3) - 2*l_
        );
}
