...
namespace eulerianDragModels
{
    ...
    addToRunTimeSelectionTable(eulerianDragModel, IshiiZuber, dictionary);
}
}
...
Foam::eulerianDragModels::IshiiZuber::IshiiZuber
(
...
Foam::eulerianDragModels::IshiiZuber::CdRe() const
{
    ...
    (
        muc
       *pow(max(1 - pair_.dispersed().volFrac(), scalar(1e-3)), -2.5*muStar)
    );
    ...
    volScalarField F((muc/muMix)*sqrt(max(1 - pair_.dispersed().volFrac(), scalar(0))));
    F.max(1e-3);
    ...
    return
       ...
       *min(CdReEllipse, Re*sqr(1 - pair_.dispersed().volFrac())*2.66667)
