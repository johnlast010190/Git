...
:
    ParentType(List<Face>(), pointField())
{
    const pointField& bPoints = pp.localPoints();
    const faceList& bFaces = pp.localFaces();

    List<pointField> gatheredPoints(Pstream::nProcs());
    gatheredPoints[Pstream::myProcNo()] = bPoints;
    Pstream::gatherList(gatheredPoints);

    List<List<face>> gatheredFaces(Pstream::nProcs());
    gatheredFaces[Pstream::myProcNo()] = bFaces;
    Pstream::gatherList(gatheredFaces);

    pointField globalPoints(0);
    ...
    if (Pstream::master())
    {
        ...
        surfZoneList newZones(0);

        reset
        (
            std::move(globalPoints),
            std::move(globalFaces),
            std::move(newZones)
        );
        if (triangulate)
        {
...
)
{
    ...
    else
    {
        ...
        forAll(faceLst, facei)
        {
            ...
            {
                newFaces[nTri] = triEngine.triPoints(triI, f);
                faceMap[nTri] = facei;
                nTri++;
            }
        }
...
template<class Face>
Foam::List<Face> Foam::MeshedSurface<Face>::stdMoveFaces()
{
...
template<class Face>
Foam::pointField Foam::MeshedSurface<Face>::stdMovePoints()
{
    // Topology changed because of transfer
    ParentType::clearOut();
...
template<class Face>
Foam::surfZoneList Foam::MeshedSurface<Face>::stdMoveZones()
{
    return std::move(this->storedZones());
}
