...
Foam::viscosityModels::approachingNu::calcNu() const
{
    ...
    if (current_time < some_fixed_time_.value())
    {
        ...
        return
        (
            strainRate() * dimensionedScalar(nu_desired_.dimensions()*dimTime, 0) // dummy for adding strainRate dependend model
...
Foam::viscosityModels::approachingNu::calcNu() const
{
    ...
    if (current_time < some_fixed_time_.value())
    {
        ...
        (
            strainRate()
           *dimensionedScalar(nu_desired_.dimensions()*dimTime, 0)
