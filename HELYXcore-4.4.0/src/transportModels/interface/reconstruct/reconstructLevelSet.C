...
void reconstructLevelSet::solveReinitializationEqn()
{
    ...
    //- calculate psi0_
    psi0_.forceAssign((2.0*alpha1_ - 1.0)*gamma_);
    psi0_.correctBoundaryConditions();

    //- correct psi_
    psi_.forceAssign(psi0_);

    dimensionedScalar ds("ds", dimLength, 1.0);
    for (int corr=0; corr<epsC_*10; corr++)
    {
        psi_.forceAssign
        (
            psi_
          + psi0_/mag(psi0_ + VSMALL)*(1.0 - mag(fvc::grad(psi_)*ds))*deltaTau_
        );
...
:
    ...
    epsC_(transpProp.subDict("reconstruct").lookup<scalar>("epsC")),
