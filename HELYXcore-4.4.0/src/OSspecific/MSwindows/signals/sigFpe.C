...
{
    return (isTrue("FOAM_SIGFPE", switchFpe_) || isTrue("HELYX_SIGFPE", switchFpe_));
}
...
void Foam::sigFpe::set(bool verbose)
{
    if (!sigActive_ && requested())
    {
        ...
        if (verbose)
        {
            ...
            if (sigActive_)
            {
                Info<< "enabled (HELYX_SIGFPE)." << endl;
            }
            else
            {
    ...
    nanActive_ = false;
    if (isTrue("FOAM_SETNAN", switchNan_) || isTrue("HELYX_SETNAN", switchNan_))
    {
        if (verbose)
        {
