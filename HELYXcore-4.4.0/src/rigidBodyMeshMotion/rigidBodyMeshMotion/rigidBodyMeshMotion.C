...
Foam::List<Foam::septernion> Foam::rigidBodyMeshMotion::transforms0() const
{
    ...
    forAll(bodyMeshes_, bi)
    {
        ...
        transforms0[bi] = septernion(model_.transform0(bodyMeshes_[bi].bodyID_));
    }
...
void Foam::rigidBodyMeshMotion::solve()
{
    ...
    {
        const septernion transform0(model_.transform0(bodyMeshes_[0].bodyID_));
            ...
            if (oldPointi >= 0)
            {
                ...
                else
                {
                    if (bodyMeshes_.size() == 1)
                    {
                        ...
                        {
                            points0[pointi] =
                                model_.transform0(bodyMeshes_[0].bodyID_).inv()
                        ...
                        else
                        {
                            points0[pointi] =
                                ...
                                (
                                    septernion::I,
                                    septernion(model_.transform0(bodyMeshes_[0].bodyID_)),
