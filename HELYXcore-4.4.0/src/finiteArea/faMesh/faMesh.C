...
const Foam::HashSet<Foam::word> Foam::faMesh::geometryFields;
...
:
    ...
    (
        pMesh,
        pMesh.boundaryMesh()[polyPatchID].name()
    ),
    edgeInterpolation(*this),
    data(pMesh.thisDb()),
    faceLabels_
...
const Foam::edgeScalarField& Foam::faMesh::phi() const
{
    ...
    if (phiPtr_->timeIndex() != time().timeIndex())
    {
        (*phiPtr_) = dimensionedScalar(dimArea/dimTime, 0);
    }
...
bool Foam::faMesh::write(const bool write) const
{
    bool ok = true;
    if (phiPtr_)
    {
