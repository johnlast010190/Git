...
void Foam::faMesh::topoChange(const polyTopoChangeMap& map)
{
    if (debug)
    {
        Info<< "bool faMesh::topoChange(const polyTopoChangeMap& map) const : "
            << "Updating mesh" << endl;
    }

    // Create fa mesh mapper, using the old mesh
    const faMeshMapper mapper(*this, map);
...
void Foam::faMesh::mapFields(const faMeshMapper& mapper) const
{
    ...
    MapGeometricFields<scalar, faePatchField, faMeshMapper, faEdgeMesh>(mapper);
    MapGeometricFields<vector, faePatchField, faMeshMapper, faEdgeMesh>(mapper);
    MapGeometricFields<sphericalTensor, faePatchField, faMeshMapper, faEdgeMesh>
        (mapper);
    MapGeometricFields<symmTensor, faePatchField, faMeshMapper, faEdgeMesh>
        (mapper);
    MapGeometricFields<tensor, faePatchField, faMeshMapper, faEdgeMesh>(mapper);
}
