...
void Foam::faPatch::calcPointEdges() const
{
    ...
    forAll(pointEdgs, pointI)
    {
        ...
        label i = 0;
        forAllIter(SLList<label>, pointEdgs[pointI], curEdgesIter)
        {
...
{
    return edgeLengths()/magEdgeLengths();
}
...
{
    const vectorField cf(edgeFaceCentres());
    const vectorField& ce = edgeCentres();

    tmp<vectorField> deltast(ce - cf);
    vectorField& deltas = deltast.ref();

    // Use patch-normal delta for all non-coupled BCs
    const vectorField nHat(edgeNormals());

    scalarField nHatEEf ( nHat & deltas );
...
void Foam::faPatch::movePoints(PstreamBuffers&, const pointField& points)
{}
