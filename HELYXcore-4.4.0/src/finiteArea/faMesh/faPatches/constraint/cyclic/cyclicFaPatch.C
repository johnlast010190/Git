...
void Foam::cyclicFaPatch::calcTransforms()
{
    if (size() > 0)
    {
        ...
        for (label edgei = 0; edgei < size()/2; edgei++)
        {
            ...
            else if (mag(magLe - nbrMagLe)/avLe > matchTolerance_)
            {
        ...
        if (maxMatchError > matchTolerance_)
        {
            ...
            FatalErrorInFunction
                ...
                << " matching tolerance:" << matchTolerance_
                << endl
...
void Foam::cyclicFaPatch::initMovePoints(PstreamBuffers& pBufs, const pointField& p)
{
    faPatch::initMovePoints(pBufs, p);
}


void Foam::cyclicFaPatch::movePoints(PstreamBuffers& pBufs, const pointField& p)
{
...
Foam::tmp<Foam::vectorField> Foam::cyclicFaPatch::delta() const
{
    ...
    else
    {
        for (label edgei = 0; edgei < sizeby2; edgei++)
        {
            ...
            pdv[edgei] = ddi - Foam::transform(forwardT()[0], dni);
            pdv[edgei + sizeby2] = - Foam::transform(reverseT()[0], pdv[edgei]);
        }
    }
