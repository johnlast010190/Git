...
//     }
    ...
    GeometricField<scalar, faePatchField, faEdgeMesh>::Boundary&
    ...
    forAll(bWeights, patchI)
    {
        if (bWeights[patchI].coupled())
        {
            ...
            vectorField CP ( mesh.areaCentres().boundaryField()[patchI].patchInternalField() );

            vectorField CN ( mesh.areaCentres().boundaryField()[patchI].patchNeighbourField() );

            vectorField nP ( mesh.faceAreaNormals().boundaryField()[patchI].patchInternalField() );

            vectorField nN ( mesh.faceAreaNormals().boundaryField()[patchI].patchNeighbourField() );
