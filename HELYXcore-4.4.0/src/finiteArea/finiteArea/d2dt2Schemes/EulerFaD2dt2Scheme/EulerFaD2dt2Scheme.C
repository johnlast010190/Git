...
#include "finiteArea/d2dt2Schemes/EulerFaD2dt2Scheme/EulerFaD2dt2Scheme.H"
...
)
{
    ...
    if (mesh().moving())
    {
        ...
        tdt2dt2.ref().primitiveFieldRef() = halfRdeltaT2*dt.value()
                                            *(coefft*SS0 - (coefft*SS0 + coefft00*S0S00)  + coefft00*S0S00)/mesh().S();
...
)
{
    ...
    else
    {
        ...
        scalarField rhoRho0 ( (rho.primitiveField() + rho.oldTime().primitiveField()) );
