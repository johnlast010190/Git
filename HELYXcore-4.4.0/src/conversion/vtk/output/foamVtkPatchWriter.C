...
void Foam::vtk::patchWriter::writePolysLegacy(const label pointOffset)
{
    ...
    if (parallel_)
    {
        vtk::writeListParallel(format_.rawRef(), vertLabels);
    }
    else
    {
...
void Foam::vtk::patchWriter::writePolys(const label pointOffset)
{
    ...
    //
    {
        ...
        label off =
        (
            parallel_ ? globalIndex(nLocalVerts_).offset(Pstream::myProcNo()) : 0
        );
        ...
        if (parallel_)
        {
            vtk::writeListParallel(format_.rawRef(), vertOffsets);
        }
        else
        {
            vtk::writeList(format_.rawRef(), vertOffsets);
        }
...
bool Foam::vtk::patchWriter::writeGeometry()
{
    ...
    (
        parallel_ ? globalIndex(nLocalPoints_).offset(Pstream::myProcNo()) : 0
    );
...
bool Foam::vtk::patchWriter::writeNeighIDs()
{
    ...
    if (parallel_ ? Pstream::master() : true)
    {
        for (const label patchId : patchIDs_)
        {
            ...
            const auto* pp = dynamic_cast<const processorPolyPatch*>(&patches[patchId]);
