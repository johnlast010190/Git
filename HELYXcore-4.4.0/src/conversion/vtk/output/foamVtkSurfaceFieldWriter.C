...
) const
{
    ...
    {
        const polyPatch& temp = mesh_.boundaryMesh()[patchi];
...
void Foam::vtk::surfaceFieldWriter::write(const surfaceVectorField& field)
{
    ...
    {
        // Internal field
        vtk::writeListParallel(format_.rawRef(), internal);

        // Boundary field
        vtk::writeListParallel(format_.rawRef(), boundary);
    }
    else
    {
        // Non-parallel
        ...
        // Internal field
        vtk::writeList(format_.rawRef(), internal);

        // Boundary field
        vtk::writeList(format_.rawRef(), boundary);
    }
