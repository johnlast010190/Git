...
void Foam::vtk::internalWriter::beginPiece()
{
    ...
    {
        reduce(
            std::tie(numberOfPoints_, numberOfCells_),
...
void Foam::vtk::internalWriter::writeCellsLegacy(const label pointOffset)
{
    ...
    // CELL_TYPES
    {
        ...
        if (parallel_)
        {
            vtk::writeListParallel(format_.rawRef(), cellTypes);
        }
        else
        {
...
void Foam::vtk::internalWriter::writeCellsConnectivity(const label pointOffset)
{
    ...
    //
    {
        ...
        if (parallel_)
        {
            ...
            vtk::writeListParallel(format_.rawRef(), vertOffsets, procOffset);
        }
        else
        {
        ...
        if (parallel_)
        {
            ...
            if (faceOffsets.size()) // Or check procOffset.localSize()
            {
                faceOffsetsRenumber =
                    ...
                    (
                        faceOffsets,
                        procOffset.offset(Pstream::myProcNo())
                    );
            }
            else
            {
            ...
            vtk::writeListParallel(format_.rawRef(), faceOffsetsRenumber);
        }
        else
        {
...
bool Foam::vtk::internalWriter::beginFile(std::string title)
{
    ...
    fileName caseName = mesh_.time().globalCaseName();
    caseName.removeRepeated('-');

    fileName meshName = mesh_.name();
    meshName.removeRepeated('-');
...
bool Foam::vtk::internalWriter::writeGeometry()
{
    ...
    (
        parallel_ ? globalIndex(vtuCells_.nFieldPoints()).offset(Pstream::myProcNo()) : 0
    );
...
void Foam::vtk::internalWriter::writeCellIDs()
{
    ...
    if (parallel_)
    {
        ...
        vtk::writeListParallel(format_.rawRef(), cellMap, globalCellOffset);
    }
    else
    {
...
void Foam::vtk::internalWriter::writePointIDs()
{
    ...
    const label pointOffset =
    (
        parallel_ ? globalIndex(vtuCells_.nPoints()).offset(Pstream::myProcNo()) : 0
    );

    // Cell offset for *regular* mesh cells (without decomposed)
    const label cellOffset =
    (
        parallel_ ? globalIndex(vtuCells_.nCells()).offset(Pstream::myProcNo()) : 0
    );


    // TODO:  There's a new identity override in 1906 that depends on labelUList::begin and end functions
    // I'm not backporting those, so check that this call to identiy returns the right range of point IDs
    // labelList pointIds = identity(vtuCells_.nFieldPoints(), pointOffset);
    labelList pointIds = identity(pointOffset);
    ...
    if (parallel_)
    {
        vtk::writeListParallel(format_.rawRef(), pointIds);
    }
    else
    {
