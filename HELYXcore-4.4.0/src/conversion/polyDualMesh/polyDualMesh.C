...
)
{
    ...
    return labelList(dualFace, true);
}
...
)
{
    ...
    forAll(meshEdges, patchEdgeI)
    {
...

        {
            ...
            vector n = f.areaNormal(dualPoints);
    ...
    forAll(patches, patchi)
    {
        ...
        dualPatches[patchi] = patch.clone
        (
            boundaryMesh(),
            patchi,
            0, //patchSizes[patchi],
...
:
    polyMesh(mesh, pointField(), faceList(), cellList()),
    cellPoint_
    (
        IOobject
        (
