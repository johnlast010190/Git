...
) const
{
    ...
    // 3 labels: owner face, neighbour face and couple index). These will be
    // used to create the non-conformal faces, so sort them to make sure the
                    ...
                    else
                    {
                        c =
                            ...
                            (
                                ...
                                (
                                    patchToPatch::small
                                   *origPp.faceAreas()[origFacei],
                                    origPp.faceCentres()[origFacei]
                                ),
                                part
                                (
                                    patchToPatch::small
                                   *tOrigSfNbrBf()[patchi][patchFacei],
        ...
        forAll(patchEdgeParts[patchi], patchEdgei)
        {
            ...
            const label sign =
                ...
                (
                    meshEdge
                    (
                        refCast<const directPolyPatch>(patch),
                        patchEdgei
                    ),
...
) const
{
    ...
    tmp<surfaceLabelField> tChanged =
        nonConformalPolyPatch::debug
      ? tmp<surfaceLabelField>
        (
            new surfaceLabelField
            (
                IOobject
                (
                    "changed",
    ...
    forAll(ownerOrigBoundaryEdgeMeshEdge, ownerOrigBoundaryEdgei)
    {
        ...
        switch (ownerOrigBoundaryEdgeNOrigFaces[ownerOrigBoundaryEdgei])
        {
            ...
            case 1:
            {
                ...
                {
                    const label facei =
                        mesh_.edgeFaces()[meshEdgei][edgeFacei];
            ...
            case 2:
            {
                forAll(mesh_.edgeFaces()[meshEdgei], edgeFacei)
                {
                    ...
                    if (nonConformalPolyPatch::debug && patchi != -1)
                    {
                        tChanged->boundaryFieldRef()
            ...
            {
                FatalErrorInFunction
                    << "Boundary is not manifold at edge "
                    << mesh_.edges()[meshEdgei].centre(mesh_.points())
                    << exit(FatalError);
            }
        }
    }
...
) const
{
    ...
    forAll(allOrigPatchIDs, i)
    {
        ...
        {
            const vectorField::subField pa = origPp.faceAreas();
            const vectorField::subField pc = origPp.faceCentres();
            const vector& a = pa[origPatchFacei];
            const vector& c = pc[origPatchFacei];
            ...
            SAndCf += part(patchToPatch::small*mag(a)*dSfHat, c);
...
) const
{
    ...
    if (haveTopology)
    {
        ...
        forAll(tOrigCfNbrBf(), patchi)
        {
            ...
            {
                const transformer& t =
                    refCast<const coupledFvPatch>(Cfp.patch()).transform();
...
) const
{
    ...
    forAll(mesh_.boundary(), patchi)
    {
        ...
        SfBf[patchi].forceAssign
        (
            vectorField
            (
                patchToPatch::small*origPp.faceAreas(),
                polyFacesBf[patchi] - origPp.start()
            )
        );
        CfBf[patchi].forceAssign
        (
            vectorField
            (
                origPp.faceCentres(),
...
void Foam::fvMeshStitcher::intersectNonConformalDiscreteMixing
(
...
)
{
    ...
    forAll(mesh_.boundary(), patchi)
    {
        ...
        if (!isA<nonConformalDiscreteMixingFvPatch>(fvp)) continue;

        const nonConformalDiscreteMixingFvPatch& ncdmFvp =
            refCast<const nonConformalDiscreteMixingFvPatch>(fvp);

        // Alias the original poly-patch
        const polyPatch& origPp = ncdmFvp.origPatch().patch();

        // Resize the non-conformal discrete mixing geometry fields
        const label patchSize = origPp.size();
        ...
        // Create couplings by transferring geometry from the original to the
        // non-conformal patches. Each coupling will be identical to the
        // corresponding original face, implying in non-matching faces on both
        // sides of the patches, opposed to the standard non-conformal cyclics.
        forAll(origPp, patchFacei)
        {
            polyFacesBf[patchi][patchFacei] = origPp.start() + patchFacei;

            SfBf[patchi][patchFacei] = SfBf[origPp.index()][patchFacei];
            CfBf[patchi][patchFacei] = CfBf[origPp.index()][patchFacei];

            // Stabilise original patch faces
            const part smallP
            (
                patchToPatch::small*origPp.faceAreas()[patchFacei],
                origPp.faceCentres()[patchFacei]
            );

            SfBf[origPp.index()][patchFacei] = smallP.area;
            CfBf[origPp.index()][patchFacei] = smallP.centre;
        }

        // For all zero-weight faces, i.e. faces that don't have any couplings
        // across all rotational intersections, keep the original patches' face
        // and stabilise the non-conformal face instead.
        const auto& intersect =
            ncdmFvp.owner()
          ? ncdmFvp.nonConformalPolyPatch().intersection()
          : ncdmFvp.nbrPatch().nonConformalPolyPatch().intersection();

        const labelHashSet& zeroWeightFaces =
            ncdmFvp.owner()
          ? intersect.srcZeroWeightFaces()
          : intersect.tgtZeroWeightFaces();

        forAllConstIter(labelHashSet, zeroWeightFaces, iter)
        {
            const label& facei = iter.key();

            Swap(SfBf[origPp.index()][facei], SfBf[patchi][facei]);
            Swap(CfBf[origPp.index()][facei], CfBf[patchi][facei]);
        }
    }
}
...
void Foam::fvMeshStitcher::postUnconformSurfaceVelocities()
{
    ...
    forAll(Ufs, i)
    {
        ...
        forAll(Uf.boundaryField(), patchi)
        {
            ...
            {
                boundaryFieldRefNoUpdate(Uf)[patchi].forceAssign
                (
    ...
    return
       ...
       /max
        (
            ...
            (patchToPatch::small*sqr(cbrt(mesh_.V())))()
        )();
}
...
)
{
    ...
    if (mesh_.moving())
    {
        ...
        {
            phi.oldTime(i).forceAssign(mesh_.phi().oldTime(i));
        }
...
)
{
    ...
    if (coupled && (geometric || !haveTopology))
    {
        ...
        // For non-conformal discrete mixing patches
        forAll(mesh_.boundary(), patchi)
        {
            const fvPatch& fvp = mesh_.boundary()[patchi];
            ...
            if (!ncdmFvp.owner()) continue;

            ncdmFvp.nonConformalPolyPatch().intersections();
        }
    }
    ...
    if (coupled && (geometric || !haveTopology))
    {
        ...
        // Do the intersection and move faces geometry from original to
        // non-conformal discrete mixing patches
        intersectNonConformalDiscreteMixing(polyFacesBf, Sf, Cf);
    }
    else
    {
    ...
    if (coupled && geometric)
    {
        ...
        if (mesh_.moving())
        {
            ...
            // on either side of interfaces (like 'phiErrorbs' in
            // fvMeshStitchers::moving::unconformErrorFaceCorrectMeshPhi())
            ...
            scalarField sumMfe(nNonProcPatches, 0);
            scalarField nSumMfe(nNonProcPatches, 0);
            ...
            forAll(minMfe, patchi)
            {
                ...
                )
                {
                    Info<< indent << fvp.name()
                        ...
                        << '/' << sumMfe[patchi]
                                 /(nSumMfe[patchi] + patchToPatch::vSmall)
                        << '/' << maxMfe[patchi] << endl;
                }
            }
        }
