...
{
    const word fvMeshTopoChangerTypeName(dict.lookup("type"));

    Info<< "Selecting fvMeshTopoChanger " << fvMeshTopoChangerTypeName << endl;
    ...
    auto ctor =
        ...
        (
            ...
            fvMeshTopoChangerTypeName
        );

    return autoPtr<fvMeshTopoChanger>(ctor(mesh, dict));
}
