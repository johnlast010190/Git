...
void Foam::fvSchemes::update()
{
    bool doOverwrite = localSchemes_.lookupOrDefault("overwrite", false);

    if (found("select"))
    {
        subDict(word(lookup("select"))).merge(localSchemes_, doOverwrite);
    }
    else
    {
        this->merge(localSchemes_, doOverwrite);
    }
}

void Foam::fvSchemes::read(const dictionary& dict)
{
    if (localSchemes_.size())
    {
        update();
    }
    ...
    {
        WarningInFunction << "Cannot find divSchemes"<<endl;
    }
    ...
    {
        WarningInFunction << "Cannot find gradSchemes"<<endl;
    }
    ...
    {
        WarningInFunction << "Cannot find laplacianSchemes"<<endl;
    }
...
void Foam::fvSchemes::setLocalSchemeDict(const dictionary& input)
{
...
void Foam::fvSchemes::resetReadOpt(const readOption& option)
{
    readOpt() = option;
}
