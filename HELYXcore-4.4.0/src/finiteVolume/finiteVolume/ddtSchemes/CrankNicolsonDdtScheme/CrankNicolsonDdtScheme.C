...
{
    const word name = IOobject::modelName(unTypedName, typeName);
...
template<class Type>
tmp<VolField<Type>> CrankNicolsonDdtScheme<Type>::fvcDdt
(
    const dimensioned<Type>& dt
)
{
    DDt0Field<VolField<Type>>& ddt0 =
        ddt0_<VolField<Type>>("ddt0(" + dt.name() + ')', dt.dimensions());
...
{
    DDt0Field<VolField<Type>>& ddt0 =
        ddt0_<VolField<Type>>("ddt0(" + vf.name() + ')', vf.dimensions());
    ...
    if (mesh().moving())
    {
        ...
        (
            ...
            (
                ...
                (
                    mesh().V()*rho()*vf()
                  - mesh().V0()*rho.oldTime()()*vf.oldTime()()
...
)
{
    ...
    else
    {
        ...
        (
            ddtName,
            rDtCoef*(alpha*rho*vf - alpha.oldTime()*rho.oldTime()*vf.oldTime())
          - offCentre_(ddt0())
        );
    }
}
