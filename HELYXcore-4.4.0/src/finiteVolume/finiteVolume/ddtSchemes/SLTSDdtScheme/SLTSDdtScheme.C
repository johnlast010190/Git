...
#include "finiteVolume/ddtSchemes/SLTSDdtScheme/SLTSDdtScheme.H"
...
template<class Type>
tmp<volScalarField> SLTSDdtScheme<Type>::SLrDeltaT
(
...
template<class Type>
tmp<VolField<Type>> SLTSDdtScheme<Type>::fvcDdt(const dimensioned<Type>& dt)
{
    const word ddtScheme("ddt(" + dt.name() + ')');
...
template<class Type>
tmp<VolField<Type>>SLTSDdtScheme<Type>::fvcDdt(const VolField<Type>& vf)
{
    const volScalarField rDeltaT(SLrDeltaT(vf));
...
template<class Type>
tmp<VolField<Type>> SLTSDdtScheme<Type>::fvcDdt
(
...
template<class Type>
tmp<fvMatrix<Type>> SLTSDdtScheme<Type>::fvmDdt(const VolField<Type>& vf)
{
    ...
    scalarField rDeltaT(SLrDeltaT(vf)().primitiveField());
...
template<class Type>
tmp<fvMatrix<Type>> SLTSDdtScheme<Type>::fvmDdt
(
    const dimensionedScalar& rho,
    const VolField<Type>& vf
)
...
{
    const surfaceScalarField rDeltaT(fvc::interpolate(SLrDeltaT(U)));
...
template<class Type>
tmp<surfaceScalarField> SLTSDdtScheme<Type>::meshPhi(const VolField<Type>&)
{
