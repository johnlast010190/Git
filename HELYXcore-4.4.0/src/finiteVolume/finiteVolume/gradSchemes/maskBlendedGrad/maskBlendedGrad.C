...
#include "finiteVolume/gradSchemes/maskBlendedGrad/maskBlendedGrad.H"
...
>
Foam::fv::maskBlendedGrad<Type>::calcGrad
(
...
{
    // Uatc size = 4
    const word maskName("mask" + vsf.name().substr(4));

    const volScalarField& blendingFactor =
        (
            this->mesh().objectRegistry::template
            lookupObject<const volScalarField>(maskName)
        );
