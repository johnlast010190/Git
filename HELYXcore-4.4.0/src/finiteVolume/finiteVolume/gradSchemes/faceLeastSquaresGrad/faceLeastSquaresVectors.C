...
void Foam::faceLeastSquaresVectors::makeLeastSquaresVectors() const
{
        ...
        if (pw.coupled())
        {
            forAll(pw, patchFacei)
            {
                ...
                meanPoints[faceCells[patchFacei]] +=
                    pmWeight[patchFacei]*pCe[patchFacei];
            }
        }
        else
        {
            forAll(pw, patchFacei)
            {
                // Should coincide with Cf on boundary
                // pCe[patchFacei] = C[faceCells[patchFacei]] + pd[patchFacei];
                // Boundary values are assumed specified at a point
                // projected orthogonally to the boundary from
                // the internal point. Should use deltaCoeff since this
                // might be stabilised and not end up quite on the wall
                // - for consistency with snGrad
                pCe[patchFacei] =
                    C[faceCells[patchFacei]]
                  + (
                       ...
                       /(
                           ...
                           *pw.patch().deltaCoeffs()[patchFacei]
                        )
                    );
    ...
    forAll(owner, facei)
    {
        ...
        dd[own] +=
            mWeight[facei]/norm[own]*Ce[facei]*(Ce[facei]-meanPoints[own]);
        dd[nei] +=
            mWeight[facei]/norm[nei]*Ce[facei]*(Ce[facei]-meanPoints[nei]);
    }
    ...
    forAll(blsP, patchi)
    {
        ...
        forAll(pw, patchFacei)
        {
            dd[faceCells[patchFacei]] +=
                pmWeight[patchFacei]
               /norm[faceCells[patchFacei]]*pCe[patchFacei]
    ...
    forAll(blsP, patchi)
    {
        ...
        forAll(pw, patchFacei)
        {
            patchLsP[patchFacei] =
               pmWeight[patchFacei]
              /norm[faceCells[patchFacei]]
              *(
                    invDd[faceCells[patchFacei]]
                  & Ce.boundaryField()[patchi][patchFacei]
                );
        }
    }
