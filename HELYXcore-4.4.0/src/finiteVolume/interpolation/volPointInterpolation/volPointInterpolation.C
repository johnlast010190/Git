...
void Foam::volPointInterpolation::makeWeights()
{
        ...
        forAll(pFaces, pPointFacei)
        {
            ...
            forAll(patches, i)
            {
                ...
                (
                 ...
                 && !isA<indirectPolyPatch>(fvbm[patchi].patch())
                )
                {
    ...
    forAll(points, pointi)
    {
        ...
        forAll(pointCells[pointi], pointCelli)
        {
            ...
            const scalar delta =
                max(mag(points[pointi] - mesh().C()[celli]), ROOTSMALL);

            pointWeights_[pointi][pointCelli] = (1 - f)/delta;
        }
    }
        ...
        forAll(pFaces, bPointFacei)
        {
            ...
            forAll(patches, i)
            {
                ...
                )
                {
                    ...
                    const scalar delta =
                        max(mag(points[pointi] - C), ROOTSMALL);
                ...
                )
                {
                    ...
                    const scalar delta =
                        max(mag(points[pointi] - Cf), ROOTSMALL);

                    boundaryPointWeights_[bPointi][bPointFacei][i] = f*a/delta;
                }
            }
        }
    }
