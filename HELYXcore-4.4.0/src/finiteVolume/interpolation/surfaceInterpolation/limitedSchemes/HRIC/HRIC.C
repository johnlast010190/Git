...
) const
{
    ...
    (
       ...
       *upwind<scalar>(mesh, faceFlux).interpolate(outFlux)
    );
    ...
    forAll(bLim, patchi)
    {
        ...
        if (bLim[patchi].coupled())
        {
            ...
            const scalarField& pblendingFactor =
                blendingFactor_.boundaryField()[patchi];
...
) const
{
    ...
    corr =
        surfaceInterpolationScheme<scalar>::interpolate(vf, calcWeights(vf))
...
void Foam::HRIC::calculateTheta() const
{
    ...
    const volScalarField& vsf =
        mesh.objectRegistry::lookupObject<const volScalarField>(blendingField_);
    const volVectorField vsfGrad(fvc::grad(vsf,"alphaS"));
    blendingFactor_ =
    (
        linearInterpolate(vsfGrad/(mag(vsfGrad) + deltaN_))
      & mesh.Sf()/mesh.magSf()
    );
