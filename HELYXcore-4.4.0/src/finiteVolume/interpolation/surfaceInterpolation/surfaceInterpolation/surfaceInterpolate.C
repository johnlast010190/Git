...
)
{
    return surfaceInterpolationScheme<Type>::New
    (
        mesh, db, mesh.schemes().interpolationScheme(name)
    );
}


template<class Type>
Foam::tmp<Foam::SurfaceField<Type>> Foam::fvc::interpolate
(
...
{
    tmp<SurfaceField<Type>> tsf = interpolate(tvf(), faceFlux, name);
    tvf.clear();
    return tsf;
}
...
{
    tmp<SurfaceField<Type>> tsf = interpolate(vf, tFaceFlux(), name);
    tFaceFlux.clear();
    return tsf;
}
...
)
{
    ...
    return scheme<Type>(vf.mesh(), vf.db(), schemeData)().interpolate(vf);
}
...
)
{
    ...
    return scheme<Type>(vf.mesh(), vf.db(), name)().interpolate(vf);
}
...
{
    tmp<SurfaceField<Type>> tsf = interpolate(tvf(), name);
    tvf.clear();
    return tsf;
}
...
{
    tmp<SurfaceField<Type>> tsf = interpolate(tvf());
    tvf.clear();
    return tsf;
}
...
template<class Type>
Foam::tmp<Foam::FieldField<Foam::fvsPatchField, Type>> Foam::fvc::interpolate
(
...
)
{
    ...
    word interName("dotInterpolate(" + Sf.name() + ',' + vf.name() + ')');
    if (interpolationName!= word::null)
    {
        interName = interpolationName;
    }
...
)
{
    ...
    > tsf = dotInterpolate(Sf, tvf(), interpolationName);
    tvf.clear();
    return tsf;
}
