...
(
    ...
    const tmp<SurfaceField<Type>> tsfCorr
) const
{

    tmp<SurfaceField<Type>> tpsi
    (
    ...
    SurfaceField<Type>& limiterField = tpsi.ref();
    limiterField.writeOpt() = IOobject::AUTO_WRITE;
    const SurfaceField<Type>& sfCorr = tsfCorr();
...
) const
{
    ...
    tmp<Field<Type>> tField(new Field<Type>(vf.primitiveField()));
...
(
    const tmp<SurfaceField<Type>> tpsi
) const
{
