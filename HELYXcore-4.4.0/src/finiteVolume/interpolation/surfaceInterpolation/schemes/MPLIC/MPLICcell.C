...
)
{
    ...
    // original cell alpha.
    if (mag(cutAlpha_) > ROOTVSMALL && (1 - mag(cellAlpha/cutAlpha_)) > 0.1)
    {
        return 0;
    }
    else
...
Foam::FixedList<Foam::scalar, 4> Foam::MPLICcell::solveVanderMatrix() const
{
    ...
    {
      static_cast<scalar>(- 4.5*b[0] + 13.5*b[1] - 13.5*b[2] + 4.5*b[3]),
      static_cast<scalar>(  9.0*b[0] - 22.5*b[1] + 18.0*b[2] - 4.5*b[3]),
      static_cast<scalar>(- 5.5*b[0] +  9.0*b[1] -  4.5*b[2] + 1.0*b[3]),
      static_cast<scalar>(  1.0*b[0])
    };
}
