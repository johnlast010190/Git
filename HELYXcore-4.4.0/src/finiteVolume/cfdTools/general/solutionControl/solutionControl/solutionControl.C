...
void Foam::solutionControl::readControls(const bool absTolOnly)
{
    ...
    // Read new solution control information for each corrector
    const dictionary* solnControlDictPtr =
        mesh_.solution().dict().subDictPtr(solutionControl::typeName);
    if (solnControlDictPtr)
    {
        // Iterate over sub-dicts for correctors
        forAllConstIters(*solnControlDictPtr, iter)
        {
            if (iter().isDict())
            {
                const dictionary residualDict
                (
                    iter().dict().subOrEmptyDict("residualControl")
                );
                readResidualControls
                (
                    iter().keyword(), residualDict, absTolOnly
                );

                if (iter().dict().found("minIter"))
                {
                    minIters_.set
                    (
                        iter().keyword(),
                        iter().dict().lookup<label>("minIter")
                    );
                }
                if (iter().dict().found("nIter"))
                {
                    nIters_.set
                    (
                        iter().keyword(),
                        iter().dict().lookup<label>("nIter")
                    );
                }
            }
        }
    ...
    // Read backward-compatible residual information
    const dictionary residualDict
    (
    ...
    if
    (
        residualControl_.found("outerCorrector")
     && residualControl_["outerCorrector"].size()
     && solutionDict.found("residualControl")
    )
    {
        IOWarningInFunction(solutionDict.subDict("residualControl"))
            << "residualControl has been specified inside the "
            << solutionControl::typeName << " sub-dictionary. "
            << "It should not be specified inside " << solutionDict.dictName()
            << " as well." << nl << endl;
    }
    readResidualControls(word::null, residualDict, absTolOnly);
}


void Foam::solutionControl::readResidualControls
(
...
{
    DynamicList<fieldData> data
    (
        residualControl_.lookup(correctorName, DynamicList<fieldData>())
    );

    forAllConstIter(dictionary, residualDict, iter)
    {
        const word& fName = iter().keyword();
        const label fieldi = applyToField(fName, false, correctorName);
        if (fieldi == -1)
        {
            fieldData fd;
            fd.name = fName.c_str();
            ...
            if (absTolOnly)
            {
                fd.absTol = residualDict.lookup<scalar>(fName);
                fd.relTol = -1;
            ...
            else
            {
                if (iter().isDict())
                {
                    ...
                    fd.absTol = fieldDict.lookup<scalar>("tolerance");
                    fd.relTol = fieldDict.lookup<scalar>("relTol");
    ...
    residualControl_.set(correctorName, List<fieldData>());
    residualControl_[correctorName].transfer(data);

    if (debug)
    {
        forAll(residualControl_[correctorName], i)
        {
            const fieldData& fd = residualControl_[correctorName][i];
            Info<< "residualControl[" << i << "] for corrector "
                << correctorName << ":" << nl
                << "    name     : " << fd.name << nl
                << "    absTol   : " << fd.absTol << nl
                << "    relTol   : " << fd.relTol << nl
...
{
    if (!residualControl_.found(correctorName))
    {
        return -1;
    }

    const List<fieldData>& residualControl(residualControl_[correctorName]);
    forAll(residualControl, i)
    {
        if
        (
            useRegEx && residualControl[i].name.match(fieldName)
        )
        {
            return i;
        }
...
)
{
            ...
            for (const auto& p : dictionaryConstructorTable_()) {
                ...
                if (entryPtr && entryPtr->isDict())
                {
                    if (!entryPtr->dict().empty() || i == 1)
                    {
                        if (solnControlName != word::null)
                        {
                            FatalIOErrorInFunction(entryPtr->dict())
                                << "Found dictionaries for two possible "
                                << "solution controls: " << solnControlName
                                << " and " << type
                                << ". Please remove one of them to "
                                << "avoid ambiguity." << nl
                                << exit(FatalIOError);
                        }
    ...
    if (solnControlName == word::null)
    {
        OSstream& s(FatalIOErrorInFunction(fvSoln));
        s   << "None of the following solution control dictionaries were found "
            << "in " << fvSoln.name() << ":" << nl;
