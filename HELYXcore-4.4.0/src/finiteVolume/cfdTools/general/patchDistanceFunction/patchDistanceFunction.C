...
Foam::autoPtr<Foam::labelHashSet> Foam::patchDistanceFunction::wallDistPatches() const
{
    const fvMesh& mesh(patch_.boundaryMesh().mesh());
    autoPtr<labelHashSet> pIDs(new labelHashSet);

    forAll(mesh.boundary(), pI)
    {
        if
        (
            !excludedPatches_.found(mesh.boundary()[pI].name())
            && mesh.boundary()[pI].type()=="wall"
        )
        {
            pIDs().insert(pI);
        }
...
:
    ...
    excludedPatches_(dict.lookupOrDefault<wordList>("excludedPatches", wordList(0)))
{
    if
    (
        coorFramePtr_
...
:
    ...
    excludedPatches_(pdf.excludedPatches_)
{
    if
    (
        coorFramePtr_
...
const Foam::scalarField& Foam::patchDistanceFunction::value() const
{
    if (!value_.valid())
    {
        ...
        switch(distanceType_)
        {
            ...
            case dtWall:
            {
                wallDist d(patch_.boundaryMesh().mesh(), wallDistPatches());
                value_() =
...
void Foam::patchDistanceFunction::write(Ostream& os, bool writeFrame) const
{
    ...
    os.writeEntry("excludedPatches", excludedPatches_);
