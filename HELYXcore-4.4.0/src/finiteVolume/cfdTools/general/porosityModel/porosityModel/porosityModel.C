...
Foam::tmp<Foam::vectorField> Foam::porosityModel::calculateUrel
(
    const vectorField& U
) const
{
    ...
    frameVelocity(Urel, false);

    return tUrel;
}

void Foam::porosityModel::substractMeshVelocity(vectorField& Urel) const
{
...
void Foam::porosityModel::frameVelocity(vectorField& Urel, const bool add) const
{
    if (coorFramePtr_)
    {
        forAll(cellZoneIDs_, zoneI)
        {
            ...
            const vectorField cellCentres(UIndirectList<vector>
            (
                mesh_.C(), cells)
            );
            tmp<vectorField> tUframe = coorFramePtr_->frameVelocity
            (
                cellCentres, true
            );
            if (add)
    ...
    coorFramePtr_(nullptr)
{
    if (zoneName_ == word::null)
    {
        ...
        zoneName_ = dict_.lookup<keyType>("cellZone");
    }
...
bool Foam::porosityModel::read(const dictionary& dict)
{
    ...
    zoneName_ = dict.lookup<keyType>("cellZone");
    cellZoneIDs_ = mesh_.cellZones().findIndices(zoneName_);
