...
)
{
    ...
    {
        tmp<scalarField> xSingleI = xSingle.component(cmptI);
        psiIn.replace(localDir, xSingleI());
...
(
    const direction dir,
    const VolField<fieldType>& matrixPsi
)
{
    typename VolField<Type>::
        Boundary& bFields = psi_.boundaryFieldRef();

    const typename VolField<fieldType>::
...
)
{
    ...
    if (psi_.mesh().schemes().fluxRequired(bIndex_.variableName(dirI)))
    {
        if (bs.faceFluxCorrectionPtr())
        {
            setFaceFluxCorrection(*(bs.faceFluxCorrectionPtr()), dirI);
        }
    }
}
...
(
    SurfaceField<Type2>& field,
    const direction sI
) const
{
    typename SurfaceField<Type2>::Boundary& ffbf = field.boundaryFieldRef();
...
(
    const SurfaceField<Type2>& matrixFluxCorr,
    const direction sI
)
{
    ...
    for (direction cmptI = 0; cmptI < nCmpts2; cmptI++)
    {
        direction sII = sI+cmptI;
        scalarField intField
        (
            mfluxCorrIn.component(cmptI)+fluxCorrIn.component(sII)
        );

        fluxCorrIn.replace(sII, intField);
        ...
        forAll(matrixFluxCorr.boundaryField(), pI)
        {
            ...
            scalarField bField
            (
                pmFluxCorr.component(cmptI) + pFluxCorr.component(sII)
            );

            pFluxCorr.replace(sII, bField);
        }
    }
}
...
(
    SurfaceField<Type2>& matrixFluxCorr,
    const direction sI
) const
{
    if (this->faceFluxCorrectionPtr_)
...
)
{
    ...
    if (this->thereIsLower())
    {
        this->lower().expandCoeffs(this->diag());
    }
    if (this->thereIsUpper())
    {
        this->upper().expandCoeffs(this->diag());
    }
...
:
    BlockLduSystem<Type, Type>
    (
        matrix.ref().psi().mesh(),
        matrix.ref().dimensions()
    ),
    psi_
    (
        const_cast<VolField<Type>&>
        ...
        if (this->symmetric() || this->asymmetric())
        {
            ...
            forAll(c, j)
            {
                ...
                else
                {
                    ...
                    else if (isA<nonConformalOrigFvPatch>(patch))
                    {
                        const label bFacei = fI - mesh.nInternalFaces();
                        ...
                        if (this->symmetric())
                        {
                            if (this->coupleUpper()[pI].activeType() != blockCoeffBase::SQUARE)
                            {
                                forAll(patches, i)
                                {
                                    typename CoeffField<Type>::linearTypeField& cUpper =
                                        this->coupleUpper()[patches[i]].asLinear();
                                    cUpper[patchFaces[i]](sI) = 0;
                                }
                            }
                            else
                            {
                                forAll(patches, i)
                                {
                                    typename CoeffField<Type>::squareTypeField& cUpper =
                                        this->coupleUpper()[patches[i]].asSquare();

                                    for (direction cmptI = 0; cmptI < nCmpts; cmptI++)
                                    {
                                        cUpper[patchFaces[i]](sI, cmptI) = 0;
                                    }
                                }
                            }
                        }
                        else
                        {
                            if (this->coupleUpper()[pI].activeType() != blockCoeffBase::SQUARE)
                            {
                                forAll(patches, i)
                                {
                                    typename CoeffField<Type>::linearTypeField& cUpper =
                                        this->coupleUpper()[patches[i]].asLinear();
                                    typename CoeffField<Type>::linearTypeField& cLower =
                                        this->coupleLower()[patches[i]].asLinear();

                                    cUpper[patchFaces[i]](sI) = 0;
                                    cLower[patchFaces[i]](sI) = 0;
                                }
                            }
                            else
                            {
                                forAll(patches, i)
                                {
                                    typename CoeffField<Type>::squareTypeField& cUpper =
                                        this->coupleUpper()[patches[i]].asSquare();
                                    typename CoeffField<Type>::squareTypeField& cLower =
                                        this->coupleLower()[patches[i]].asSquare();

                                    for (direction cmptI = 0; cmptI < nCmpts; cmptI++)
                                    {
                                        cUpper[patchFaces[i]](sI, cmptI) = 0;
                                        cLower[patchFaces[i]](sI, cmptI) = 0;
                                    }
                                }
                            }
                        }
...
template<class Type>
Foam::tmp<Foam::SurfaceField<Type>> Foam::fvBlockMatrix<Type>::flux() const
{
...
template<class Type>
template<class Type2>
Foam::tmp<Foam::SurfaceField<Type2>> Foam::fvBlockMatrix<Type>::flux
(
    const VolField<Type2>& field
) const
{
    ...
    tmp<SurfaceField<Type2>> tfieldFlux
    (
        SurfaceField<Type2>::New
        (
            "flux(" + field.name() + ')',
            psi_.mesh(),
            dimensioned<Type2>
    ...
    SurfaceField<Type2>& fieldFlux = tfieldFlux.ref();
...
)
{
    ...
    const scalarField& mV = psi_.mesh().V();
    scalarField coeffIJVol(coeffIJ*mV);
