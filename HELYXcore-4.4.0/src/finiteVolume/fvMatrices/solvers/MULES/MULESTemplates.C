...
)
{
    ...
    explicitSolve(rho, psi, phi, phiPsi, Sp, Su, psiMax, psiMin, MULEScontrols);
}
...
)
{
    ...
    {
        sumlPhip = dimensionedScalar("sumlPhip", phiCorr.dimensions(), 0);
        mSumlPhim = dimensionedScalar("sumlPhim", phiCorr.dimensions(), 0);
