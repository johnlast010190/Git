...
(
    const tmp<fvMatrix<Type>>& tA,
    const VolField<scalar>& dOlddNew,
    const VolField<Type>& newVar
)
{
    const fvMesh& mesh = newVar.mesh();

    tmp<SurfaceField<Type>> fluxCorr;
    if (tA().hasLower() || tA().hasUpper())
    {
        fluxCorr = tA().flux();
    }

    const VolField<Type>& oldVar =
        tA().psi();
    ...
    if (ALin.hasDiag())
    {
        ...
        {
            // The 'flux' part of diag's explicit correction will be accounted
            // for as part of fluxCorr, but we need to account for any
            // non-flux component
            scalarField prevDiag(ALin.diag());
            ALin.sumDiag();
            ALin.source() += ALin.diag()*(dOlddNew*newVar-oldVar);
            ALin.diag() = prevDiag*dOlddNew;
        }
        else
        {
            ALin.source() -= ALin.diag()*oldVar;
            ALin.diag() *= dOlddNew;
            ALin.source() += ALin.diag()*newVar;
        }
    }
    ...
    const VolField<scalar>::Boundary& bdOlddNew =
