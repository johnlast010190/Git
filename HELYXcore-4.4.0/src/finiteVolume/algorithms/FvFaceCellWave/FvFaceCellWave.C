...
Foam::labelList Foam::FvFaceCellWave<Type, TrackingData>::listListSizes
(
    const ListList& ll
)
{
...
(
    const label celli,
    const labelPair& nbrPatchAndFacei,
    const Type& nbrInfo,
    const scalar tol,
    Type& cellInfo
)
...
)
{
    for (label changedFacei = 0; changedFacei < nFaces; changedFacei++)
    {
        ...
        const Type& nbrInfo = changedPatchFacesInfo[changedFacei];

        if (!info.equal(nbrInfo, td_))
        {
            updateFace
            (
...
template<class Type, class TrackingData>
void Foam::FvFaceCellWave<Type, TrackingData>::
handleNonConformalDiscreteMixingPatches()
{
    // Define combine operator for interpolation
    class combine
    {
        FvFaceCellWave<Type, TrackingData>& solver_;

        const nonConformalDiscreteMixingFvPatch& patch_;
        ...
        public:
            ...
            (
                ...
                const nonConformalDiscreteMixingFvPatch& patch
            )
            :
                solver_(solver),
                patch_(patch)
    ...
    // Transfer information across non-conformal discrete mixing pair
    forAll(mesh_.boundaryMesh(), patchi)
    {
        const fvPatch& patch = mesh_.boundary()[patchi];

        if (isA<nonConformalDiscreteMixingFvPatch>(patch))
        {
            const nonConformalDiscreteMixingFvPatch& ncdmPatch =
                refCast<const nonConformalDiscreteMixingFvPatch>(patch);
            const nonConformalDiscreteMixingFvPatch& otherPatch =
                ncdmPatch.nbrPatch();

            // Create a combine operator to transfer sendInfo from other
            // to this patch
            combine cop(*this, ncdmPatch);

            // Get other patch data (so not just changed faces)
            List<Type> sendInfo(patchFaceInfo_[otherPatch.index()]);

            // Interpolate from other to this patch
            List<Type> receiveInfo;
            ncdmPatch.interpolate(sendInfo, receiveInfo, cop);

            // Shuffle up to remove invalid info
            DynamicList<label> receiveFaces(ncdmPatch.size());
...
template<class Type, class TrackingData>
void Foam::FvFaceCellWave<Type, TrackingData>::handleCyclicAMIPatches()
{
    ...
    forAll(mesh_.boundaryMesh(), patchi)
    {
        ...
        if (isA<cyclicAMIFvPatch>(patch))
        {
            ...
            if (cycPatch.applyLowWeightCorrection())
            {
                List<Type> defVals
                (
                    cycPatch.cyclicAMIPatch().patchInternalList(cellInfo_)
                );

                cycPatch.cyclicAMIPatch().interpolate
                (
                    sendInfo,
                    cmb,
                    receiveInfo,
...
:
    ...
    hasNonConformalDiscreteMixingPatches_
    (
        hasPatch<nonConformalDiscreteMixingFvPatch>()
    ),
...
Foam::label Foam::FvFaceCellWave<Type, TrackingData>::faceToCell()
{
    ...
    forAll(changedPatchAndFaces_, changedFacei)
    {
        ...
        // Propagate to the neighbour cell
        if (patchi == -1)
        {
            const label nbrCelli = neighbour[facei];

            Type& nbrInfo = cellInfo_[nbrCelli];

            if (!nbrInfo.equal(info, td_))
            {
                updateCell
                (
                    nbrCelli,
...
Foam::label Foam::FvFaceCellWave<Type, TrackingData>::cellToFace()
{
    ...
    forAll(changedCells_, changedCelli)
    {
        ...
        forAll(cells[celli], cellFacei)
        {
            ...
            // Propagate to the connected FV faces
            forAll(patches, i)
            {
    ...
    if (hasNonConformalDiscreteMixingPatches_)
    {
        // Transfer changed faces across non-conformal discrete mixing couples
        handleNonConformalDiscreteMixingPatches();
    }
