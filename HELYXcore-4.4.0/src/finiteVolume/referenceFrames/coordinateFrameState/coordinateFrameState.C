...
:
    ...
    rotations_(frameState.rotations_),
...
const Foam::coordinateSystem Foam::coordinateFrameState::cartesianSys() const
{
    return
        coorSys().uniform()
      ? coorSys()
...
const Foam::coordinateSystem Foam::coordinateFrameState::cartesianSys0() const
{
    return
        coorSys0().uniform()
      ? coorSys0()
...
const Foam::vector& Foam::coordinateFrameState::rotation() const
{
    return rotations_[nFrameCorrector()];
}
...
)
{
    ...
    if (dict.found("transformation"))
    {
        ...
        if (dict.found("rotation"))
        {
            state.rotations_ = dict.lookup<List<vector>>("rotation");
        }
    }
    else
    {
        ...
        if (dict.found("rotations"))
        {
            state.rotations_ = dict.lookup<List<vector>>("rotations");
        }

        // On restart use the data from the last corrector
        if (state.transformations_.size() > 1)
        {
            state.outerCorrectorUpdate_ = state.transformations_.size();
        }
    }
}
...
) const
{
    ...
    frame.add("rotations", state.rotations_);
