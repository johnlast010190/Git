...
:
    ...
    calculatedMode_(false),
    restart_(dict.lookupOrDefault("restart", Switch(false)))
{
    if (valueRequired)
    {
        if (dict.found("value"))
...
:
    ...
    calculatedMode_(ptf.calculatedMode_),
...
template<class Type>
word Foam::fvPatchField<Type>::detectPhiName(const word& phiName) const
{
    if (phiName == word::null)
    {
        // Try to auto-detect flux name
        if (internalField().group() == word::null)
        {
            return "phi";
        }
        else
        {
            // Default to global mass-flux phi if it exists, otherwise phasic
            // volumetric phi
            if (db().template foundObject<surfaceScalarField>("phi"))
            {
                return "phi";
            }
            else
            {
                const word phasicPhi
                (
                    IOobject::groupName("phiv", internalField().group())
                );
                return phasicPhi;
            }
        }
...
void Foam::fvPatchField<Type>::write(Ostream& os) const
{
    ...
    // The 'value' and 'restart' fields are only written by boundary conditions
    // that need them
}


template<class Type>
void Foam::fvPatchField<Type>::writeValueEntry(Ostream& os) const
{
    this->writeEntry("value", os);
    writeEntryIfDifferent(os, "restart", Switch(false), Switch(restart_));
}
