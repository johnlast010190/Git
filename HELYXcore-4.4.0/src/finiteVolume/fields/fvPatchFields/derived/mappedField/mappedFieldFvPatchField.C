...
template<class Type>
Foam::scalar Foam::mappedFieldFvPatchField<Type>::getUfromFlowRate() const
{
    scalar Umean;
    if (this->isMassFlow_)
    {
        ...
        Umean = this->flowRate_/gSum(this->patch().magSf())/rhoMean;
    }
    else
    {
        Umean = this->flowRate_/gSum(this->patch().magSf());
    }
