    ...
    p_("p", dict, p.size())
{
    ...
    {
        fvPatchField<scalar>::forceAssign(p_);
    }
}
...
void Foam::prghPressureFvPatchScalarField::reset
(
    const fvPatchScalarField& ptf
)
{
...
void Foam::prghPressureFvPatchScalarField::autoMapGIB
(
    const gibFvPatchFieldMapper& mapper
)
{
    fixedValueFvPatchScalarField::autoMapGIB(mapper);
    mapper.map(p_, scalar(0));
}
...
void Foam::prghPressureFvPatchScalarField::updateCoeffs()
{
    ...
    const scalarField& rhop = patch().lookupPatchFieldInDb<volScalarField, scalar>
    (
        db(),
        rhoName_
    );
    ...
    forceAssign(p_ - rhop*((g.value() & patch().Cf()) - ghRef.value()));
