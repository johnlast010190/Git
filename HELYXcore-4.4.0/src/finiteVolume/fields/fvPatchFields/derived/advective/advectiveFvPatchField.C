...
:
    ...
    fieldInf_(Zero),
    lInf_(-GREAT),
    fieldInfMod_(p.size(), pTraits<Type>::zero)
{
    this->refValue() = Zero;
    this->refGrad() = Zero;
    ...
    fieldInfMod_(p.size(), Zero)
{
    ...
    {
        fieldInf_ = dict.lookup<Type>("fieldInf");
...
:
    ...
    lInf_(ptpsf.lInf_),
    fieldInfMod_(ptpsf.fieldInfMod_)
{}
...
Foam::advectiveFvPatchField<Type>::advectionSpeed() const
{
    ...
    if (phi.dimensions() == dimDensity*dimVelocity*dimArea)
    {
        const fvPatchScalarField& rhop =
            this->patch().template lookupPatchFieldInDb<volScalarField, scalar>
            (
                this->db(),
                rhoName_
            );
...
void Foam::advectiveFvPatchField<Type>::updateCoeffs()
{
    ...
    word ddtScheme
    (
        mesh.schemes().ddtScheme(this->internalField().name())
    );
    if (ddtScheme == "bounded" || ddtScheme == "smartBounded")
    {
         //- A bit of a hack
         //  In v4 the smartBounded and bounded schemes are used in temporal
         //  schemes. Therefore, in order to see the actual scheme we need to
         //  cast it. Otherwise, refactoring and moving the schemes to
         //  thermophysical properties is needed.
         tmp<fv::ddtScheme<Type>> ddtSchemePtr
         (
         ...
         const fv::boundedDdtScheme<Type>& bScheme =
             refCast<const fv::boundedDdtScheme<Type>>(ddtSchemePtr());
         ddtScheme = bScheme.scheme()->type();
    }
