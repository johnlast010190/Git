...
#include "fields/fvPatchFields/derived/activePressureForceBaffleVelocity/activePressureForceBaffleVelocityFvPatchVectorField.H"
...
:
    ...
    baffleActivated_(0),
    opening_(dict.lookup<bool>("opening"))
{
...
void Foam::activePressureForceBaffleVelocityFvPatchVectorField::recalculate()
{
...
)
{
    ...
    {
        if (patch().boundaryMesh().mesh().magFaceAreas()[i] == 0)
        {
...
void Foam::activePressureForceBaffleVelocityFvPatchVectorField::autoMapGIB
(
...
void Foam::activePressureForceBaffleVelocityFvPatchVectorField::updateCoeffs()
{
    ...
    if (curTimeIndex_ != this->db().time().timeIndex())
    {
        ...
        scalarField::subField magSfw = patch().patch().magFaceAreas();
        ...
        const_cast<scalarField&>(cyclicPatch.magSf()) =
            mag(areaFraction*initCyclicSf_);
