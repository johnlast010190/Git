...
void Foam::turbulentDFSEMInletFvPatchVectorField::autoMapGIB
(
...
)
{
    ...
    mapper.map(L_, scalar(0));
    mapper.map(U_, vector::zero);

    // Clear interpolator
    mapperPtr_.clear();

    mapper.map(sigmax_, scalar(0));
}
...
void Foam::turbulentDFSEMInletFvPatchVectorField::updateCoeffs()
{
    ...
    if (curTimeIndex_ != db().time().timeIndex())
    {
        if (debug)
        {
            ...
            Info<< "Number of eddies: " << returnReduceToMaster(n, sumOp<label>())
                << endl;
        }
