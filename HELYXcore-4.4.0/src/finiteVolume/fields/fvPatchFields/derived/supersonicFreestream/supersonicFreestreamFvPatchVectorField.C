...
#include "fields/fvPatchFields/derived/supersonicFreestream/supersonicFreestreamFvPatchVectorField.H"
    ...
    gamma_(dict.lookup<scalar>("gamma"))
{
    ...
    if (dict.found("value"))
    {
        fvPatchField<vector>::forceAssign
        (
    ...
    {
        fvPatchField<vector>::forceAssign(patchInternalField());
    }
...
void Foam::supersonicFreestreamFvPatchVectorField::updateCoeffs()
{
    ...
    const fvPatchField<scalar>& pT =
        patch().lookupPatchFieldInDb<volScalarField, scalar>(db(), TName_);
