...
#include "fields/fvPatchFields/derived/turbulentDigitalFilterInlet/turbulentDigitalFilterInletFvPatchVectorField.H"
...
namespace Foam
{
    ...
    <
        turbulentDigitalFilterInletFvPatchVectorField::variantType,
        2
        >::names[] =
    {
        "digitalFilter",
...
<
    ...
    2
>   Foam::turbulentDigitalFilterInletFvPatchVectorField::variantTypeNames_;
...
Foam::turbulentDigitalFilterInletFvPatchVectorField::patchIndexPairs()
{
    ...
    forAll(*this, facei)
    {
        ...
        j = min(label((centre0 - localMinPt[0])*invDelta_[0]), n[0]-1);
        k = min(label((centre1 - localMinPt[1])*invDelta_[1]), n[1]-1);
...
{
    const auto faceCentres = this->patch().patch().faceCentres();
...
)
{
    for (direction dir = 0; dir < pTraits<vector>::nComponents; ++dir)
    {
        ...
        (
            dir,
            U0_.component(dir)*constList1FSM_[dir]
...
:
    ...
    variant_
    (
        variantTypeNames_.readOrDefault(dict.lookup("variant"), dFilter)
    ),
    mapperPtr_(nullptr),
    isGaussian_
    (
        (variant_ == rdFilter)
      ? false
      : dict.lookupOrDefault("isGaussian", true)
    ),
    isFixedSeed_(dict.lookupOrDefault("isFixedSeed", true)),
    isContinuous_(dict.lookupOrDefault("isContinuous", false)),
    isCorrectedFlowRate_(dict.lookupOrDefault("isCorrectedFlowRate", true)),
    interpolateR_(dict.lookupOrDefault("interpolateR", false)),
    interpolateUMean_(dict.lookupOrDefault("interpolateUMean", false)),
    isInsideMesh_(dict.lookupOrDefault("isInsideMesh", false)),
    isTaylorHypot_(dict.lookupOrDefault("isTaylorHypot", true)),
    mapMethod_(dict.lookupOrDefault<word>("mapMethod", "nearestCell")),
    curTimeIndex_(-1),
    tiny_(dict.lookupOrDefault<scalar>("threshold", 1e-8)),
    patchNormalSpeed_
    (
    ...
    planeDivisions_
    (
        dict.lookup<Tuple2<label, label>>("planeDivisions")
    ),
    invDelta_(),
    ...
    (
      ...
      ? dict.lookupOrDefault<List<List<scalar>>>
        (
            "randomBox",
    ...
    )
{
    ...
    bool isAdjustTimeStep =
        db().time().controlDict().lookupOrDefault("adjustTimeStep", false);
...
) const
{
    ...
    os.writeEntry("variant", variantTypeNames_[variant_]);
    ...
    writeEntryIfDifferent<bool>(os, "isGaussian", true, isGaussian_);
    writeEntryIfDifferent<bool>(os, "isFixedSeed", true, isFixedSeed_);
    writeEntryIfDifferent<bool>(os, "isContinuous", false, isContinuous_);
    writeEntryIfDifferent<bool>(os, "isCorrectedFlowRate", true, isCorrectedFlowRate_);
    writeEntryIfDifferent<bool>(os, "isInsideMesh", false, isInsideMesh_);
    writeEntryIfDifferent<bool>(os, "isTaylorHypot", true, isTaylorHypot_);
    ...
    os.writeEntry("delabella", delabella_);
