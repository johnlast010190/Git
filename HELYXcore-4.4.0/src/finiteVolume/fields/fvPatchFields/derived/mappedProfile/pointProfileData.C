...
)
{
    ...
    if (fieldIndex != -1)
    {
        ...
        (
            ...
            treeBoundBox(treeBoundBox(points_).extend(1e-4)), // overall search
            8,                              // maxLevel
            10,                             // leafsize
            3.0                             // duplicity
        );
        ...
        forAll(targetPoints, pI)
        {
            ...
            pointIndexHit info = pointTree.findNearest(p, spanSqr);

            if (!info.hit())
            {
                info = pointTree.findNearest(p, GREAT*spanSqr);
            }
