    ...
    targetType_(targetType)
{
    ...
    temperatureVarying_ = dict.lookupOrDefault<Switch>("temperatureVarying", false);
...
:
    ...
    temperatureVarying_(frb.temperatureVarying_),
    TName_(frb.TName_),
...
Foam::scalar Foam::flowRateBase::getFlowRate() const
{
    if (temperatureVarying_ && db_.foundObject<volScalarField>(TName()))
    {
        const fvPatchField<scalar>& TPatch =
        ...
        scalar TAve = gSum(TPatch*patch_.magSf())
                      / gSum(patch_.magSf());

        return flowRate_->value(TAve);
    }
    else
        return flowRate_->value(db_.time().timeOutputValue());
}

Foam::scalar Foam::flowRateBase::currentFlowRate() const
{
    scalar fr = getFlowRate();
...
void Foam::flowRateBase::write(Ostream& os) const
{
    ...
    fvPatchVectorField::writeEntryIfDifferent<Switch>
    (
        os, "temperatureVarying", false, temperatureVarying_
    );
