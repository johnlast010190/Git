...
template<class Type>
void Foam::partialSlipFvPatchField<Type>::autoMapGIB
(
...
Foam::partialSlipFvPatchField<Type>::transformationField() const
{
    const vectorField nHat(this->patch().nf());
    tmp<tensorField> ttrField(new tensorField(this->size(), Zero));
    tensorField& trField = ttrField.ref();
    forAll(trField, fI)
    {
        trField[fI] =
            valueFraction_[fI]*tensor::I
          + (1.0-valueFraction_[fI])*nHat[fI]*nHat[fI];
    }
    return ttrField;
}
...
template<class Type>
Foam::tmp<Foam::Field<Type>>
Foam::partialSlipFvPatchField<Type>::valueDivInternalCoeffs
(
...
template<class Type>
Foam::tmp<Foam::Field<Type>>
Foam::partialSlipFvPatchField<Type>::valueDivBoundaryCoeffs
(
    const tmp<Field<scalar>>& w
) const
{
