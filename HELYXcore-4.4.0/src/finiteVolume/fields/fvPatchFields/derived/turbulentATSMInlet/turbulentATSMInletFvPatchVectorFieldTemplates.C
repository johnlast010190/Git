...
) const
{
    ...
    if (mag(N) > 0)
    {
        ...
        if (mag(e3) == 0)
        {
            ...
            e3 = vector
            (
                Foam::sin(beta*constant::mathematical::twoPi/360)*N[1],
               -Foam::sin(beta*constant::mathematical::twoPi/360)*N[0],
                Foam::cos(beta*constant::mathematical::twoPi/360)
            );
        }
    }
    else
    ...
    if (dict.found(subDictName))
    {
        ...
        else if (profile == "exponential")
        {
            ...
            refDire.component(vector::Y) = Foam::sin(refAngl*constant::mathematical::twoPi/360);
            refDire.component(vector::Z) = Foam::cos(refAngl*constant::mathematical::twoPi/360);
...
template<>
inline Foam::tmp<Foam::symmTensorField>
