...
void Foam::turbulentATSMInletFvPatchVectorField::writeFileHeader(const label i)
{
    ...
    writeHeaderValue(filePtrs_[i], "mean velocity magnitude", mag(U_[outputFaceIndices_[i]]));
...
void Foam::turbulentATSMInletFvPatchVectorField::initialisePatch()
{
    ...
    Pstream::allGatherList(patchSize_);
    ...
    forAll(patch, faceI)
    {
        ...
        forAll(triEngine.triPoints(), triI)
        {
            ...
            triFace.append(triEngine.triPoints(triI, f));
            triMagSf.append(triEngine.triPoints(triI, f).mag(points));
        }
    }
    ...
    Pstream::listCombineReduce(sumTriMagSf_, maxOp<scalar>());
...
void Foam::turbulentATSMInletFvPatchVectorField::initialiseVortons()
{
    ...
    if (nVortonGlobal_ && !isCleanRestart_)
    {
...
)
{
    ...
    forAll(vortons_, vortonI)
    {
        ...
        v.move(deltaT*mag(U_[v.patchFaceI()]));
        ...
        if (position0 > maxSigmaX_)
        {
            ...
            while (search && iter++ < seedIterMax_)
            {
               ...
               scalar Urand = rndGen_.position<scalar>(0, UMax_);

               label iter0 = 0;

               while (Urand > mag(U_[faceI]) && iter0++ < seedIterMax_)
               {
                   pointIndexHit pos(setNewPosition(false));
                   faceI = pos.index();
                   Urand = rndGen_.position<scalar>(0, UMax_);
               }
               ...
               v = vorton
                    (
                        ...
                        faceI,
                        pos.hitPoint(),
                        -maxSigmaX_ + rndGen_.position<scalar>(0, deltaT*mag(U_[faceI])),
                        L_[faceI],
                        R_[faceI],
                        rndGen_
                    );
...
:
    ...
    patchNormal_(Zero),
    v0_(Zero),
    rndGen_((Pstream::myProcNo()+1)*time(nullptr)),
    maxSigmaX_(Zero),
    curTimeIndex_(-1),
    patchBounds_(boundBox::invertedBox),
    singleProc_(false),
...
void Foam::turbulentATSMInletFvPatchVectorField::autoMapGIB
(
...
void Foam::turbulentATSMInletFvPatchVectorField::updateCoeffs()
{
    ...
    if (curTimeIndex_ != db().time().timeIndex())
    {
        if (debug)
        {
            ...
            Info<< "Number of vortons: " << returnReduceToMaster(n, sumOp<label>())
                << endl;
        }
        ...
        // Set velocity
        vectorField& U = *this;
        U = (U_ & patchNormal_)*patchNormal_;
...
void Foam::turbulentATSMInletFvPatchVectorField::write(Ostream& os) const
{
    ...
    writeEntryIfDifferent<scalar>(os, "delabella", Switch(true), delabella_);
