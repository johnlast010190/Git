...
template<class Type>
void Foam::directionMixedFvPatchField<Type>::autoMapGIB
(
...
)
{
    ...
    mapper.map(valueFraction_, symmTensor::zero);
}
...
Foam::directionMixedFvPatchField<Type>::transformationField() const
{
    tmp<tensorField> ttrField(new tensorField(this->size(), Zero));
    tensorField& trField = ttrField.ref();
    forAll(trField, fI)
    {
        trField[fI] = valueFraction_[fI];
    }
