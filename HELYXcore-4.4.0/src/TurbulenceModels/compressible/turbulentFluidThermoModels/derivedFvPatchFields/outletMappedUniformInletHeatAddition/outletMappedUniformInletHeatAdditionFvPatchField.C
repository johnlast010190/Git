...
#include "turbulentFluidThermoModels/derivedFvPatchFields/outletMappedUniformInletHeatAddition/outletMappedUniformInletHeatAdditionFvPatchField.H"
...
void Foam::outletMappedUniformInletHeatAdditionFvPatchField::updateCoeffs()
{
    ...
    if (sumOutletPatchPhi > SMALL)
    {
        ...
        const scalarField Cpf(thermo.Cp(pT, outletPatchID));

        scalar totalPhiCp = gSum(outletPatchPhi)*gAverage(Cpf);

        forceAssign(min(max(averageOutletField + Q_/totalPhiCp, TMin_), TMax_));
    }
    else
    {
        scalar averageOutletField =
        ...
        forceAssign(averageOutletField);
    }
