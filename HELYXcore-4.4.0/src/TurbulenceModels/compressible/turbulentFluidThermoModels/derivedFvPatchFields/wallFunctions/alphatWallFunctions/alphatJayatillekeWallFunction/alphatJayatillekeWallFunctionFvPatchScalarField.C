...
void alphatJayatillekeWallFunctionFvPatchScalarField::updateCoeffs()
{
    ...
    // These lookup-or-allocate-and-dereference allocators are a terrible idea, and they
    // confuse GCC's value tracking :D.
#if defined(__clang__)
#elif defined(__GNUC__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdangling-reference"
#endif
    // Lookup the phasic thermo sub-model
    const fluidThermo& thermo =
        refCast<const fluidThermo>
        (
            multiphaseThermo::lookupOrCreate(db(), internalField().group())
        );
#if defined(__clang__)
#elif defined(__GNUC__)
#pragma GCC diagnostic pop
    ...
    const tmp<scalarField> tmuw = turbModel.mu().boundaryField()[patchi];
    ...
    const scalarField qDot(thermo.alphaEff(alphatw, patchi)*hew.snGrad());
...
(
    ...
    compressible__alphatJayatillekeWallFunction
);
