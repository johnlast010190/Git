...
void turbulentTemperatureCoupledBaffleMixedFvPatchScalarField::updateCoeffs()
{
    ...
    if (acht_)
    {
        ...
        */
        ...
        scalarField Tp
        (
            valueFraction()*refValue() + (1.0 - valueFraction())*intFld()
        );
        ...
        forAll(Tp, facei)
        {
            ...
            if (intFld()[facei] > nbrIntFld()[facei] - switchTol_)
            {
                refValue()[facei] =
                    (
                        nbrIntFld()[facei]*Cie_*valueFraction()[facei]
                      + Tp[facei] * CieCiemo
                    )/beta[facei];
                ...
                refGrad()[facei] =
                    ...
                    (
                        (
                            Cie_*(1 - valueFraction()[facei])*intFld()[facei]
                          + Ciemo2*(intFld()[facei] + deltaTib[facei])
                        )/(1 - beta[facei]) - intFld()[facei]
                    );

                valueFraction()[facei] = beta[facei];
            }
        }
    }
...
(
    ...
    compressible__turbulentTemperatureCoupledBaffleMixed
);
