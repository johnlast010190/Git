...
:
    ...
    mapped_(false),
    TinfName_("Tinf"),
    alphaWallName_("alphaWall"),
    numNbrs_(5),
    mapTinf_(nullptr),
    mapAlphaWall_(nullptr)
{
    refValue() = 0.0;
    refGrad() = 0.0;
    valueFraction() = 0.0;
...
:
    ...
    mapped_(false),
    TinfName_(ptf.TinfName_),
    alphaWallName_(ptf.alphaWallName_),
    numNbrs_(ptf.numNbrs_),
    mapTinf_
    (
        mapWall_
      ? new mapScalar(ptf.mapTinf_, TinfName_)
      : nullptr
    ),
    mapAlphaWall_
    (
        mapWall_
      ? new mapScalar(ptf.mapAlphaWall_, alphaWallName_)
      : nullptr
    )
{
    forAll(ptf.t_, li)
...
:
    ...
    mapWall_(dict.lookupOrDefault<Switch>("mapValuesOnWall", false)),
    Tinf_
    (
        mapWall_
        ? scalarField(p.size(), 0.0)
        : scalarField("Tinf", dict, p.size())
    ),
    alphaWall_
    (
        (!mapWall_ && dict.found("alphaWall"))
    ...
    mapped_(false),
    TinfName_(dict.lookupOrDefault<word>("TinfName", "Tinf")),
    alphaWallName_(dict.lookupOrDefault<word>("alphaWallName", "alphaWall")),
    numNbrs_(dict.lookupOrDefault<label>("neighboursKNN", 5)),
    mapTinf_
    (
        mapWall_
      ? new mapScalar(TinfName_, dict, (*this).db().time().value(), numNbrs_)
      : nullptr
    ),
    mapAlphaWall_
    (
        mapWall_
      ? new mapScalar(alphaWallName_, dict, (*this).db().time().value(), numNbrs_)
      : nullptr
    )
{
...
:
    ...
    mapped_(false),
    TinfName_(tppsf.TinfName_),
    alphaWallName_(tppsf.alphaWallName_),
    numNbrs_(tppsf.numNbrs_),
    mapTinf_
    (
        mapWall_
      ? new mapScalar(tppsf.mapTinf_, TinfName_)
      : nullptr
    ),
    mapAlphaWall_
    (
        mapWall_
      ? new mapScalar(tppsf.mapAlphaWall_, alphaWallName_)
      : nullptr
    )
{}
...
void turbulentConvectiveTemperatureFvPatchScalarField::updateCoeffs()
{
    ...
    if (mapWall_ && !mapped_)
    {
        ...
        Tinf_ = mapTinf_().interpolateSampledData(db().time(), patch())();

        mapAlphaWall_().checkTable
        (
            db().time(),
            patch(),
            coorFramePtr(),
        ...
        (
            new scalarField
            (
                mapAlphaWall_().interpolateSampledData(db().time(), patch())()
            )
        );
    ...
    if (this->KMethod() != mtLookup)
    {
        ...
        )
        {
            ...
            if (!db().foundObject<volVectorField>(gradTName))
            {
                ...
                pGrad.set(fvc::grad(thermo.T()).ptr());
...
const
{
    ...
    os.writeEntry("mapValuesOnWall", mapWall_);
    os.writeEntry("TinfName", TinfName_);
    os.writeEntry("alphaWallName", alphaWallName_);
    os.writeEntry("neighboursKNN", numNbrs_);
...
(
    fvPatchScalarField,
    turbulentConvectiveTemperatureFvPatchScalarField,
    compressible::turbulentConvectiveTemperature,
    compressible__turbulentConvectiveTemperature
);
