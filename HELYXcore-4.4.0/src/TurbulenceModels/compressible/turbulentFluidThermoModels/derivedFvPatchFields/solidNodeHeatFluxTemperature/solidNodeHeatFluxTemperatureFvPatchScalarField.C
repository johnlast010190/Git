...
#include "turbulentFluidThermoModels/derivedFvPatchFields/solidNodeHeatFluxTemperature/solidNodeHeatFluxTemperatureFvPatchScalarField.H"
...
void solidNodeHeatFluxTemperatureFvPatchScalarField::updateCoeffs()
{
    ...
    scalarField kappaw( turbModel.kappaEff(patchi) );
    scalarField alphaConv( kappaw * patch().deltaCoeffs() ); // [W/m2 K]
    ...
    const radiationModels::fvDOM* domPtr(nullptr);

    // radiation handling
    if
    (
        db().foundObject<radiationModel>("radiationProperties")
    ...
    )
    {
        ...
        domPtr = &(refCast<const radiationModels::fvDOM>(radiation));
    }

    scalarField qabs( Qfixed*faceArea );
...
(
    fvPatchScalarField,
    solidNodeHeatFluxTemperatureFvPatchScalarField,
    compressible::solidNodeHeatFluxTemperature,
    compressible__solidNodeHeatFluxTemperature
);
