...
#include "turbulentFluidThermoModels/derivedFvPatchFields/turbulentTemperatureRadCoupledMixed/turbulentTemperatureRadCoupledMixedFvPatchScalarField.H"
...
void turbulentTemperatureRadCoupledMixedFvPatchScalarField::updateCoeffs()
{
    ...
    if (contactRes_ == 0.0)
    {
        KDeltaNbr = nbrField.kappa()*nbrPatch.deltaCoeffs();
    }
    else
    {
    ...
    scalarField KDelta(kappa()*patch().deltaCoeffs());
    ...
    if (thermalInertia_)
    {
...

        {
            ...
            if (thermo)
            {
                ...
                mCpDtNbr =
                    (
                        thermo->Cp(Tpn, samplePatchi)*thermo->rho(samplePatchi)
        ...
        // Local inertia therm
        {
            ...
            if (thermo)
            {
                mCpDt =
                    (
                        thermo->Cp(Tp, patchi)*thermo->rho(patchi)
...
(
    ...
    compressible__turbulentTemperatureRadCoupledMixed
);
