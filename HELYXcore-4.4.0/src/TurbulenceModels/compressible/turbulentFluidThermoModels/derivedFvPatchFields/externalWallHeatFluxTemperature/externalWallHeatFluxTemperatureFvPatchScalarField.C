...
#include "turbulentFluidThermoModels/derivedFvPatchFields/externalWallHeatFluxTemperature/externalWallHeatFluxTemperatureFvPatchScalarField.H"
...
:
    ...
    boundaryKappa(db(), patch(), ewhftpsf),
    mode_(ewhftpsf.mode_),
    Q_(ewhftpsf.Q_),
    q_(ewhftpsf.q_),
    h_(ewhftpsf.h_),
...
)
{
    ...
    switch (mode_)
    {
        ...
        case fixedHeatFlux:
        {
            q_.reset(ewhftpsf.q_);

            break;
        }
        case fixedHeatTransferCoeff:
        {
            h_.reset(ewhftpsf.h_);
            Ta_.reset(ewhftpsf.Ta_);
    ...
    if (qrName_ != "none")
    {
        qrPrevious_.reset(ewhftpsf.qrPrevious_);
    }
}


void Foam::externalWallHeatFluxTemperatureFvPatchScalarField::autoMapGIB
(
...
)
{
    ...
    switch (mode_)
    {
        ...
        case fixedHeatTransferCoeff:
        {
            mapper.map(h_, scalar(0));
            mapper.map(Ta_, gAverage(Ta_));
            break;
        }
    }
    ...
    if (qrName_ != "none")
    {
        mapper.map(qrPrevious_, scalar(0));
    }
}
...
void Foam::externalWallHeatFluxTemperatureFvPatchScalarField::updateCoeffs()
{
    ...
    switch (mode_)
    {
        case fixedPower:
        {
            refGrad() = (Q_/gSum(patch().magSf()) + qr)/kappa();
            refValue() = 0;
            valueFraction() = 0;
