...
void turbulentConvectiveTemperatureFvPatchScalarField::updateCoeffs()
{
    ...
    if (mapWall_ && !mapped_)
    {
        ...
        Tinf_ = mapTinf_().interpolateSampledData
        (
            (*this).db().time(),
            (*this).patch()
        )();
        ...
        alphaWall_.reset(new scalarField
        (
            mapAlphaWall_().interpolateSampledData
            (
                (*this).db().time(),
                (*this).patch()
            )()
