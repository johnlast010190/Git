...
#include "lumpedServerOutletTemperatureFvPatchScalarField.H"
...
:
    ...
    Ms_(dict.lookup<scalar>("mass")),
    Cps_(dict.lookup<scalar>("Cps")),
    effectiveness_(dict.lookup<scalar>("effectiveness")),
...
void lumpedServerOutletTemperatureFvPatchScalarField::updateCoeffs()
{
    ...
    const scalarField& Cpp = turbModel.Cp().boundaryField()[patchi];
    const scalar Cpa(gAverage(Cpp));
    ...
    if (curTimeIndex_ > 1)
    {
        ...
        const scalarField& rhop = trho().boundaryField()[patchi];
