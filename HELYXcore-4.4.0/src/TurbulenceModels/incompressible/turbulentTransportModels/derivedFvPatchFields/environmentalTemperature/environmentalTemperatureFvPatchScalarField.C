...
void environmentalTemperatureFvPatchScalarField::calculateEnvRadiation()
{
    if (rSourcesPtr_.size())
    {
        ...
        forAllIter(PtrList<entry>, rSourcesPtr_, iter)
        {
            ...
            scalar radiationFlux(envRadDict.lookup<scalar>("intensity"));
    ...
    QPtr_()
{
    ...
    {
        Tconv_.reset(new scalar(dict.lookup<scalar>("Twall")));
        refValue() = Tconv_;
        valueFraction() = 1.0;
    }
    else
    {
        Tconv_.reset(new scalar(dict.lookup<scalar>("Tenv")));

        alphaOutside_.reset
        (
            new scalar(dict.lookup<scalar>("alphaConv"))
        );

        if (dict.found("Trad"))
        {
            Trad_.reset(new scalar(dict.lookup<scalar>("Trad")));
        }
