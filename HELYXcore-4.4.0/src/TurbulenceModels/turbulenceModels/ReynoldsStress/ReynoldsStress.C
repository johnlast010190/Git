...
{
    const_cast<volVectorField&>(this->U_).updateBoundaryCoeffs();
...
) const
{
    ...
    if (couplingFactor_.value() > 0.0)
    {
        ...
        (
          ...
          - fvc::div
            (
                this->alpha_*this->rho_*this->nu()*dev2(T(fvc::grad(U))),
                scheme
            )
        );
    }
...
Foam::ReynoldsStress<BasicTurbulenceModel>::bDivDevRhoReff
(
    volVectorField& U
) const
{
