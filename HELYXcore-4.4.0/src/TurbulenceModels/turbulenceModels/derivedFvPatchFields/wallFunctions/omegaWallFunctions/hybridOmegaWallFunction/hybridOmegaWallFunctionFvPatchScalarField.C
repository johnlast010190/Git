...
void hybridOmegaWallFunctionFvPatchScalarField::updateCoeffs()
{
    if (!this->updated())
    {
        ...
        forAll(nutw, faceI)
        {
            ...
            scalar utau = max(SMALL,Foam::sqrt(tauw));
            ...
            else
            {
                G[faceCellI] =
                    ...
                    (
                        ...
                        -Cmu75 * sv/kappa_*Foam::log(E_*yPlus)
                        *Foam::sqrt(k[faceCellI]) * k[faceCellI]
                    )
                    - (1.0-phiHL) *
                    (
            ...
            scalar Rek = y[faceI]*Foam::sqrt(k[faceCellI])/nuw[faceI];
            scalar Lomega = y[faceI]*(1.0-exp(-tw_*Rek));
            omega[faceCellI] = Foam::sqrt(k[faceCellI])
