...
) const
{
    ...
    {
        if (internalField().db().foundObject<refScalarField>(TName_+"Ref"))
        {
            const refScalarField& TAbs =
                internalField().db().lookupObject<refScalarField>(TName_+"Ref");
            scalarField TpAbs(TAbs.boundaryField()[patch().index()]);

            forAll(TpAbs, fI)
            {
                linerCoeff[fI] *=
                    calcDcorrValue(TpAbs[fI], TRef_+TAbs.offset().value());
                nonLinearCoeff[fI] *=
                    calcFcorrValue(TpAbs[fI], TRef_+TAbs.offset().value());
            }
        }
        else
        {
            FatalErrorInFunction
                << "Absolute temperature field was not found."
                << nl << exit(FatalError);
        }
    }
}
...
Foam::word Foam::porousBafflePressureAMIFvPatchField::detectPhiName() const
{
