...
namespace Foam
{
    ...
    NamedEnum
    <
        porousBafflePressureFvPatchField::porousMode,
        3
    >::names[] =
    {
        "darcyForchheimer",
...
<
    Foam::porousBafflePressureFvPatchField::porousMode,
    3
> Foam::porousBafflePressureFvPatchField::porousModeNames_;


// * * * * * * * * * * * * * Private Functions   * * * * * * * * * * * * * * //

Foam::scalar Foam::porousBafflePressureFvPatchField::calcDcorrValue
(
    const scalar& T, const scalar& TRef
) const
{
    return (aT1_ + aT2_*TRef + aT3_*sqr(TRef))*(TRef/T);
}


Foam::scalar Foam::porousBafflePressureFvPatchField::calcFcorrValue
(
    const scalar& T, const scalar& TRef
) const
{
    return (bT1_ + bT2_*TRef + bT3_*sqr(TRef))*sqr(TRef/T);
}

void Foam::porousBafflePressureFvPatchField::stabiliseJump
(
...
Foam::porousBafflePressureFvPatchField::computeUn() const
{
    // Use volumetric flux if available as this is necessary for multiphase
    // (there it is not equivalent to divide by density)
    const surfaceScalarField& phi =
        db().lookupObject<surfaceScalarField>(detectPhiName());
    ...
    {
        const volScalarField& rho = db().lookupObject<volScalarField>(rhoName_);
        // Need to use interpolated rho since cyclic patches do not store the
        // actual patch value
        surfaceScalarField rhof(fvc::interpolate(rho));
        Un /= patch().patchField<surfaceScalarField, scalar>(rhof);
    }
    ...
    if (patch().boundaryMesh().mesh().moving())
    {
        Un -= fvc::meshPhi
            (
                db().lookupObject<volVectorField>("U"), patch().index()
            )/patch().magSf();
    }
    ...
    (
        coorFramePtr() &&
        !this->db().foundObject<helyxCoupledControl>(solutionControl::typeName)
    )
    {
...
void Foam::porousBafflePressureFvPatchField::computeUniformVelocity
(
    scalarField& Un
) const
{
...
void Foam::porousBafflePressureFvPatchField::correctTemperatureDependency
(
...
) const
{
    if (temperatureDependence_)
    {
        ...
        {
            basicThermo& thermo = basicThermo::lookupOrCreate
            (
            ...
            const fvPatchField<scalar>& Tp =
                patch().patchField<volScalarField, scalar>(TAbs());

            forAll(Tp, fI)
            {
                linerCoeff[fI] *=
                    calcDcorrValue(Tp[fI], TRef_+thermo.TRefValue());
                nonLinearCoeff[fI] *=
                    calcFcorrValue(Tp[fI], TRef_+thermo.TRefValue());
            }
        }
    }
}
...
void Foam::porousBafflePressureFvPatchField::computeDFJump()
{
...
void Foam::porousBafflePressureFvPatchField::computeABJump()
{
    const scalar t = db().time().timeOutputValue();
...
void Foam::porousBafflePressureFvPatchField::computePLJump()
{
    const scalar t = db().time().timeOutputValue();
...
Foam::word Foam::porousBafflePressureFvPatchField::detectPhiName() const
{
    if (phiName_ != word::null)
    {
        return phiName_;
    }

    if (db().foundObject<surfaceScalarField>("phiv"))
    {
        return "phiv";
    }
    else
...
void Foam::porousBafflePressureFvPatchField::printDebugInfo
(
...
void Foam::porousBafflePressureFvPatchField::autoMap
(
    const fvPatchFieldMapper& m
)
{
    fixedJumpFvPatchField<scalar>::autoMap(m);
    referenceFrameFvPatch<vector>::autoMap(m);
}


void Foam::porousBafflePressureFvPatchField::rmap
(
    const fvPatchScalarField& ptf,
    const labelList& addr
)
{
    fixedJumpFvPatchField<scalar>::rmap(ptf, addr);
    const porousBafflePressureFvPatchField& pbptf =
        refCast<const porousBafflePressureFvPatchField>(ptf);
    referenceFrameFvPatch<vector>::rmap(pbptf, addr);
}


void Foam::porousBafflePressureFvPatchField::autoMapGIB
(
    const gibFvPatchFieldMapper& mapper
)
{
    fixedJumpFvPatchField<scalar>::autoMapGIB(mapper);
...
void Foam::porousBafflePressureFvPatchField::updateCoeffs()
{
    ...
    if (!db().foundObject<surfaceScalarField>(detectPhiName()))
    {
        return;
    }
