...
void Foam::fixedShearStressFvPatchVectorField::updateCoeffs()
{
    ...
    )
    {
        ...
        nuEff = tmp<scalarField>
            (
                new scalarField(turbModel.nuEff(patch().index()))
            );
    }
    else if
    (
    ...
    )
    {
        ...
        nuEff = tmp<scalarField>
            (
                new scalarField
                (
                    nut.boundaryField()[patch().index()]
                )
            );
        WarningInFunction
            << "Couldn't find turbulenceModel. "
            << "Initialised nuEff with nut."
            << endl;
    }
    else
    {
        FatalErrorInFunction
            << "Couldn't find either turbulenceModel "
            << "or nut."
            << exit(FatalError);
    }
