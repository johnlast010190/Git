...
{
    //  ( Teddy + Tkol )
    return sqrt((k_*k_/sqr(epsilon_ + dimensionedScalar(epsilon_.dimensions(), SMALL))) +
                CT_*CT_*(max(this->nu(), dimensionedScalar(this->nu()().dimensions(), 0))
...
{
//    return CL_*sqrt((pow(k_,3.0)/pow(epsilon_ + dimensionedScalar(epsilon_.dimensions(), SMALL), 2.0)) +
...
//               dimensionedScalar(epsilon_.dimensions(), SMALL))));
    return CL_*sqrt((pow(k_,3.0)/pow(epsilon_ + dimensionedScalar(epsilon_.dimensions(), SMALL), 2.0)) +
               ...
               dimensionedScalar(epsilon_.dimensions(), SMALL)), 0.5));
}
...
void ellipticBlendingKE<BasicTurbulenceModel>::correct()
{
    ...
    const volScalarField L2(modelName("L2"), sqr(Ls()));
