...
tmp<volScalarField> ellipticBlendingLagKE<BasicTurbulenceModel>::Ts() const
{
    ...
    return
        ...
        (
            k_*k_
            /sqr(epsilon_ + dimensionedScalar(epsilon_.dimensions(), SMALL))
          + CT_*CT_
           *(
                max
                (
                    this->nu(),
                    dimensionedScalar(this->nu()().dimensions(), 0)
                )
               /(epsilon_ + dimensionedScalar(epsilon_.dimensions(), SMALL))
            )
        );
}
...
) const
{
    ...
    return
        CtLim_
       /(
           sqrt(3*2*magSqr(dev(symm(gradU))))*Cmu_*v2k_
         + dimensionedScalar(dimensionSet(0, 0, -1, 0, 0, 0, 0), SMALL)
        );
}
        ...
        (
            (
               ...
               /(
                    ...
                    (
                        epsilon_
                      + dimensionedScalar(epsilon_.dimensions(), SMALL),
                        2.0
                    )
                )
            )
...
)
{
    ...
    const volSymmTensorField Sbar
    (
        "Sbar",
        S
       /(
            sqrtS2 + dimensionedScalar(dimensionSet(0, 0, -1, 0, 0), SMALL)
        )
    );
...
//    const volVectorField n_(grada/(mag(grada) + dimensionedScalar(grada.dimensions(), 1)));
    const volScalarField E
    (
        "E",
        2.0*this->nu()*nut*sqr(fvc::div(mag(2*S & n_ ) * n_, "div(Sn*n)"))
    );
...
//      - fvm::SuSp(Ceps2star*alpha*rho*epsilon_/k_, epsilon_)  // for variable Ceps2
    ...
    (
       "fmu",
       (sqrtS2*k_/epsilon_ + pow(a_,3.0))
      /max(sqrtS2*k_/epsilon_, dimensionedScalar(dimless, 1.87))
    );

    dimensionedScalar C4Star = 2.0/Cmu_*(1.0 - C4_);
    dimensionedScalar C5Star = 2.0/Cmu_*(1.0 - C5_);
    rotSource_ =
        (C4Star*((b & S) && S) - (C5Star *((b & W) && S)))
       /(
            k_*S2/epsilon_
          + dimensionedScalar(dimensionSet(0, 0, -1, 0, 0), VSMALL)
        );

    fh2_.storePrevIter();
    fh2_ = fmu/Ts/Cmu_*(2.0/3.0 - C3_/2.0) + rotSource_;
    ...
    if (boundV2kSource_)
    {
        ...
        forAll(fh2_, ci)
        {
            fh2_.primitiveFieldRef()[ci] =
                max(maxFh2, fh2_.primitiveFieldRef()[ci]);
        }
...
:
    ...
    (
        IOobject
        (
            IOobject::groupName("v2k", alphaRhoPhi.group()),
            this->runTime_.timeName(),
            this->db_,
            IOobject::MUST_READ,
            IOobject::AUTO_WRITE
    ...
    (
        IOobject
        (
            IOobject::groupName("a", alphaRhoPhi.group()),
            this->runTime_.timeName(),
            this->db_,
            IOobject::MUST_READ,
            IOobject::AUTO_WRITE
    ...
    (
        IOobject
        (
            IOobject::groupName("rotSource", alphaRhoPhi.group()),
            this->runTime_.timeName(),
            this->db_,
            IOobject::NO_READ,
            IOobject::NO_WRITE
    ...
    (
        IOobject
        (
            IOobject::groupName("nutOverk", alphaRhoPhi.group()),
            this->runTime_.timeName(),
            this->db_,
            IOobject::NO_READ,
            IOobject::NO_WRITE
    ...
    (
        IOobject
        (
            IOobject::groupName("fh2", alphaRhoPhi.group()),
            this->runTime_.timeName(),
            this->db_,
            IOobject::NO_READ,
            IOobject::AUTO_WRITE
    ...
    aMin_(dimensionedScalar("aMin", a_.dimensions(), 0)),
