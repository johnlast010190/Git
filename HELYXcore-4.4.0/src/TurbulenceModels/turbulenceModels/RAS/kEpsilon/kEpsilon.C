...
void kEpsilon<BasicEddyViscosityModel>::segregatedCorrect()
{
    // Local references
...
template<class BasicEddyViscosityModel>
void kEpsilon<BasicEddyViscosityModel>::coupledCorrect()
{
    //- field and equation allocation and preparation
    volVector2Field kEpsilon
    (
        IOobject
        (
           "kEpsilon",
    ...
    fvBlockMatrix<vector2> kEpsilonEqn(kEpsilon);
    kEpsilonEqn.interfaces() = kEpsilon.boundaryField().blockInterfaces();

    wordList fieldNames(2);
    labelList fieldSizes(fieldNames.size());

    fieldNames[0] = k_.name();  fieldNames[1] = epsilon_.name();
    fieldSizes[0] =  1;   fieldSizes[1] =  1;
    blockIndex bIndex(fieldNames, fieldSizes);
    kEpsilonEqn.bIndex() = bIndex;
    //-
    // Local references
    const alphaField& alpha = this->alpha_;
    const rhoField& rho = this->rho_;
    ...
    if (deferredS1_ || deferredS2_)
    {
        if (debug)
        {
            Info<< "using time consistent turbulence "
                 << "source discretisation" << endl;
        }
        epsilon_.storePrevIter();
    }
    ...
    (
     ...
     ==
        C1_*alpha()*rho()*G*epsilon_()/k_()
      - fvm::SuSp(((2.0/3.0)*C1_ - C3_)*alpha()*rho()*divU, epsilon_)
      - fvm::Sp(2*C2_*alpha()*rho()*epsilon_()/k_(), epsilon_)
      + C2_*alpha*rho*sqr(epsilon_)/k_
      + epsilonSource()
      + fvOptions(alpha, rho, epsilon_)
    );
    ...
    epsEqn.ref().relax();
    fvOptions.constrain(epsEqn.ref());
    epsEqn->boundaryManipulate(epsilon_.boundaryFieldRef());
    kEpsilonEqn += epsEqn();

    // Coupling term
    // = -fvm::Sp(C2_*alpha()*rho()*epsilon_()/k_(), epsilon_)
    //  C2_*alpha()*rho()*sqr(epsilon_()/k_())*k_
    /*
    volScalarField::Internal cTermImp0
    (
        IOobject
        (
            "cTermImp0",
    ...
    volScalarField::Internal cTermImp00
    (
        IOobject
        (
            "cTermImp00",
    ...
    forAll( this->mesh_.boundary(), pI)
    {
         ...
         {
             const labelUList& fc = this->mesh_.boundary()[pI].faceCells();
             forAll(fc, cI)
             {
                 cTermImp00[fc[cI]] = 0.0;
             }
         }
    }
    cTermImp0 = cTermImp0*cTermImp00;
    */
    //kEpsilonEqn(epsilon_.name(), k_.name(), cTermImp0);
    ...
    (
     ...
     ==
        alpha()*rho()*G
      - fvm::SuSp((2.0/3.0)*alpha()*rho()*divU, k_)
      - fvm::Sp(2*alpha()*rho()*epsilon_()/k_(), k_)
      + alpha()*rho()*epsilon_()
      + kSource()
      + fvOptions(alpha, rho, k_)
    );
    ...
    // Coupling term
    // alpha()*rho()*epsilon
    volScalarField::Internal cTermImp1
    (
        IOobject
        (
            "cTermImp1",
...
//    forAll(cTermImp1, cI)
//    {
//        cTermImp1[cI] = alpha()[cI]*rho()[cI];
//    }
//    cTermImp1 = alpha*rho;
//    kEpsilonEqn(k_.name(), epsilon_.name(), cTermImp1);
    //-

    kEpsilonEqn.solve();
    kEpsilonEqn.retrieveSolution(0, k_.primitiveFieldRef());
    kEpsilonEqn.retrieveSolution(1, epsilon_.primitiveFieldRef());
    fvOptions.correct(k_);
    fvOptions.correct(epsilon_);
    bound(epsilon_, this->epsilonMin_);
    bound(k_, this->kMin_);
...
{
    scalar Cmu_75(Foam::pow(Cmu_.value(), 0.75));
