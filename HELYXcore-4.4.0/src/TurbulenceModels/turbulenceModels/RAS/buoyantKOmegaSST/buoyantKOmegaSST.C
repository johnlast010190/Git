...
|   o   O   o    |  Version : 4.4.0
|    o     o     |  Copyright © 2019 ENGYS Ltd. <http://engys.com/>
|       o        |
\*---------------------------------------------------------------------------
License
...
Author
    2018. Lisandro Maders (Engys Ltd.). All rights reserved.
...
buoyantKOmegaSST<BasicTurbulenceModel>::Gcoef() const
{
    ...
    (
        modelName("Gcoef"),
        (-1*this->nut_*Cg_*(g & fvc::grad(this->rho())))
    );
}
...
buoyantKOmegaSST<BasicTurbulenceModel>::omegaSource() const
{
    ...
    if (mag(g.value()) > SMALL && diss_)
    {
        ...
        (
            this->k_,
            dimensionedScalar(this->k_.dimensions(), 0)
        );
