...
) const
{
    ...
    const volScalarField Ry(y*sqrt(this->k_)/this->nu());
...
{
    return gammaIntEff_*kOmegaSST<BasicTurbulenceModel>::Pk(G,deferred);
}
...
) const
{
    return
       ...
       *kOmegaSST<BasicTurbulenceModel>::epsilonByk(F1, gradU, deferred);
}
...
) const
{
    ...
    const volScalarField::Internal& y(wallDist::New(this->mesh_).y());
    ...
    const dimensionedScalar deltaSmall("deltaSmall", delta.dimensions(), SMALL);
    ...
    (
        ...
        (
            max
            (
                Fwake*exp(-pow4((y/(delta + deltaSmall)))),
...
template<class BasicTurbulenceModel>
tmp<volScalarField::Internal> kOmegaSSTLM<BasicTurbulenceModel>::Fthetat2
(
    const volScalarField::Internal& Us,
    const volScalarField::Internal& Omega,
    const volScalarField::Internal& nu
) const
{
    ...
    (
        ...
        (
            IOobject::groupName("Fthetat2", this->alphaRhoPhi_.group()),
            min
            (
                Fwake*exp(-pow4((y/(delta + deltaSmall)))),
                scalar(1.0)
...
template<class BasicTurbulenceModel>
tmp<volScalarField::Internal> kOmegaSSTLM<BasicTurbulenceModel>::ReScf
(
...
{
    const tmp<volScalarField> tmut = this->mut();
    const volScalarField::Internal& mut = tmut()();

    const tmp<volScalarField> tmu = this->mu();
    const volScalarField::Internal& mu = tmu()();

    const volScalarField::Internal& y(wallDist::New(this->mesh_).y());

    tmp<volScalarField::Internal> tReScf
    (
        volScalarField::Internal::New
        (
            IOobject::groupName("ReScf", this->alphaRhoPhi_.group()),
            this->db_,
            this->mesh_,
            dimless
        )
    );
    volScalarField::Internal& ReScf = tReScf.ref();

    tmp<volVectorField> tvorticity = fvc::curl(Urel);
    const volVectorField& vorticity = tvorticity();

    const volScalarField::Internal omegaStream(mag((Urel/Us) & vorticity));

    label maxIter(10);
    scalar maxErr(1e-3);

    // Solving empirical correlation to calculate momentum
    // thickness using Newton-Raphson method
    forAll(ReScf, celli)
    {
        scalar thetat = 1.0;
        int iter = 0;
        ...
        scalar Hcross = y[celli]*omegaStream[celli]/Us[celli];
        scalar deltaHcross = Hcross*(1.0 + min(mut[celli]/mu[celli], 0.4));
        scalar posDeltaHcross = max(0.1066 - deltaHcross, 0.0);
        scalar negDeltaHcross = max(-1.0*(0.1066 - deltaHcross), 0.0);
        scalar fPosDeltaH = 6200*(posDeltaHcross) + 50000*sqr(posDeltaHcross);
        scalar fNegDeltaH = 75*tanh(80*negDeltaHcross);

        do
        {
            thetat = max(thetat, VSMALL);
            Ftheta = thetat*Us[celli]*1.2195122/nu[celli]
                          + 35.088*log(h_/thetat) - 319.51
                          - fPosDeltaH + fNegDeltaH;
            scalar dFtheta = Us[celli]*1.2195122/nu[celli] - 35.088/thetat;
            thetat -= Ftheta/dFtheta;
        } while (mag(Ftheta) >= maxErr && ++iter < maxIter);

        ReScf[celli] = thetat*Us[celli]*1.219512/nu[celli];
        ...
        if (iter >= maxLambdaIter_)
        {
            ...
            {
                WarningInFunction
                << "Empirical correlation for stationary crossflow "
                << " did not converge within max iteration limit: "
                << maxIter << endl;
            }
        }
    }
...
:
    ...
    maxLambdaIter_
    (
        this->coeffDict_.lookupOrDefault("maxLambdaIter", 30)
    ),
    ...
    h_(this->coeffDict_.lookupOrDefault("roughnessHeight", 3e-6)),
    ReThetat_
    (
        IOobject
        (
...
bool kOmegaSSTLM<BasicTurbulenceModel>::read()
{
    if (kOmegaSST<BasicTurbulenceModel>::read())
    {
        ...
        if (crossflow_)
        {
            Info<< "Activating stationary crossflow correction model -"
                << " Langtry et al. (2015)" << endl << endl;

            this->coeffDict().readIfPresent("roughnessHeight", h_);
            cCross_.readIfPresent(this->coeffDict());
        }
...
template<class BasicTurbulenceModel>
void kOmegaSSTLM<BasicTurbulenceModel>::segregatedReThetatGammaIntCorrect()
{
    // Local references
    ...
    const volScalarField::Internal Us(max(mag(Urel()), deltaU_));
    const volScalarField::Internal dUsds((Urel() & (Urel() & tgradU()()))/sqr(Us));
    tgradU.clear();
    ...
    {
        volScalarField::Internal t(500*nu/sqr(Us));
        ...
        if (crossflow_)
        {
            const volScalarField::Internal Fthetat2(this->Fthetat2(Us, Omega, nu));

            // Limited time scale for robunstness in high Re number flows
            t = min(500*nu/sqr(Us), rho()*sqr(pow(this->mesh_.V(), 1.0/3.0))/(nu*rho() + mut));

            // Stationary crossflow sink term
            const volScalarField::Internal Dscf
            (
                alpha()*rho()*cThetat_/t*cCross_*
                min(this->ReScf(Urel, Us, nu) - ReThetat_(), scalar(0.0))*Fthetat2
            );

            ReThetatEqn.ref() -= Dscf;
        }
...
template<class BasicTurbulenceModel>
void kOmegaSSTLM<BasicTurbulenceModel>::coupledReThetatGammaIntCorrect()
{
    //- field and equation allocation and preparation
    volVector2Field reThetaGamma
    (
        IOobject
        (
           "reThetaGamma",
    ...
    fvBlockMatrix<vector2> reThetaGammaEqn(reThetaGamma);
    reThetaGammaEqn.interfaces() = reThetaGamma.boundaryField().blockInterfaces();

    wordList fieldNames(2);
    labelList fieldSizes(fieldNames.size());

    fieldNames[0] = ReThetat_.name();  fieldNames[1] = gammaInt_.name();
    fieldSizes[0] =  1;   fieldSizes[1] =  1;
    blockIndex bIndex(fieldNames, fieldSizes);
    reThetaGammaEqn.bIndex() = bIndex;
...

    {
        ...
        ReThetatEqn.ref().relax();
        fvOptions.constrain(ReThetatEqn.ref());
        ReThetatEqn->boundaryManipulate(ReThetat_.boundaryFieldRef());
        reThetaGammaEqn += ReThetatEqn();
    }
...

    {
        ...
        gammaIntEqn.ref().relax();
        fvOptions.constrain(gammaIntEqn.ref());
        gammaIntEqn->boundaryManipulate(gammaInt_.boundaryFieldRef());
        reThetaGammaEqn += gammaIntEqn();
    }

    reThetaGammaEqn.solve();

    reThetaGammaEqn.retrieveSolution(0, ReThetat_.primitiveFieldRef());
    reThetaGammaEqn.retrieveSolution(1, gammaInt_.primitiveFieldRef());
...
void kOmegaSSTLM<BasicTurbulenceModel>::correct()
{
    ...
    if (this->solveCoupled_)
    {
        this->coupledReThetatGammaIntCorrect();
    }
    else
    {
        this->segregatedReThetatGammaIntCorrect();
    }
