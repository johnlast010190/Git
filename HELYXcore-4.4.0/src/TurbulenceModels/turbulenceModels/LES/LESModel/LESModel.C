...
) const
{
    ...
    const fvPatch& curPatch = this->mesh_.boundary()[patchi];
    ...
    {
        Yp = this->rho_.boundaryField()[patchi] * sqrt
            (
                this->muEff(patchi)
               *mag(this->U_.boundaryField()[patchi].snGrad())
               /this->rho_.boundaryField()[patchi]
            )
           /this->mu().boundaryField()[patchi]*this->y()[patchi];
    }
    else
    {
        WarningInFunction
            << "Patch " << patchi << " is not a wall. Returning null field"
            << nl << endl;
...
bool Foam::LESModel<BasicTurbulenceModel>::read()
{
    if (BasicTurbulenceModel::read())
    {
        ...
        turbulence_ = LESDict_.lookup<Switch>("turbulence");
