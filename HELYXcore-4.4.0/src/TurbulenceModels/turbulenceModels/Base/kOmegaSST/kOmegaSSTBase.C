...
template<class BasicEddyViscosityModel>
void kOmegaSSTBase<BasicEddyViscosityModel>::segregatedCorrect()
{
    // Local references
    ...
    if (deferredS1_ || deferredS2_)
    {
        ...
        {
            Info<< "using time consistent turbulence source discretisation"
                 << endl;
        }
    ...
    volScalarField F23(this->F23());
    {
        ...
        (
         ...
         ==
            alpha()*rho()*gamma*Fr1()*GbyNu(GbyNu0, F23(), S2())
    ...
    (
     ...
     ==
        alpha()*rho()*Pk(G, deferredS1_)*Fr1()
...
template<class BasicEddyViscosityModel>
void kOmegaSSTBase<BasicEddyViscosityModel>::coupledCorrect()
{
    //- field and equation allocation and preparation
    volVector2Field kOmega
    (
        IOobject
        (
           "kOmega",
    ...
    fvBlockMatrix<vector2> kOmegaEqn(kOmega);
    kOmegaEqn.interfaces() = kOmega.boundaryField().blockInterfaces();

    wordList fieldNames(2);
    labelList fieldSizes(fieldNames.size());

    fieldNames[0] = k_.name();  fieldNames[1] = omega_.name();
    fieldSizes[0] =  1;   fieldSizes[1] =  1;
    blockIndex bIndex(fieldNames, fieldSizes);
    kOmegaEqn.bIndex() = bIndex;
    //-
    ...
    volScalarField F23(this->F23());
    {
        ...
        (
         ...
         ==
            alpha()*rho()*gamma*Fr1()*GbyNu(GbyNu0, F23(), S2())
          - fvm::SuSp((2.0/3.0)*alpha()*rho()*gamma*divU, omega_)
          - fvm::Sp(2*alpha()*rho()*beta*omega_(), omega_)
          + alpha()*rho()*beta*omega_()*omega_
          + alpha()*rho()*(scalar(1) - F1())*CDkOmega
          + Qsas(S2(), gamma, beta)
          + omegaSource()
          + fvOptions(alpha, rho, omega_)
        );
        ...
        omegaEqn.ref().relax();
        fvOptions.constrain(omegaEqn.ref());
        omegaEqn->boundaryManipulate(omega_.boundaryFieldRef());
        kOmegaEqn += omegaEqn();
    }

    //- Production term that added explicitly
    volScalarField::Internal cTermExp
    (
        IOobject
        (
            "cTermExp",
        ...
        dimensionedScalar
        (
            G.dimensions()*rho.dimensions()*alpha.dimensions(),
            0
        )
    );

    const scalarField& vol = this->mesh_.V();

    //- Production term that added implicitly
    autoPtr<volScalarField::Internal> cTermImpPtr;

    if (!decoupled_)
    {
        //- Production term to check sign
        volScalarField::Internal cTerm
        (
            G -((c1_*betaStar_)*k_.internalField()*omega_.internalField())
        );
        ...
        (
            new volScalarField::Internal
                (
                    -(c1_*betaStar_)*k_*rho*alpha*Fr1()
                )
        );

        volScalarField::Internal& cTermImp = cTermImpPtr();

        //- Alter the terms based on the way the system is solved
        forAll(cTerm, cI)
        {
            if (cTerm[cI] < 0)
            {
                cTermExp[cI] = G[cI]*rho[cI]*alpha[cI]*Fr1()[cI];
                cTermImp[cI] = 0;
            }
            else
            {
    ...
    {
        //  Pk needed to be used here for the derived overloaded
        // SST classes
        cTermExp = alpha()*rho()*Pk(G, deferredS1_)*Fr1();
    }
    ...
    (
     ...
     ==
        cTermExp
      - fvm::SuSp((2.0/3.0)*alpha()*rho()*divU, k_)
      - fvm::Sp(2*alpha()*rho()*epsilonByk(F1, tgradU(), deferredS2_), k_)
      + alpha()*rho()*epsilonByk(F1, tgradU(), deferredS2_)*k_
      + kSource()
      + fvOptions(alpha, rho, k_)
    );
    ...
    {
        //- couplings terms
        // alpha()*rho()*min(G, (c1_*betaStar_)*k_*omega_)*Fr1()
        kOmegaEqn(k_.name(), omega_.name(), cTermImpPtr());
    }

    kOmegaEqn.solve();

    kOmegaEqn.retrieveSolution(0,k_.primitiveFieldRef());
    kOmegaEqn.retrieveSolution(1,omega_.primitiveFieldRef());
    fvOptions.correct(k_);
    fvOptions.correct(omega_);
    bound(omega_, this->omegaMin_);
    bound(k_, this->kMin_);
...
{
    tmp<volScalarField> CDkOmegaPlus =
        max
        (
            CDkOmega,
    ...
    tmp<volScalarField> ty2(sqr(y));
    const volScalarField& y2 = ty2();
    ...
    (
        ...
        (
            ...
            (
               ...
               /max(y2*omega_, sqr(this->ySmall_)*this->omegaMin_)
            ),
            (4*alphaOmega2_)*k_/(CDkOmegaPlus*y2)
        ),
        scalar(10)
    );
...
tmp<volScalarField> kOmegaSSTBase<BasicEddyViscosityModel>::F2() const
{
    ...
    //NOTE: Engys removed min(...,100) function around arg2 in compressible
...
)
{
    ...
    if (!limitNearWallCell_)
    {
        ...
        {
            const fvPatchScalarField& nutwpf =
            ...
            if (isA<nutWallFunctionFvPatchScalarField>(nutwpf))
            {
                ...
                forAll(nutWF, facei)
                {
                    ...
                    this->nut_[nearWallCelli] =
                        k_[nearWallCelli]/omega_[nearWallCelli];
                }
            }
        }
    }
    ...
    dimensionedScalar nutMin("nutMin", this->nut_.dimensions(), 0);
...
// Based on model kOmegaSST-RC -
// https://turbmodels.larc.nasa.gov/sst.html#sst-rc
template<class BasicEddyViscosityModel>
...
)
{
    ...
    (
        ...
        (
            symInnerProduct()/
            max(w,dimensionedScalar("minw", w().dimensions(), SMALL))
        )
    );
    ...
    (
        2.0*rT()
        /(max((w2()*D()*D()*D()),
            dimensionedScalar("limit", dimensionSet(0, 0, -4, 0, 0), SMALL)))
    );
    ...
    (
        ...
        (
            min
            (
                (scalar(1.0) + cr1_) * 2.0 * rStar() / (scalar(1) + rStar()) *
...
{
    scalar Cmu_m25(Foam::pow(betaStar_.value(), -0.25));
