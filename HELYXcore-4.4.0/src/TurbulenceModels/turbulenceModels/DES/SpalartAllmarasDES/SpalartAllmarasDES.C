...
{
    return volScalarField::New(modelName("Omega"), sqrt(2.0)*mag(skew(gradU)));
}
...
{
    return (gradNuTilda & fvc::grad(rho));
}
...
template<class BasicTurbulenceModel>
tmp<volScalarField>
SpalartAllmarasDES<BasicTurbulenceModel>::DnuTildaEff() const
{
    return volScalarField::New
    (
        "DnuTildaEff",
...
tmp<volScalarField> SpalartAllmarasDES<BasicTurbulenceModel>::k() const
{
    ...
    (
        modelName("k"),
        sqr(this->nut()/ck_/dTilda(chi, fv1, gradUnwc()))
    );
}
