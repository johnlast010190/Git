...
//NOTE: Currently Engys compressible model uses IDDESDelta2 whereas incompressible
...
tmp<volScalarField> SpalartAllmarasIDDES<BasicTurbulenceModel>::alpha() const
{
    ...
    (
        modelName("alpha"),
        max(0.25 - y/IDDESDelta_.hmax(), scalar(-5))
    );
}
...
) const
{
    ...
    (
        modelName("fdt"),
        1 - tanh(pow(Cdt1_*rd(this->nut_, magGradU), Cdt2_))
    );
}
...
) const
{
    ...
    (
        ...
        dimensionedScalar(dimLength, SMALL) // this should be relative
    );
}
