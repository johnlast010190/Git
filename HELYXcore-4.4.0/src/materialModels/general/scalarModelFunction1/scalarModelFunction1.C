...
{
    return autoPtr<scalarModelFunction1>(new scalarModelFunction1(*this));
}
...
Foam::scalar Foam::scalarModelFunction1::generalCell
(
    const label celli
) const
{
    if (useTime_)
    {
        return general(mesh().time().value());
    }
    return general(field_->operator[](celli));
}


Foam::scalar Foam::scalarModelFunction1::generalValue(scalar p, scalar T) const
{
    if (useTime_)
    {
        return general(mesh().time().value());
    }
    return general(field_->value(p, T));
}
...
const
{
    ...
    (
        ...
        (
            ...
            obr_,
            mesh(),
            dimensionedScalar(generalModel::dimensions(), Zero)
        )
    );
...
bool Foam::scalarModelFunction1::read()
{
    ...
    (
        baseModels<scalar>::dimensions
        (
            dict_->lookupOrDefault<word>("dimensions", funcName)
        )
    );
