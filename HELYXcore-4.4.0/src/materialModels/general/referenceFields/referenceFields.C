...
:
    ...
    fieldRef_
    (
        dict.lookupOrDefault<dimensioned<Type>>(fieldName, dimensioned<Type>())
    ),
    isConst_(dict.found(fieldName + "Const")),
    constVal_
    (
        dict.lookupOrDefault<dimensioned<Type>>
        (
            fieldName + "Const", dimensioned<Type>()
        )
    ),
    field_
    (
        obr.lookupObjectPtr<VolField<Type>>
        (
            fieldName
        )
    ),
    ...
    boundary_(*this)
{
    ...
    else if (field_ != nullptr)
    {
        if
        (
            dict.found(fieldName_)
         && fieldRef_.dimensions() != field_->dimensions()
        )
        {
            FatalErrorInFunction
    ...
    if (fieldName.lessExt() == "T")
    {
        fieldType_ = temperature;
    }
    else if (fieldName.lessExt() == "p")
    {
        fieldType_ = pressure;
    }
}
...
template<class Type>
bool Foam::referenceFields<Type>::write(const bool write) const
{
    NotImplemented;
}
