...
namespace Foam
{
    defineTypeNameAndDebug(rPolynomialLaw, 0);
    addToRunTimeSelectionTable(materialModel, rPolynomialLaw, dictionary);
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::rPolynomialLaw::rPolynomialLaw
(
...
Foam::baseModels<Foam::scalar>* Foam::rPolynomialLaw::castScalarModel
(
...
void Foam::rPolynomialLaw::updateTable(const word& modelName)
{
    // Model names
    ...
    fill(modelName, modList, {RModel::typeName}, {0});
}


bool Foam::rPolynomialLaw::read()
{
    initialise("T");
    initialise("p");

    C_ = coeffList(dict_->lookup("C"));
