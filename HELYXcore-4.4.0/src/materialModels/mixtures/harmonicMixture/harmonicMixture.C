...
)
{
    ...
    (
        new harmonicMixture(obr, dict, phaseName, specieName, name)
    );
}
...
{
    const dimensionedScalar s(sMod_[0].dimensions(), VSMALL);
    ...
    (
        ...
        (
            ...
            dimensionedScalar(dimless/sMod_[0].dimensions(), 0)
        )
    );
...
Foam::tmp<Foam::scalarField>
Foam::harmonicMixture::harmonicMixturePatch(const label patchi) const
{
...
Foam::scalar Foam::harmonicMixture::harmonicMixtureValue
(
    scalar p,
    scalar T
) const
