...
) const
{
    ...
    if (X_ != nullptr)
    {
        ...
        (
            min(max((Xc_ - XBc*fac_)/Xc_, scalar(0.0)), scalar(1.0))
        );
...
Foam::tmp<Foam::scalarField> Foam::BokilBewtraMu::muInternal() const
{
    ...
    if (X_ != nullptr)
    {
        const scalarField blending
        (
            min(max((Xc_ - X*fac_)/Xc_, scalar(0.0)), scalar(1.0))
        );
...
Foam::tmp<Foam::volScalarField> Foam::BokilBewtraMu::muGeometric() const
{
    ...
    (
        ...
        (
            ...
            obr_,
            mesh(),
            dimensionedScalar(muModel::modelDims, 0)
        )
    );
...
Foam::scalar Foam::BokilBewtraMu::muValue(scalar p, scalar T) const
{
    NotImplemented;
}
