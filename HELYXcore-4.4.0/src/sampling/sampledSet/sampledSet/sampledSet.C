...
)
{
    ...
    if (mergeTol_ > 0)
    {
        ...
        HashSet<label> valueToIndex;
        forAll(reverseMap, i)
        {
            label key = reverseMap[i];
            if (!valueToIndex.found(key))
            {
                valueToIndex.insert(key);
            }
            else
            {
                if (globalGathered.isLocal(key))
                {
                    valid[globalGathered.toLocal(key)] = false;
                }
            }
        }
        ...
        sz = 0;
        for (label key : valueToIndex)
        {
            if ((key < samplingPts.size()) && valid[key])
            {
                if (debug)
                    Pout<< "sz " << sz
                        << ", key " << key
                        << ",  point "<< samplingPts[key] << endl;

                operator[](sz) = samplingPts[key];
                cells_[sz] = samplingCells[key];
                faces_[sz] = samplingFaces[key];
                segments_[sz] = samplingSegments[key];
                curveDist_[sz] = samplingCurveDist[key];
                sz++;
            }
        }
    }
