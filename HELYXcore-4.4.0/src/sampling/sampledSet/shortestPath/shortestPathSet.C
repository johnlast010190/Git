    ...
    if (debug)
    {
        ...
        forAll(fld.boundaryField(), patchi)
        {
            ...
            if (isA<directPolyPatch>(pp))
            {
                ...
                label starti = dpp.start();
                forAll(pfld, i)
                {
                    pfld[i] = 1.0*allFaceInfo[starti+i].distance();
                }
                fld.boundaryFieldRef()[patchi].forceAssign(pfld);
            }
        }
...
)
{
    ...
    forAll(insidePoints_, insidei)
    {
        point insidePoint = insidePoints_[insidei];
        label insideCelli = insideCells_[insidei];
        if (returnReduce(insideCelli, maxOp<label>()) == -1)
        {
            insideCelli = mesh.findCell(insidePoint);
        }

        forAll(outsidePoints_, outsidei)
        {
            point outsidePoint = outsidePoints_[outsidei];
            label outsideCelli = outsideCells_[outsidei];
            if (returnReduce(outsideCelli, maxOp<label>()) == -1)
            {
               outsideCelli = mesh.findCell(outsidePoint);
            }
...
(
    ...
    const labelList& insideCells,
    const pointField& outsidePoints,
    const labelList& outsideCells,
    const boolList& isBlockedFace
)
:
    sampledSet(name, mesh, searchEngine, axis),
    insidePoints_(insidePoints),
    insideCells_(insideCells),
    outsidePoints_(outsidePoints),
    outsideCells_(outsideCells)
{
    if (debug)
    {
        fileName outputDir;
...
:
    ...
    insideCells_(insidePoints_.size() ,-1),
    outsidePoints_(dict.lookup("outsidePoints")),
    outsideCells_(outsidePoints_.size() ,-1)
{
