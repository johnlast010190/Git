...
const Foam::word Foam::sampledSets::volFieldType(word field)
{
    ...
    writeFormat_(word::null)
{
    ...
    if (obr().name() != word::null && obr().name() != mesh_.name())
    {
        outputPath_ = outputPath_/obr().name();
    }
    ...
    writeFormat_(word::null)
{
    ...
    // Append region name if different
    if (this->obr().name() != word::null && this->obr().name() != mesh_.name())
    {
        outputPath_ = outputPath_/this->obr().name();
    }
...
bool Foam::sampledSets::write()
{
    if (size())
    {
        ...
        {
            allSep = bool(
               checkFormatter(scalarFields_)
            && checkFormatter(vectorFields_)
            && checkFormatter(sphericalTensorFields_)
            && checkFormatter(symmTensorFields_)
            && checkFormatter(tensorFields_)
            );
        }
...
bool Foam::sampledSets::read(const dictionary& dict)
{
    ...
    if (setsFound)
    {
        fieldSelection_ = dict_.lookup<wordReList>("fields");
        clearFieldGroups();

        interpolationScheme_ = dict.lookup<word>("interpolationScheme");
        writeFormat_ = dict.lookup<word>("setFormat");
...
void Foam::sampledSets::topoChange
(
...
void Foam::sampledSets::mapMesh(const polyMeshMap& map)
{
    correct();
}
