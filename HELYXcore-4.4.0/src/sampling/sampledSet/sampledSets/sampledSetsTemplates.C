...
)
{
    ...
    else
    {
        ...
        (
            volFieldType(masterFields[0].name())
        );
        fileName fName
        (
            timeDir/formatter.getFileName(masterSampleSet, {fieldType})
        );
...
template<class Type>
bool Foam::sampledSets::checkFormatter(fieldGroup<Type>& fields)
{
    if (fields.size())
    {
        if (fields.formatter.empty())
        ...
        return fields.formatter().sepFile();
    }
    return true;
}
