...
#include "sampledSet/uniform/uniformSet.H"
...
) const
{
    ...
    while (true)
    {
        ...
        point startPt =  singleParticle.position();
        ...
        point endPt =  singleParticle.position();
        ...
        if (mag(startPt-endPt) < smallDist && !perturb && bFacei != -1)
        {
           perturb = true;
        }
        else
        {
            ...
            while (bHitI < bHits.size())
            {
                ...
                if
                (
                    (dist > -smallDist && updateBoundPt)
                    || faceChange
                )
                {
                    // hitpoint is past tracking position
                    foundValidB = true;
                    // Update with new boundary point
                    bPoint = bHits[bHitI].hitPoint();
                ...
                {
                    //Check for an entering patch
                    while (++bHitI < bHits.size())
                    {
                        point fA = mesh().faceAreas()[bHits[bHitI].index()];
                        fA /= (mag(fA) + SMALL);
                        if ((normOffset & fA) < 0.0871)
                        {
                            updateBoundPt = true;
                            if (bHits[bHitI].index() != bFacei)
                            {
                               faceChange = true;
                            }
                            break;
