...
void Foam::sampledIsoSurface::getIsoFields() const
{
    ...
    if (!subMeshPtr_.valid())
    {
        ...
        if (fvm.foundObject<pointScalarField>(pointFldName))
        {
            ...
            //Check if mesh changing
            if (fvm.changing())
            {
                const pointScalarField& pfld =
                    fvm.lookupObject<pointScalarField>
                    (
                        pointFldName
                     );
    ...
    else
    {
        ...
        if (subFvm.foundObject<pointScalarField>(pointFldName))
        {
            ...
            {
                const pointScalarField& pfld =
                    subFvm.lookupObject<pointScalarField>
                    (
                        pointFldName
                    );
...
:
    ...
    isoVal_(dict.lookup<scalar>("isoValue")),
    ...
    pointFieldPtr_(nullptr)
{
    ...
    if (zoneID_.index() != -1)
    {
        exposedPatchName_ = dict.lookup<word>("exposedPatchName");
