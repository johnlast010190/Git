...
bool Foam::sampledTriSurfaceMesh::update(const meshSearch& meshSearcher)
{
    ...
    if (sampleSource_ == cells)
    {
        ...
        scalar searchDistSqr = sqr(GREAT);
        ...
        if (useEdgeLength_)
        {
            ...
            {
                edge e = edges[edgei];
                scalar eLenSqr = magSqr
                    (mesh().points()[e[1]] - mesh().points()[e[0]]);
                 searchDistSqr = max(eLenSqr,searchDistSqr);
            }
            reduce(searchDistSqr, maxOp<scalar>());
            Info<<"Using edge length squared search distance : "
                << searchDistSqr << endl;
        }
...
:
    ...
    surface_
    (
        isFile(dict.lookup("surface"))
        ?
        IOobject
        (
            fileName(dict.lookup("surface")),
            mesh,                   // registry
            IOobject::MUST_READ,
            IOobject::NO_WRITE,
            false
    ...
    useEdgeLength_(dict.lookupOrDefault<Switch>("maxEdgeLengthSearch", false)),
    originalIds_(),
    zoneIds_(),
    sampleElements_(0),
    samplePoints_(0)
...
bool Foam::sampledTriSurfaceMesh::update()
{
    ...
    if (!bb.overlaps(mesh().bounds()))
    {
        Warning
            << "sampledTriSurfaceMesh: " << name()
            << " does not intersect any cell. Skipping..."
            << endl;
        return false;
    }
