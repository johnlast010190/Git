...
#include "meshes/primitiveShapes/triangle/triPoints.H"
...
{
    if (tri[0] < 0 or tri[0] > pointValues.size() - 1) return false;
    if (tri[1] < 0 or tri[1] > pointValues.size() - 1) return false;
    if (tri[2] < 0 or tri[2] > pointValues.size() - 1) return false;
...
) const
{
    ...
    else if (localTris.size() == 2)
    {
        ...
        if (shared[0] != -1)
        {
            vector n0 = tri0.unitNormal(localPoints);
            vector n1 = tri1.unitNormal(localPoints);
