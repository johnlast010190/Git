...
) const
{
    ...
    if (Pstream::parRun())
    {
        ...
        Pstream::allGatherList(cellsPresentOnProc);
...
) const
{
    ...
    label maxLevel = dict_.lookupOrDefault<label>("maxLevel", 8);
    label minBinSize = dict_.lookupOrDefault<label>("minBinSize", 0);
