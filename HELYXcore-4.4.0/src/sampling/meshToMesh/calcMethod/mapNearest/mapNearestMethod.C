...
#include "meshToMesh/calcMethod/mapNearest/mapNearestMethod.H"
...
)
{
    ...
    bool useTreeOnly = dict_.lookupOrDefault<bool>("useTreeSearch", false);

    {
        ...
        do
        {
            ...
            if (findNearestCell(src_, tgt_, srcCelli, tgtCelli))
            {
    ...
    // Set maximum neighbouring cells to search to a sensible value
    label maxIter = dict_.lookupOrDefault<label>("maxCellsWalked", 125);

    forAll(tgtToSrc, tgtCelli)
    {
        if (tgtToSrc[tgtCelli].empty())
        {
            label srcCelli = findMappedSrcCell(tgtCelli, tgtToSrc, maxIter);

            if (srcCelli != -1)
            {
                if (findNearestCell(tgt_, src_, tgtCelli, srcCelli, maxIter))
                {
                    tgtToSrc[tgtCelli].append(srcCelli);
                }
            }
...
bool Foam::mapNearestMethod::findNearestCell
(
...
) const
{
    ...
    } while (cells2.size() > 0 && nIter < maxIter);

    boundBox cell1Bb
    (
        mesh1.cells()[cell1].points(mesh1.faces(), mesh1.points()),
        false
    );

    boundBox cell2Bb
    (
        mesh2.cells()[cell2].points(mesh2.faces(), mesh2.points()),
        false
    );

    if (cell2Bb.mag() > cell1Bb.mag())
    {
        cell2Bb.inflate(0.5);
    }
    else
...
{
    FIFOStack<label> testCells;
    DynamicList<label> visitedCells(10);

    testCells.push(tgtCelli);

    label nIter = 0;
    do
    {
        // search target tgtCelli neighbours for match with source cell
        label tgtI = testCells.pop();
        nIter++;
        ...
        if (findIndex(visitedCells, tgtI) == -1)
        {
            ...
            {
                appendNbrCells(tgtI, tgt_, visitedCells, testCells);
            }
        }
    } while (testCells.size() && nIter < maxIter);
