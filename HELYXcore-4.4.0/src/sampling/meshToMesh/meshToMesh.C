...
(
    const VolField<sphericalTensor>& field,
    const plusEqOp<sphericalTensor>& cop,
    VolField<sphericalTensor>& result,
...
void Foam::meshToMesh::mapInternalSrcToTgt
(
    const VolField<symmTensor>& field,
    const plusEqOp<symmTensor>& cop,
    VolField<symmTensor>& result,
    const bool secondOrder
) const
{
    mapSrcToTgt(field, cop, result.primitiveFieldRef());
...
void Foam::meshToMesh::mapInternalSrcToTgt
(
    const VolField<tensor>& field,
    const plusEqOp<tensor>& cop,
    VolField<tensor>& result,
    const bool secondOrder
) const
{
    mapSrcToTgt(field, cop, result.primitiveFieldRef());
...
{
    const word coeffsName = methodName + "Coeffs";
    dictionary coeffsDict
    (
        dict_.found(coeffsName)
        ? dict_.subDict(coeffsName)
        : dictionary()
    );
...
void Foam::meshToMesh::calculatePatchAMIs(const word& AMIMethodName)
{
    ...
    forAll(srcPatchID_, i)
    {
        ...
        const directPolyPatch& srcPP =
            refCast<const directPolyPatch>
            (
                srcRegion_.boundaryMesh()[srcPatchi]
            );
        const directPolyPatch& tgtPP =
            refCast<const directPolyPatch>
            (
                tgtRegion_.boundaryMesh()[tgtPatchi]
            );
...
)
{
    ...
    forAll(cuttingPatches_, i)
    {
        ...
        label cuttingPatchi = tgtBm.findPatchID(patchName);

        if (cuttingPatchi == -1)
        {
            FatalErrorInFunction
                << "Unable to find patch '" << patchName
                << "' in mesh '" << tgtRegion_.name() << "'. "
                << " Available patches include:" << tgtBm.names()
                << exit(FatalError);
        }
