...
// Check if "ENGYS multi" appears as the first non-space content.
// If found file is binary multi-block format
static bool startsWithBlock(const char header[STLHeaderSize])
{
    unsigned pos = 0;
    while (std::isspace(header[pos]) && pos < STLHeaderSize)
    {
    ...
    return
    (
        pos < (STLHeaderSize-11)  // At least 11 chars remaining
     ...
     && std::toupper(header[pos+6]) == 'M'
     && std::toupper(header[pos+7]) == 'U'
     && std::toupper(header[pos+8]) == 'L'
     && std::toupper(header[pos+9]) == 'T'
     && std::toupper(header[pos+10]) == 'I'
    );
}
//! \endcond
...
)
{
    ...
    else if (startsWithBlock(header))
    {
        blockBinary = true;
        return 1;
    }
    ...
    else if (!compressed)
    {
            ...
            const off_t dataFileSize = Foam::fileSize(filename,false);
        #else
