...
)
{
    ...
    DebugInformation        << "Read " << celli << " cells;" << facei << " faces." << nl;
...
{
    if (vtkDataTypeNames.hasEnum(dataType))
    {
        switch (vtkDataTypeNames[dataType])
        {
            ...
            case VTK_STRING:
            {
                DebugInformation                    << "Reading strings:" << size << nl;
...
) const
{
    ...
    DebugInformation        << "dataName:" << dataName << nl;
    ...
    for (label i = 0; i < numArrays; i++)
    {
        ...
        DebugInformation            << "Reading field " << arrayName
...
void Foam::vtkUnstructuredReader::read(ISstream& inFile)
{
    ...
    DebugInformation << "Header   : " << header_ << nl;

    inFile.getLine(title_);
    DebugInformation << "Title    : " << title_ << nl;

    inFile.getLine(dataType_);
    DebugInformation << "dataType : " << dataType_ << nl;
    ...
    while (inFile.read(tok).good() && tok.isWord())
    {
        ...
        DebugInformation            << "line:" << inFile.lineNumber()
            << " tag:" << tag << nl;

        if (tag == "DATASET")
        {
            word geomType(inFile);
            DebugInformation << "geomType : " << geomType << nl;
        ...
        else if (tag == "POINTS")
        {
            ...
            DebugInformation                << "Reading " << nPoints << " numbers representing "
        ...
        else if (tag == "CELLS")
        {
            ...
            DebugInformation                << "Reading " << nCells << " cells or faces." << nl;
        ...
        else if (tag == "LINES")
        {
            ...
            label nNumbers(readLabel(inFile));
            DebugInformation                << "Reading " << nLines << " lines." << nl;
        ...
        else if (tag == "POLYGONS")
        {
            ...
            label nNumbers(readLabel(inFile));
            DebugInformation                << "Reading " << nFaces << " faces." << nl;
        ...
        else if (tag == "SCALARS")
        {
            ...
            DebugInformation                << "Reading scalar " << dataName
                << " of type " << dataType
        ...
        else if (tag == "VECTORS" || tag == "NORMALS")
        {
            ...
            word dataName(is);
            word dataType(is);
            DebugInformation                << "Reading vector " << dataName
                << " of type " << dataType << nl;
        ...
        else if (tag == "TEXTURE_COORDINATES")
        {
            ...
            DebugInformation                << "Reading texture coords " << dataName
                << " dimension " << dim
                << " of type " << dataType << nl;
        ...
        else if (tag == "TRIANGLE_STRIPS")
        {
            ...
            label nNumbers(readLabel(inFile));
            DebugInformation                << "Reading " << nStrips << " triangle strips." << nl;
        ...
        else if (tag == "METADATA")
        {
            ...
            DebugInformation                << "Consuming " << nInfo<< " metadata information." << nl;
