    ...
    for (; it0.More(); it0.Next())
    {
        ...
        for (Standard_Integer j = 1; j <= shapeMap.Extent(); j++)
        {
            ...
            for (Standard_Integer aTriIter = 1; aTriIter <= aTriangulation->NbTriangles(); aTriIter++)
            {
                ...
                Standard_Integer anId0, anId1, anId2;

                aTri.Get(anId0, anId1, anId2);

                if (anOrientation == TopAbs_REVERSED)
                {
                    // Swap 1, 2.
                    Standard_Integer aTmpIdx = anId1;
                    anId1 = anId2;
                    anId2 = aTmpIdx;
                }
                ...
                labelledTri aLabelledTri(anId0, anId1, anId2, patchIDs[patchName]);
