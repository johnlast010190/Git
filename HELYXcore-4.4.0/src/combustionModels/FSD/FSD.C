...
:
    ...
    YFuelFuelStream_(dimensionedScalar(dimless, 1)),
...
void Foam::combustionModels::FSD::calculateSourceNorm()
{
    ...
    // is 1.5 mm)
    volScalarField deltaF(delta/dimensionedScalar(dimLength, 1.5e-3));
    ...
    this->wFuel_.forceAssign(mgft*pc*omegaFuelBar);
}
