...
void Foam::treeBoundBox::cornerPerturb
(
   const direction exclDir,
   point& pt
) const
{
    forAll(pt, dir)
    {
        if (dir != exclDir)
        {
            ...
            {
#if defined(HELYX_SP)
                pt[dir] = nextafterf(min()[dir],INFINITY);
#else
                pt[dir] = nextafter(min()[dir],INFINITY);
#endif
            }
            else if (pt[dir] == max()[dir])
            {
#if defined(HELYX_SP)
                pt[dir] = nextafterf(max()[dir],-INFINITY);
#else
                pt[dir] = nextafter(max()[dir],-INFINITY);
#endif
            }
        }
    }
