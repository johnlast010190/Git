...
{
    return faceCentre(UIndirectList<point>(points, *this));
}


Foam::vector Foam::face::areaNormal(const UList<point>& ps) const
{
    return faceArea(UIndirectList<point>(ps, *this));
}
...
Foam::label Foam::face::longestEdge(const UList<point>& pts) const
{
    ...
    for (const edge e : this->walkEdges())
    {
        scalar edgeLength = e.mag(pts);
