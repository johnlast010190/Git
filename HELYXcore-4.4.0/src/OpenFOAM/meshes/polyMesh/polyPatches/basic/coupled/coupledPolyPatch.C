...
) const
{
    ...
    else
    {
        ...
        forAll(ownerSeedFacePoints, regioni)
        {
            ...
            // The seed face and face-point are the ones which give the
            // smallest total displacement between all corresponding points.
            // Note that owner and neighbour point order is reversed.
            ...
            forAll(pp, facei)
            {
                ...
                auto reversedList = reverseList(facePts);
                forAll(facePts, facePointi)
                {
                    inplaceRotateList(reversedList, 1);
                    const scalar sumSqrDisplacement =
                        sum
                        (
                            magSqr
                            (
                                reversedList - ownerFacePts
                            )
                        );
...
)
{
    ...
    forAll(faces, facei)
    {
        ...
        // 2. As a measure of the truncation error when comparing two
        //    coordinates, use SMALL * maximum component.
