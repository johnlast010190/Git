...
void Foam::directPolyPatch::topoChange(PstreamBuffers&)
{
    primitivePatch::clearGeom();
    clearAddressing();
}
...
void Foam::directPolyPatch::reorder(const labelUList& newToOldIndex)
{
...
:
    polyPatch(name, dict.lookup<label>("startFace"), dict, index, bm, patchType),
    primitivePatch
    (
        faceSubList
        (
