...
namespace Foam
{
    defineTypeNameAndDebug(outletPolyPatch, 0);

    addToRunTimeSelectionTable(polyPatch, outletPolyPatch, word);
    addToRunTimeSelectionTable(polyPatch, outletPolyPatch, dictionary);
}

// * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * * * * * //

Foam::outletPolyPatch::outletPolyPatch
(
    const word& name,
    const label size,
    const label start,
