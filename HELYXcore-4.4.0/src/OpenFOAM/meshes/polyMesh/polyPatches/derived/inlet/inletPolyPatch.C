...
namespace Foam
{
    defineTypeNameAndDebug(inletPolyPatch, 0);

    addToRunTimeSelectionTable(polyPatch, inletPolyPatch, word);
    addToRunTimeSelectionTable(polyPatch, inletPolyPatch, dictionary);
}

// * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * * * * * //

Foam::inletPolyPatch::inletPolyPatch
(
    const word& name,
    const label size,
    const label start,
