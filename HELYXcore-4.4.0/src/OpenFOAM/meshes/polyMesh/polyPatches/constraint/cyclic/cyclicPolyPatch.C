...
namespace Foam
{
    ...
    addToRunTimeSelectionTable(directPolyPatch, cyclicPolyPatch, word);
    addToRunTimeSelectionTable(directPolyPatch, cyclicPolyPatch, dictionary);
}
    ...
    ownToNbrDebugOrderDataPtr_(nullptr)
{
    ...
    {
        sectors_ = sign(rotationAngle())*
            constant::mathematical::twoPi/rotationAngle();
    }
...
) const
{
    ...
    if (owner())
    {
        ...
        const scalarField magAreas(pp.magFaceAreas());
