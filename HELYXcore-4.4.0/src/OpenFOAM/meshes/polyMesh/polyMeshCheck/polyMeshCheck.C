...
) const
{
    ...
    forAll(ortho, facei)
    {
            ...
            else
            {
                ...
                if (detailedReport && errorNonOrth == 0)
                {
                    ...
                    WarningInFunction
                        ...
                        << " and " << nei[facei]
                        << ": Angle = "
                        << radToDeg(Foam::acos(min(1.0, max(-1.0, ortho[facei]))))
                        << " deg." << endl;
                }
        ...
        if (isMasterFace[facei])
        {
            ...
            sumDDotS += radToDeg(Foam::acos(min(1.0, max(-1.0, ortho[facei]))));
            nSummed++;
        }
    }
        ...
        if (nSummed > 0)
        {
            if (debug || report)
            {
                Info<< "    Mesh non-orthogonality Max: "
                    << radToDeg(Foam::acos(min(1.0, max(-1.0, minDDotS))))
                    << " average: "
                    << sumDDotS/nSummed
                    << endl;
            }
