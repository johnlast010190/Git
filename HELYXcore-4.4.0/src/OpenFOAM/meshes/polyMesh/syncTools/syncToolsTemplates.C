...
)
{
    ...
    EdgeMap<edge> syncedEdges(potentialSharedEdge.size());
    ...
    forAllConstIter(typename EdgeMap<edge>, potentialSharedEdge, iter)
    {
        ...
        {
            typename EdgeMap<T>::iterator iter = edgeValues.find(meshEdge);
            if (iter != edgeValues.end())
            {
                edgeValues.erase(iter);
            }

            syncedEdges.insert(meshEdge,sharedEdge);
            combine
            (
                edgeValues,
                cop,
        ...
        forAll(patches, patchi)
        {
            ...
            )
            {
                ...
                forAll(edges, i)
                {
                    ...
                    if (iter != edgeValues.end())
                    {
                        typename EdgeMap<edge>::const_iterator sharedFnd =
                            syncedEdges.find(meshEdge);

                        if (sharedFnd == syncedEdges.end())
                        {
...
)
{
    ...
    const Map<label>& map = cpp.meshPointMap();
    ...
    forAll(meshPoints, i)
    {
        ...
        Map<label>::const_iterator iter = map.find(pointi);
        if (iter != map.end())
        {
...
)
{
    ...
    const globalMeshData& gd = mesh.globalData();
    const indirectPrimitivePatch& cpp = gd.coupledPatch();
    const Map<label>& map = gd.coupledPatchMeshEdgeMap();
    ...
    forAll(meshEdges, i)
    {
        ...
        Map<label>::const_iterator iter = map.find(edgeI);
        if (iter != map.end())
        {
...
{
    //TODO see fvMeshDistribute::reorderCoupledFaces for multithreaded sync //GGG
    // when dealing with list of meshes
    ...
    if (parRun)
    {
        if (threaded && Pstream::haveThreads())
        {
            ...
            (
                Pstream::nProcs(), // receiving proc
                PstreamBuffers(Pstream::commsTypes::nonBlocking)
            );

            tbb::task_group sendTasks;
                ...
                )
                {
                    ...
                    (
                        [&]
                        {
                            ...
                            const label toProc = procPatch.neighbProcNo();

                            UOPstream toNbr(toProc, pBufs[toProc]);
            ...
            for (auto i = 0; i < Pstream::nProcs(); ++i)
            {
            ...
            //                UIPstream fromNeighb(procPatch.neighbProcNo(), pBufs[Pstream::myProcNo()]);
            //                fromNeighb >> nbrPatchInfo;

            //                //TODO nested threaded?
            //                top(procPatch, nbrPatchInfo);
            ...
            //                    std::max<size_t>
            //                    (
            //                        nbrPatchInfo.size() / tbb::this_task_arena::max_concurrency(),
            //                        1024
            //                    );

            //                tbb::parallel_for
            //                (
            //                    tbb::blocked_range<size_t>(0, nbrPatchInfo.size(), grainSize),
            //                    [&](const tbb::blocked_range<size_t>& r)
            //                    {
            //                        for (size_t i = r.begin(); i < r.end(); ++i)
            //                        {
            //                            cop(faceValues[bFacei+i], nbrPatchInfo[i]);
            //                        }
            //                    },
            ...
            if (cycPatch.owner())
            {
                ...
                if (threaded && nbrVals.size() > 2048)
                {
                    ...
                    const auto grainSize =
                        std::max<size_t>
                        (
                            nbrVals.size() / tbb::this_task_arena::max_concurrency(),
                            1024
                        );

                    tbb::parallel_for
                    (
                        tbb::blocked_range<size_t>(0, nbrVals.size(), grainSize),
                        [&](const tbb::blocked_range<size_t>& r)
                        {
                            for (size_t i = r.begin(); i < r.end(); ++i)
                            {
                                cop(faceValues[i0+i], nbrVals[i]);
                            }
                        },
                        tbb::simple_partitioner()
                    );
                ...
                if (threaded && ownVals.size() > 2048)
                {
                    ...
                    const auto grainSize =
                        std::max<size_t>
                        (
                            ownVals.size() / tbb::this_task_arena::max_concurrency(),
                            1024
                        );

                    tbb::parallel_for
                    (
                        tbb::blocked_range<size_t>(0, ownVals.size(), grainSize),
                        [&](const tbb::blocked_range<size_t>& r)
                        {
                            for (size_t i = r.begin(); i < r.end(); ++i)
                            {
                                cop(faceValues[i1+i], ownVals[i]);
                            }
                        },
                        tbb::simple_partitioner()
                    );
