...
{
    return this->findMatching(this->findIndices(key));
}
...
{
    return this->findMatching(this->findIndices(matcher));
}


template<class ZoneType, class MeshType>
void Foam::ZoneMesh<ZoneType, MeshType>::clearZeroSized()
{
    PtrList<ZoneType>& zones = *this;
    labelList newToOld(zones.size());
    label nValid = 0;
    DynamicList<word> emptyZones(zones.size());
    forAll(zones, zonei)
    {
        label sz = zones[zonei].size();
        reduce(sz, sumOp<label>());
        if (sz > 0)
        {
            newToOld[nValid++] = zonei;
        }
        else
        {
            emptyZones.append(names()[zonei]);
        }
    }
    if (emptyZones.size() > 0)
    {
        Info<< nl;
        Info<<"Clearing empty " << ZoneType::typeName << endl;
        Info<<"-----------------------" << endl;
        forAll(emptyZones, emptyi)
        {
            Info<<"Removing : " << emptyZones[emptyi] <<endl;
        }
        Info<< nl;
    }
    newToOld.setSize(nValid);
    zones.shuffle(newToOld);
}
