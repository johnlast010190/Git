...
const Foam::distributionMap& Foam::globalMeshData::globalCoPointSlavesMap() const
{
...
void Foam::globalMeshData::topoChange()
{
    ...
    // Find the total number of faces, cells, and points.
    FixedList<label, 3> tup {
        mesh_.nFaces(),
        mesh_.nCells(),
        mesh_.nPoints()
    };
    ...
    if (debug)
    {
        Pout<< "globalMeshData : nTotalFaces_:" << nTotalFaces_ << endl
             << "globalMeshData : nTotalCells_:" << nTotalCells_ << endl
             << "globalMeshData : nTotalPoints_:" << nTotalPoints_ << endl;
    }
}
