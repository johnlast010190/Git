...
// Distribute list in list of meshes.
template<class T, class negateOp>
void Foam::distributionMapBase::distribute
(
    const List<label>& constructSizes,
    const List<labelListList>& subMaps,
    const boolList subHasFlips,
    const List<labelListList>& constructMaps,
    const boolList constructHasFlips,
    PtrList<IOList<T>>& fields,
    const negateOp& negOp,
    const labelList& meshToProc,
    const labelListList& meshNoInProc
)
{
    const label nFinalProcs = meshToProc.size();
    ...
    label bufSize = ceil(scalar(nFinalProcs)/scalar(Pstream::nProcs()));
    ...
    for (label domain = 0; domain < nFinalProcs; domain++)
    {
        //only first mesh in list has cells to send
        const labelList& map = subMaps[0][domain];

        if (map.size() == 0 || domain == Pstream::myProcNo()) continue;

        // findout if neighbour mesh is on this processor
        const label recvProc = meshToProc[domain];

        //Pout<< "Sending domain " << domain;
        ...
        if (recvProc != Pstream::myProcNo())
        {
            ...
            UOPstream toDomain(recvProc, pBufs[domain]);
        ...
        else
        {
            ...
            toLocalNeighbour[nStr] << subField;
            nStr++;
        }
    } // forAll my meshes send subMap

    // Start sending&receiving from buffers
    for (auto i = 0; i < nFinalProcs; ++i)
    {
        pBufs[i].finishedSends();
    }
    nStr = 0;

    // Set up 'send' to first mesh in list (myProcNo)
    {
        const labelList& mySub = subMaps[0][Pstream::myProcNo()];

        //Pout<<"Subsetting for my domain " << Pstream::myProcNo()
        //    <<" - nCells " << mySub.size()
        //    <<endl;
        ...
        fields[0].setSize(constructSizes[0]);

        // Receive sub field from myself
        {
            const labelList& map = constructMaps[0][Pstream::myProcNo()];
            ...
            //Pout<<"Receiving to my domain " << Pstream::myProcNo()
            //    <<" - nCells " << map.size()
            //    <<endl;
        }
    }
    ...
    // Consume
    forAll(subMaps, meshi)
    {
        const label myDomain = meshNoInProc[Pstream::myProcNo()][meshi];

        const labelListList& constructMap = constructMaps[meshi];

        for (label sendProc = 0; sendProc < Pstream::nProcs(); sendProc++)
        {
            const labelList& map = constructMap[sendProc];

            // first mesh in list already subsetted
            if (map.size() == 0 || sendProc == myDomain) continue;

            //Pout<< "Receiving subset of domain " << myDomain;
            ...
            if (sendProc != Pstream::myProcNo())
            {
                ...
                //    << "    nCells:" << map.size()
                //    << endl;
                UIPstream str(sendProc, pBufs[myDomain]);

                List<T> recvField(str);

                checkReceivedSize(sendProc, map.size(), recvField.size());

                flipAndCombine
                (
                    map,
                    constructHasFlips[meshi],
                    recvField,
                    eqOp<T>(),
                    negOp,
                    fields[meshi]
            ...
            else
            {
                ...
                List<T> recvField(fromNeighbour);
    ...
    else if (commsType == Pstream::commsTypes::nonBlocking)
    {
        ...
        else
        {
            ...
            // Receive sub field from myself (sendFields[Pstream::myProcNo()])
            {
                ...
                //Pout<< "Recv from " << Pstream::myProcNo() << ": " << subField << " " << map << endl;
            ...
            for (label domain = 0; domain < Pstream::nProcs(); domain++)
            {
                ...
                if (domain != Pstream::myProcNo() && map.size())
                {
                    ...
                    //Pout<< "Recv from " << domain << ": " << recvFields[domain] << " " << map << endl;
...
template<class T>
void Foam::distributionMapBase::send(PstreamBuffers& pBufs, const List<T>& field)
const
{
    // Stream data into buffer
    for (label domain = 0; domain < Pstream::nProcs(); domain++)
