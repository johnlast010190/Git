...
const Foam::labelList& Foam::polyBoundaryMesh::patchID() const
{
    if (!patchIDPtr_.valid())
    {
        ...
        forAll(bm, patchi)
        {
            if (!isA<indirectPolyPatch>(bm[patchi]))
            {
...
Foam::label Foam::polyBoundaryMesh::nDirectPatches() const
{
    const polyPatchList& patches = *this;
    label nDPatches = 0;
    for (const polyPatch& p : patches)
...
Foam::label Foam::polyBoundaryMesh::nIndirectPatches() const
{
    const polyPatchList& patches = *this;
    label nIndPatches = 0;
    for (const polyPatch& p : patches)
...
) const
{
    ...
    forAllConstIters(groupPatchIDs, iter)
    {
        ...
        {
            // No full match. Undo.
            nonGroupPatches = std::move(oldNonGroupPatches);
        }
    }
...
bool Foam::polyBoundaryMesh::checkParallelSync(const bool report) const
{
    ...
    forAll(bm, patchi)
    {
        if (!isA<processorPolyPatch>(bm[patchi]) && !isA<indirectPolyPatch>(bm[patchi]))
        {
            if (nonProci != patchi)
            {
                ...
                if (debug || report)
                {
                    Pout<< " Warning for boundary patch " << patchi
                        << " name: " << bm[patchi].name()
                        << " type: " <<  bm[patchi].type()
                        << ". Seems preceeded by processor patches. "
...
void Foam::polyBoundaryMesh::updateGIB()
{
    forAll(*this, patchI)
    {
        operator[](patchI).updateGIB();
    }
}
...
void Foam::polyBoundaryMesh::findNeighbProPatchIDs() const
{
    if (Pstream::parRun())
    {
        List<wordList> wL(Pstream::nProcs());
        wL[Pstream::myProcNo()] = names();
        Pstream::allGatherList(wL);
        ...
        forAll(patches, patchI)
        {
            ...
            {
                const processorPolyPatch& cprocPatch =
                    refCast<const processorPolyPatch>(patches[patchI]);

                processorPolyPatch& procPatch =
                const_cast<processorPolyPatch&>(cprocPatch);

                label& neiIndex = procPatch.neighbProPatchID();

                label neiPro = procPatch.neighbProcNo();
                label myPro = procPatch.myProcNo();

                const wordList& nwL = wL[neiPro];

                word patchName = "procBoundary" + Foam::name(neiPro) + "to" + Foam::name(myPro);
