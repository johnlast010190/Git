...
)
{
    ...
    forAll(f, faceBasePtI)
    {
        ...
        for (label tetPtI = 1; tetPtI < f.size() - 1; tetPtI++)
        {
            ...
            if (std::min(tetQualities[0], tetQualities[1]) < thisBaseMinTetQuality)
            {
                thisBaseMinTetQuality = std::min(tetQualities[0], tetQualities[1]);
            }
        }
...
)
{
    ...
    return List<tetIndices>(cellTets, true);
}
