...
template<class Mesh, template<class> class MeshObjectType, class Type>
Foam::MeshObject<Mesh, MeshObjectType, Type>::MeshObject(const Mesh& mesh, const word& typeName)
:
    MeshObjectType<Mesh>(typeName, mesh.thisDb()),
    mesh_(mesh)
{}
...
)
{
    ...
    if (meshObject::debug)
    {
        Pout<< "meshObject::distribute(objectRegistry&, "
               "const polyDistributionMap& map) : updating " << Mesh::typeName
            << " meshObjects for region " << obr.name() << endl;
    }
