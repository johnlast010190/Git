...
) const
{
    ...
    {
        // Gather all points
        List<List<point>> gatheredPoints =
            gatherList(points_);

        // Gather all faces
        List<List<face>> gatheredFaces =
                gatherList(faces_);
        ...
        {
            List<List<label>> gatheredValues =
                gatherList(faceLabelData_[setI]);
            if (Pstream::master())
            {
        ...
        {
            List<List<scalar>> gatheredValues =
                gatherList(faceScalarData_[setI]);
            if (Pstream::master())
            {
                // Combine values into single field
        ...
        {
            List<List<vector>> gatheredValues =
                gatherList(faceVectorData_[setI]);
            if (Pstream::master())
            {
                // Combine values into single field
        ...
        {
            List<List<label>> gatheredValues =
                gatherList(pointLabelData_[setI]);
            if (Pstream::master())
            {
                // Combine values into single field
                Field<label> allValues
        ...
        {
            List<List<scalar>> gatheredValues =
                gatherList(pointScalarData_[setI]);
            if (Pstream::master())
            {
                // Combine values into single field
                Field<scalar> allValues
        ...
        {
            List<List<vector>> gatheredValues =
                gatherList(pointVectorData_[setI]);
            if (Pstream::master())
            {
                // Combine values into single field
                Field<vector> allValues
...
template<class Type>
Foam::List<Foam::List<Type>> Foam::simpleVTKWriter::gatherList
(
   const List<Type>& fld
) const
{
    label nTransfers = 1;
    label sumSz = returnReduce(fld.size(), sumOp<label>());
    if (sumSz > 0)
    {
        nTransfers = (sumSz/20000000)+1;
    }

    // Split transfers
    if (nTransfers > 1)
    {
        Info<<"Large simpleVTK write : " <<  sumSz <<endl;
        label maxSz = returnReduce(fld.size(), maxOp<label>());
        label remain = (maxSz % nTransfers);
        label exactSz =  maxSz;
        if (remain != 0)
        {
            exactSz += (nTransfers - remain);
        }
        label maxLocalSize = exactSz / nTransfers;

        List<List<Type>> gatheredValues(Pstream::nProcs(),List<Type>(0));
        for (int transfer=0; transfer<nTransfers; transfer++)
        {
            label start = transfer*maxLocalSize;
            label end = (transfer+1)*maxLocalSize;
            DynamicList<Type> sfld(fld.size());
            forAll(fld, fldi)
            {
                if (fldi >= start && fldi <end)
                {
                    sfld.append(fld[fldi]);
                }
            }
            List<List<Type>> subValues(Pstream::nProcs());
            subValues[Pstream::myProcNo()] = sfld;
            Pstream::gatherList(subValues);
            for (label proci = 0; proci < Pstream::nProcs(); proci++)
            {
                List<Type>& gValue = gatheredValues[proci];
                const List<Type>& procValue = subValues[proci];
                label sz = gValue.size();
                label newsz = sz + procValue.size();
                gValue.setSize(newsz);
                forAll(procValue, pvi)
                {
                    gValue[sz++] = procValue[pvi];
                }
            }
        }
