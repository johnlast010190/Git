...
{
    point fc = fCtrs[facei];
    vector fa = fAreas[facei];
    ...
    else
    {
        ...
        skewness = max(mag(fc - faceIntersection)/(charDist + VSMALL),skewness);
...
Foam::scalar Foam::primitiveMeshTools::pyramidVol
(
    const point& apex,
    const point& fC,
    const vector& fA
...
{
    vector height = apex-fC;
    return (1.0/3.0)*(fA&height);
}
...
)
{
...
#if defined(HELYX_SP)
    vectorField compClosed(mesh.nCells(), Zero);
    vectorField compMagClosed(mesh.nCells(), Zero);
    forAll(owner, facei)
    {
        // Add to owner
        label own = owner[facei];
        primitiveMesh::kahanSum(areas[facei],compClosed[own],sumClosed[own]);
        vector cmptArea = cmptMag(areas[facei]);
        primitiveMesh::kahanSum(cmptArea,compMagClosed[own],sumMagClosed[own]);
    }
    ...
    forAll(neighbour, facei)
    {
        ...
        primitiveMesh::kahanSum(-areas[facei],compClosed[nei],sumClosed[nei]);
        vector cmptArea = cmptMag(areas[facei]);
        primitiveMesh::kahanSum(cmptArea,compMagClosed[nei],sumMagClosed[nei]);
    }
#else
    forAll(owner, facei)
    {
        // Add to owner
        sumClosed[owner[facei]] += areas[facei];
        sumMagClosed[owner[facei]] += cmptMag(areas[facei]);
    }

    forAll(neighbour, facei)
    {
        // Subtract from neighbour
        sumClosed[neighbour[facei]] -= areas[facei];
        sumMagClosed[neighbour[facei]] += cmptMag(areas[facei]);
    }
#endif
