...
#if defined(HELYX_SP)
Foam::scalar Foam::primitiveMesh::closedThreshold_  = 1.0e-3;
#else
...
Foam::scalar Foam::primitiveMesh::skewThreshold_    = 10;
...
) const
{
    ...
    forAll(magFaceAreas, facei)
    {
        if (magFaceAreas[facei] < minFaceArea)
        {
            ...
            if (detailedReport)
            {
                ...
                {
                    Pout<< "Face area less than " << minFaceArea
                        << " detected for internal face "<< facei
                        << " between cells "
                        << faceOwner()[facei] << " and "
                        << faceNeighbour()[facei]
                ...
                {
                    Pout<< "Face area less than " << minFaceArea
                        << " detected for boundary face " << facei
                        << " next to cell "
    ...
    if (minArea < minFaceArea)
    {
        if (debug || report)
        {
            Info<< " ***face with area less than " << minFaceArea
                << " detected. Minimum area: " << minArea << endl;
        }
...
) const
{
    ...
    const scalar severeNonorthogonalityThreshold =
        Foam::cos(degToRad(nonOrthThreshold_));
    ...
    forAll(ortho, i)
    {
        sumDDotS += radToDeg(Foam::acos(min(1.0, max(-1.0, ortho[i]))));
    }
...
) const
{
    ...
    forAll(c, celli)
    {
        ...
        // Neighbouring cells, annotated with their original index.
        List<std::pair<label, label>> nbr(curFaces.size());
        ...
        std::sort(nbr.begin(), nbr.end(), [](
            const std::pair<label, label>& x,
            const std::pair<label, label>& y
        ...
        label prevCell = nbr[0].second;
        label prevFace = curFaces[nbr[0].first];

        bool hasMultipleFaces = false;

        for (label i = 1; i < nbr.size(); i++)
        {
            label thisCell = nbr[i].second;
            label thisFace = curFaces[nbr[i].first];
...
) const
{
    ...
    forAll(c, celli)
    {
        ...
        {
            for (const edge curEdge : f[curFaces[facei]].walkEdges())
            {
