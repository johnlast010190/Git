...
void Foam::primitiveMesh::calcEdges(const bool doFaceEdges) const
{
    ...
    else
    {
        ...
        // no reallocations
        List<std::pair<label, label>> nbrPoints(primitiveMesh::edgesPerPoint_);
        ...
        forAll(pe, pointi)
        {
            ...
            forAll(pEdges, i)
            {
                ...
                nbrPoints[i] = {i, nbrPointi};
            }

            std::sort(nbrPoints.begin(), nbrPoints.end(), [](
            ...
            if (nInternalPoints_ == -1)
            {
                ...
                for (const auto& [index, value] : nbrPoints)
                {
                    if (value != -1)
                    {
                        label edgeI = pEdges[index];
            ...
            else
            {
                if (pointi < nInternalPoints_)
                {
                    for (const auto& [index, nbrPointi] : nbrPoints)
                    {
                        label edgeI = pEdges[index];
...
void Foam::primitiveMesh::calcFaceEdges() const
{
    HELYX_ASSERT(!fePtr_)
    {
        FatalErrorInFunction << "Should not re-calculate face edges." << abort(FatalError);
    }
