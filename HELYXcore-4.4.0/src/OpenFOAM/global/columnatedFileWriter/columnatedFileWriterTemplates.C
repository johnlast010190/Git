...
template<class Type>
void Foam::columnatedFileWriter::writeHeaderValue
(
...
void Foam::columnatedFileWriter::writeDelimited
(
    const T1& value
) const
{
    ...
    os_ << delimiter_ << std::string(spacesAfterDelimiter_, ' ').c_str()
        << setw(width) << buf.str().c_str();
}

// For backwards-compatibility, don't print "" around strings
template<>
void Foam::columnatedFileWriter::writeDelimited<Foam::string>
(
    const string& str
) const
{
    writeDelimited(str.c_str());
}

// For backwards-compatibility, don't print "" around strings
template<>
void Foam::columnatedFileWriter::writeDelimited<std::string>
(
...
void Foam::columnatedFileWriter::writeDelimitedComment(
    const string& str,
    const Args&... args
) const
{
    writeDelimitedComment(str.c_str(), args...);
}
