...
{
    queried = Foam::home()/HELYX_USER_RESOURCE_DIRNAME;
...
// Looks for
//   - $HELYX_CUSTOM_DATA_DIR
//   - HELYX_PROJECT_DIR/../site
static inline bool groupResourceDir(Foam::fileName& queried)
{
    queried = Foam::getEnv("HELYX_CUSTOM_DATA_DIR");
    if (queried.size())
    {
        return Foam::isDir(queried);
    }

    // Fallback (when HELYX_CUSTOM_DATA_DIR is unset)
    queried = Foam::getEnv("HELYX_PROJECT_DIR")/".."/"site";
...
{
    queried = Foam::getEnv("HELYX_PROJECT_DIR")/"etc";
