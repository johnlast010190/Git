...
#include "signals/sigInt.H"
#include "signals/sigQuit.H"
#include "signals/sigSegv.H"
...
void Foam::argList::resetMaxOpenFileStreams()
{
#if defined(WIN64) || defined(WIN32)
    _setmaxstdio
    (
        fileOperations::masterUncollatedFileOperation::maxOpenFileStreams
    );
#endif
}
...
void Foam::argList::getRootCase()
{
    ...
    setEnv("HELYX_EXECUTABLE", executable_, false);
    ...
    if (rootPath_.isAbsolute())
    {
        ...
        setEnv("HELYX_CASE", rootPath_/globalCase_, true);
        setEnv("HELYX_CASENAME", globalCase_, true);
    ...
    else
    {
        ...
        setEnv("HELYX_CASE", casePath, true);
        setEnv("HELYX_CASENAME", casePath.name(), true);
    ...
    distributed_(false)
{
    ...
    needsThread = needsThread || appNeedsThread;
    ...
    for (int argI = 1; argI < args_.size(); ++argI)
    {
        ...
        if (args_[argI][0] == '-')
        {
            ...
            )
            {
                ...
                if (strcmp(optionName, "debug-switch") == 0)
                {
                ...
                else if (strcmp(optionName, "info-switch") == 0)
                {
                ...
                else if (strcmp(optionName, "opt-switch") == 0)
                {
    ...
    if (parRunControl_.parRun())
    {
        ...
        if (Pstream::master())
        {
            ...
            else if (checkProcessorDirectories_)
            {
                ...
                {
                    distributed_ = true;
                    roots = decompDict.lookup<fileNameList>("roots");
                }
            }
            else if (fileHandler().type() == "uncollated")
            {
                IFstream decompDictStream(source);
...
void Foam::argList::printUsage() const
{
    ...
    Info<< nl
        <<"Using: HELYXcore-" << Foam::FOAMversion
        << " (see www.engys.com)" << nl
        << "Build: " << Foam::FOAMbuild << nl
        << "Arch:  " << Foam::FOAMbuildArch << nl
        << endl;
}
...
void Foam::argList::displayDoc(bool source) const
{
    ...
    if (found)
    {
        ...
        if (docBrowser.empty())
        {
            docBrowser = docDict.lookup<string>("docBrowser");
        }
