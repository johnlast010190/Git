...
namespace fileOperations
{
    ...
    float masterUncollatedFileOperation::maxOpenFileStreams
    (
        Foam::debug::floatOptimisationSwitch("maxOpenFileStreams", 1024)
    );
    registerOptSwitch
    (
        "maxOpenFileStreams",
        float,
        masterUncollatedFileOperation::maxOpenFileStreams
    );
}
}
    ...
    if (Pstream::master(comm))
    {
        ...
        else
        {
            if (read[0])
            {
                ...
                {
                    Info<< "\n\nAttempting to re-read the file.\n" << endl;
...
) const
{
    ...
    else
    {
        if (Pstream::nProcs(comm_) != Pstream::nProcs(Pstream::worldComm))
        {
            ...
            Pstream::gatherList(filePaths, comm_);
        }
...
) const
{
    ...
    else
    {
        ...
        else
        {
            ...
            Pstream::gatherList(procValid, comm_);

            // Uniform in local comm
            bool uniform = uniformFile(filePaths);

            return this->read(io, comm_, uniform, filePaths, procValid);
        }
    }
}
...
) const
{
    ...
    if (iter.found())
    {
        DynamicList<instant>& times = *iter();
...
) const
{
    if (Pstream::parRun())
    {
        ...
        Pstream::gatherList(filePaths, Pstream::worldComm);
