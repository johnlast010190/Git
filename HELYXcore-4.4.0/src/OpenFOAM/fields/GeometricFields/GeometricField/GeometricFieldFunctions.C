...
{
    cmptMax(gcf.primitiveFieldRef(), gf.primitiveField());
    cmptMax(gcf.boundaryFieldRef(), gf.boundaryField());
    gcf.oriented() = gf.oriented();
}
...
<
    ...
    >
>
cmptMax(const GeometricField<Type, PatchField, GeoMesh>& gf)
{
    typedef typename GeometricField<Type, PatchField, GeoMesh>::cmptType
        cmptType;

    tmp<GeometricField<cmptType, PatchField, GeoMesh>> CmptMax
    (
        GeometricField<scalar, PatchField, GeoMesh>::New
        (
            "cmptMax(" + gf.name() + ')',
...
<
    ...
    >
>
cmptMax(const tmp<GeometricField<Type, PatchField, GeoMesh>>& tgf)
{
    typedef typename GeometricField<Type, PatchField, GeoMesh>::cmptType
        cmptType;
...
{                                                                              \
    ...
    (                                                                          \
        ...
        (                                                                      \
            ...
            PatchField<Type1>::calculatedType(),                               \
            gf1.boundaryField().patchTypes()                                   \
        )                                                                      \
    );                                                                         \
                                                                               \
