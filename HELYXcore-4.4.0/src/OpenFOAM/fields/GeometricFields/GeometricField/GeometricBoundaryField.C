...
Copyright
    (c) 2010 H. Jasak All rights reserved
    (c) 2016 OpenCFD Ltd.
...
(
    const DimensionedField<Type, GeoMesh>& field,
    GeometricBoundaryField<Type, PatchField, GeoMesh>& btf,
    bool reuse
)
:
    FieldField<PatchField, Type>(btf.size()),
...
updatePatchFields()
{
    if (GeometricField<Type, PatchField, GeoMesh>::debug)
    {
        Info<< "GeometricField<Type, PatchField, GeoMesh>::"
               "Boundary::"
               "updatePatchFields()" << endl;
    }

    forAll(*this,patchi)
    {
        this->operator[](patchi).updateFromInternalField();
    }
}
...
{
    evaluatePatchFields(bmesh_.mesh(), *this);
}
...
)
{
    ...
    )
    {
        ...
        forAll(ff, patchi)
        {
            ff[patchi].initEvaluate(Pstream::defaultCommsType);
        }

        // Before blocking, allow the other solvers in the consolidated regions
        // to catch up to us
        serialThreads::yieldToNext();
        ...
        forAll(ff, patchi)
        {
            ff[patchi].evaluate(Pstream::defaultCommsType);
        }
    }
    else if (Pstream::defaultCommsType == Pstream::commsTypes::scheduled)
    {
        ...
        forAll(patchSchedule, patchEvali)
        {
            ...
            {
                ff[patchSchedule[patchEvali].patch]
...
)
{
    ...
    {
        const DimensionedField<Type, GeoMesh>& field
            = this->operator[](patchi).internalField();
        this->set(patchi, btf[patchi].clone(field));
    }
}
...
template<class Type, template<class> class PatchField, class GeoMesh>
Foam::wordList
Foam::GeometricBoundaryField<Type, PatchField, GeoMesh>::patchTypes() const
{
    const FieldField<PatchField, Type>& pff = *this;
    ...
    forAll(pff, patchi)
    {
        Types[patchi] = pff[patchi].patchType();
    }
...
template<class Type, template<class> class PatchField, class GeoMesh>
Foam::wordList
Foam::GeometricBoundaryField<Type, PatchField, GeoMesh>::combinePatchTypes
(
    const Foam::wordList& pt1,
    const Foam::wordList& pt2
) const
{
    wordList pTypes(pt1.size());
    forAll(pTypes, pI)
    {
        if (pt1[pI]!="" || pt2[pI]!="")
        {
...
Foam::GeometricBoundaryField<Type, PatchField, GeoMesh>::oldTimeField() const
{
    List<Field<Type>> OldTimeFields;
    OldTimeFields.setSize(bmesh_.size());


    forAll(OldTimeFields, patchi)
    {
        if (isA<indirectPolyPatch>(bmesh_[patchi].patch()))
        {
            OldTimeFields[patchi] =
                this->operator[](patchi).oldTimeField();
        }
    }

    return OldTimeFields;
}
...
template<class Type, template<class> class PatchField, class GeoMesh>
void Foam::GeometricBoundaryField<Type, PatchField, GeoMesh>::forceAssign
(
    const GeometricBoundaryField<Type, PatchField, GeoMesh>& bf
)
{
    forAll(*this, patchi)
    {
        this->operator[](patchi).forceAssign(bf[patchi]);
    }
}
...
)
{
    forAll(*this, patchi)
    {
        this->operator[](patchi).forceAssign(ptff[patchi]);
    }
}
...
)
{
    forAll(*this, patchi)
    {
        this->operator[](patchi).forceAssign(t);
    }
}
