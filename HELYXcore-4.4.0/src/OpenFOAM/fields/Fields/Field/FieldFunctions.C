...
/**
 * Calculate multiple "gFuncs" on the same list at the same type, emitting the results
 * as a tuple. This operation performs only one MPI reduce operation, regardless of how
 * many "gFuncs" you asked for.
 *
 * Combine with destructuring for maximum niceness as well as speed:
 *    auto [myMax, myMin, mySum] = gFuncs<GMax, GMin, GSum>(myList);
 */
template<template<typename> typename... GOps, typename EltT>
auto gFuncs(const UList<EltT>& f, const label comm = Pstream::worldComm) {
    // Compute the min/max/whatevers locally on our UList.
    std::tuple<typename GOps<EltT>::RetType...> outs{
        GOps<EltT>{}.applyLocal(f)...
    };

    // Do a reduction that calculates all of those things at once. The thing being transmitted during
    // the reduction will be the above tuple (which consists of the in-progress average, min, max, etc.),
    // so is just a handful of numbers.
    reduce(outs, ParallelOp<typename GOps<EltT>::ReductionOperator...>{}, comm);

    // Ta-da.
    return outs;
}

#define G_UNARY_FUNCTION(GFuncOp, ReturnType, gFunc, Func, rFunc) \
template<typename Type>                                                        \
struct GFuncOp {                                                               \
    using RetType = ReturnType;                                                \
                                                                               \
    /* Reduction operator to apply between nodes*/                             \
    using ReductionOperator = rFunc ## Op<Type>;                               \
                                                                               \
    /* Reduce the list on this node as far as possible */                      \
    /* TODO: Should be implemented using ReductionOperator */                  \
    ReturnType applyLocal(const UList<Type>& f) {                              \
        return Func(f);                                                        \
    }                                                                          \
};                                                                             \
                                                                               \
...
{                                                                              \
    return std::get<0>(gFuncs<GFuncOp>(f, comm));                              \
}                                                                              \
TMP_UNARY_FUNCTION(ReturnType, gFunc)


G_UNARY_FUNCTION(GMax, Type, gMax, max, max)
G_UNARY_FUNCTION(GMin, Type, gMin, min, min)
G_UNARY_FUNCTION(GSum, Type, gSum, sum, sum)
G_UNARY_FUNCTION(GMaxMagSqr, Type, gMaxMagSqr, maxMagSqr, maxMagSqr)
G_UNARY_FUNCTION(GMinMagSqr, Type, gMinMagSqr, minMagSqr, minMagSqr)
G_UNARY_FUNCTION(GSumSqr, scalar, gSumSqr, sumSqr, sum)
G_UNARY_FUNCTION(GSumMag, scalar, gSumMag, sumMag, sum)
G_UNARY_FUNCTION(GSumCmptMag, Type, gSumCmptMag, sumCmptMag, sum)
...
)
{
    ...
    reduce(SumProd, sumOp<scalar>(), comm);
    return SumProd;
}
...
)
{
    ...
    reduce(SumProd, sumOp<Type>(), comm);
    return SumProd;
}
