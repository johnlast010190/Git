...
#include "fields/expandContract/expandTensorField.H"
...
)
{
    ...
    {
        (*scalarCoeffPtr_)[index] = coeff.asScalar();
    }
    else if (coeff.activeType() == blockCoeffBase::LINEAR)
    {
        (*linearCoeffPtr_)[index] = coeff.asLinear();
    }
    else if (coeff.activeType() == blockCoeffBase::SQUARE)
    {
        (*squareCoeffPtr_)[index] = coeff.asSquare();
    }
}


template<class Type>
void Foam::CoeffField<Type>::subtractCoeff
(
    const label index,
    const BlockCoeff<Type>& coeff
)
...
template<class Type>
template<class Type2>
void Foam::CoeffField<Type>::expandCoeffs
(
    const CoeffField<Type2>& cf
)
{
    if (cf.activeType() == blockCoeffBase::SCALAR)
    {
        if (this->activeType() == blockCoeffBase::UNALLOCATED)
        {
            this->toScalar();
        }
    }
    else if (cf.activeType() == blockCoeffBase::LINEAR)
    {
        if
        (
            (this->activeType() == blockCoeffBase::UNALLOCATED) ||
    ...
    else if (cf.activeType() == blockCoeffBase::SQUARE)
    {
        if
        (
            (this->activeType() == blockCoeffBase::UNALLOCATED) ||
...
#include "fields/CoeffField/CoeffFieldFunctions.C"
