...
Copyright
    (c) 2011-2025 OpenFOAM Foundation
    (c) 2025 Engys Ltd.
...
Type Foam::Function1Types::TableBase<Type>::value(const scalar x) const
{
    ...
    Type t = Type(currentWeights_[0]*table_[currentIndices_[0]].second());
...
template<class Type>
Type Foam::Function1Types::TableBase<Type>::derivative(const scalar x) const
{
    // Use interpolator
...
) const
{
    scalar bx1 = x1;
    scalar bx2 = x2;
    checkMinBounds(x1, bx1);
    checkMaxBounds(bx1, bx1);
    checkMinBounds(x2, bx2);
    checkMaxBounds(bx2, bx2);

    // Use interpolator
    interpolator().integrationWeights
    (
        bx1, bx2, currentIndices_, currentWeights_
    );

    Type sum = Type(currentWeights_[0]*table_[currentIndices_[0]].second());
    for (label i = 1; i < currentIndices_.size(); i++)
    {
       sum += currentWeights_[i]*table_[currentIndices_[i]].second();
    }

    if (boundsHandling_ == REPEAT)
    {
        ...
        if (n != 0)
        {
            ...
            interpolator().integrationWeights
            (
                t0, t1, currentIndices_, currentWeights_
            );

            forAll(currentWeights_, i)
            {
                sum2 += currentWeights_[i]*table_[currentIndices_[i]].second();
            }
...
template<class Type>
Type Foam::Function1Types::TableBase<Type>::integrateYoverX
(
    const scalar x1,
    const scalar x2
) const
{
    ...
    const Tuple2<scalar, Type>& tableEntry = table_[currentIndices_[0]];
    Type sum = Type(currentWeights_[0]*tableEntry.second()/tableEntry.first());
    for (label i = 1; i < currentIndices_.size(); i++)
    {
        const Tuple2<scalar, Type>& tableEntry = table_[currentIndices_[0]];
       sum += currentWeights_[i]*tableEntry.second()/tableEntry.first();
    }
...
template<class Type>
void Foam::Function1Types::TableBase<Type>::shiftTimeBase(const Time& t)
{
    forAll(table_, i)
    {
        table_[i].first() += t.value();
    }
}
