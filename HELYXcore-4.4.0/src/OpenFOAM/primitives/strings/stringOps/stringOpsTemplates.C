...
template<class StringType>
StringType Foam::stringOps::center
(
    const StringType& str,
    const std::string::size_type requestedSize,
    const char padChar
)
{
    ...
    {
        const std::string::size_type extraLength = requestedSize - str.size();
        const std::string::size_type leftPadSize = extraLength / 2;
        const std::string::size_type rightPadSize = extraLength - leftPadSize;

        // assert(0 < leftPadSize);
        // assert(0 <= rightPadSize);
        const StringType leftPad(leftPadSize, padChar);
        const StringType rightPad(rightPadSize, padChar);
