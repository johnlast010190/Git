...
Foam::vector Foam::eigenValues(const tensor& T)
{
    ...
    C = Txx*Tyy + Txx*Tzz + Tyy*Tzz
      - Txy*Tyx - Txz*Tzx - Tyz*Tzy;
    C = C/3;

    D = - Txx*Tyy*Tzz - Txy*Tyz*Tzx
      - Txz*Tyx*Tzy + Txz*Tyy*Tzx
      + Txy*Tyx*Tzz + Txx*Tyz*Tzy;
#else
    //Creating coefficients
    B = -T.xx() - T.yy() - T.zz();
    B = B/3;

    C = T.xx()*T.yy() + T.xx()*T.zz() + T.yy()*T.zz()
      - T.xy()*T.yx() - T.xz()*T.zx() - T.yz()*T.zy();
    C = C/3;

    D = - T.xx()*T.yy()*T.zz() - T.xy()*T.yz()*T.zx()
...
Foam::vector Foam::eigenValues(const symmTensor& T)
{
    ...
    if (Di<SMALL)
    {
        //- If is 0<Di<SMALL then the denominators later can go zero
        if (fabs(d1)<SMALL)
        {
            i = -B;
            ii = i;
...
Foam::tensor Foam::hinv(const tensor& t)
{
    ...
    else
    {
        ...
        else //degenerate
        {
            ...
            //- Last safety net check - probably all elements below small
