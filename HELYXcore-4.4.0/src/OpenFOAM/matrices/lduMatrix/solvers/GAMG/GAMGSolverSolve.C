...
) const
{
    ...
    if (!solveForCorrection_)
    {
        //- solve for the uknown field x
        //  Ax = b
        solveForField
        (
            psi,
    ...
    {
        //- solve for the correction of the uknown field:
        // A dx = Res; x += dx
        scalarField psiOld(psi);
        psi = Zero;
        Apsi = Zero;
        scalarField sourceCorrection(finestResidual);
...
void Foam::GAMGSolver::solveForField
(
    scalarField& psi,
    scalarField& Apsi,
...
) const
{
    ...
    else if (smoothSolveCoarsest_)
    {
        coarsestCorrField = 0.0;
        ...
        if (maxIterCoarsest_ > 1)
        {
            scalarField src(coarsestSource);
            for
            (
                label solveIter = 0;
                solveIter < maxIterCoarsest_-1;
                solveIter++
            )
            {
                scalarField ApsiCorr(coarsestSource.size());
                matrixLevels_[coarsestLevel].Amul
                (
                    ApsiCorr,
                    coarsestCorrField,
    ...
    else
    {
        ...
        {
            WarningInFunction
                << "The coarsest-level solve in GAMG appears to have diverged:"
                << nl;
            coarseSolverPerf.print(Info.masterStream(coarseComm));
            Info<< "Consider enabling 'smoothSolveCoarsest'"
                << endl;
