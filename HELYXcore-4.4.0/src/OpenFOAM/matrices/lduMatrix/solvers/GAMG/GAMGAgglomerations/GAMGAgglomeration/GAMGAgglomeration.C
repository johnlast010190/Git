...
{
    label nTotalCoarseCells = nCoarseCells;
    label nTotalFineCells = nFineCells;

    reduce(
        std::tie(nTotalCoarseCells, nTotalFineCells),
        UniformParallelOp<sumOp<label>, 2>{}
    );

    // Make sure at least one level is created, even if it means agglomerating
    // a mesh with fewer than nCellsInCoarsestLevels
    if
    (
        nTotalCoarseCells < Pstream::nProcs()*nCellsInCoarsestLevel_
     && nCreatedLevels
    )
    {
        return false;
...
)
{
    ...
    )
    {
        ...
        libs.open(controlDict, "geometricGAMGAgglomerationLibs");

        const auto ctor =
            ctorTableLookup
            (
                "GAMGProcAgglomeration type",
                lduMeshConstructorTable_(),
                agglomeratorType
            );
        return store(ctor(mesh, controlDict).ptr());
    }
    else
    {
...
)
{
    ...
    )
    {
        ...
        libs.open(controlDict, "algebraicGAMGAgglomerationLibs");

        if (lduMatrixConstructorTable_().count(agglomeratorType) == 0)
        {
        ...
        else
        {
            lduMatrixConstructorTable::iterator cstrIter =
                lduMatrixConstructorTable_().find(agglomeratorType);

            return store(cstrIter->second(matrix, controlDict).ptr());
        }
    }
    else
    {
...
)
{
    ...
    const auto ctor =
        ctorTableLookup
        (
            "GAMGAgglomeration type",
            geometryConstructorTable_(),
            agglomeratorType
        );
