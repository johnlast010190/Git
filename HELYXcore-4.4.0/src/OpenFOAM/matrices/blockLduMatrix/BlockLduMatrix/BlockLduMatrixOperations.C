...
template<class Type>
void Foam::BlockLduMatrix<Type>::relaxAndCheckDominance
(
    const Field<Type>& x,
    Field<Type>& b,
    const scalar alpha
...
)
{
    ...
    TypeCoeffField& bDiag = this->diag();
    Field<Type>& source = b;

    const scalar invAlpha = 1/alpha;

    if (bDiag.activeType() == blockCoeffBase::SCALAR)
    {
        typedef typename TypeCoeffField::scalarTypeField scalarTypeField;
        scalarTypeField& abDiag = bDiag.asScalar();

        forAll(source, cI)
        {
            scalar abDiagOld = abDiag[cI];
            abDiag[cI] = mult.diagMultiply(abDiagOld, invAlpha);
            source[cI] += mult(abDiag[cI]-abDiagOld, x[cI]);
        }
    }
    if (bDiag.activeType() == blockCoeffBase::LINEAR)
    {
        Field<Type>& abDiag = bDiag.asLinear();

        //Type invAlphaType;
        //expandScalar(invAlphaType, invAlpha);
        forAll(source, cI)
        {
            Type abDiagOld = abDiag[cI];
            abDiag[cI] = mult.diagMultiply(abDiagOld, invAlpha);
            source[cI] += mult(abDiag[cI]-abDiagOld, x[cI]);
        }
    }
    if (bDiag.activeType() == blockCoeffBase::SQUARE)
    {
        typedef typename TypeCoeffField::squareTypeField squareTypeField;
        typedef typename outerProduct<Type, Type>::type squareType;
        squareTypeField& abDiag = bDiag.asSquare();
        forAll(source, cI)
        {
            squareType abDiagOld = abDiag[cI];
            abDiag[cI] = mult.diagMultiply(abDiagOld, invAlpha);
            source[cI] += mult(abDiag[cI]-abDiagOld, x[cI]);
        }
    }
...
template<class Type>
void Foam::BlockLduMatrix<Type>::relaxVec
(
    const Field<Type>& x,
    Field<Type>& b,
    const Field<Type>& alpha
)
{
    ...
    Field<Type> invAlpha(x.size(), Type::one);
    forAll(invAlpha, cI)
    {
        invAlpha[cI] = cmptDivide(invAlpha[cI], alpha[cI]);
    }
    ...
    if (bDiag.activeType() == blockCoeffBase::SCALAR)
    {
        ...
        {
            scalar abDiagOld = abDiag[cI];
            abDiag[cI] = mult.diagTypeMultiply(abDiagOld, invAlpha[cI]);
            source[cI] += mult(abDiag[cI]-abDiagOld, x[cI]);
        }
        */
    }
    if (bDiag.activeType() == blockCoeffBase::LINEAR)
    {
        ...
        {
            Type abDiagOld = abDiag[cI];
            //abDiag[cI] = mult.diagMultiply(abDiagOld, invAlpha[cI]);
            for (direction cmptI = 0; cmptI < nCmpts; cmptI++)
            {
                abDiag[cI](cmptI) = abDiagOld(cmptI)*invAlpha[cI](cmptI);
            }
            source[cI] += mult(abDiag[cI]-abDiagOld, x[cI]);
        }
    }
    if (bDiag.activeType() == blockCoeffBase::SQUARE)
    {
        ...
        forAll(source, cI)
        {
            ...
            for (direction cmptI = 0; cmptI < nCmpts; cmptI++)
            {
                abDiag[cI](cmptI, cmptI) = abDiagOld(cmptI, cmptI)*invAlpha[cI](cmptI);
            }
            source[cI] += mult(abDiag[cI]-abDiagOld, x[cI]);
        }
    }
...
)
{
    ...
    if (bDiag.activeType() == blockCoeffBase::SCALAR)
    {
        ...
        forAll(cells, cI)
        {
            const label gcI = cells[cI];
            scalar abDiagOld = abDiag[gcI];
            abDiag[gcI] = mult.diagMultiply(abDiagOld, invAlpha);
            source[gcI] += mult(abDiag[gcI]-abDiagOld, x[gcI]);
        }
    }
    if (bDiag.activeType() == blockCoeffBase::LINEAR)
    {
    ...
    if (bDiag.activeType() == blockCoeffBase::SQUARE)
    {
        ...
        {
            const label gcI = cells[cI];
            squareType abDiagOld = abDiag[gcI];
            abDiag[gcI] = mult.diagMultiply(abDiagOld, invAlpha);
            source[gcI] += mult(abDiag[gcI]-abDiagOld, x[gcI]);
        }
    }
