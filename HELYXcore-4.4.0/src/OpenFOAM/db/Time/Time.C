...
void Foam::Time::setControls()
{
    ...
    {
        startTime_ = controlDict_.lookup<scalar>("startTime");
    }
    else
    {
    ...
    timeDict.readIfPresent("compensation", compensation_);
...
Foam::Time& Foam::Time::operator++()
{
    ...
    //- kahan summation to find new time value
    //  This reduces numerical errors
    //  especially for SP runs where the time firectories are not exact
    scalar dtComp = deltaT_ - compensation_;
    scalar newValue = value() + dtComp;
    //- calulate the lost digits
    compensation_ = (newValue - value()) - dtComp;

    // Increment time
    setTime(newValue, timeIndex_ + 1);
    ...
    if (!subCycling_)
    {
        ...
        // Notify function objects of time increment
        functionObjects_.timeIncremented();
    }
