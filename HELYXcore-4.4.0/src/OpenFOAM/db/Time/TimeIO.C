...
void Foam::Time::overrideSwitches
(
    const word& dictName,
    simpleObjectRegistry& objects
)
{
    if (controlDict_.found(dictName))
    {
        Info<< "Overriding " << dictName << " according to "
            << controlDict_.name() << endl;

        const dictionary& localSettings = controlDict_.subDict(dictName);
...
void Foam::Time::readDict()
{
    ...
    if (controlDict_.readIfPresent("application", application))
    {
        ...
        setEnv("HELYX_APPLICATION", application, false);
    ...
    // Debug switches
    overrideSwitches("DebugSwitches", debug::debugObjects());

    // Optimisation Switches
    const word optDictName("OptimisationSwitches");
    overrideSwitches(optDictName, debug::optimisationObjects());
    if (controlDict_.found(optDictName))
    {
        // Handle fileHandler override explicitly since interacts with
        // local dictionary monitoring.
        word fileHandlerName;
        const dictionary& localSettings = controlDict_.subDict(optDictName);
        if
        (
    ...
    // Info switches
    overrideSwitches("InfoSwitches", debug::infoObjects());
    ...
    if (!deltaTchanged_ && controlDict_.found("deltaT"))
    {
        deltaTcontrol_ = controlDict_.lookupOrDefault<word>
        (
            "deltaTcontrol",
            "index"
        );

        if (deltaTcontrol_ != "index" && deltaTcontrol_ != "time")
        {
            FatalIOErrorInFunction(deltaTcontrol_)
                << "expected time or index"
                << " found '" << deltaTcontrol_ << "'"
                << exit(FatalIOError);
        }

        deltaTDataPtr_ = Function1<scalar>::New("deltaT", controlDict_);
        ...
        if (deltaTDataPtr_->type() == Function1Types::Constant<scalar>::typeName)
        {
    ...
    else if (controlDict_.found("writeFrequency"))
    {
        writeInterval_ = controlDict_.lookup<scalar>("writeFrequency");
    }
    ...
    if (controlDict_.found("stopAt"))
    {
        ...
        {
            endTime_ = controlDict_.lookup<scalar>("endTime");
        }
        else if (stopAt_ == saClockTime)
        {
            endTime_ = controlDict_.lookup<scalar>("endTime");
            endClockTime_ = controlDict_.lookup<label>("endClockTime");
        }
        else
        {
            endTime_ = GREAT;
...
bool Foam::Time::writeTimeDict() const
{
    ...
    timeDict.add("compensation", timeToUserTime(compensation_));
    ...
    if (writeTime())
    {
        ...
        if (writeOK)
        {
            ...
            if (writeTime_ && purgeWrite_)
            {
                ...
                else if (previousWriteTimes_.last() != timeName())
                {
                    previousWriteTimes_.push(timeName());
