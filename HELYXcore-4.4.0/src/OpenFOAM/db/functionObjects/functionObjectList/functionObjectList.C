...
void Foam::functionObjectList::timeIncremented()
{
    if (execution_)
    {
        forAll(*this, objectI)
        {
            ...
            #if !defined( WIN32 ) && !defined( WIN64 )
            addProfiling(fo, "functionObject::"+objName+"::timeIncremented");
            #endif

            operator[](objectI).timeIncremented();
        }
    }
}
