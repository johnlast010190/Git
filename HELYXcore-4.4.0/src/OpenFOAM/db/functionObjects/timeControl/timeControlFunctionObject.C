...
void Foam::functionObjects::timeControl::readControls()
{
    ...
    else if (dict_.readIfPresent("averagingStartTime", timeStart_))
    {
        //backwards compatibility fix for fieldAverage
        timeStart_ = time_.userTimeToTime(timeStart_);
    }
...
void Foam::functionObjects::timeControl::timeIncremented()
{
    if (active())
    {
        foPtr_->timeIncremented();
    }
}
