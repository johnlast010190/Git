...
void Foam::primitiveEntry::write(Ostream& os, const bool contentsOnly) const
{
    ...
    //explicit hack to write lists of dictionaries (functionObjects)
    //in a nicer way
    //check if primitive entry is a list of dictionaries
    //first non-space token is (
    //third non-space token is a {
    label nNonSpaceToken = 0;
    bool firstTokenBeginList = false;
    bool thirdTokenBeginBlock = false;
    for (label i=0; i<size(); i++)
    {
        if (operator[](i) != token::SPACE)
        {
            nNonSpaceToken ++;
            if (nNonSpaceToken == 1)
            {
                if (operator[](i) == token::BEGIN_LIST)
                {
                    firstTokenBeginList = true;
                }
                else
                {
                    break;
            ...
            if (nNonSpaceToken == 3)
            {
                if (operator[](i) == token::BEGIN_BLOCK)
                {
                    thirdTokenBeginBlock = true;
                }
                break;
            }
        }
    ...
    if (firstTokenBeginList && thirdTokenBeginBlock)
    {
        //this is a list of dictionaries
        PtrList<entry> dictList(ITstream(*this).rewind());
        os << dictList;
    }
    else
    {
