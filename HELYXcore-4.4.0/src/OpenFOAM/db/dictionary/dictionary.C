    ...
    patternRegexps_(std::move(dict.patternRegexps_))
{
    ...
    {
        name() = parent_.name() + '.' + name();
    }
}


Foam::dictionary::dictionary(dictionary&& dict)
:
    dictionaryName(std::move(dict.name())),
    IDLList<entry>(std::move(dict)),
    hashedEntries_(std::move(dict.hashedEntries_)),
...
{
    return lookupEntry(keywords, recursive, patternMatch).stream();
}
...
{
    const entry* entryPtr = lookupEntryPtr(keywords, false, true);
