...
) const
{
    ...
    for (const auto& cmpt : cmpts)
    {
        ...
        {
            // Find entry
            const word key = word::validated(cmpt);
...
) const
{
    ...
    if ((matchOpt & keyType::REGEX) && patternEntries_.size())
    {
        pattern_const_iterator wcLink = patternEntries_.begin();
        regexp_const_iterator  reLink = patternRegexps_.begin();
...
bool Foam::dictionary::remove(const word& keyword)
{
    ...
    if (iter.found())
    {
        ...
        pattern_iterator wcLink = patternEntries_.begin();
        regexp_iterator  reLink = patternRegexps_.begin();
...
)
{
    ...
    if (newKeyword.isPattern())
    {
        ...
        patternRegexps_.insert(autoPtr<regExp>::New(newKeyword));
    }
