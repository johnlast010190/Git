...
const char* const Foam::dynamicCode::libTargetRoot =
    "LIB = $(PWD)/../platforms/$(HELYX_OPTIONS)/lib/lib";
...
)
{
    ...
    if (!allowSystemOperations)
    {
        FatalIOErrorInFunction(dict)
            ...
            << "    ~/.HELYXcore/$HELYX_PROJECT_VERSION/controlDict" << nl << nl
            << "or" << nl << nl
            << "    $HELYX_PROJECT_DIR/etc/controlDict" << nl
            << endl
            << exit(FatalIOError);
    }
}
...
bool Foam::dynamicCode::createMakeFile() const
{
    ...
    string helyxOptions = stringOps::expand("$HELYX_OPTIONS", true);
    if (string::null == helyxOptions)
    {
        WarningInFunction
            << "The environment variable HELYX_OPTIONS was not set."
            << "  Unless significant changes have been made to the dynamic"
            << " code Makefile template, compilation is extremely likely to"
            << " fail."
            << endl;
    }
    fileName makefileTemplate =
        findEtcFile
        (
            fileName("codeTemplates/dynamicCode/makefiles") / helyxOptions /
                fileName("dynamicCode.Makefile"),
            true
        );


    // Now read, string replace, write out

    // Scoped file read, to ensure f is closedby ifstream destructor
    string contents;
    {
        std::ifstream f;
        f.open(makefileTemplate);
    ...
    for (const fileName& sourceFile : compileFiles_)
    {
        sourceFiles << " " << sourceFile;
    }
    contents.replaceAll("|SOURCE_FILES|", sourceFiles.str());

    contents.replaceAll("|DYNAMIC_CODE_OPTIONS|", options_);
    contents.replaceAll("|LINK_LIBS|", linkLibs_);

    contents.replaceAll("|LIBRARY_NAME|", libPath());

    // Now write to file
    const fileName dstFile(this->codePath()/"Makefile");
    mkDir(dstFile.path());

    // OFstream outStream(dstFile);
    std::ofstream outStream(dstFile);
    if (!outStream.good())
    {
        FatalErrorInFunction
...
:
    ...
    libSubDir_(stringOps::expand("platforms/$HELYX_OPTIONS/lib")),
    codeName_(codeName),
    codeDirName_(codeDirName)
{
    ...
    List<string> requiredEnvVars{"$HELYX_OPTIONS", "$HELYX_PROJECT_DIR"};
    for (string& requiredVar : requiredEnvVars)
    {
        string result = stringOps::expand(requiredVar, true);
        if (string::null == result)
        {
            WarningInFunction
                << "The environment variable " << requiredVar << " was not set."
                << "  Unless significant changes have been made to the dynamic"
                << " code Makefile template, compilation is extremely likely to"
                << " fail."
                << endl;
...
void Foam::dynamicCode::setOptions(const std::string& content)
{
    options_ = stringOps::trim(content);
    stringOps::inplaceExpand(options_);
}


void Foam::dynamicCode::setLinkLibs(const std::string& content)
{
    linkLibs_ = content;
}
...
bool Foam::dynamicCode::makeLibso() const
{
    ...
    FatalError
        << "Dynamic code generation is only available on Linux"
        << exit(FatalError);
#endif
    Info<< "Running make in " << this->codePath().c_str() << endl;
    fileName currentDir = cwd();
    bool returnCode = false;
    if (chDir(this->codePath()))
    {
        mkDir(codeRoot_/libSubDir_);
        if (!Foam::system("make"))
        {
            returnCode = true;
        }
    ...
    {
        FatalError << "Failed to move to directory " << this->codePath()
            << exit(FatalError);
    }
    chDir(currentDir);
    return returnCode;
