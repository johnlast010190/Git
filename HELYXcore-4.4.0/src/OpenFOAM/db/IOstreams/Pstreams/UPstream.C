...
// Some of the things you need to do inside (or when assembling) mpi error handlers can themselves cause
// mpi errors. What fun.
#define MPI_ASSERT(X) \
    if (X) { \
        FatalErrorInFunction \
            << "A fatal MPI error was encountered while trying to handle another fatal MPI error." \
            << Foam::abort(Foam::FatalError); \
    }

// OpenMPI's documentation says:
//
// > Before the error value is returned, the current MPI error handler is called.  By default, this error handler
// > aborts the MPI job, except for I/O function errors.
//
// That is: the `MPI_ERRORS_ARE_FATAL` default error handler is actually "errors except IO errors are fatal".
// The manual also says:
//
// > MPI does not guarantee that an MPI program can continue past an error.
//
// This suggests that the default behaviour in the case of an IO error is to ignore it and enter undefined behaviour
// land. Not helpful!
//
// This is an MPI error handler function that always crashes the program, which we use with all communicators to
// restore sanity: ie. actually crash whenever anything goes wrong.
void mpiErrorHandler(MPI_Comm* comm, int* errCode, ...) {
    char errStorage[MPI_MAX_ERROR_STRING];
    int len;
    MPI_ASSERT(MPI_Error_string(*errCode, &errStorage[0], &len));

    // Stupid copy.
    std::string errString{(const char*) errStorage, (size_t) len};

    FatalErrorInFunction
            << "Fatal error performing MPI operation: " << errString
            << Foam::abort(Foam::FatalError);
}

// Connect an MPI communicator to the above error handler.
void configureMpiErrorHandler(MPI_Comm& comm) {
    static MPI_Errhandler MPIErrorHandler;
    static bool errHandlerCreated = false;

    if (!errHandlerCreated) {
        MPI_ASSERT(MPI_Comm_create_errhandler(&mpiErrorHandler, &MPIErrorHandler));
        errHandlerCreated = true;
    }

    MPI_ASSERT(MPI_Comm_set_errhandler(comm, MPIErrorHandler));
}
...
int Foam::UPstream::useMpiAllToAll
(
    Foam::debug::optimisationSwitch("useMpiAllToAll", 0)
);
registerOptSwitch
(
    "useMpiAllToAll",
    int,
    Foam::UPstream::useMpiAllToAll
);
...
)
{
    ...
    if (parentIndex == -1)
    {
        ...
        configureMpiErrorHandler(PstreamGlobals::MPICommunicators_[index]);
