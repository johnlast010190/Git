...
) const
{
    ...
    else
    {
        ...
        // Note: Here order of the operations matters so it might be worth to
        // consider two divisions instead of one division and after that 1 - f
        const scalar f =
            (samples_[index_ + 1] - t)
           /(samples_[index_ + 1] - samples_[index_]);
...
bool Foam::linearInterpolationWeights::derivationWeights
(
    const scalar t,
    labelList& indices,
    scalarField& weights
) const
{
    ...
    {
        indices[0] = index_;
        weights[0] = samples_[index_];
        indices[1] = index_ + 1;
        weights[1] = samples_[index_ + 1];
    }
