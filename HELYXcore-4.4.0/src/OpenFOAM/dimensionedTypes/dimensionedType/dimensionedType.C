...
Foam::dimensioned<Type>::read(Istream& is, const dictionary& readSet)
{
    ...
    // Read dimensions if provided, otherwise use existing
    scalar multiplier = 1.0;

    if (nextToken == token::BEGIN_SQR)
    {
        dimensions_.read(is, multiplier, readSet);
    }
...
{
    // Write the name
    // This should be changed so that double keys are not written
    os << dt.name() << token::SPACE;

    // Write the dimensions
    scalar multiplier;
    dt.dimensions().write(os, multiplier);

    os << token::SPACE;

    // Write the value
    os << dt.value()/multiplier;
