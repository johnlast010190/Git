...
bool Foam::isValidBoundSphere(const Tuple2<point, scalar>& sphere)
{
...
)
{
    switch (nPs)
    {
        ...
        case 3:
        {
            ...
            return
                triPointRef(ps[pis[0]], ps[pis[1]], ps[pis[2]]).circumCircle();
        }
...
Foam::boundSphere(const PointField& ps, Random& rndGen)
{
    ...
    else
    {
        ...
        forAll(pis, i)
        {
            Swap(pis[i], pis[rndGen.position<label>(i, ps.size())]);
        }
