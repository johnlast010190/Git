...
)
{
    ...
    {
        perturbVec[dir] = nextafterf(span[dir],INFINITY)-span[dir];
    }
#else
    ...
    if (pushInside)
    {
        for (direction dir = 0; dir < vector::nComponents; dir++)
        {
            ...
            {
                // Close to 'left' side. Push inside.
                perturbedPt[dir] = nextafterf(bb.min()[dir],INFINITY);
            }
            else if (mag(pt[dir]-bb.max()[dir]) < mag(perturbVec[dir]))
            {
                // Close to 'right' side. Push inside.
                perturbedPt[dir] = nextafterf(bb.max()[dir],-INFINITY);
            }
#else
            if (mag(pt[dir]-bb.min()[dir]) < mag(perturbVec[dir]))
            {
    ...
    else
    {
        for (direction dir = 0; dir < vector::nComponents; dir++)
        {
            ...
            if (mag(pt[dir]-bb.min()[dir]) < mag(perturbVec[dir]))
            {
                // Close to 'left' side. Push outside.
                perturbedPt[dir] = nextafterf(bb.min()[dir],-INFINITY);
            }
            else if (mag(pt[dir]-bb.max()[dir]) < mag(perturbVec[dir]))
            {
                // Close to 'right' side. Push outside.
                perturbedPt[dir] = nextafterf(bb.max()[dir],INFINITY);
            }
#else
            if (mag(pt[dir]-bb.min()[dir]) < mag(perturbVec[dir]))
...
)
{
    ...
    if (faceID & treeBoundBox::LEFTBIT)
    {
        ...
        {
#if defined(HELYX_SP)
            perturbedPt[0] = nextafterf(bb.min()[0],INFINITY);
#else
    ...
    else if (faceID & treeBoundBox::RIGHTBIT)
    {
        ...
        {
#if defined(HELYX_SP)
            perturbedPt[0] = nextafterf(bb.max()[0],-INFINITY);
#else
    ...
    if (faceID & treeBoundBox::BOTTOMBIT)
    {
        ...
        {
#if defined(HELYX_SP)
            perturbedPt[1] = nextafterf(bb.min()[1],INFINITY);
#else
    ...
    else if (faceID & treeBoundBox::TOPBIT)
    {
        ...
        {
#if defined(HELYX_SP)
            perturbedPt[1] = nextafterf(bb.max()[1],-INFINITY);
#else
    ...
    if (faceID & treeBoundBox::BACKBIT)
    {
        ...
        {
#if defined(HELYX_SP)
            perturbedPt[2] = nextafterf(bb.min()[2],INFINITY);
#else
    ...
    else if (faceID & treeBoundBox::FRONTBIT)
    {
        ...
        {
#if defined(HELYX_SP)
            perturbedPt[2] = nextafterf(bb.max()[2],-INFINITY);
#else
...
) const
{
    ...
    pointIndexHit anyHit(false, treeStart, -1);
    ...
    for (; i < 100000; i++)
    {
        ...
        if (!ok)
        {
            // Hit the edge of the tree. Return miss unless any earlier hit registered.
            if (!hitInfo.hit() && anyHit.hit())
            {
                hitInfo = anyHit;
            }
...
) const
{
    ...
    if (nodes_.size())
    {
        findSphere(0, centre, radiusSqr, elements, nOverlapChecks);
    }
...
template<class Type>
Foam::label Foam::indexedOctree<Type>::findNearestCell
(
    const point& sample,
    scalar& distSqr
) const
{
    ...
    if (isContent(contentIndex))
    {
        ...
        shapes_.findNearest
        (
            indices, sample, distSqr, nearestItem, dpoint
        );
    }
