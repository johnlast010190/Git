...
#include "coordinate/rotation/coordinateRotation.H"
...
)
{
    ...
    const auto ctor =
        ctorTableLookup
        (
            "coordinateRotation type",
            dictionaryConstructorTable_(),
            modelType
        );
    return autoPtr<coordinateRotation>(ctor(dict));
}
