...
#include "coordinate/rotation/STARCDCoordinateRotation.H"
...
void Foam::coordinateRotations::starcd::update
(
    const vector& e1,
    const vector& e2
)
{
    NotImplemented;
    // This is draft solution. Not yet working correctly! When rotation matrix
    // is applied to the base vectors (R() & vector(1, 0, 0)) it does recover
    // magnitudes of the new rotated e1, however the signs are wrong.

    // cy*cz - sx*sy*sz, -cx*sz,  sx*cy*sz + sy*cz,
    // cy*sz + sx*sy*cz,  cx*cz,  sy*sz - sx*cy*cz,
    // -cx*sy,            sx,     cx*cy

    // tensor e123(e1, e2, e1 ^ e2);
    // tensor Rinv = e123.T().inv();

    // // Rotation around x
    // angles_[1] = asin(min(max(Rinv.yz(), -1.0), 1.0));

    // // Rotation around y
    // angles_[2] = acos(min(max(Rinv.zz()/cos(angles_[1]), -1.0), 1.0));

    // // Rotation around z
    // angles_[0] = acos(min(max(Rinv.yy()/cos(angles_[1]), -1.0), 1.0));
