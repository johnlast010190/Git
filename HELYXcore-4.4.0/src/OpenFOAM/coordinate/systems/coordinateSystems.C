...
void Foam::coordinateSystems::readFromStream(const bool read)
{
    ...
    if (read)
    {
        ...
        {
            (*this).PtrList::read(is, coordinateSystem::iNew());
            close();
        }
        else if (headerClassName() == headerTypeCompat)
        {
            ...
            (
                is, headerTypeCompat, "header class name", 1806,
...
//    const coordinateSystems* ptr = obr.findObject<coordinateSystems>(typeName);
    const coordinateSystems* ptr = obr.lookupObjectPtr<coordinateSystems>(typeName);
