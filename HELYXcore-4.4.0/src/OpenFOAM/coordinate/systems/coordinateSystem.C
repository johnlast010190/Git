...
#include "coordinate/rotation/axesRotation.H"
...
void Foam::coordinateSystem::errorCoordinateSystem
(
    const dictionary& dict,
    const word& whereIsItDefined
)
{
    if (dict.found(coordinateSystem::typeName))
    {
        FatalErrorIn(whereIsItDefined)
            << "Found old definition of " << coordinateSystem::typeName
            << " that is no longer supported. Use coordinateFrame instead."
            << exit(FatalError);
    }
}
...
void Foam::coordinateSystem::update
(
    const point& origin,
    const vector& e1,
    const vector& e2
...
void Foam::coordinateSystem::setLocalToFrame
(
    const coordinateSystem& csysParent
)
{
    fdata_.transformToLocal(*this, csysParent);
}
