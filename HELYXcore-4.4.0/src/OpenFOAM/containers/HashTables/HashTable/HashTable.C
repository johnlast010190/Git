...
:
    ...
    table_()
{
    label tableSize = HashTableCore::canonicalSize(size);
    if (tableSize)
    {
        table_.resize(tableSize);

        for (label hashIdx = 0; hashIdx < tableSize; ++hashIdx)
        {
...
:
    HashTable<T, Key, Hash>(ht.table_.size())
{
...
)
{
    ...
    if (!existing)
    {
        ...
        if (double(nElmts_)/table_.size() > 0.8 && table_.size() < maxTableSize)
        {
            #ifdef FULLDEBUG
            if (debug)
            {
    ...
    else
    {
        ...
        existing->key_ = key; // <- Isn't this pointless by definition? :D

        if constexpr (HasData) {
            existing->obj_.~T(); // Casually calling the destructor...
            new(&(existing->obj_)) T{obj};
        }
    }
...
void Foam::HashTable<T, Key, Hash>::resize(const label sz)
{
    ...
    // Make new hash buckets, recalculate all the hashes, and shuffle everything about.
    // No need for any allocations: we just have to do some pointer knitting.
    List<hashedEntry*> newTable(newSize, nullptr);

    for (hashedEntry* e : table_) {
        hashedEntry* ep = e;
        while (ep) {
            hashedEntry* next = ep->next_;

            label hashIdx = hashKeyIndex(ep->key_, newSize);
            hashedEntry* oldHead = newTable[hashIdx];
            newTable[hashIdx] = ep;
            ep->next_ = oldHead;
...
void Foam::HashTable<T, Key, Hash>::clear()
{
    if (nElmts_)
    {
        for (label hashIdx = 0; hashIdx < table_.size(); hashIdx++)
        {
