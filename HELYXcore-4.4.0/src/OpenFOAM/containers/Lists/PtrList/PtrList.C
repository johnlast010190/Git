...
template<class T>
Foam::PtrList<T>::PtrList(PtrList<T>&& a)
{
    *this = std::move(a);
}
...
Foam::PtrList<T>& Foam::PtrList<T>::operator=(const PtrList<T>& a)
{
    ...
    if (this->size() == 0)
    {
        ...
        forAll(*this, i)
        {
            // TODO: Use actual copy-assignment operators to copy the elements, not
            //       weird/nonstandard `clone()`...? :D
            this->ptrs_[i] = (T*) (a[i]).clone().ptr();
        }
    }
...
template<class T>
Foam::PtrList<T>& Foam::PtrList<T>::operator=(PtrList<T>&& a) noexcept
{
    clear();
    this->ptrs_ = std::move(a.ptrs_);
    return *this;
}
