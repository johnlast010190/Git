...
)
{
    ...
    const scalar r1 = sampleDiskDict.lookup<scalar>("r1");
    const scalar r2 = sampleDiskDict.lookup<scalar>("r2");

    nTheta_ = sampleDiskDict.lookup<label>("nTheta");
    nRadial_ = sampleDiskDict.lookup<label>("nRadial");
...
void Foam::functionObjects::propellerInfo::topoChange(const polyTopoChangeMap& map)
{
    updateSampleDiskCells();
}
