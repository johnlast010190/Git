...
:
    ...
    forceMomentFilePtr_(nullptr),
    outputTotalForceMoment_(false)
{
    liftDrag::read(dict);
}
...
bool Foam::functionObjects::liftDrag::execute()
{
    ...
    if (averagingIndex_ == nAveragingSteps_)
    {
        ...
        if (Pstream::master() || !Pstream::parRun())
        {
            ...
            if (outputTotalForceMoment_)
            {
                forceMomentFilePtr_() << mesh_.time().timeName() << tab
                                      << totalForce_.x() << tab
                                      << totalForce_.y() << tab
                                      << totalForce_.z() << tab
                                      << totalMoment_.x() << tab
            ...
            if (liftDragRegions_)
            {
                forAll(liftDragRegions_.filePtr_, fileI)
                {
                    ...
                    {
                        forAll(liftDragRegions_.force_, i)
                        {
                            liftDragRegions_.filePtr_[fileI]()
                                << liftDragRegions_.force_[i].x()
                                << tab
                                << liftDragRegions_.force_[i].y()
                                << tab
                                << liftDragRegions_.force_[i].z()
                                << tab;
                        }
                    }
                    else
                    {
                        forAll(liftDragRegions_.moment_, i)
                        {
                            liftDragRegions_.filePtr_[fileI]()
                                << liftDragRegions_.moment_[i].x()
                                << tab
                                << liftDragRegions_.moment_[i].y()
                                << tab
                                << liftDragRegions_.moment_[i].z()
                                << tab;
                        }
                    }
...
void Foam::functionObjects::liftDrag::calculate()
{
    ...
    tmp<volSymmTensorField::Boundary> tdevRhoReffb = forces::bDevRhoReff();
    ...
    forAllConstIter(labelHashSet, patchSet(), iter)
    {
        ...
        if (liftDragRegions_)
        {
            ...
            if (liftDragRegions_.outputForceMoment_)
            {
                liftDragRegions_.force_[regionI] *= alpha;
                liftDragRegions_.force_[regionI] +=
                    beta*(viscousPatchForce +pressurePatchForce);
                liftDragRegions_.moment_[regionI] *= alpha;
                liftDragRegions_.moment_[regionI] +=
                    beta*(viscousPatchMoment +pressurePatchMoment);
            }
        }
    ...
    if (porosity_)
    {
        ...
        forAll(porousZoneNames_, i)
        {
            ...
            if (liftDragRegions_)
            {
                ...
                {
                    liftDragRegions_.force_[regionI] *= alpha;
                    liftDragRegions_.force_[regionI] += beta*porousZoneForce;
                    liftDragRegions_.moment_[regionI] *= alpha;
                    liftDragRegions_.moment_[regionI] += beta*porousZoneMoment;
                }
            }
            regionI++;
        }
    ...
    const vector totalMoment = viscousMoment + pressureMoment + porousMoment;
    if (outputTotalForceMoment_)
    {
        totalForce_ = alpha*totalForce_ + beta*totalForce;
        totalMoment_ = alpha*totalMoment_ + beta*totalMoment;
    }
...
bool Foam::functionObjects::liftDrag::read(const dictionary& dict)
{
    ...
    if (dict.found("maxCp"))
    {
        maxCp_ = dict.lookup<scalar>("maxCp");
    }

    minCp_ = -GREAT;
    if (dict.found("minCp"))
    {
        minCp_ = dict.lookup<scalar>("minCp");
    }
    ...
    {
        nAveragingSteps_ = dict.lookup<label>("nAveragingSteps");
    }
    ...
    {
        cOfPressure_ = dict.lookup<bool>("cOfPressure");
    }

    wheelBase_ = dict.lookup<scalar>("wheelbase");
    if (wheelBase_ <= 0.0)
    {
        FatalErrorInFunction
    ...
    outputTotalForceMoment_ =
        dict.lookupOrDefault<Switch>("outputTotalForceMoment", false);
    ...
    if (Pstream::master() || !Pstream::parRun())
    {
        ...
        if (outputTotalForceMoment_)
        {
            fileName forceMomentFileName("forceMoment");
            forceMomentFilePtr_ = createFile(forceMomentFileName);
            // Add headers
            writeCommented(forceMomentFilePtr_(), "Time");
            writeDelimited(forceMomentFilePtr_(), "Xforce");
            writeDelimited(forceMomentFilePtr_(), "Yforce");
            writeDelimited(forceMomentFilePtr_(), "Zforce");
            writeDelimited(forceMomentFilePtr_(), "Xmoment");
            writeDelimited(forceMomentFilePtr_(), "Ymoment");
            writeDelimited(forceMomentFilePtr_(), "Zmoment");
            forceMomentFilePtr_() << endl;
        }
    }
    ...
    if (dict.found("outputRegionData"))
    {
        ...
        if (liftDragRegions_)
        {
            ...
            liftDragRegions_.outputForceMoment_ =
                dict.lookupOrDefault<Switch>("outputRegionForceMoment", false);

            if (liftDragRegions_.outputForceMoment_)
            {
                liftDragRegions_.force_ = vectorField(nRegions, Zero);
                liftDragRegions_.moment_ = vectorField(nRegions, Zero);
            }
            ...
            if (Pstream::master() || !Pstream::parRun())
            {
                ...
                if (liftDragRegions_.outputForceMoment_)
                {
                    liftDragRegions_.filePtr_.setSize(6);
                }
                else
                {
                ...
                if (liftDragRegions_.outputForceMoment_)
                {
                    liftDragRegions_.filePtr_[4] = createFile("regionalForces");
                    liftDragRegions_.filePtr_[5] = createFile("regionalMoments");
                }
    ...
    if (dict.found("binData"))
    {
        ...
        forAll(binNames, bI)
        {
            if (binDict.isDict(binNames[bI]))
            {
                ...
                if (mag(normalDir & normalised(checkBinAxis)) < SMALL)
                {
                    ...
                    ldBin.nBins_ = binDictAxis.lookup<label>("nBins");
        ...
        if (oldFormat)
        {
            ...
            liftDragBinPtr_[0].nBins_ = binDict.lookup<label>("nBins");
