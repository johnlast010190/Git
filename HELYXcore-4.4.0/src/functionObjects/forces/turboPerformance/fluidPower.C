...
Foam::functionObjects::fluidPower::calcDEmHead(const Switch& turbine) const
{
    ...
    forAllConstIter(labelHashSet, inletPatchSet_, iter)
    {
        ...
        scalarField patchFlux(patch.size(), Zero);
        forAll(patch, patchFacei)
        {
            const label polyFacei = mesh_.polyFacesBf()[patchi][patchFacei];
            ...
            forAll(patches, i)
            {
                patchFlux[patchFacei] += phib[patches[i]][patchFaces[i]];
            }
        }

        mflowInlet  += rho(p)*sum(patchFlux) + VSMALL;    // sign: inflow (-)
        EmInlet     += rho(p)*sum(patchFlux*ptInlet);
    }
    ...
    forAllConstIter(labelHashSet, outletPatchSet_, iter)
    {
        ...
        mflowOutlet += rho(p)*sum(patchFlux) + VSMALL;    // sign: outflow (+)
        EmOutlet    += rho(p)*sum(patchFlux*ptOutlet);
    }
...
bool Foam::functionObjects::fluidPower::execute()
{
    ...
    dEmHead  dEmH = calcDEmHead(turbine_);
