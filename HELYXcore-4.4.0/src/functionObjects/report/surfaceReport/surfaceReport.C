...
void functionObjects::surfaceReport::relativeFlux()
{
    ...
    forAll(flux, rfI)
    {
        if (rfI < mesh_.nInternalFaces())
        {
            if (reportFaces_.get(rfI))
            {
                label zonei = mesh_.faceZones().whichZone(rfI);
                if (useFlipMap_ && zonei != -1)
                {
                    const faceZone& fZone = mesh_.faceZones()[zonei];
                    bool zoneFlip = fZone.flipMap()[fZone.whichFace(rfI)];
                    if (zoneFlip)
                    {
                        relativeFlux_ -= flux[rfI];
                    }
                    else
                    {
                        relativeFlux_ += flux[rfI];
        ...
        if (isA<processorPolyPatch>(pp))
        {
            forAll(flux.boundaryField()[pI], pfI)
            {
                if (reportFaces_.get(pstart + pfI))
                {
                    ...
                    {
                        const faceZone& fZone = mesh_.faceZones()[zonei];
                        bool zoneFlip = fZone.flipMap()[fZone.whichFace(facei)];
                        if (zoneFlip)
                        {
    ...
    forAll(flux, rfI)
    {
        if (rfI < mesh_.nInternalFaces())
        {
            if (reportFaces_.get(rfI))
            {
                ...
                else if (sign(Sf[rfI] & surfaceDirectionPtr_())*flux[rfI] < 0)
                {
                    backFlowArea_ += mesh_.magSf()[rfI];
                }
            }
            ...
            forAll(flux.boundaryField()[pI], pfI)
            {
                ...
                if (reportFaces_.get(facei))
                {
                    ...
                    if (useFlipMap_ && zonei != -1)
                    {
                        ...
                        if
                        (
                            (zoneFlip && flux.boundaryField()[pI][pfI] > 0)
                            || (!zoneFlip && flux.boundaryField()[pI][pfI] < 0)
                        )
                        {
                            backFlowArea_ +=
                                mesh_.magSf().boundaryField()[pI][pfI];
                        }
                    }
                    else if
                    (
...
)
{
    ...
    if (fluxWeighting_)
    {
        data[2] /= max(totalFlux_ , SMALL);
    }
    else
    {
        data[2] /= max(totalSurface_, SMALL);
    }
    ...
    if (fluxWeighting_)
    {
        data[3] /= max(totalFlux_ , SMALL);
    }
    else
    {
        data[3] /= max(totalSurface_, SMALL);
    }
...
)
{
    ...
    if (fluxWeighting_)
    {
        if (directionAware_)
        {
            data[2] /= max(mag(relativeFlux_), SMALL);
        }
        else
        {
            data[2] /= max(totalFlux_ , SMALL);
        ...
        if (addDirectionMeanOutput_)
        {
            data[4] /= max(relativeFlux_, SMALL);
        }
    }
    else
    {
...
void functionObjects::surfaceReport::unmarkFaces(const dictionary& dict)
{
    ...
    forAll(conDicts, dI)
    {
        ...
        else if (word(cD.lookup("type")) == "sphere")
        {
            ...
            scalar sr(cD.lookup<scalar>("radius"));
...
bool functionObjects::surfaceReport::read(const dictionary& dict)
{
    ...
    useFlipMap_ =
       dict.lookupOrDefault<Switch>("useFlipMap", "false");
    ...
    {
        const word& defaultFluxName = "phi";
        fluxName_ = dict.lookupOrDefault<word>("fluxName", defaultFluxName);
        ...
        else
        {
            ...
            if
            (
                foundObject<surfaceScalarField>(defaultFluxName)
                &&
                (lookupObject<surfaceScalarField>(defaultFluxName).dimensions()
                == dimDensity*dimVelocity*dimArea)
            )
            {
                compressible_ = true;
    ...
    {
        homogeneity_ = dict.lookup<bool>("homogeneity");
    }
...
void functionObjects::surfaceReport::topoChange(const polyTopoChangeMap& map)
{
    fvMeshFunctionObject::topoChange(map);
    meshChanged_ = true;
}
