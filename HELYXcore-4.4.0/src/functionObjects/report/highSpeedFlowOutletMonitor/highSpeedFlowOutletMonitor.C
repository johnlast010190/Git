...
:
    ...
    thermo_(refCast<const fluidThermo>(basicThermo::lookupOrCreate(obr_))),
    p_(thermo_.p()),
    T_(thermo_.T()),
...
void functionObjects::highSpeedFlowOutletMonitor::calculate()
{
    ...
    const scalar T0Abs(T0 + thermo_.TRefValue());
    ...
    const scalar rho0 =
       ...
       *pow
        (
            T0Abs/(inletT+thermo_.TRefValue()),
            1.0/(gamma - 1.0)
        );
    ...
    if (Pr <= PrCrit)
    {
        ...
        Uis = sqrt(2.0*gamma*R*T0Abs/(gamma + 1.0));
    }
    else
    {
...
Foam::scalar functionObjects::highSpeedFlowOutletMonitor::calcT0()
{
    ...
    volScalarField& he = const_cast<volScalarField&>(thermo_.he());
    ...
    if (inFaceZoneIDs_.size() > 0)
    {
        ...
        TOld = new scalarField(T.primitiveField());
