...
Foam::functionObjects::powerLossObjectiveFunctionObject::execute()
{
    ...
    if (compressible())
    {
        ...
        {
            hTmp = tmp<volScalarField>::New(lookupObject<volScalarField>("h"));
        }
        else if (foundObject<volScalarField>("e"))
        {
            hTmp = lookupObject<volScalarField>("e") + p()/rho();
        }
        else
        {
    ...
    forAll(mesh_.boundary(), patchI)
    {
        if (objectivePatch_[patchI])
        {
            ...
            if (compressible())
            {
                ...
                const scalarField& hp = hTmp().boundaryField()[patchI];
