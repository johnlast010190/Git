...
)
{
    ...
    forAll(wallHeatFluxBf, patchi)
    {
        ...
        if (!patch.coupled())
        {
            ...
            wallHeatFluxBf[patchi] = (kappaEff & gradTBf[patchi]);
        }
    }
    ...
    patchSet_()
{
    ...
    wallHeatFlux::read(dict);
    wallHeatFlux::writeFileHeader(file());
}
...
bool Foam::functionObjects::wallHeatFlux::read(const dictionary& dict)
{
    ...
    Info<< wallHeatFlux::type() << " " << wallHeatFlux::name() << ":" << nl;
...
bool Foam::functionObjects::wallHeatFlux::execute()
{
    ...
    )
    {
        if (obr().foundObject<compressible::turbulenceModel>(turbName))
        {
            ...
            calcHeatFlux(turbModel.transport().T(), wallHeatFlux);
        }
        else if (obr().foundObject<fluidThermo>(thermoName))
        {
            ...
            calcHeatFlux(thermo.T(), wallHeatFlux);
        }
