...
bool Foam::functionObjects::regionSizeDistribution::read(const dictionary& dict)
{
    ...
    writeFile::read(dict);
    writeDroplets_.readIfPresent("logDroplets", dict);
    phaseGradDet_.readIfPresent("phaseGradientDropletDetection", dict);

    alphaName_ = dict.lookup<word>("field");
    patchNames_ = dict.lookup<wordReList>("patches");
    lThreshold_ = dict.lookup<scalar>("lowerThreshold");
    uThreshold_ = dict.lookup<scalar>("upperThreshold");
    maxDiam_ = dict.lookup<scalar>("maxDiameter");
    minDiam_ = 0.0;
    dict.readIfPresent("minDiameter", minDiam_);
    nBins_ = dict.lookup<label>("nBins");
    fields_ = dict.lookup<wordReList>("fields");
    ...
    if (isoPlanes_)
    {
        ...
        maxDiameter_ = dict.lookup<scalar>("maxDiameter");
        nDownstreamBins_ = dict.lookup<scalar>("nDownstreamBins");
        maxDownstream_ = dict.lookup<scalar>("maxDownstream");
...
//    label nBlocked = 0;
    ...
    {
        //enhanced droplet boundary detection using gradient sign change
        //specifically to separate droplets in close proximity
        ...
        for (label facei = 0; facei < mesh_.nInternalFaces(); facei++)
        {
            ...
            if (phaseGradDet_)
            {
                label Gfown
                (
                    sign101(gradAlpha[mesh_.faceOwner()[facei]] & Sf[facei])
                );
                label Gfnei
                (
                    sign101
                    (
                        gradAlpha[mesh_.faceNeighbour()[facei]] & Sf[facei]
                    )
                );
                ...
                (
                    ((Gfown <= 0 && Gfnei > 0)
                 || (Gfown < 0 && Gfnei >= 0))
                 && (lThreshold_ < ownVal && ownVal < uThreshold_)
                 && (lThreshold_ < neiVal && neiVal < uThreshold_)
                )
                {
            ...
            if
            (
                (ownVal < lThreshold_ && neiVal > lThreshold_)
             || (ownVal > lThreshold_ && neiVal < lThreshold_)
            )
            {
                blockedFace[facei] = true;
...
//                nBlocked++;
        ...
        forAll(alpha.boundaryField(), patchi)
        {
            ...
            if (fvp.coupled())
            {
                ...
                tmp<vectorField> townGrad(fvg.patchInternalField());
                const vectorField& ownGrad = townGrad();
                tmp<vectorField> tnbrGrad(fvg.patchNeighbourField());
                const vectorField& nbrGrad = tnbrGrad();

                const vectorField& Sfp = fvp.patch().Sf();
                ...
                forAll(ownFld, i)
                {
                    ...
                    if (phaseGradDet_)
                    {
                        label Gfown
                        (
                            sign101(ownGrad[i] & Sfp[i])
                        );
                        label Gfnei
                        (
                            sign101(nbrGrad[i] & Sfp[i])
                        );
                        ...
                        (
                         ...
                         || (Gfown < 0 && Gfnei >= 0))
                         && (ownVal > lThreshold_ && neiVal > lThreshold_)
                         && (ownVal < uThreshold_ && neiVal < uThreshold_)
                        )
                        {
...
//                        nBlocked++;
    ...
    if (writeDroplets_)
    {
        // write raw droplet data via simpleVTKWriter

        const vectorField alphaVolPosition
        (
            alpha.internalField()*mesh_.V()*mesh_.C()
        );
        Map<vector> allRegionPosition(regionSum(regions, alphaVolPosition));

        const volVectorField& U = mesh_.lookupObject<volVectorField>(UName_);
        const vectorField alphaMomentum
        (
            alpha.internalField()*mesh_.V()*U.internalField()
        );
        Map<vector> allRegionMomentum(regionSum(regions, alphaMomentum));

        //delete regions with low alpha concentration
        Map<scalar>::iterator vIter = allRegionVolume.begin();
        Map<scalar>::iterator aIter = allRegionAlphaVolume.begin();
        Map<vector>::iterator pIter = allRegionPosition.begin();
        Map<vector>::iterator mIter = allRegionMomentum.begin();

        DynamicList<scalar> volume(allRegionVolume.size());
        DynamicList<scalar> alphaVol(allRegionVolume.size());
        DynamicList<vector> position(allRegionVolume.size());
        DynamicList<vector> velocity(allRegionVolume.size());
        ...
        (
            ...
            vIter != allRegionVolume.end()
         && aIter != allRegionAlphaVolume.end()
         && pIter != allRegionPosition.end()
         && mIter != allRegionMomentum.end();
            ++vIter, ++aIter, ++pIter, ++mIter
        )
        {
            label regionI = vIter.key();
            scalar meanAlphaInRegion = aIter()/vIter();

            if
            (
                patchRegions.found(regionI)
             || vIter() >= maxDropletVol
             || meanAlphaInRegion < lThreshold_
            )
            {
                allRegionVolume.erase(vIter);
                allRegionAlphaVolume.erase(regionI);
                allRegionNumCells.erase(regionI);
                allRegionPosition.erase(regionI);
                allRegionMomentum.erase(regionI);
            }
            else if (meanAlphaInRegion > lThreshold_)
            {
                volume.append(vIter());
                alphaVol.append(aIter());
                position.append(pIter()/aIter());
                velocity.append(mIter()/aIter());
            }
        }

        //write discrete droplets to VTK format output
        volume.shrink();
        alphaVol.shrink();
        position.shrink();
        velocity.shrink();

        simpleVTKWriter vtkOut(position);
        vtkOut.addPointDataToPoints("phaseVolume", alphaVol);
        vtkOut.addPointDataToPoints("volume", volume);
        vtkOut.addPointDataToPoints("U", velocity);
        ...
        (
            outfileDir/
            word("droplets_") + Foam::name(obr_.time().timeIndex())
            + word(".vtk")
        );
    }
    else
    {
        ...
        forAllIter(Map<scalar>, allRegionVolume, vIter)
        {
            ...
            (
                patchRegions.found(regioni)
             || vIter() >= maxDropletVol
             || (allRegionAlphaVolume[regioni]/vIter() < lThreshold_)
            )
            {
                allRegionVolume.erase(vIter);
                allRegionAlphaVolume.erase(regioni);
