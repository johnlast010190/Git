...
{
    if (obr().foundObject<compressible::turbulenceModel>(modelName))
    {
        return true;
    }
    else if (obr().foundObject<incompressible::turbulenceModel>(modelName))
    {
        return false;
    }
    else
...
bool Foam::functionObjects::turbulenceFields::execute()
{
    ...
    if (comp)
    {
        const compressible::turbulenceModel& model =
            obr().lookupObject<compressible::turbulenceModel>(modelName);
    ...
    else
    {
        const incompressible::turbulenceModel& model =
            obr().lookupObject<incompressible::turbulenceModel>(modelName);
