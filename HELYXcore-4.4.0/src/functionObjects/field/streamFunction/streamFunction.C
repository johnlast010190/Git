    ...
    do
    {
        ...
        forAll(patches, patchi)
        {
            ...
            )
            {
                ...
                const primitivePatch& bouFaces = dpp;

                if (!isType<emptyPolyPatch>(dpp))
                {
                    forAll(bouFaces, facei)
                    {
                        if (Foam::magSqr(phi.boundaryField()[patchi][facei]) < SMALL)
                        {
