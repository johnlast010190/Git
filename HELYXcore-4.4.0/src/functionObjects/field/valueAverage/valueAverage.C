...
#include "valueAverage/valueAverage.H"
...
void Foam::functionObjects::valueAverage::writeFileHeader(Ostream& os) const
{
    ...
    {
        writeDelimited(os, fieldNames_[fieldi]);
    }
    os  << endl;
}
    ...
    if (resetOnRestart_)
    {
        forAll(fieldNames_, fieldi)
        {
            ...
            if (dict.found(fieldName))
            {
                ...
                totalTime_[fieldi] = valueDict.lookup<scalar>("totalTime");
            }
        }
    }
...
bool Foam::functionObjects::valueAverage::read(const dictionary& dict)
{
    ...
    functionObjectName_ = dict.lookup<word>("functionObject");
