...
{
    typedef SurfaceField<Type> sf;
    typedef VolField<Type> vf;
...
)
{
    ...
    if (validField<scalar>(fieldName))
    {
        ...
        {
            meanVal = dict.lookup<scalar>("meanValue");
        }
        else
        {
            scalar areaSum = 0;
...
)
{
    ...
    {
        axialVelSum = dict.lookup<scalar>("meanAxialVelocity");
    }
    else
    {
        forAll(filterFaces_, i)
...
)
{
    ...
    point swirlOrigin(dict.lookup("origin"));
    scalar swirlRadius(dict.lookup<scalar>("radius"));
