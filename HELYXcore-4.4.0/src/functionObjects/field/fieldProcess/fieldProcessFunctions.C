...
const tmp<volScalarField> functionObjects::fieldProcess::getMuEff()
{
    ...
    if (foundObject<eulerianPhaseSystem>(eulerianPhaseSystem::typeName))
    {
        const eulerianPhaseSystem& phaseSystem =
            obr_.lookupObjectRef<eulerianPhaseSystem>
            (
                eulerianPhaseSystem::typeName
            );

        return phaseSystem.muEff();
    }
    else if (foundObject<cmpTurbModel>(turbulenceModel::propertiesName))
    {
        const auto& model =
...
const tmp<volVectorField> functionObjects::fieldProcess::wallShear()
{
    ...
    forAll(mesh.boundary(), pI)
    {
        if  (isA<wallFvPatch>(mesh.boundary()[pI]))
        {
            ...
            Up -= cPatch.Sf()*(cPatch.Sf() & Up)
                / max(magSqr(cPatch.Sf()),VSMALL);
...
)
{
    ...
    {
        nearCellValue = dict.lookup<bool>("nearCellValue");
    }
    if (nearCellValue)
    {
...
)
{
    ...
    if (dict.found("write"))
    {
        writeStatus = dict.lookup<bool>("write");
    }
...
scalar functionObjects::fieldProcess::referenceValue
(
    const dictionary& dict,
    const word refName,
    const word fieldName,
    const scalar defaultRefValue
)
{
    if (dict.found(refName))
    {
        return dict.lookup<scalar>(refName);
    }
    else if (dict.found("referencePoint"))
    {
        const word fName = dict.lookupOrDefault<word>(fieldName, fieldName);
        scalar closestRefValue(0);

        if (volFieldType(fieldName) == ftvector)
        {
            vector refVectorValue =
                closestReferenceValueParallel<vector, volVectorField>(fName, dict);
            closestRefValue = mag(refVectorValue);
        }
        else if (volFieldType(fieldName) == ftscalar)
        {
            closestRefValue =
                closestReferenceValueParallel<scalar, volScalarField>(fName, dict);
        }
        else
        {
            FatalErrorInFunction
                << fieldName
                << " is neither a volVectorField nor a volScalarField."
                << exit(FatalError);
            closestRefValue = -1;
        }
        ...
        {
            WarningInFunction
                << refName << " entry not found in dictionary. "
                << closestRefValue <<" was used as reference, "
                << "as the cell value closest to the reference point." << endl;
        }
        return closestRefValue;
    }
    else
    {
        if (defaultRefValue == -1)
        {
            FatalErrorInFunction
                << "Neither reference value for " << refName
                << ", nor referencePoint have been specified!"
                << exit(FatalError);
            return 0;
        }
        else
        {
            if (debug)
            {
                Info<< "Returning default value: " << defaultRefValue << endl;
            }
            return defaultRefValue;
        }
    }
}
...
Type functionObjects::fieldProcess::closestReferenceValueParallel
(
    const word fieldName,
    const dictionary& dict
)
{
    const point pI = dict.lookup<point>("referencePoint");
    label cellI = mesh_.findNearestCell(pI);
    const auto& pt = lookupObject<Type2>(fieldName);

    List<scalar> cellDist(Pstream::nProcs());
    cellDist[Pstream::myProcNo()] = GREAT;

    List<Type> refValue(Pstream::nProcs());
    refValue[Pstream::myProcNo()] = -1;

    scalar& myCellDist = cellDist[Pstream::myProcNo()];
    if (cellI>-1)
    {
        myCellDist = mag(mesh_.cellCentres()[cellI] -pI);
        refValue[Pstream::myProcNo()] = pt.internalField()[cellI];
    }
    Pstream::allGatherList(cellDist);
    Pstream::allGatherList(refValue);

    scalar closestCellDist = GREAT;
    Type closestRefValue = -1;
...
void functionObjects::fieldProcess::ptot(const dictionary& dict)
{
    ...
    const Switch isIsentropic =
        dict.lookupOrDefault<Switch>("isentropic", false);
    ...
    )
    {
        ...
        const auto& U = lookupObject<volVectorField>(Uname);
        ...
        if (pt.dimensions() == dimensionSet(0, 2, -2, 0, 0))
        {
            ...
            (
                fieldName,
                (rhoIncomp*pt + 0.5*rhoIncomp*magSqr(U)).ptr(),
                dict
            );
        }
        else if (pt.dimensions() == dimensionSet(1, -1, -2, 0, 0))
        {
            // Rather try a default of thermo:rho if rho not found
            if (!dict.found("rho"))
            {
                rhoName =
                    !foundObject<volScalarField>("rho")
                 && foundObject<volScalarField>("thermo:rho")
                  ? "thermo:rho" : "rho";
            }

            const auto& rho = lookupObject<volScalarField>(rhoName);
            const volVectorField& U = lookupObject<volVectorField>(Uname);

            const rhoThermo& thermo =
                refCast<rhoThermo>
                (
                    multiphaseThermo::lookupOrCreate(obr_, word::null)
                );
            const tmp<volScalarField> tpAbs(thermo.pAbs());
            const volScalarField& pAbs = tpAbs();

            if (isIsentropic && !thermo.incompressible())
            {
                const tmp<volScalarField> tgamma(thermo.gamma());
                const volScalarField& gamma = tgamma();
                const tmp<volScalarField> M(Foam::mag(U)/sqrt(gamma*pAbs/rho));
                ...
                (
                    ...
                    (
                        tpAbs
                       *pow(1 + 0.5*(gamma - 1)*sqr(M), gamma/(gamma - 1))
                       -thermo.pRef()
                    ).ptr(),
                    dict
                );
            ...
            {
                setField(fieldName, (pt + 0.5*rho*magSqr(U)).ptr(), dict);
            }
        }
        else
        {
...
void functionObjects::fieldProcess::Cp(const dictionary& dict)
{
    ...
    {
        const auto& pt = lookupObject<volScalarField>(pName);
        scalar Uref = referenceValue(dict, "Uref", "U");
        ...
        if (pt.dimensions() == dimensionSet(0, 2, -2, 0, 0))
        {
            ...
            (
                "Pref",
                pt.dimensions(),
                referenceValue(dict, "pRef", "p", 0.0)
            );
            setField
            (
                fieldName,
        ...
        else if (pt.dimensions() == dimensionSet(1,-1,-2,0,0))
        {
            ...
            (
              ...
              ? pRefValue
              : referenceValue(dict, "pRef", "p")
            );
            dimensionedScalar rhoRef
            (
                "rhoRef",
                dimDensity,
                referenceValue(dict, "rhoRef", "rho")
            );
...
void functionObjects::fieldProcess::pGauge(const dictionary& dict)
{
    ...
    if (foundObject<volScalarField>(pName))
    {
        ...
        (
            ...
            dict.isDict("referenceFields")
          ? pRefValue
          : referenceValue(dict, "pRef", "p", 101325)
        );
...
void functionObjects::fieldProcess::Cg(const dictionary& dict)
{
    ...
    )
    {
        ...
        if (pt.dimensions() == dimensionSet(0, 2, -2, 0, 0))
        {
            ...
            (
                fieldName,
                ((pt -pRef + 0.5*magSqr(Ut))/U2).ptr(),
                dict
            );
        }
        else if (pt.dimensions() == dimensionSet(1,-1,-2,0,0))
        {
            // Try a default of thermo:rho if rho not found
            word rhoRefName =
                !foundObject<volScalarField>("rho")
                && foundObject<volScalarField>("thermo:rho")
                ? "thermo:rho" : "rho";
...
void functionObjects::fieldProcess::y(const dictionary& dict)
{
    word fieldName = dict.lookup<word>("fieldName");

    const fvMesh& mesh = refCast<const fvMesh>(obr_);
    volScalarField::Boundary d = nearWallDist::New(mesh).y();
    ...
    y.boundaryFieldRef().forceAssign(d);

    setField(fieldName, y, dict);
}
...
void functionObjects::fieldProcess::frictionVelocity(const dictionary& dict)
{
    // Name for the field
    word fieldName(dict.lookupOrDefault<word>("fieldName", "frictionVelocity"));
    ...
    if (foundObject<cmpTurbModel>(turbulenceModel::propertiesName))
    {
        ...
        forAll(mesh.boundary(),patchi)
        {
            ...
            {
                frictionVelocity.boundaryFieldRef()[patchi].forceAssign
                (
                    sqrt(mag(wallShearStress->boundaryField()[patchi])/model.rho().boundaryField()[patchi])
                );
            }
        }
    }
    ...
    {
        FatalErrorInFunction
            << "No valid model for rho"
            << exit(FatalError);
        ::abort();
    }

    setField(fieldName, frictionVelocity, dict);
}
...
void functionObjects::fieldProcess::Urel(const dictionary& dict)
{
    ...
    )
    {
        ...
        {
            // Specification by reference frames allows to create single Urel
            // for the whole domain relative to one frame (that can be nested)
            const coordinateFrame& frame =
                *coordinateFrame::lookupNew(mesh_, dict);

            const volVectorField& C = mesh_.C();
            Urel.primitiveFieldRef() -= frame.frameVelocity(C, true);

            volVectorField::Boundary& Ubf = Urel.boundaryFieldRef();
            forAll(Ubf, patchi)
            {
                Ubf[patchi] -=
                    frame.frameVelocity(C.boundaryField()[patchi], true);
            }
        }
        else
        {
            if (fvOptionsObj.optionList::size())
            {
                for (int i=0; i < (fvOptionsObj.optionList::size()); ++i)
                {
                    ...
                    {
                        fvOptionsObj.optionList::operator[](i).makeRelative
                        (
                            Urel
                        );
                    }
...
void functionObjects::fieldProcess::Urtz(const dictionary& dict)
{
    ...
    const Switch useForFrameForUrelCalc =
        dict.lookupOrDefault<Switch>("useFrameForUrelCalc", false);

    if
    (
        fvOptionsObj.optionList::size()
     || mesh_.moving()
     || useForFrameForUrelCalc
    )
    {
        const volVectorField& U = lookupObject<volVectorField>(Uname);
        ...
        if (useForFrameForUrelCalc)
        {
            const volVectorField& C = mesh_.C();
            Urtz.primitiveFieldRef() -= framePtr->frameVelocity(C, true);

            volVectorField::Boundary& Ubf = Urtz.boundaryFieldRef();
            forAll(Ubf, patchi)
            {
                Ubf[patchi] -=
                    framePtr->frameVelocity(C.boundaryField()[patchi], true);
            }
        }
        else
        {
...
void functionObjects::fieldProcess::UCartesianToCylindrical
(
    const dictionary& dict
)
{
    ...
    // Load optional specification of the field name
    const word fieldName =
        dict.lookupOrDefault<word>("fieldName", Uname + word("Cylindrical"));
    ...
    else
    {
        WarningInFunction
            << "Could not find referenceFrame in the dict." << nl
            << "Skipping calculation of UCartesianToCylindrical." << endl;
        return;
    }

    const volVectorField& U = lookupObject<volVectorField>(Uname);

    volVectorField UCartesianToCylindrical
    (
        IOobject
        (
            fieldName + "temp",
    ...
    // Internal field
    UCartesianToCylindrical.primitiveFieldRef() =
        csys.invTransform
        (
            mesh_.cellCentres(),
            UCartesianToCylindrical.primitiveField()
        );

    forAll(UCartesianToCylindrical.boundaryField(), patchi)
    {
        vectorField& vfp = UCartesianToCylindrical.boundaryFieldRef()[patchi];
        vfp = csys.invTransform(mesh_.boundary()[patchi].Cf(), vfp);
    }

    UCartesianToCylindrical.correctBoundaryConditions();

    setField(fieldName, UCartesianToCylindrical, dict);
}


void functionObjects::fieldProcess::UCylindricalToCartesian
(
    const dictionary& dict
)
{
    ...
    // Load optional specification of the field name
    const word fieldName =
        dict.lookupOrDefault<word>("fieldName", Uname + word("Cartesian"));
    ...
    else
    {
        WarningInFunction
            << "Could not find referenceFrame in the dict." << nl
            << "Skipping calculation of UCylindricalToCartesian." << endl;
        return;
    }

    const volVectorField& U = lookupObject<volVectorField>(Uname);

    volVectorField UCylindricalToCartesian
    (
        IOobject
        (
            fieldName + "temp",
    ...
    // Internal field
    UCylindricalToCartesian.primitiveFieldRef() =
        csys.transform
        (
            mesh_.cellCentres(),
            UCylindricalToCartesian.primitiveField()
        );

    forAll(UCylindricalToCartesian.boundaryField(), patchi)
    {
        vectorField& vfp = UCylindricalToCartesian.boundaryFieldRef()[patchi];
        vfp = csys.transform(mesh_.boundary()[patchi].Cf(), vfp);
    }

    UCylindricalToCartesian.correctBoundaryConditions();

    setField(fieldName, UCylindricalToCartesian, dict);
}
...
void functionObjects::fieldProcess::wallHeatFlux(const dictionary& dict)
{
    ...
    (
        ...
        mesh_,
        dimensionedScalar(dimensionSet(1, 0, -3, 0, 0, 0, 0), 0)
    );
    ...
    if (foundObject<cmpTurbModel>(turbulenceModel::propertiesName))
    {
        ...
        (
            ...
            (
               ...
               /sqrt
                (
                    (thermo.Cp()/thermo.Cv())*(thermo.Cp() - thermo.Cv())
                   *thermo.TAbs()
                )
            ).ptr(),
            dict
...
void functionObjects::fieldProcess::skinFriction(const dictionary& dict)
{
    ...
    scalar Uref = referenceValue(dict, "Uref", "U");
    dimensionedScalar U2("U2", dimVelocity*dimVelocity, 0.5*magSqr(Uref));
    scalar rho = referenceValue(dict, "rhoRef", "rho");
