...
bool Foam::functionObjects::streamLineBase::read(const dictionary& dict)
{
    ...
    trackForward_ = dict.lookup<bool>("trackForward");
    lifeTime_ = dict.lookup<label>("lifeTime");
    if (lifeTime_ < 1)
    {
        FatalErrorInFunction
    ...
    {
        trackLength_ = dict.lookup<scalar>("trackLength");
...
bool Foam::functionObjects::streamLineBase::write()
{
    ...
    fileName scalarVtkFile = fileName
    (
        vtkPath
    ...
    fileName vectorVtkFile = fileName
    (
        vtkPath
      / vectorFormatterPtr_().getFileName("track0", vectorNames_)
    );
...
void Foam::functionObjects::streamLineBase::movePoints(const polyMesh& mesh)
{
...
void Foam::functionObjects::streamLineBase::topoChange(const polyTopoChangeMap& map)
{
    if (&map.mesh() == &mesh_)
    {
        read(dict_);
...
void Foam::functionObjects::streamLineBase::mapMesh(const polyMeshMap& map)
{
    // Moving mesh affects the search tree
    read(dict_);
}
