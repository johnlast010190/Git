...
<
    Foam::functionObjects::runTimeControls::runTimeControl::logicalOps, 2
>::names[]=
{
    "and",
    "or"
...
const Foam::NamedEnum<Foam::functionObjects::runTimeControls::runTimeControl::logicalOps, 2>
    Foam::functionObjects::runTimeControls::runTimeControl::conditionLogicalOps_;
...
:
    ...
    logicalOperator_
    (
        dict.lookupOrDefault<word>("conditionLogicalOperator", "or")
    ),
    postProcessMode_(dict.lookupOrDefault<bool>("postProcessMode", false))
{
    read(dict);
}
...
{
    if (functionObject::postProcess && !postProcessMode_)
    {
...
bool Foam::functionObjects::runTimeControls::runTimeControl::execute()
{
    ...
    List<bool> satisfied(groupList_.size(), true);
    ...
    forAll(conditions_, conditioni)
    {
        ...
        if (condition.active())
        {
            bool conditionSatisfied = condition.apply(postProcessMode_);
            label groupi = condition.groupID();

            auto conditionIter = groupMap_.cfind(groupi);
            auto conditionListIter = groupList_.find(conditioni);
            ...
            )
            {
                FatalErrorInFunction
                    << "group " << groupi << " not found in list or map"
            ...
            if (conditionSatisfied)
            {
                ...
                if (groupi == -1)
                {
                    satisfied[conditionListIter] = true;
                }
                else
                {
                    //- all conditions within the same group must be satisfied
            ...
            else
            {
                if (groupi == -1)
                {
                    satisfied[conditionListIter] = false;
                }
                if (groupi != -1)
                {
    ...
    switch
    (
        conditionLogicalOps_.read(IStringStream(logicalOperator_)())
    )
    {
        case logicalAND:
            done = true;
            forAll(satisfied, conditioni)
            {
                done &= satisfied[conditioni];
            }
            doneGroup = true;
            forAll(groupSatisfied, groupi)
            {
                doneGroup &= groupSatisfied[groupi];
            }
            done &= doneGroup;
            break;
        ...
        default:
            WarningInFunction
                << logicalOperator_ << " is an invalid operation." << endl;
            break;
    }
