...
tmp<volScalarField> Foam::functionObjects::heatBalance::diffusivity()
{
    ...
    if (obr_.foundObject<incompressible::turbulenceModel>("turbulenceModel"))
    {
        alphaEff =
...
tmp<volScalarField> Foam::functionObjects::heatBalance::enthalpy()
{
    ...
    if (compressible_)
    {
        ...
        enthalpy =
        (
            thermo.he() - thermo.Cp()*(refTemp_ + thermo.TRefValue())
        );
    }
    else if
    (
        obr_.foundObject<incompressible::turbulenceModel>("turbulenceModel")
    )
    {
...
:
    ...
    compressible_(false),
    phiName_("phi"),
    refTemp_("referenceTemperature", dimTemperature, 273.15),
...
{
    heatBalance::read(dict);
    heatBalance::writeFileHeader(file());
...
bool Foam::functionObjects::heatBalance::execute()
{
    ...
    scalar ddtHeat =
        (domainHeat_.value() - domainHeatOld_.value())
...
bool Foam::functionObjects::heatBalance::read(const dictionary& dict)
{
    ...
    {
        dimensionedScalar refTempInDict = dict.lookup("referenceTemperature");
        basicThermo& thermo = basicThermo::lookupOrCreate(obr_);
        refTemp_ = refTempInDict + thermo.TRef();
        Info<< "    Using " << refTemp_
