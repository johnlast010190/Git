    ...
    Ttc_()
{
    ...
    {
        Ttc_ = probeDict.lookup<scalarField>("Tc");
    }
    else
    {
...
) const
{
    ...
    dydx =
        (
            epsilon_*(G/4 - sigma*pow4(y + thermo_.TRefValue()))*area
          + htc*(Tc - y)*area
        )
      / (rho_*Cp_*volume);
}
...
bool Foam::functionObjects::thermoCoupleProbes::execute()
{
    if (this->size())
    {
        ...
        odeSolver_->solve(t, t + dt, Ttc_, 0, dt);
        return true;
    }
