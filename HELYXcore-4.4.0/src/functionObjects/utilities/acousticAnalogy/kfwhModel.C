    ...
    forAll(observerSets_, obsSetI)
    {
        ...
        forAll(pos, obsI)
        {
            ...
            label currentTimeStep =
                floor
                (
                    (time_.time().value() - startSamplingTime_)/dtAco_ + ROOTSMALL
                );
...
bool Foam::functionObjects::kfwhModel::read(const dictionary& dict)
{
    ...
    if (sobject)
    {
        ...
        if (type == "smartBoundBox")
        {
            ...
            {
                FatalErrorInFunction
                << "Minimum and maximum vertices need to be defined for smartBoundBox"
                << exit(FatalError);
            }
            ...
            forAll(Cf(), jf)
            {
                ...
                if (advancedTime < endSamplingTime_)
                {
                    ...
                    label previousTimeStep =
                        floor
                        (
                            (advancedTime - startSamplingTime_)/dtAco_ + ROOTSMALL
                        );
