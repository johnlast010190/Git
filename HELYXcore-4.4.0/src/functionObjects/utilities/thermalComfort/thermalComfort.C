...
Foam::functionObjects::thermalComfort::CpRef() const
{
    if (mesh_.foundObject<fluidThermo>(basicThermo::dictName))
    {
        return mesh_.lookupObject<fluidThermo>(basicThermo::dictName).Cp();
    }
    else if (mesh_.foundObject<transportModel>("transportProperties"))
    {
        return mesh_.lookupObject<transportModel>("transportProperties").Cp();
    }
...
bool Foam::functionObjects::thermalComfort::read(const dictionary& dict)
{
    ...
    if
    (
        ( calculateASHRAE55_ || calculateASHRAE55PMV_)
        && clo_ > 1.5
    )
    {
        WarningInFunction
    ...
    if (calculateISO7933_ && ((clo_ < 0.1) || (clo_ > 1.0)))
    {
        WarningInFunction
    ...
    if (calculateISO7933_ && ((met_ < 1.7) || (met_ > 7.7)))
    {
        WarningInFunction
...
bool Foam::functionObjects::thermalComfort::write()
{
    ...
    if (foundObject<volScalarField>("w"))
    {
        ...
        Psat.primitiveFieldRef() =
            2337*Foam::exp(6879*(1/293.15 - 1./TAbs.primitiveField())
          - 5.031*Foam::log(max(TAbs.primitiveField(), SMALL)/293.15));
        ...
        forAll(Psat.boundaryField(), patchi)
        {
            Psat.boundaryFieldRef()[patchi] =
                2337*Foam::exp
                (
                    6879*(1/293.15 - 1/TAbs.boundaryField()[patchi])
                  - 5.031*Foam::log(TAbs.boundaryField()[patchi]/293.15)
                );
        }
    ...
    const radiationModel& radiation =
        mesh_.lookupObject<radiationModel>("radiationProperties");

    volScalarField& radiantTemperature =
        lookupObjectRef<volScalarField>("Tmrt");

    if (isA<radiationModels::fvDOM>(radiation))
    {
        const radiationModels::fvDOM& dom =
            const_cast<radiationModels::fvDOM&>
            (
    ...
    else
    {
        ...
        forAll(patches, patchi)
        {
            ...
            if (isType<wallFvPatch>(fvp))
            {
                ...
                {
                    scalar Pavg =
                        gSum(TAbs.boundaryField()[patchi]*fvp.magSf())/Apatch;
        ...
        forAll(radiantTemperature, celli)
        {
            ...
            {
                radiantTemperature[celli] = TAbs[celli];
            }
        }
    }
    ...
    if (calculateWetBulbTemp_ && !calculateISO7243_)
    {
        ...
        {
            wetBulb[celli] =
                comfort.wetBulb(TAbs[celli], relativeHumidity[celli]);
        }
    ...
    if (calculateISO7243_)
    {
        ...
        if (flagSolarRad)
        {
            ...
            const radiationModel& domRadObj =
                lookupObject<radiationModel>("radiationProperties");

            if (isA<radiationModels::fvDOM>(domRadObj))
            {
                const radiationModels::fvDOM& fvDOMobj =
                    refCast<const radiationModels::fvDOM>(domRadObj);
