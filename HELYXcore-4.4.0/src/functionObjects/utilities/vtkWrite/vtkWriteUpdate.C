...
namespace Foam
{
    ...
    const Enum<topoSetSource::setAction> functionObjects::vtkWrite::actionNames
    ({
        { topoSetSource::NEW, "use" },  // Reuse NEW for "use" action name
        { topoSetSource::ADD, "add" },
        // TODO:  Check that cellSet and topoSetSource are suitable replacements for
        // cellBitSet and topoSetCellSource.H in this context
...
) const
{
    ...
    cellSet cellsToSelect(mesh, "vtkWriteUpdateCellSet", IOobject::NO_READ);
        ...
        switch (action)
        {
            ...
            // case topoSetSource::SUBTRACT:
                if (topoSetSource::NEW == action)
                {
                    ...
                    // TODO:  Check that cellSet and topoSetSource are suitable replacements for
                    // cellBitSet and topoSetCellSource.H in this context
                    // reset sets blocks_ to zero in packedList - not sure how to implement that
                    // for cellSet
                    // cellsToSelect.reset();
                }
            ...
            case topoSetSource::SUBSET:
            {
                ...
                cellSet other(mesh, "vtkWriteUpdateOtherCellSet");
    ...
    // TODO:  Check that cellSet and topoSetSource are suitable replacements for
    // cellBitSet and topoSetCellSource.H in this context
    // In 1906, topoBitSet::addressing() just returns the underlying bitset
...
) const
{
    ...
    for (const polyPatch& temp : patches)
    {
        const directPolyPatch& pp = reinterpret_cast<const directPolyPatch&>(temp);
...
bool Foam::functionObjects::vtkWrite::update()
{
    ...
    for (auto& subsetter : meshSubsets_)
    {
        ...
        vtuMappings_[regioni].reset(subsetter.baseMesh());
        ++regioni;
    }
...
void Foam::functionObjects::vtkWrite::topoChange(const polyTopoChangeMap&)
{
    meshState_ = polyMesh::TOPO_CHANGE;
}
