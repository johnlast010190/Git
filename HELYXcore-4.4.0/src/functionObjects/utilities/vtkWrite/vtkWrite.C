...
bool Foam::functionObjects::vtkWrite::read(const dictionary& dict)
{
    ...
    if (outputDir_.size())
    {
        ...
        {
            outputDir_ = time_.rootPath()/time_.globalCaseName()/outputDir_;
        }
    }
    else
    {
        // Standard postProcessing/ naming
        outputDir_ = time_.rootPath()/time_.globalCaseName()/functionObject::outputPrefix/name();
    }
    outputDir_.clean();
...
bool Foam::functionObjects::vtkWrite::write()
{
    ...
    for (const word& regionName : meshes_.sortedToc())
    {
        ...
        // const fvMeshSubset& baseMesh = meshProxy;
        ...
        (
          ...
          ? (
        // The below implementation is used in 1906, but we don't
        // need to backport foam::fieldTypes for anything else
        //   ? baseMesh.count
        //     (
        //         stringListOps::foundOp<word>(fieldTypes::volume),
        //         acceptField
                obr_.names<volScalarField>(selectFields_).size() +
                obr_.names<volVectorField>(selectFields_).size() +
                obr_.names<volSphericalTensorField>(selectFields_).size() +
                obr_.names<volSymmTensorField>(selectFields_).size() +
                obr_.names<volTensorField>(selectFields_).size()
            )
          : 0
        );
        ...
        if (doInternal_)
        {
            if (interpolate_)
            {
                // TODO:  The 1906 fvMeshSubsetProxy::mesh() function returns
                // either the baseMesh or the subMesh - check that this work-around
                // is okay
                pInterp.reset(new volPointInterpolation(meshProxy.mesh()));
                // pInterp.reset(new volPointInterpolation(meshProxy));
            }
            ...
            Info<< "    Internal  : "
                << time_.TimePaths::relativePath(internalWriter->output())
                << endl;
        ...
        // const polyBoundaryMesh& patches = meshProxy.boundaryMesh();
        ...
        if (oneBoundary_ && patchIds.size())
        {
            ...
            Info<< "    Boundaries: "
                << time_.TimePaths::relativePath(writer->output()) << nl;
        ...
        else if (patchIds.size())
        {
            ...
            {
                const polyPatch& temp = patches[patchId];
                const directPolyPatch& pp = refCast<const directPolyPatch>(temp);
