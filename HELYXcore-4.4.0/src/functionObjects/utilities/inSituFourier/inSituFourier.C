        ...
        if (!integralPhiSinWinDt_.set(m))
        {
            ...
            (
                ...
                (
                    ...
                    mesh_,
                    dimensionedScalar(Phi.dimensions()*dimTime, 0)
                )
            );
...
bool Foam::functionObjects::inSituFourier::execute()
{
    ...
    if (!PhiRescaling_.valid())
    {
        ...
        PhiRescaling_.reset(new dimensionedScalar("PhiRescaling", dimless, 1));
        if
        (
...
bool Foam::functionObjects::inSituFourier::read(const dictionary& dict)
{
    ...
    flow_.clear();
    fhigh_.clear();
    if (dict.lookup<bool>("generateFrequencyBands"))
    {
        scalar fl = dict.lookup<scalar>("fl");
        if (fl <= 0)
        {
            FatalErrorInFunction
        ...
        scalar fU = dict.lookup<scalar>("fU");
