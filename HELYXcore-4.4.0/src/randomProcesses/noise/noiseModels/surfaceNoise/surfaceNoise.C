...
#include "sampledSurface/readers/surfaceReader.H"
...
bool surfaceNoise::read(const dictionary& dict)
{
    if (noiseModel::read(dict))
    {
        if (dict.found("file"))
        {
            ...
            inputFileNames_[0] = dict.lookup<fileName>("file");
        }
        else
        {
            inputFileNames_ = dict.lookup<List<fileName>>("files");
        }

        dict.readIfPresent("fftWriteInterval", fftWriteInterval_);
        dict.readIfPresent("p", pName_);

        readerType_ = dict.lookup<word>("reader");

        word writerType = dict.lookup<word>("writer");
        dictionary optDict
        (
