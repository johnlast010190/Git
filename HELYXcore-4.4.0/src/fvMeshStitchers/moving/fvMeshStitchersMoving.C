...
#include "fvMesh/fvMeshStitchers/fvMeshStitcher/conformedFvsPatchFieldsFwd.H"
...
template<>
constexpr bool contiguous<layerAndWeight>() {return true;}
...
)
{
    ...
    forAll(origPatchIDs, i)
    {
        ...
        SfSf.boundaryFieldRef()[errorPatchi] =
            repeat
            (
                (patchToPatch::rootVSmall*origPp.faceNormals())(),
                (-patchToPatch::rootVSmall*origPp.faceNormals())()
            );
...
)
{
    ...
    // For each boundary face, mark if it is an owner orig face and sum
    // the total non-conformal coupled area if so
    boolList bFaceIsOwnerOrig
    (
    ...
    scalarList regionMagSf(subNRegions, patchToPatch::vSmall);
    forAll(phiBf, nccPatchi)
    {
        const fvPatch& fvp = mesh().boundary()[nccPatchi];
    ...
    // Read fvSchemes and fvSolution dictionaries for the sub-mesh
    fvSolution& solDict = const_cast<fvSolution&>(subMesh.solution());
    fvSchemes& schemeDict = const_cast<fvSchemes&>(subMesh.schemes());

    schemeDict.resetReadOpt(IOobject::READ_IF_PRESENT);
    schemeDict.read();

    solDict.resetReadOpt(IOobject::READ_IF_PRESENT);
    solDict.read();
...
)
{
        ...
        if (isA<nonConformalCoupledFvPatch>(fvp))
        {
            ...
            forAll(nccFvp, nccPatchFacei)
            {
                const label origPatchFacei =
                  ...
                  - origFvp.start();
                const point& origC = faceCentres[origPatchFacei];
                const point origC0 =
        ...
        forAll(origPp, origPatchFacei)
        {
            ...
            const vector errorSf =
                ...
                (
                   ...
                   /max
                    (
                        meshMagUfb[origPatchi][origPatchFacei],
                        patchToPatch::vSmall
                    ),
    ...
    //
    // An exception for this procedure happens whenever a second-order temporal
    // scheme is employed for meshPhi: because the old-time mesh fluxes don't
    // get corrected here in the same way as the current field, wrong values
    // for the velocity error patches might be calculated in the
    // movingWall-type boundary conditions, so we avoid it in that case.

    if (phi.nOldTimes(false) > 0) return;
...
)
{
    ...
    // Resize the patch in the flux field
    for (label i = 0; i <= phi.nOldTimes(false); ++i)
    {
        ...
        forAll(polyFacesBf, ncPatchi)
        {
            ...
            {
                if (isA<conformedFvsPatchScalarField>(phi0Bf[ncPatchi]))
                {
                    conformedFvsPatchScalarField& cptf =
                        refCast<conformedFvsPatchScalarField>(phi0Bf[ncPatchi]);

                    cptf.map
                    (
                        cptf,
                        fvPatchFieldSetSizer(polyFacesBf[ncPatchi].size())
                    );
                }
                else
                {
                    phi0Bf[ncPatchi].autoMap
                    (
                        fvPatchFieldSetSizer(polyFacesBf[ncPatchi].size())
                    );
                }
